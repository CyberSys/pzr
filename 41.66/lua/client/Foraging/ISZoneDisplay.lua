...
local function getToolTipText(_text, _value, _isBonus)
	...
	if _isBonus then
		return " " .. rgbWhite .. _text .. getRGBForTooltip(_value > 0) .. ": " .. formatPercent(_value) .. " \% <LINE> ";
	else
		return " " .. rgbWhite .. _text .. getRGBForTooltip(_value < 0) .. ": " .. formatPercent(_value) .. " \% <LINE> ";
	end;
end
...
function ISZoneDisplay:getVisionTooltipText()
	...
	text = text .. rgbWhite .. getText("IGUI_SearchMode_Vision_Effect_Bonus_Radius") .. getRGBForTooltip(modifiers.professionBonus + modifiers.traitBonus > 0) .. ": " .. string.format("%+.2f", modifiers.professionBonus + modifiers.traitBonus) .. " <LINE> ";
...
function ISZoneDisplay:getZoneTooltipText()
	local fuzzyChanceTable = {
		...
		[2] = {text = getText"Sandbox_Rarity_option2", chance = 10},
...
function ISZoneDisplay:updateTooltip()
	...
	if self.tooltip then
		...
		elseif (self:isMouseOver() or tooltipForced == "Area") and (not self.tipPanel:getIsVisible()) then
