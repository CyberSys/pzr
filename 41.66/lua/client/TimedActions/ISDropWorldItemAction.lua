...
function ISDropWorldItemAction:perform()
	...
	if instanceof(worldItem, "Radio") then
		local _obj = IsoRadio.new(getCell(), worldItem:getWorldItem():getSquare(), nil)
		local deviceData = worldItem:getDeviceData();
		if deviceData then
			_obj:setDeviceData(deviceData);
		end
		worldItem:getWorldItem():getSquare():AddSpecialObject( _obj, worldItem:getWorldItem():getSquare():getObjects():size() );
		worldItem:getModData().IsoRadio = _obj
	end
