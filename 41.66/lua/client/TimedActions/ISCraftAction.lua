	...
	if resultItemCreated and self.recipe:getResult():getCount() > 1 then
		...
		if itemsAdded and instanceof(resultItemCreated, "Food") then
			for i=0, itemsAdded:size()-1 do
				...
				--avoid compounding the frozen item penalties
				if resultItemCreated:isFrozen() and resultItemCreated:getType() ~= "Icecream" then
					newItem:setBoredomChange(resultItemCreated:getBoredomChange() - 30);
					newItem:setUnhappyChange(resultItemCreated:getUnhappyChange() - 30);
				else
