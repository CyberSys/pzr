...
function ConnectToServer:OnConnectionStateChanged(state, message, arg)
	...
	if state == "Disconnected" then self:OnConnectFailed(getText("UI_servers_connectionfailed"), message) end
	--if state == "Disconnected" then return end
	if state == "Disconnecting" then return end
	if state == "Failed" and message and message ~= "Unknown" then self.failMessage = getText('UI_servers_'..message); return end
