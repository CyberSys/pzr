...
public final class ErosionMain {
	...
	private void initGridSquare(IsoGridSquare square, ErosionData.Square square2) {
		...
		float float1 = this.noiseMain.layeredNoise((float)int1 / 10.0F, (float)int2 / 10.0F);
		square2.noiseMainByte = Bits.packFloatUnitToByte(float1);
		square2.noiseMain = float1;
		...
		float float2 = (float)square2.rand(int1, int2, 100) / 100.0F;
		square2.magicNumByte = Bits.packFloatUnitToByte(float2);
		square2.magicNum = float2;
	...
	private boolean initConfig() {
		...
		if (GameClient.bClient) {
			...
			if (!file.exists() && !Core.getInstance().isNoSave()) {
				File file2 = ZomboidFileSystem.instance.getMediaFile("data" + File.separator + string);
				if (file2.exists()) {
					try {
						String string3 = file2.getAbsolutePath();
						DebugLog.log("erosion: copying " + string3 + " to " + file.getAbsolutePath());
