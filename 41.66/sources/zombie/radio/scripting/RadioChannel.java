				...
				if (radioLine == null) {
					...
					if (!ZomboidRadio.DISABLE_BROADCASTING) {
						...
						if (this.louisvilleObfuscate && ZomboidRadio.LOUISVILLE_OBFUSCATION) {
							...
							ZomboidRadio.getInstance().SendTransmission(0, 0, this.frequency, string, "", 0.7F, 0.5F, 0.5F, -1, this.isTv);
						} else {
							ZomboidRadio.getInstance().SendTransmission(0, 0, this.frequency, string, radioLine.getEffectsString(), radioLine.getR(), radioLine.getG(), radioLine.getB(), -1, this.isTv);
						}
					}
