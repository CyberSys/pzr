...
public final class DebugChunkState extends GameState {
	...
	private void drawZones() {
		...
		for (int int4 = 0; int4 < arrayList.size(); ++int4) {
			...
			if (!zone.isPolyline()) {
				if (!zone.points.isEmpty()) {
					for (int int5 = 0; int5 < zone.points.size(); int5 += 2) {
						int1 = zone.points.get(int5);
						int int6 = zone.points.get(int5 + 1);
						int2 = zone.points.get((int5 + 2) % zone.points.size());
						int3 = zone.points.get((int5 + 3) % zone.points.size());
		...
		float float3;
		for (int1 = 0; int1 < arrayList.size(); ++int1) {
			IsoMetaGrid.Zone zone2 = (IsoMetaGrid.Zone)arrayList.get(int1);
			if (zone2 != null && zone2.isPolyline() && !zone2.points.isEmpty()) {
				for (int2 = 0; int2 < zone2.points.size() - 2; int2 += 2) {
					int3 = zone2.points.get(int2);
					int int9 = zone2.points.get(int2 + 1);
					int7 = zone2.points.get(int2 + 2);
					int8 = zone2.points.get(int2 + 3);
				...
				if (zone2.polylineOutlinePoints != null) {
					float[] floatArray = zone2.polylineOutlinePoints;
