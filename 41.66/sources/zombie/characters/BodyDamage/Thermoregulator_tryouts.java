...
public class Thermoregulator_tryouts {
	...
	public Thermoregulator_tryouts(BodyDamage bodyDamage) {
	...
	public void save(ByteBuffer byteBuffer) throws IOException {
		...
		for (int int1 = 0; int1 < this.nodes.length; ++int1) {
			Thermoregulator_tryouts.ThermalNode thermalNode = this.nodes[int1];
	...
	public void load(ByteBuffer byteBuffer, int int1) throws IOException {
		...
		for (int int3 = 0; int3 < int2; ++int3) {
			...
			Thermoregulator_tryouts.ThermalNode thermalNode = this.getNodeForType(BodyPartType.FromIndex(int4));
			if (thermalNode != null) {
	...
	private void initNodes() {
		...
		for (int1 = 0; int1 < this.bodyDamage.getBodyParts().size(); ++int1) {
			...
			Thermoregulator_tryouts.ThermalNode thermalNode = null;
			switch (bodyPart.getType()) {
			case Torso_Upper: 
				thermalNode = new Thermoregulator_tryouts.ThermalNode(true, 37.0F, bodyPart, 0.25F);
				this.core = thermalNode;
				break;
			
			case Head: 
				thermalNode = new Thermoregulator_tryouts.ThermalNode(37.0F, bodyPart, 1.0F);
				break;
			
			case Neck: 
				thermalNode = new Thermoregulator_tryouts.ThermalNode(37.0F, bodyPart, 0.5F);
				break;
			
			case Torso_Lower: 
				thermalNode = new Thermoregulator_tryouts.ThermalNode(37.0F, bodyPart, 0.25F);
				break;
		...
		for (int1 = 0; int1 < this.nodes.length; ++int1) {
			Thermoregulator_tryouts.ThermalNode thermalNode2 = this.nodes[int1];
			...
			if (bodyPartTypeArray != null && bodyPartTypeArray.length > 0) {
				thermalNode2.downstream = new Thermoregulator_tryouts.ThermalNode[bodyPartTypeArray.length];
	...
	private void updateMetabolicRate() {
		...
		if (!PZMath.equal(this.metabolicRate, this.metabolicTarget)) {
			...
			if (this.metabolicTarget > this.metabolicRate) {
				this.metabolicRate += float4 * this.getSimulationMultiplier(Thermoregulator_tryouts.Multiplier.MetabolicRateInc);
			} else {
				this.metabolicRate += float4 * this.getSimulationMultiplier(Thermoregulator_tryouts.Multiplier.MetabolicRateDec);
			}
		}
	...
	private void updateTest() {
		...
		for (int int1 = 0; int1 < this.nodes.length; ++int1) {
			...
			float float3 = this.metabolicRateReal;
			float3 *= 1.0F + thermalNode.insulation * 0.25F;
			thermalNode.heatDelta += float3;
			float float4 = 1.0F;
			float1 += thermalNode.heatDelta * thermalNode.skinSurface * float4;
			...
			float float7 = thermalNode.skinCelcius + thermalNode.heatDelta;
		...
		this.coreHeatDelta = float1;
		float float10 = 0.0025F;
		...
		Thermoregulator_tryouts.ThermalNode thermalNode2 = this.core;
		thermalNode2.celcius += this.coreHeatDelta * float10;
	}
	...
	private void updateTest_4() {
		...
		for (int int1 = 0; int1 < this.nodes.length; ++int1) {
			Thermoregulator_tryouts.ThermalNode thermalNode = this.nodes[int1];
			thermalNode.calculateInsulation();
			float2 = 1.0F - thermalNode.getDistToCore();
			float float3 = this.airTemperature;
			if (this.airAndWindTemp < this.airTemperature) {
				float3 -= (this.airTemperature - this.airAndWindTemp) / (1.0F + thermalNode.windresist);
			}

			thermalNode.heatDelta = float3 - thermalNode.skinCelcius;
			thermalNode.heatDelta /= 1.0F + thermalNode.insulation;
			...
			float float7 = this.core.celcius - this.setPoint;
			if (float7 > 0.0F) {
				float7 *= 2.0F;
				float7 *= 1.0F + float2 * 0.25F;
			} else {
				float7 *= 1.0F + thermalNode.distToCore * 0.25F;
			}

			float float8 = thermalNode.skinCelcius + thermalNode.heatDelta + float7;
			float8 = PZMath.clamp(float8, float5, float6);
			float float9 = float8 - thermalNode.skinCelcius;
			float float10 = 0.0025F;
			if (thermalNode.skinCelcius < 33.0F && float9 < 0.0F || thermalNode.skinCelcius > 33.0F && float9 > 0.0F) {
				float10 = 2.5E-4F;
			}

			thermalNode.skinCelcius += float9 * float10;
		}

		float float11 = this.metabolicRateReal / Metabolics.Default.getMet() - 1.0F;
		if (float11 > 0.0F) {
			float11 /= Metabolics.Default.getMet() * 2.0F;
		}

		this.coreHeatDelta = float1 + float11;
		float2 = 0.0025F;
		if (this.core.celcius < this.setPoint && this.coreHeatDelta < 0.0F) {
			float2 = 5.0E-5F;
		}
		...
		Thermoregulator_tryouts.ThermalNode thermalNode2 = this.core;
		thermalNode2.celcius += this.coreHeatDelta * float2;
	}
	...
	private void updateTest_3() {
		for (int int1 = 0; int1 < this.nodes.length; ++int1) {
			...
			thermalNode.heatDelta /= 1.0F + thermalNode.insulation;
			float float2 = 1.0F + (1.0F - thermalNode.getDistToCore() * 0.5F);
			...
			float float6 = thermalNode.skinCelcius + thermalNode.heatDelta / 12.0F;
			float6 = PZMath.clamp(float6, float4, float5);
			float float7 = float6 - thermalNode.skinCelcius;
			thermalNode.skinCelcius += float7 * this.getSimulationMultiplier(Thermoregulator_tryouts.Multiplier.SkinCelcius);
		}
	}
	...
	private void updateTest_2() {
		...
		float float4;
		for (int1 = 0; int1 < this.nodes.length; ++int1) {
			thermalNode = this.nodes[int1];
			thermalNode.calculateInsulation();
			float1 = this.airTemperature;
			if (this.airAndWindTemp < this.airTemperature) {
				float1 = (this.airTemperature - this.airAndWindTemp) / (1.0F + thermalNode.windresist);
			}
			...
			thermalNode.heatDelta /= 1.0F + thermalNode.insulation;
			float3 = 1.0F + (1.0F - thermalNode.getDistToCore() * 0.8F);
			float4 = this.metabolicRateReal * float3;
			float4 *= 1.0F + thermalNode.insulation;
			thermalNode.heatDelta += float4;
			float float5 = thermalNode.heatDelta / 12.0F;
			float5 = PZMath.clamp(float5, -1.0F, 1.0F);
			float float6 = float5 - thermalNode.primaryDelta;
			thermalNode.primaryDelta += float6 * this.getSimulationMultiplier(Thermoregulator_tryouts.Multiplier.PrimaryDelta);
			thermalNode.primaryDelta = PZMath.clamp(thermalNode.primaryDelta, -1.0F, 1.0F);
		}

		for (int1 = 0; int1 < this.nodes.length; ++int1) {
			thermalNode = this.nodes[int1];
			float1 = this.core.celcius - 20.0F;
			float2 = this.core.celcius;
			if (float1 < this.airTemperature) {
				float1 = this.airTemperature;
			}

			float3 = this.core.celcius - 4.0F;
			if (thermalNode.primaryDelta < 0.0F) {
				float3 = 33.0F - 20.0F * PZMath.abs(thermalNode.primaryDelta);
			} else {
				float3 = 33.0F + 20.0F * thermalNode.primaryDelta;
			}

			float3 = PZMath.clamp(float3, float1, float2);
			float4 = float3 - thermalNode.skinCelcius;
			thermalNode.skinCelcius += float4 * this.getSimulationMultiplier(Thermoregulator_tryouts.Multiplier.SkinCelcius);
			thermalNode.skinCelcius = float3;
		}
	}
	...
	private void updateTest_OLD() {
		...
		for (int1 = 0; int1 < this.nodes.length; ++int1) {
			...
			float2 = float1 - thermalNode.skinCelcius;
			thermalNode.heatDelta = float2;
			thermalNode.heatDelta /= 1.0F + thermalNode.insulation;
			float3 = 1.0F + (1.0F - thermalNode.getDistToCore() * 0.8F);
			float4 = this.metabolicRateReal * float3;
			float4 *= 1.0F + thermalNode.insulation;
			thermalNode.heatDelta += float4;
			thermalNode.primaryDelta = thermalNode.heatDelta / 12.0F;
			thermalNode.primaryDelta = PZMath.clamp(thermalNode.primaryDelta, -1.0F, 1.0F);
		}
	...
	private void updateNodesHeatDelta() {
		float float1 = PZMath.clamp_01((this.player.getNutrition().getWeight() / 75.0F - 0.5F) * 0.666F);
		...
		for (int int1 = 0; int1 < this.nodes.length; ++int1) {
			...
			if (thermalNode.primaryDelta > 0.0F) {
				...
				float9 = Metabolics.Default.getMet() * thermalNode.primaryDelta * thermalNode.skinSurface / thermalNode.insulation;
			...
			if (thermalNode.secondaryDelta > 0.0F) {
				...
				float9 = Metabolics.MAX.getMet() * 0.75F * thermalNode.secondaryDelta * thermalNode.skinSurface / thermalNode.insulation;
				float9 *= float10;
	...
	private void updateNodesHeatDelta_OLD() {
		float float1 = this.player.getNutrition().getWeight();
		float float2 = PZMath.clamp_01((float1 / 75.0F - 0.5F) * 0.666F);
		float float3 = (float2 - 0.5F) * 2.0F;
		float float4 = this.stats.getFitness();
		float float5 = 1.0F;
		if (this.airAndWindTemp > this.setPoint - 2.0F) {
			if (this.airTemperature < this.setPoint + 2.0F) {
				float5 = (this.airTemperature - (this.setPoint - 2.0F)) / 4.0F;
				float5 = 1.0F - float5;
			} else {
				float5 = 0.0F;
			}
		...
		float float7;
		if (this.climate.getHumidity() > 0.5F) {
			float7 = (this.climate.getHumidity() - 0.5F) * 2.0F;
			float6 -= float7;
		}
		...
		for (int int1 = 0; int1 < this.nodes.length; ++int1) {
			...
			float float10;
			if (this.airAndWindTemp < this.airTemperature) {
				float10 = this.airTemperature - this.airAndWindTemp;
				float10 /= 1.0F + thermalNode.windresist;
				float9 -= float10;
			}

			float10 = 1.0F + thermalNode.insulation;
			float float11 = float9 - thermalNode.skinCelcius;
			if (float11 < 0.0F) {
				float11 *= 1.0F + 0.25F * thermalNode.bodyWetness;
			} else if (float11 > 0.0F) {
				float11 /= 1.0F + thermalNode.bodyWetness * 4.0F;
			}

			float11 *= 0.3F;
			float11 /= float10;
			thermalNode.heatDelta = float11 * thermalNode.skinSurface;
			float float12;
			float float13;
			if (thermalNode.primaryDelta > 0.0F) {
				float13 = 0.2F + 0.8F * this.getBodyFluids();
				float12 = Metabolics.Default.getMet() * 1.0F * thermalNode.primaryDelta * thermalNode.skinSurface / float10;
				float12 *= float13 * (0.1F + 0.9F * float5);
				float12 *= float6;
				float12 *= 1.0F - 0.2F * float3;
				float12 *= 1.0F + 0.2F * float4;
				thermalNode.heatDelta -= float12;
			} else {
				float13 = 0.2F + 0.8F * this.getEnergy();
				float12 = Metabolics.Default.getMet() * 1.0F * PZMath.abs(thermalNode.primaryDelta) * thermalNode.skinSurface;
				float12 *= float13;
				float12 *= 1.0F + 0.2F * float3;
				float12 *= 1.0F + 0.2F * float4;
				float12 *= 0.25F + 0.75F * float7;
				thermalNode.heatDelta += float12;
			}

			if (thermalNode.secondaryDelta > 0.0F) {
				float13 = 0.1F + 0.9F * this.getBodyFluids();
				float12 = Metabolics.MAX.getMet() * 0.75F * thermalNode.secondaryDelta * thermalNode.skinSurface / float10;
				float12 *= float13;
				float12 *= 0.75F + 0.25F * float6;
				float12 *= 1.0F - 0.2F * float3;
				float12 *= 1.0F + 0.2F * float4;
				thermalNode.heatDelta -= float12;
			} else {
				float13 = 0.1F + 0.9F * this.getEnergy();
				float12 = Metabolics.Default.getMet() * 1.0F * PZMath.abs(thermalNode.secondaryDelta) * thermalNode.skinSurface;
				float12 *= float13;
				float12 *= 1.0F + 0.2F * float3;
				float12 *= 1.0F + 0.2F * float4;
				float12 *= 0.25F + 0.75F * float7;
	...
	private void updateHeatDeltas() {
		...
		Thermoregulator_tryouts.ThermalNode thermalNode = this.core;
	...
	private void updateNodes() {
		...
		for (int int1 = 0; int1 < this.nodes.length; ++int1) {
			...
			thermalNode.skinCelcius += float8 * float9 * this.getSimulationMultiplier(Thermoregulator_tryouts.Multiplier.SkinCelcius);
			if (thermalNode != this.core) {
	...
	private void updateBodyMultipliers() {
		...
		this.fluidsMultiplier = 1.0;
		this.fatigueMultiplier = 1.0;
		float float1 = this.metabolicRateReal / Metabolics.Default.getMet();
		this.energyMultiplier = (double)PZMath.clamp(float1 * float1, 1.0F, 5.0F);
		float float2 = PZMath.abs(this.primTotal);
		float2 *= float2;
		if (this.primTotal < 0.0F) {
			this.energyMultiplier += (double)(0.75F * float2);
			this.fatigueMultiplier += (double)(0.5F * float2);
		} else if (this.primTotal > 0.0F) {
			this.fluidsMultiplier += (double)(0.75F * float2);
			this.fatigueMultiplier += (double)(0.5F * float2);
		}

		float2 = PZMath.abs(this.secTotal);
		float2 *= float2;
		if (this.secTotal < 0.0F) {
			this.energyMultiplier += (double)(8.0F * float2);
			this.fatigueMultiplier += (double)(3.5F * float2);
		} else if (this.secTotal > 0.0F) {
			this.fluidsMultiplier += (double)(6.0F * float2);
			this.fatigueMultiplier += (double)(3.5F * float2);
		}
	}
	...
	public float getEnergy() {
		float float1 = 1.0F - this.stats.getHunger() * this.stats.getHunger();
		float float2 = 1.0F - this.stats.getFatigue() * this.stats.getFatigue();
