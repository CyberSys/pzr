					...
					if (Rand.Next(100) > byte5) {
						...
						label208: while (true) {
							do {
								...
								boolean4 = true;
								int5 = Rand.Next(BodyPartType.ToIndex(BodyPartType.MAX));
							}					 while (int5 != BodyPartType.ToIndex(BodyPartType.Head) && int5 != BodyPartType.ToIndex(BodyPartType.Neck));
				...
				if (this.getOverallBodyHealth() != 0.0F) {
					...
					if (this.PoisonLevel > 0.0F) {
						if (this.PoisonLevel > 10.0F && this.getParentChar().getMoodles().getMoodleLevel(MoodleType.Sick) >= 1) {
							float3 += 0.0035F * Math.min(this.PoisonLevel / 10.0F, 3.0F) * GameTime.instance.getMultiplier();
						}

						float4 = 0.0F;
						if (this.getParentChar().getMoodles().getMoodleLevel(MoodleType.FoodEaten) > 0) {
							float4 = 1.5E-4F * (float)this.getParentChar().getMoodles().getMoodleLevel(MoodleType.FoodEaten);
						}

						this.PoisonLevel = (float)((double)this.PoisonLevel - ((double)float4 + ZomboidGlobals.PoisonLevelDecrease * (double)GameTime.instance.getMultiplier()));
						if (this.PoisonLevel < 0.0F) {
							this.PoisonLevel = 0.0F;
						}
					...
					if (this.getParentChar().getMoodles().getMoodleLevel(MoodleType.Hungry) == 4) {
						float3 += this.getHealthReductionFromSevereBadMoodles() / 50.0F * GameTime.instance.getMultiplier();
					}

					if (this.getParentChar().getMoodles().getMoodleLevel(MoodleType.Sick) == 4 && this.FoodSicknessLevel > this.InfectionLevel) {
						float3 += this.getHealthReductionFromSevereBadMoodles() * GameTime.instance.getMultiplier();
					}
					...
					if (this.getParentChar().getMoodles().getMoodleLevel(MoodleType.Thirst) == 4) {
						float3 += this.getHealthReductionFromSevereBadMoodles() / 10.0F * GameTime.instance.getMultiplier();
					}
					...
					if (this.getParentChar().getMoodles().getMoodleLevel(MoodleType.HeavyLoad) > 2 && this.getParentChar().getVehicle() == null && !this.getParentChar().isAsleep() && !this.getParentChar().isSitOnGround() && this.getThermoregulator().getMetabolicTarget() != Metabolics.SeatedResting.getMet() && this.getHealth() > 75.0F && Rand.Next(Rand.AdjustForFramerate(10)) == 0) {
						float3 += this.getHealthReductionFromSevereBadMoodles() / ((float)(5 - this.getParentChar().getMoodles().getMoodleLevel(MoodleType.HeavyLoad)) / 10.0F) * GameTime.instance.getMultiplier();
					}
					...
					if (this.isInfected()) {
						...
						if (int3 == 1) {
							...
							float float5 = (float4 - this.InfectionTime) / this.InfectionMortalityDuration;
							...
							if (float5 == 1.0F) {
								...
								float5 *= float5;
								float5 *= float5;
								float float6 = (1.0F - float5) * 100.0F;
								float float7 = this.getOverallBodyHealth() - float6;
