	...
	public void render() {
		if (this.isVisible()) {
			if (this.Parent == null || this.Parent.getMaxDrawHeight() == -1.0 || !(this.Parent.getMaxDrawHeight() <= this.getY())) {
				...
				int int1;
				float float4;
				if (this.m_renderer.getBoolean("Players") && float1 < 20.0F) {
					for (int1 = 0; int1 < IsoPlayer.numPlayers; ++int1) {
						IsoPlayer player = IsoPlayer.players[int1];
						if (player != null && !player.isDead()) {
							float4 = player.x;
							float float5 = player.y;
							if (player.getVehicle() != null) {
								float4 = player.getVehicle().getX();
								float5 = player.getVehicle().getY();
							}

							float float6 = this.m_APIv1.worldToUIX(float4, float5, float1, float2, float3, this.m_renderer.getProjectionMatrix(), this.m_renderer.getModelViewMatrix());
							float float7 = this.m_APIv1.worldToUIY(float4, float5, float1, float2, float3, this.m_renderer.getProjectionMatrix(), this.m_renderer.getModelViewMatrix());
				...
				int1 = TextManager.instance.getFontHeight(UIFont.Small);
				float float8;
				double double1;
				int int2;
				if (Core.bDebug && this.m_renderer.getBoolean("DebugInfo")) {
					...
					float8 = this.m_APIv1.mouseToWorldX();
					float4 = this.m_APIv1.mouseToWorldY();
					double1 = 0.0;
					double double2 = 0.0;
					double double3 = 0.0;
					double double4 = 1.0;
					byte byte1 = 0;
					this.DrawText("SQUARE = " + (int)float8 + "," + (int)float4, 0.0, (double)byte1, double1, double2, double3, double4);
					int2 = byte1 + int1;
					this.DrawText("CELL = " + (int)(float8 / 300.0F) + "," + (int)(float4 / 300.0F), 0.0, (double)int1, double1, double2, double3, double4);
					int2 += int1;
				...
				if (Core.bDebug && this.m_renderer.getBoolean("HitTest")) {
					...
					while (iterator.hasNext()) {
						...
						if (worldMapData.isReady()) {
							worldMapData.hitTest(float8, float4, s_tempFeatures);
						}
					}
					...
					if (!s_tempFeatures.isEmpty()) {
						...
						for (int int8 = 0; int8 < worldMapPoints.numPoints(); ++int8) {
							...
							float float9 = this.m_APIv1.worldToUIX((float)(int4 + int9), (float)(int5 + int2));
							float float10 = this.m_APIv1.worldToUIY((float)(int4 + int9), (float)(int5 + int2));
							float float11 = this.m_APIv1.worldToUIX((float)(int4 + int10), (float)(int5 + int11));
							float float12 = this.m_APIv1.worldToUIY((float)(int4 + int10), (float)(int5 + int11));
							SpriteRenderer.instance.renderline((Texture)null, int6 + (int)float9, int7 + (int)float10, int6 + (int)float11, int7 + (int)float12, 1.0F, 0.0F, 0.0F, 1.0F);
						}
					}
				}
