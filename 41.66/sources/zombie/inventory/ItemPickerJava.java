		...
		for (int int1 = 0; (float)int1 < itemPickerContainer.rolls; ++int1) {
			...
			for (int int2 = 0; int2 < itemPickerItemArray.length; ++int2) {
				...
				if ((float)Rand.Next(10000) <= float2 * 100.0F * float3 + float1 * 10.0F) {
					...
					if (inventoryItem.getScriptItem().getRecordedMediaCat() != null) {
						String string3 = inventoryItem.getScriptItem().getRecordedMediaCat();
						MediaData mediaData = ZomboidRadio.getInstance().getRecordedMedia().getRandomFromCategory(string3);
