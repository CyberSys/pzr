...
public final class PlayerOnGroundState extends State {
	...
	public void execute(IsoGameCharacter gameCharacter) {
		if (!GameServer.bServer && gameCharacter.isDead() && gameCharacter.getVariableBoolean("bAnimEnd")) {
			this.becomeCorpse((IsoPlayer)gameCharacter);
		} else {
	...
	private void becomeCorpse(IsoPlayer player) {
		if (!player.isOnDeathDone()) {
			if (GameClient.bClient) {
				player.networkAI.processDeadBody();
			}
