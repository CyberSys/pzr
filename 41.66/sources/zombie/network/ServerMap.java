...
public class ServerMap {
		...
		try {
			for (int int2 = 0; int2 < this.LoadedCells.size(); ++int2) {
				...
				if (!serverCell.bLoaded) {
					...
					int int3 = serverCell.WX - this.getMinX();
					int int4 = serverCell.WY - this.getMinY();
					if (!boolean1) {
						ServerLOS.instance.suspend();
						boolean1 = true;
					}
	...
	private boolean outsidePlayerInfluence(ServerMap.ServerCell serverCell) {
		...
		for (int int5 = 0; int5 < GameServer.udpEngine.connections.size(); ++int5) {
			...
			if (udpConnection.RelevantTo((float)int3, (float)int2)) {
				return false;
			}

			if (udpConnection.RelevantTo((float)int3, (float)int4)) {
				return false;
			}

			if (udpConnection.RelevantTo((float)int1, (float)int4)) {
				return false;
			}
		}
	...
	public void saveZoneInsidePlayerInfluence(short short1) {
		for (int int1 = 0; int1 < GameServer.udpEngine.connections.size(); ++int1) {
			...
			for (int int2 = 0; int2 < udpConnection.players.length; ++int2) {
				if (udpConnection.players[int2] != null && udpConnection.players[int2].OnlineID == short1) {
					IsoGridSquare square = IsoWorld.instance.CurrentCell.getGridSquare((double)udpConnection.players[int2].x, (double)udpConnection.players[int2].y, (double)udpConnection.players[int2].z);
					if (square != null) {
	...
	private boolean InsideThePlayerInfluence(ServerMap.ServerCell serverCell, short short1) {
		int int1 = serverCell.WX * 50;
		int int2 = serverCell.WY * 50;
		int int3 = (serverCell.WX + 1) * 50;
		int int4 = (serverCell.WY + 1) * 50;
		for (int int5 = 0; int5 < GameServer.udpEngine.connections.size(); ++int5) {
			UdpConnection udpConnection = (UdpConnection)GameServer.udpEngine.connections.get(int5);
			for (int int6 = 0; int6 < udpConnection.players.length; ++int6) {
				if (udpConnection.players[int6] != null && udpConnection.players[int6].OnlineID == short1) {
					if (udpConnection.RelevantToPlayerIndex(int6, (float)int1, (float)int2)) {
						return true;
					}

					if (udpConnection.RelevantToPlayerIndex(int6, (float)int3, (float)int2)) {
						return true;
					}

					if (udpConnection.RelevantToPlayerIndex(int6, (float)int3, (float)int4)) {
						return true;
					}

					if (udpConnection.RelevantToPlayerIndex(int6, (float)int1, (float)int4)) {
						return true;
					}
	...
	public void setSoftResetChunk(IsoChunk chunk) {
		...
		if (this.isValidCell(int1, int2)) {
			...
			int int3 = chunk.wx % 5;
			int int4 = chunk.wy % 5;
			serverCell.chunks[int3][int4] = chunk;
		}
	}
	...
	public void clearSoftResetChunk(IsoChunk chunk) {
		...
		if (serverCell != null) {
			int int3 = chunk.wx % 5;
			int int4 = chunk.wy % 5;
			serverCell.chunks[int3][int4] = null;
		}
	}
	...
	public static class ZombieIDMap {
		...
		public void put(short short1, IsoZombie zombie) {
			this.idToZombie2.put(short1, zombie);
		}

		public void remove(short short1) {
			this.idToZombie2.remove(short1);
		}

		public IsoZombie get(short short1) {
			return (IsoZombie)this.idToZombie2.get(short1);
		}

		private short allocateID() {
			short short1 = this.next;
			this.next = (short)(short1 + 1);
			return short1;
		}
		...
		private void loadVehicles() {
			for (int int1 = 0; int1 < 5; ++int1) {
				for (int int2 = 0; int2 < 5; ++int2) {
					IsoChunk chunk = this.chunks[int1][int2];
						...
						if (chunk != null) {
							try {
								...
								for (int int3 = 0; int3 < chunk.vehicles.size(); ++int3) {
									BaseVehicle baseVehicle = (BaseVehicle)chunk.vehicles.get(int3);
									VehiclesDB2.instance.updateVehicle(baseVehicle);
								}
							} catch (Exception exception) {
								exception.printStackTrace();
