...
public final class BaseVehicle extends IsoMovingObject implements Thumpable,IFMODParameterUpdater {
	...
	public void setNetPlayerAuthorization(byte byte1) {
		this.netPlayerAuthorization = byte1;
	}
	...
	public void switchSeat(IsoGameCharacter gameCharacter, int int1) {
		...
		if (int2 != -1) {
			...
			VehicleManager.instance.sendSwitchSeat(this, int1, gameCharacter);
		}
	}
		...
		if (this.removedFromWorld) {
			...
			if (this.physics != null) {
				...
				if (square == null && !this.chunk.refs.isEmpty()) {
					...
					float float4 = this.x;
					float float5 = this.y;
					this.x = Math.max(this.x, (float)int3 + float3);
					this.x = Math.min(this.x, (float)int4 - float3);
					this.y = Math.max(this.y, (float)int1 + float3);
				...
				this.updateTransform();
				Vector3f vector3f2 = allocVector3f().set((Vector3fc)(GameServer.bServer ? this.netLinearVelocity : this.jniLinearVelocity));
				if (this.jniIsCollide && this.limitCrash.Check()) {
					...
					if (DebugLog.isEnabled(DebugType.Vehicle)) {
						short short2 = this.VehicleID;
						DebugLog.log(DebugType.Vehicle, "Vehicle #" + short2 + " last velocity=" + this.lastLinearVelocity + "/" + this.lastLinearVelocity.length() + " current velocity=" + vector3f2 + "/" + vector3f2.length() + " delta=" + float6);
					}

					if (vector3f2.lengthSquared() > this.lastLinearVelocity.lengthSquared() && float6 > 4.0F) {
						DebugLog.log(DebugType.Vehicle, "Vehicle #" + this.VehicleID + " received a sharp increase in speed: delta=" + float6);
						float6 = 4.0F;
					}
					...
					if (float6 > 1.0F) {
						...
						if (Core.bDebug) {
							DebugLog.log(DebugType.Vehicle, "CRASH ID=" + this.VehicleID + " delta=" + float6);
						}
			...
			VehiclePart vehiclePart2 = this.getPartById("GasTank");
			if (vehiclePart2 != null && vehiclePart2.getContainerContentAmount() > (float)vehiclePart2.getContainerCapacity()) {
				vehiclePart2.setContainerContentAmount((float)vehiclePart2.getContainerCapacity());
			}
	...
	public void netPlayerFromServerUpdate(byte byte1, short short1) {
		if (IsoPlayer.getPlayerIndex() >= 0 && IsoPlayer.players[IsoPlayer.getPlayerIndex()] != null && (byte1 != this.netPlayerAuthorization || this.netPlayerId != short1)) {
			if (byte1 == 3) {
				if (short1 == IsoPlayer.players[IsoPlayer.getPlayerIndex()].OnlineID) {
					this.setNetPlayerAuthorization((byte)3);
					this.netPlayerId = short1;
	...
	public void checkPhysicsValidWithServer() {
		if (this.limitPhysicValid.Check()) {
			float[] floatArray = physicsParams;
			float float1 = 0.05F;
		...
		if (GameClient.bClient) {
			...
			if (short1 != -1) {
				...
				if (baseVehicle != null) {
					if (this.canAttachTrailer(baseVehicle, this.towAttachmentSelf, this.towAttachmentOther)) {
						this.addPointConstraint(baseVehicle, this.towAttachmentSelf, this.towAttachmentOther, this.towConstraintZOffset, true);
					}
				}
			}
			...
			if (this.vehicleTowingID != -1) {
				...
				if (baseVehicle2 != null) {
					if (this.canAttachTrailer(baseVehicle2, this.towAttachmentSelf, this.towAttachmentOther)) {
						this.addPointConstraint(baseVehicle2, this.towAttachmentSelf, this.towAttachmentOther, this.towConstraintZOffset, false);
					}
				}
			}
		}
	...
	public void hitCharacter(IsoGameCharacter gameCharacter, BaseVehicle.HitVars hitVars) {
		...
		if (hitVars.vehicleSpeed >= 5.0F || this.zombiesHits > 10) {
			...
			if (vector3f.z > 0.0F) {
				...
				if (Core.bDebug) {
					DebugLog.log(DebugType.Damage, "Damage car front hits=" + this.zombiesHits + " damage=" + int1);
				}
				...
				if (Core.bDebug) {
					DebugLog.log(DebugType.Damage, "Damage car rear hits=" + this.zombiesHits + " damage=" + int1);
				}
