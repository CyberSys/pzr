...
public final class CarController {
	...
	public CarController(BaseVehicle baseVehicle) {
		...
		DebugLog.Vehicle.debugln("Vehicle #" + short1 + " of " + vehicleScript.getFullName() + " has been added at " + baseVehicle.x + ", " + baseVehicle.y + ", " + float1 + " with netPlayerAuthorization=" + baseVehicle.netPlayerAuthorization + " as " + (baseVehicle.netPlayerAuthorization == 4 ? "static" : "dynamic"));
	}
	...
	public void update() {
		if (this.vehicleObject.getVehicleTowedBy() == null) {
			...
			if (baseVehicle != null && baseVehicle.getDriver() == null && this.vehicleObject.getDriver() != null) {
				this.vehicleObject.addPointConstraint(baseVehicle, this.vehicleObject.getTowAttachmentSelf(), baseVehicle.getTowAttachmentSelf());
			}

			if (this.vehicleObject.getVehicleTowing() != null) {
				BaseVehicle baseVehicle2 = this.vehicleObject.getVehicleTowing();
				this.vehicleObject.updateConstraint(baseVehicle2);
			}
	...
	public void debug() {
		if (Core.bDebug && DebugOptions.instance.VehicleRenderOutline.getValue()) {
			...
			for (int1 = 0; int1 < this.vehicleObject.getScript().getWheelCount(); ++int1) {
				...
				float2 = this.tempVec3f.x;
				float3 = this.tempVec3f.y;
				this.vehicleObject.getWheelForwardVector(int1, this.tempVec3f);
				LineDrawer.addLine(float2, float3, 0.0F, float2 + this.tempVec3f.x, float3 + this.tempVec3f.z, 0.0F, 1.0F, 1.0F, 1.0F, (String)null, true);
				this.drawRect(this.tempVec3f, float2 - WorldSimulation.instance.offsetX, float3 - WorldSimulation.instance.offsetY, wheel.width, wheel.radius);
			}
			...
			float float4;
			if (int1 != -1) {
				float4 = JoypadManager.instance.getMovementAxisX(int1);
				float2 = JoypadManager.instance.getMovementAxisY(int1);
				float3 = JoypadManager.instance.getDeadZone(int1, 0);
				if (Math.abs(float2) > float3 || Math.abs(float4) > float3) {
					Vector2 vector2 = this.tempVec2.set(float4, float2);
			...
			float4 = this.vehicleObject.x;
			float2 = this.vehicleObject.y;
			float3 = this.vehicleObject.z;
			LineDrawer.DrawIsoLine(float4 - 0.5F, float2, float3, float4 + 0.5F, float2, float3, 1.0F, 1.0F, 1.0F, 0.25F, 1);
			LineDrawer.DrawIsoLine(float4, float2 - 0.5F, float3, float4, float2 + 0.5F, float3, 1.0F, 1.0F, 1.0F, 0.25F, 1);
		}
	}
