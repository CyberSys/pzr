		...
		public void doStage(IsoGameCharacter gameCharacter, IsoThumpable thumpable, boolean boolean1) {
			...
			if (this.bonusHealthSkill) {
				while (iterator.hasNext()) {
					...
					int4 += gameCharacter.getPerkLevel(PerkFactory.Perks.FromString(string2)) * byte1;
				}
			}

			thumpable2.setMaxHealth(int2 + int3 + int4);
			thumpable2.setHealth(int1 + int3 + int4);
					...
					do {
						while (true) {
							...
							do {
								...
								int5 = (Integer)this.items.get(string3);
								item = ScriptManager.instance.getItem(string3);
							}				 while (item == null);

							if (item.getType() == Item.Type.Drainable) {
								inventoryItem = itemContainer.getFirstRecurse((thumpablex)->{
									return thumpablex.getFullType().equals(item.getFullName()) && ((DrainableComboItem)thumpablex).getDrainableUsesInt() >= int5;
								});
							...
							for (int int6 = 0; int6 < int5; ++int6) {
								InventoryItem inventoryItem2 = itemContainer.getFirstTypeRecurse(string3);
			...
			int int8 = thumpable.getSquare().transmitRemoveItemFromSquare(thumpable);
			thumpable2.getSquare().AddSpecialObject(thumpable2, int8);
			thumpable2.getSquare().RecalcAllWithNeighbours(true);
			thumpable2.transmitCompleteItemToServer();
			if (gameCharacter != null && !StringUtils.isNullOrWhitespace(this.completionSound)) {
				gameCharacter.playSound(this.completionSound);
			}
		}
