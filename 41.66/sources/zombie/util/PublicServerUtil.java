...
public final class PublicServerUtil {
	...
	private static void insertDatas() throws Exception {
		if (isEnabled) {
			...
			string2 = webSite;
			callUrl(string2 + "write.php?name=" + ServerOptions.instance.PublicName.getValue().replaceAll(" ", "%20") + string + "&port=" + ServerOptions.instance.DefaultPort.getValue() + "&players=" + int1 + "&ip=" + string5 + "&open=" + (ServerOptions.instance.Open.getValue() ? "1" : "0") + "&password=" + ("".equals(ServerOptions.instance.Password.getValue()) ? "0" : "1") + "&maxPlayers=" + ServerOptions.getInstance().getMaxPlayers() + "&version=" + Core.getInstance().getVersionNumber().replaceAll(" ", "%20") + string3 + "&mac=" + getMacAddress());
