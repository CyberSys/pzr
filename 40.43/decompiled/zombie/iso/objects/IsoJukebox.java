...
public class IsoJukebox extends IsoObject {
   ...
   public boolean onMouseLeftClick(int var1, int var2) {
      if (IsoPlayer.getInstance() == null) {
         ...
         float var3 = 0.0F;
         int var4 = Math.abs(this.square.getX() - IsoPlayer.getInstance().getCurrentSquare().getX()) + Math.abs(this.square.getY() - IsoPlayer.getInstance().getCurrentSquare().getY() + Math.abs(this.square.getZ() - IsoPlayer.getInstance().getCurrentSquare().getZ()));
         if (var4 < 4) {
            if (!this.Activated) {
