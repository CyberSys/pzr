   ...
   public boolean Update() {
      ...
      if ((this.Type == MoodleType.Hyperthermia || this.Type == MoodleType.Hypothermia) && this.Parent instanceof IsoPlayer) {
         if (!(this.Parent.getBodyDamage().getTemperature() < 36.5F) && !(this.Parent.getBodyDamage().getTemperature() > 37.5F)) {
            ...
            Temperature.PlayerTempVars var10 = Temperature.getPlayerTemperatureVars((IsoPlayer)this.Parent);
            if (var10 == null) {
               var12 = 0;
            } else {
               var8 = var10.getTickChangePm() >= 0.0F;
               if (var10.getChangeSpeed() >= 0.0F) {
                  if (var10.getChangeSpeed() > 0.66F) {
                     var12 = 3;
                  } else if (var10.getChangeSpeed() > 0.33F) {
                     var12 = 2;
                  } else {
                     var12 = 1;
                  }
      ...
      if (this.Type == MoodleType.Windchill) {
         ...
         if (this.Parent instanceof IsoPlayer) {
            Temperature.PlayerTempVars var6 = Temperature.getPlayerTemperatureVars((IsoPlayer)this.Parent);
            if (var6 != null && !var6.IsInVehicle() && !var6.IsInside()) {
               if (var6.getWindChillAmount() > 5.0F) {
                  var11 = 1;
               }

               if (var6.getWindChillAmount() > 10.0F) {
                  var11 = 2;
               }

               if (var6.getWindChillAmount() > 15.0F) {
                  var11 = 3;
               }

               if (var6.getWindChillAmount() > 20.0F) {
                  var11 = 4;
               }
            }
         }
