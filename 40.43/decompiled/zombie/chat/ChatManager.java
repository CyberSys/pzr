...
public class ChatManager {
      ...
      if (var1.getType() == ChatType.radio) {
         ...
         if (var1.isEnabled()) {
            if (!this.isSinglePlayerMode() && !var2.isLocal()) {
               ...
               if (this.isPlayerCanUseRadioChat() && var1.isSendingToRadio()) {
                  ChatMessage var3 = this.radioChat.createMessage(var2.getText());
                  var3.setRadioChannel(this.player.getEquipedRadio().getDeviceData().getChannel());
                  this.radioChat.sendToServer(var3);
               }
            }
         } else {
   ...
   private WhisperChat getWhisperChat(String var1) throws TimeoutException {
      if (this.whisperChats.containsKey(var1)) {
         ...
         PacketTypes.doPacket((short)187, var2);
