...
public class ZombieGroupManager {
   public static ZombieGroupManager instance = new ZombieGroupManager();
   private ArrayList groups = new ArrayList();
   private ArrayDeque freeGroups = new ArrayDeque();
   ...
   public void preupdate() {
      if (!GameClient.bClient) {
         ++this.tickCount;
         if (this.tickCount > PerformanceSettings.LockFPS) {
            this.tickCount = 0;
         }
   ...
   public void update(IsoZombie var1) {
      if (!GameClient.bClient) {
         if (!this.shouldBeInGroup(var1)) {
         ...
         } else if (this.tickCount % PerformanceSettings.LockFPS == 0) {
            if (var1.group == null) {
            ...
            if (var1.getCurrentState() == ZombieStandState.instance()) {
               int var5;
               float var10;
               if (var1 == var1.group.getLeader()) {
                  ...
                  if (!(var1.group.lastSpreadOutTime + 0.083333336F > var10)) {
                     ...
                     if (var5 >= 1) {
                        if (GameClient.bClient || GameServer.bServer || !(IsoPlayer.instance.getHoursSurvived() < 2.0D)) {
