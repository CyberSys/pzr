...
public class UIDebugConsole extends NewWindow {
   ...
   public void ProcessCommand() {
      if (this.CommandLine.internalText != null) {
         ...
         LuaClosure var3 = null;

         try {
            var3 = LuaCompiler.loadstring(var1, "console", LuaManager.env);
            LuaReturn var4 = LuaManager.caller.protectedCall(LuaManager.thread, var3);
            if (!var4.isSuccess()) {
               this.SpoolText(var4.getErrorString());
               this.SpoolText(var4.getJavaException().toString());
               this.SpoolText(var4.getLuaStackTrace());
            }
         } catch (IOException var5) {
            Logger.getLogger(UIDebugConsole.class.getName()).log(Level.SEVERE, (String)null, var5);
         }
