...
public class RenderSettings {
   ...
   public static class PlayerRenderSettings {
      ...
      private void updateRenderSettings(int var1, IsoPlayer var2) {
         ...
         this.CM_GlobalLight = var3.getGlobalLight();
         this.CM_GlobalLightIntensity = var3.getGlobalLightIntensity();
         this.CM_Ambient = var3.getAmbient();
         this.CM_DayLightStrength = var3.getDayLightStrength();
         this.CM_NightStrength = var3.getNightStrength();
         this.CM_Desaturation = var3.getDesaturation();
         this.CM_ViewDistance = var3.getViewDistance();
         this.CM_FogIntensity = var3.getFogIntensity();
         WorldFlares.applyFlaresForPlayer(this, var1, var2);
         var3.getThunderStorm().applyLightningForPlayer(this, var1, var2);
         int var4 = SandboxOptions.instance.NightDarkness.getValue();
         ...
         this.viewDistance = this.CM_ViewDistance;
         --var4;
         float var5 = 0.2F + 0.1F * (float)var4;
         var5 += 0.075F * ClimateMoon.getMoonFloat() * this.night;
         if (!this.isExterior) {
            var5 *= 0.925F - 0.075F * this.darkness;
            this.desaturation *= 0.25F;
         }

         if (this.ambient < 0.2F && var2.HasTrait("NightVision")) {
            this.ambient = 0.2F;
         }

         this.ambient = var5 + (1.0F - var5) * this.ambient;
         if (Core.getInstance().RenderShader != null && Core.getInstance().getOffscreenBuffer() != null) {
      ...
      private void legacyPostRender(int var1) {
         ...
         if (this.applyNightVisionGoggles) {
            ...
            SpriteRenderer.instance.render(RenderSettings.texture, 0.0F, 0.0F, (float)Core.getInstance().getOffscreenWidth(var1), (float)Core.getInstance().getOffscreenHeight(var1), 0.05F, 0.95F, 0.05F, this.goggleMod);
            IndieGL.glBlendFunc(770, 771);
         } else {
            IndieGL.glBlendFunc(774, 774);
            SpriteRenderer.instance.render(RenderSettings.texture, 0.0F, 0.0F, (float)Core.getInstance().getOffscreenWidth(var1), (float)Core.getInstance().getOffscreenHeight(var1), this.blendInfo.r, this.blendInfo.g, this.blendInfo.b, 1.0F);
            IndieGL.glBlendFunc(770, 771);
         }
