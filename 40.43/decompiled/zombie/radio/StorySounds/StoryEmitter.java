      ...
      if (this.max != -1 && this.max <= this.Instances.size() + this.ToStart.size()) {
         ...
         if (var8 == null) {
            ...
            if (var10 == 0L) {
               ...
               javafmod.FMOD_Channel_Set3DAttributes(var12.channel, var12.x - IsoPlayer.instance.x, var12.y - IsoPlayer.instance.y, var12.z - IsoPlayer.instance.z, 0.0F, 0.0F, 0.0F);
   ...
   public void tick() {
      ...
      for(var1 = 0; var1 < this.Instances.size(); ++var1) {
         ...
         if (!javafmod.FMOD_Channel_IsPlaying(var2.channel)) {
            ...
            float var3 = IsoUtils.DistanceManhatten(var2.x, var2.y, IsoPlayer.instance.x, IsoPlayer.instance.y, var2.z, IsoPlayer.instance.z) / var2.maxRange;
            if (var3 > 1.0F) {
               var3 = 1.0F;
            }

            if (!this.coordinate3D) {
               javafmod.FMOD_Channel_Set3DAttributes(var2.channel, Math.abs(var2.x - IsoPlayer.instance.x), Math.abs(var2.y - IsoPlayer.instance.y), Math.abs(var2.z - IsoPlayer.instance.z), 0.0F, 0.0F, 0.0F);
            } else {
               javafmod.FMOD_Channel_Set3DAttributes(var2.channel, Math.abs(var2.x - IsoPlayer.instance.x), Math.abs(var2.z - IsoPlayer.instance.z), Math.abs(var2.y - IsoPlayer.instance.y), 0.0F, 0.0F, 0.0F);
            }
            ...
            float var4 = 0.0F;
            float var5 = 0.0F;
            IsoGridSquare var6 = IsoPlayer.instance.getCurrentSquare();
            ...
            float var8 = (float)Math.toDegrees((double)IsoPlayer.instance.angle.getDirection());
