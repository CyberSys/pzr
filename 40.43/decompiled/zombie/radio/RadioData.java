...
public class RadioData {
   ...
   private ArrayList radioChannels = new ArrayList();
   private ArrayList translationDataList = new ArrayList();
   private RadioTranslationData currentTranslation;
   private Node rootNode;
   private Map advertQue = new HashMap();
   ...
   private static ArrayList fetchRadioData(boolean var0) {
      return fetchRadioData(var0, true);
   }
   ...
   private static ArrayList fetchRadioData(boolean var0, boolean var1) {
      ...
      try {
         ...
         searchForFiles(new File("media" + File.separator + "radio"), "xml", var4);
         ...
         while(true) {
            ...
            var4.clear();
            searchForFiles(new File("media" + File.separator + "radio"), "txt", var4);
            if (var0) {
               for(var6 = 0; var6 < var3.size(); ++var6) {
                  var7 = ZomboidFileSystem.instance.getModDir((String)var3.get(var6));
                  if (var7 != null) {
   ...
   public static ArrayList fetchAllRadioData() {
      ArrayList var0 = fetchRadioData(true);

      for(int var1 = var0.size() - 1; var1 >= 0; --var1) {
         RadioData var2 = (RadioData)var0.get(var1);
         if (var2.loadRadioScripts()) {
            System.out.println(" Adding" + (var2.isVanilla ? " (vanilla)" : "") + " file: " + var2.xmlFilePath);
            System.out.println(" - GUID: " + var2.GUID);
            var2.currentTranslation = null;
            var2.translationDataList.clear();
         } else {
            System.out.println("[Failure] Failed to load radio scripts for GUID: " + var2.GUID);
            System.out.println("          File: " + var2.xmlFilePath);
            var0.remove(var1);
         }
      }
   ...
   private static RadioData ReadFile(String var0) {
      ...
      try {
         System.out.println("Reading xml: " + var0);
         ...
         System.out.println("RadioData nodes len: " + var7.getLength());
         if (var7.getLength() > 0) {
            var1.rootNode = var7.item(0);
            var2 = var1.loadRootInfo();
            System.out.println("valid file: " + var2);
         }
   ...
   private ArrayList getChildNodes(Node var1) {
      ...
      if (var1.hasChildNodes()) {
         ...
         while(var3 != null) {
            if (!(var3 instanceof ElementImpl)) {
   ...
   private boolean loadRootInfo() {
      System.out.println("Reading RootInfo...");
      Iterator var1 = this.getChildNodes(this.rootNode).iterator();
      ...
      while(true) {
         ...
         do {
            ...
            var2 = (Node)var1.next();
         } while(!this.nodeNameIs(var2, "RootInfo"));

         System.out.println("RootInfo found");
         Iterator var3 = this.getChildNodes(var2).iterator();
         ...
         while(var3.hasNext()) {
            ...
            if (var5 != null && var6 != null) {
               var5 = var5.trim();
               System.out.println("Found element: " + var5);
               if (var5.equals("Version")) {
                  System.out.println("Version = " + this.version);
                  this.version = Integer.parseInt(var6);
               } else if (var5.equals("FileGUID")) {
                  System.out.println("GUID = " + var6);
                  this.GUID = var6;
               }
            }
         }
