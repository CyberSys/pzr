...
public class PublicServerUtil {
   ...
   private static void insertDatas() throws Exception {
      if (isEnabled) {
         ...
         callUrl(webSite + "write.php?name=" + ServerOptions.instance.PublicName.getValue().replaceAll(" ", "%20") + var0 + "&port=" + ServerOptions.instance.DefaultPort.getValue() + "&players=" + var5 + "&ip=" + var4 + "&open=" + (ServerOptions.instance.Open.getValue() ? "1" : "0") + "&password=" + ("".equals(ServerOptions.instance.Password.getValue()) ? "0" : "1") + "&maxPlayers=" + ServerOptions.instance.MaxPlayers.getValue() + "&version=" + Core.getInstance().getVersionNumber().replaceAll(" ", "%20") + var1 + "&mac=" + getMacAddress());
   ...
   private static void callUrl(final String var0) {
      (new Thread("openUrl") {
         public void run() {
            try {
