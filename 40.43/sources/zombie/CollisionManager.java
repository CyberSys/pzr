...
public class CollisionManager {
	...
	public void ResolveContacts() {
		...
		for (int3 = 0; int3 < int2; ++int3) {
			IsoPushableObject pushableObject2 = (IsoPushableObject)pushables.get(int3);
			float float2 = 0.0F;
			float1 = 0.0F;
			int int4;
			...
			for (int4 = 0; int4 < pushableObject2.connectList.size(); ++int4) {
				...
				int int5 = pushables.indexOf(pushableObject2.connectList.get(int4));
				pushables.remove(pushableObject2.connectList.get(int4));
				if (int5 <= int3) {
					--int3;
				}
			}
		...
		for (int int6 = 0; int6 < int3; ++int6) {
			...
			if (!(Math.abs(contact.a.getZ() - contact.b.getZ()) > 0.3F)) {
				this.vel.x = contact.a.getNx() - contact.a.getX();
				this.vel.y = contact.a.getNy() - contact.a.getY();
				this.vel2.x = contact.b.getNx() - contact.b.getX();
				this.vel2.y = contact.b.getNy() - contact.b.getY();
				if (this.vel.x != 0.0F || this.vel.y != 0.0F || this.vel2.x != 0.0F || this.vel2.y != 0.0F || contact.a.getImpulsex() != 0.0F || contact.a.getImpulsey() != 0.0F || contact.b.getImpulsex() != 0.0F || contact.b.getImpulsey() != 0.0F) {
					...
					CollisionManager.PolygonCollisionResult polygonCollisionResult = this.PolygonCollision(this.vel);
		...
		for (int int9 = 0; int9 < arrayList3.size(); ++int9) {
			IsoMovingObject movingObject = (IsoMovingObject)arrayList3.get(int9);
			movingObject.postupdate();
			if (!arrayList3.contains(movingObject)) {
				--int9;
			}
		}
