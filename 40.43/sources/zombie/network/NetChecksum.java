...
public final class NetChecksum {
		...
		public void serverPacket(ByteBuffer byteBuffer, UdpConnection udpConnection) {
			if (GameServer.bServer) {
				...
				case 1: 
					...
					boolean boolean2 = string2.equals(ScriptManager.instance.getChecksum());
				...
				case 2: 
					...
					if (short2 >= 0 && short3 >= short2 && short3 < short2 + 10) {
						...
						while (true) {
							...
							PacketTypes.doPacket((short)91, byteBufferWriter2);
							byteBufferWriter2.putShort((short)2);
							byteBufferWriter2.putShort(short2);
				...
				case 3: 
					short2 = byteBuffer.getShort();
					short3 = byteBuffer.getShort();
					if (short2 < 0 || short3 <= 0 || short3 > 20) {
						this.sendError(udpConnection, "PacketFileChecksums: groupIndex and/or fileCount are invalid");
						return;
					}

					if (short2 >= NetChecksum.GroupOfFiles.groups.size()) {
						String string3 = GameWindow.ReadStringUTF(byteBuffer);
						this.sendFileMismatch(udpConnection, short2, string3, (byte)2);
						return;
					}

					NetChecksum.GroupOfFiles groupOfFiles2 = (NetChecksum.GroupOfFiles)NetChecksum.GroupOfFiles.groups.get(short2);
					for (short4 = 0; short4 < short3; ++short4) {
						...
						if (short4 >= groupOfFiles2.fileCount) {
							this.sendFileMismatch(udpConnection, short2, string4, (byte)2);
							return;
						}

						if (!string4.equals(groupOfFiles2.relPaths[short4])) {
							String string5 = ZomboidFileSystem.instance.getString(string4);
							if (string5.equals(string4)) {
								this.sendFileMismatch(udpConnection, short2, string4, (byte)2);
								return;
							}

							this.sendFileMismatch(udpConnection, short2, groupOfFiles2.relPaths[short4], (byte)3);
							return;
						}

						if (byte1 > groupOfFiles2.checksums[short4].length) {
							this.sendFileMismatch(udpConnection, short2, groupOfFiles2.relPaths[short4], (byte)1);
							return;
						}

						byteBuffer.get(this.checksum, 0, byte1);
						if (!this.checksumEquals(groupOfFiles2.checksums[short4])) {
							this.sendFileMismatch(udpConnection, short2, groupOfFiles2.relPaths[short4], (byte)1);
							return;
						}
					}

					if (groupOfFiles2.fileCount > short3) {
						this.sendFileMismatch(udpConnection, short2, groupOfFiles2.relPaths[short3], (byte)3);
						return;
					}
	...
	public static final class Checksummer {
		...
		public String checksumToString() {
			...
			for (int int1 = 0; int1 < byteArray.length; ++int1) {
				stringBuffer.append(Integer.toString((byteArray[int1] & 255) + 256, 16).substring(1));
			}
