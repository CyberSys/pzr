...
public class GameServerWorkshopItems {
	...
	private static class WorkshopItem implements ISteamWorkshopCallback {
		...
		void CheckItemState() {
			...
			GameServerWorkshopItems.noise("GetItemState()=" + SteamWorkshopItem.ItemState.toString(long1) + " ID=" + this.ID);
		...
		void DownloadPending() {
			...
			if (this.downloadQueryTime + 100L <= long1) {
				...
				GameServerWorkshopItems.noise("DownloadPending GetItemState()=" + SteamWorkshopItem.ItemState.toString(long2) + " ID=" + this.ID);
