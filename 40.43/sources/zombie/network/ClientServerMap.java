...
public final class ClientServerMap {
	private static final int ChunksPerServerCell = 7;
	private static final int SquaresPerServerCell = 70;
	int playerIndex;
	int centerX;
	int centerY;
	int chunkGridWidth;
	...
	public int getMinX() {
		return (this.centerX / 10 - this.chunkGridWidth / 2) / 7;
	}

	public int getMinY() {
		return (this.centerY / 10 - this.chunkGridWidth / 2) / 7;
	}
		...
		if (!GameClient.bClient) {
			...
			for (int int3 = 0; int3 < IsoPlayer.numPlayers; ++int3) {
				...
				if (clientServerMap != null) {
					int int4 = int1 / 7 - clientServerMap.getMinX();
					int int5 = int2 / 7 - clientServerMap.getMinY();
	...
	public void sendPacket(UdpConnection udpConnection) {
		if (GameServer.bServer) {
			...
			PacketTypes.doPacket((short)15, byteBufferWriter);
					...
					for (int4 = 0; int4 < IsoChunkMap.ChunkGridWidth; ++int4) {
						...
						if (chunk != null) {
							...
							if (boolean1 && texture != null) {
								...
								if (chunk2 != null && !isLoaded[int4 + (int3 - 1) * IsoChunkMap.ChunkGridWidth]) {
									for (int int5 = 0; int5 < byte1; ++int5) {
										...
										SpriteRenderer.instance.render(texture, float6 - (float)(texture.getWidth() / 2), float7, (float)texture.getWidth(), (float)texture.getHeight(), 1.0F, 1.0F, 1.0F, 1.0F);
									}
								}
								...
								if (chunk3 != null && !isLoaded[int4 + (int3 + 1) * IsoChunkMap.ChunkGridWidth]) {
									for (int int6 = 0; int6 < byte1; ++int6) {
										...
										SpriteRenderer.instance.render(texture, float7 - (float)(texture.getWidth() / 2), float8, (float)texture.getWidth(), (float)texture.getHeight(), 1.0F, 1.0F, 1.0F, 1.0F);
									}
								}
								...
								if (chunk4 != null && !isLoaded[int4 - 1 + int3 * IsoChunkMap.ChunkGridWidth]) {
									for (int int7 = 0; int7 < byte1; ++int7) {
										...
										SpriteRenderer.instance.render(texture, float8 - (float)(texture.getWidth() / 2), float9, (float)texture.getWidth(), (float)texture.getHeight(), 1.0F, 1.0F, 1.0F, 1.0F);
									}
								}
								...
								if (chunk5 != null && !isLoaded[int4 + 1 + int3 * IsoChunkMap.ChunkGridWidth]) {
									for (int int8 = 0; int8 < byte1; ++int8) {
										...
										SpriteRenderer.instance.render(texture, float9 - (float)(texture.getWidth() / 2), float10, (float)texture.getWidth(), (float)texture.getHeight(), 1.0F, 1.0F, 1.0F, 1.0F);
									}
								}
							}
							...
							if (!boolean1) {
								...
								SpriteRenderer.instance.renderPoly((int)float6, (int)float7, (int)(float6 + (float)(byte1 * 64 / 2 * int2)), (int)(float7 - (float)(byte1 * 32 / 2 * int2)), (int)(float6 + (float)(byte1 * 64 * int2)), (int)float7, (int)(float6 + (float)(byte1 * 64 / 2 * int2)), (int)(float7 + (float)(byte1 * 32 / 2 * int2)), float1, float2, float3, float4);
							}
						}
					}
				}
