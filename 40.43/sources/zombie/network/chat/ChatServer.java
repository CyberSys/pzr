...
public class ChatServer {
	...
	public void initPlayer(int int1) {
		logger.write("Player with id = \'" + int1 + "\' tries to connect", "info");
		synchronized (players) {
			if (players.contains(int1)) {
				logger.write("Player already connected!", "warning");
				return;
			}
		}
		logger.write("Adding player \'" + int1 + "\' to chat server", "info");
		...
		if (udpConnection != null && player != null) {
			...
			this.addDefaultChats(int1);
			logger.write("Player joined to default chats", "info");
			if (udpConnection.accessLevel.equals("admin")) {
				this.joinAdminChat(int1);
			}

			Faction faction = Faction.getPlayerFaction(player);
			if (faction != null) {
				this.addMemberToFactionChat(faction.getName(), int1);
			}

			SafeHouse safeHouse = SafeHouse.hasSafehouse(player);
			if (safeHouse != null) {
				this.addMemberToSafehouseChat(safeHouse.getId(), int1);
			}

			ByteBufferWriter byteBufferWriter = udpConnection.startPacket();
			PacketTypes.doPacket((short)191, byteBufferWriter);
			udpConnection.endPacketImmediate();
			synchronized (players) {
				players.add(int1);
			}

			logger.write("Player " + player.getUsername() + "(" + int1 + ") joined to chat server successfully", "info");
		} else {
	...
	public void processPlayerStartWhisperChatPacket(ByteBuffer byteBuffer) {
		...
		if (!ChatUtility.chatStreamEnabled(ChatType.whisper)) {
			...
			if (player == null) {
				...
				logger.write("Whisper chat (id = " + whisperChat.getID() + ") between \'" + player.getUsername() + "\' and \'" + player2.getUsername() + "\' started", "info");
			}
		}
	}
	...
	private void sendPlayerNotFoundMessage(UdpConnection udpConnection) {
		ByteBufferWriter byteBufferWriter = udpConnection.startPacket();
		PacketTypes.doPacket((short)192, byteBufferWriter);
		udpConnection.endPacketImmediate();
	...
	public void disconnectPlayer(int int1) {
		logger.write("Player " + int1 + " disconnecting...", "info");
		synchronized (chats) {
			Iterator iterator = chats.values().iterator();
			while (iterator.hasNext()) {
				ChatBase chatBase = (ChatBase)iterator.next();
				chatBase.removeMember(int1);
		...
		logger.write("Disconnecting player " + int1 + " finished", "info");
	}
	...
	public void joinAdminChat(Integer integer) {
		if (adminChat == null) {
			logger.write("Admin chat is null! Can\'t add player to it", "warning");
		} else {
			adminChat.addMember(integer);
			logger.write("Player joined admin chat", "info");
		}
	}

	public void leaveAdminChat(Integer integer) {
		logger.write("Player " + integer + " are leaving admin chat...", "info");
		...
		if (adminChat == null) {
			...
			adminChat.leaveMember(integer);
			((ChatTab)tabs.get("admin")).sendRemoveTabPacket(udpConnection);
			logger.write("Player " + integer + " leaved admin chat", "info");
		}
	}
		...
		if (string != null && string2 != null && arrayList != null) {
			synchronized (factionChats) {
				...
				while (iterator.hasNext()) {
					...
					stringBuilder.append("\'").append(ChatUtility.findPlayerName(int1)).append("\', ");
				}
	...
	private void addMemberToSafehouseChat(String string, Integer integer) {
		if (!safehouseChats.containsKey(string)) {
			logger.write("Safehouse chat is not initialized!", "warning");
		} else {
			synchronized (safehouseChats) {
				SafehouseChat safehouseChat = (SafehouseChat)safehouseChats.get(string);
				safehouseChat.addMember(integer);
			}
	...
	private void addMemberToFactionChat(String string, Integer integer) {
		if (!factionChats.containsKey(string)) {
			logger.write("Faction chat is not initialized!", "warning");
		} else {
			synchronized (factionChats) {
				FactionChat factionChat = (FactionChat)factionChats.get(string);
				factionChat.addMember(integer);
			}
	...
	private void sendInitPlayerChatPacket(UdpConnection udpConnection) {
		ByteBufferWriter byteBufferWriter = udpConnection.startPacket();
		PacketTypes.doPacket((short)182, byteBufferWriter);
