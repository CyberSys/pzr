...
public class Item extends BaseScriptObject {
	...
	public void setUseDelta(String string) {
	...
	public void Load(String string, String[] stringArray) {
		this.name = string;
		int int1 = IDMax++;
		NetIDToItem.put(int1, this.module.name + "." + string);
		NetItemToID.put(this.module.name + "." + string, int1);
		...
		if (this.type == Item.Type.Clothing) {
			for (int int4 = 0; int4 < this.PaletteChoices.size(); ++int4) {
				this.InstanceItem((String)this.PaletteChoices.get(int4));
			}
	...
	public InventoryItem InstanceItem(String string) {
		...
		if (this.type == Item.Type.Food) {
			...
			food.setThirstChange(this.ThistChange / 100.0F);
		...
		if (this.type == Item.Type.Literature) {
			...
			handWeapon.ProjectileCount = this.ProjectileCount;
			...
			handWeapon.setSwingAnim(this.SwingAnim);
			...
			handWeapon.setAmmoType(this.AmmoType);
			...
			((Clothing)object).setBodyLocation(this.bodyLocation);
			...
			drainableComboItem.setDisappearOnUse(this.DisappearOnUse);
		...
		((InventoryItem)object).setAlwaysWelcomeGift(this.AlwaysWelcomeGift);
		((InventoryItem)object).setCanBandage(this.CanBandage);
		...
		((InventoryItem)object).setTwoHandWeapon(this.TwoHandWeapon);
		...
		((InventoryItem)object).setTrap(this.Trap);
		((InventoryItem)object).setFishingLure(this.FishingLure);
		...
		((InventoryItem)object).setMechanicType(this.vehicleType);
		...
		if (string.trim().length() != 0) {
			try {
				...
				if (string2.trim().equalsIgnoreCase("BodyLocation")) {
					Item.ClothingBodyLocation clothingBodyLocation = this.bodyLocation;
					this.bodyLocation = Item.ClothingBodyLocation.valueOf(string3.trim());
				} else {
					String[] stringArray2;
					int int1;
					if (string2.trim().equalsIgnoreCase("Palettes")) {
						...
						if (string2.trim().equalsIgnoreCase("Icon")) {
						...
						} else if (string2.trim().equalsIgnoreCase("ResizeWorldIcon")) {
							...
							this.ThistChange = Float.parseFloat(string3);
							...
							this.NumberOfPages = new Integer(string3.trim());
							...
							this.LvlSkillTrained = new Integer(string3.trim());
						} else if (string2.trim().equalsIgnoreCase("NumLevelsTrained")) {
							this.NumLevelsTrained = new Integer(string3.trim());
						} else if (string2.trim().equalsIgnoreCase("Capacity")) {
							this.Capacity = new Integer(string3.trim());
						} else if (string2.trim().equalsIgnoreCase("MaxCapacity")) {
							this.maxCapacity = new Integer(string3.trim());
						} else if (string2.trim().equalsIgnoreCase("ItemCapacity")) {
							this.itemCapacity = new Integer(string3.trim());
							...
							this.LightDistance = new Integer(string3.trim());
							...
							this.Trap = new Boolean(string3.trim());
							...
							this.FishingLure = new Boolean(string3.trim());
						} else if (string2.trim().equalsIgnoreCase("CanBeWrite")) {
							this.canBeWrite = new Boolean(string3.trim());
							...
							this.OnlyAcceptCategory = string3.trim();
							...
							this.twoWay = new Boolean(string3.trim());
							...
							this.isPortable = new Boolean(string3.trim());
						} else if (string2.trim().equalsIgnoreCase("IsTelevision")) {
							this.isTelevision = new Boolean(string3.trim());
							...
							this.usesBattery = new Boolean(string3.trim());
						} else if (string2.trim().equalsIgnoreCase("IsHighTier")) {
							this.isHighTier = new Boolean(string3.trim());
							...
							this.evolvedRecipeName = Translator.getDisplayItemName(string3.trim());
							...
							if (string2.trim().equalsIgnoreCase("TeachedRecipes")) {
								...
								for (int1 = 0; int1 < stringArray2.length; ++int1) {
									...
									if (!string6.contains(":")) {
										...
										if (!string7.contains("|")) {
											int2 = Integer.parseInt(string6.split(":")[1]);
										} else {
											String[] stringArray3 = string7.split("\\|");
											for (int int3 = 0; int3 < stringArray3.length; ++int3) {
												if ("Cooked".equals(stringArray3[int3])) {
													boolean1 = true;
												}
											}

											int2 = Integer.parseInt(stringArray3[0]);
										}
									}

									ItemRecipe itemRecipe = new ItemRecipe(this.name, this.module.getName(), int2);
