...
public class BodyPart {
	...
	public void DamageUpdate() {
		...
		if (this.getDeepWoundTime() > 0.0F) {
			if (this.bandaged()) {
				this.setDeepWoundTime(this.getDeepWoundTime() - (float)(5.0E-5 * (double)GameTime.getInstance().getMultiplier()));
				this.setBandageLife(this.getBandageLife() - (float)(1.0E-4 * (double)GameTime.getInstance().getMultiplier()));
				if (this.getPlantainFactor() > 0.0F) {
					this.setDeepWoundTime(this.getDeepWoundTime() - (float)(1.0E-5 * (double)GameTime.getInstance().getMultiplier()));
					this.setPlantainFactor(this.getPlantainFactor() - (float)(8.0E-4 * (double)GameTime.getInstance().getMultiplier()));
					if (this.getPlantainFactor() < 0.0F) {
				...
				this.setDeepWoundTime(this.getDeepWoundTime() - (float)(5.0E-6 * (double)GameTime.getInstance().getMultiplier()));
			}
	...
	public void setScratched(boolean boolean1) {
		...
		if (boolean1) {
			...
			if (float1 > 13.0F) {
				float float2 = Rand.Next(float1 * 0.2F, float1 * 0.5F);
				this.setBleedingTime(float2);
			}
	...
	public void SetScratchedWeapon(boolean boolean1) {
		...
		if (boolean1) {
			...
			this.setScratchTime(float1);
			float float2 = Rand.Next(this.scratchTime * 0.5F, this.scratchTime * 1.0F);
			switch (SandboxOptions.instance.InjurySeverity.getValue()) {
			case 1: 
				float2 *= 0.5F;
				break;
	...
	public void generateDeepWound() {
		float float1 = Rand.Next(7.0F, 12.0F);
		if (this.ParentChar.HasTrait("FastHealer")) {
			float1 = Rand.Next(3.0F, 7.0F);
		} else if (this.ParentChar.HasTrait("SlowHealer")) {
			float1 = Rand.Next(12.0F, 24.0F);
		}
		...
		float float2 = Rand.Next(this.getDeepWoundTime() * 0.7F, this.getDeepWoundTime());
		switch (SandboxOptions.instance.InjurySeverity.getValue()) {
		case 1: 
			float2 *= 0.5F;
			break;
	...
	public void SetScratchedWindow(boolean boolean1) {
		if (boolean1) {
			...
			if (Rand.Next(7) == 0) {
				...
				this.setScratchTime(float1);
				float float2 = Rand.Next(this.getScratchTime() * 0.2F, this.getScratchTime() * 0.5F);
				switch (SandboxOptions.instance.InjurySeverity.getValue()) {
				case 1: 
					float2 *= 0.5F;
					break;
	...
	public void damageFromFirearm(float float1) {
		...
		float float2 = Rand.Next(float1 / 2.0F, float1);
		switch (SandboxOptions.instance.InjurySeverity.getValue()) {
		case 1: 
			float2 *= 0.5F;
			break;
	...
	public void setHaveBullet(boolean boolean1, int int1) {
		if (this.haveBullet && !boolean1) {
			float float1 = Rand.Next(9.0F, 15.0F) - (float)(int1 / 2);
			if (this.ParentChar.HasTrait("FastHealer")) {
				float1 = Rand.Next(4.0F, 10.0F) - (float)(int1 / 2);
			} else if (this.ParentChar.HasTrait("SlowHealer")) {
				float1 = Rand.Next(14.0F, 20.0F) - (float)(int1 / 2);
			}
