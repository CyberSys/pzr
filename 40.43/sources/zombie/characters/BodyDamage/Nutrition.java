...
public class Nutrition {
	...
	private void updateCalories() {
		...
		if (this.parent.getCurrentState() == SwipeStatePlayer.instance() || this.parent.getCurrentState() == ClimbOverFenceState.instance() || this.parent.getCurrentState() == ClimbOverFenceState2.instance() || this.parent.getCurrentState() == ClimbThroughWindowState.instance() || this.parent.getCurrentState() == ClimbThroughWindowState2.instance()) {
			float1 = 8.0F;
		}

		float float2 = this.getWeight() / 80.0F;
		if (this.parent.IsRunning()) {
			float1 = 1.0F;
			this.setCalories(this.getCalories() - (this.isFemale ? this.caloriesDecreraseFemaleExercise : this.caloriesDecreaseMaleExercise) * float1 * float2 * GameTime.getInstance().getGameWorldSecondsSinceLastUpdate());
		} else if (this.parent.isAsleep()) {
			this.setCalories(this.getCalories() - (this.isFemale ? this.caloriesDecreraseFemaleSleeping : this.caloriesDecreaseMaleSleeping) * float1 * float2 * GameTime.getInstance().getGameWorldSecondsSinceLastUpdate());
		} else {
			this.setCalories(this.getCalories() - (this.isFemale ? this.caloriesDecreraseFemaleNormal : this.caloriesDecreaseMaleNormal) * float1 * float2 * GameTime.getInstance().getGameWorldSecondsSinceLastUpdate());
		}
	...
	public void applyWeightFromTraits() {
		if (this.parent.getTraits() != null && !this.parent.getTraits().isEmpty()) {
			if (this.parent.getTraits().contains("Emaciated")) {
				this.setWeight(50.0F);
			}

			if (this.parent.getTraits().contains("Very Underweight")) {
				this.setWeight(60.0F);
			}

			if (this.parent.getTraits().contains("Underweight")) {
				this.setWeight(70.0F);
			}

			if (this.parent.getTraits().contains("Overweight")) {
				this.setWeight(95.0F);
			}

			if (this.parent.getTraits().contains("Obese")) {
	...
	public boolean characterHaveWeightTrouble() {
		return this.parent.getTraits().contains("Emaciated") || this.parent.getTraits().contains("Obese") || this.parent.getTraits().contains("Very Underweight") || this.parent.getTraits().contains("Underweight") || this.parent.getTraits().contains("Overweight");
	}
	...
	public boolean canAddFitnessXp() {
		if (this.parent.getPerkLevel(PerkFactory.Perks.Fitness) >= 9 && this.characterHaveWeightTrouble()) {
			...
			return !this.parent.getTraits().contains("Emaciated") && !this.parent.getTraits().contains("Obese") && !this.parent.getTraits().contains("Very Underweight");
		}
	}
