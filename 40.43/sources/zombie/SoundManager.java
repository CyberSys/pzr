...
public class SoundManager extends BaseSoundManager {
	...
	public void resumeSoundAndMusic() {
		if (this.muted) {
			...
			javafmod.FMOD_Channel_SetPaused(FMODManager.instance.channelGroupInGameNonBankSounds, false);
			GameSounds.soundIsPaused = false;
		}
	}
	...
	private void debugScriptSound(Item item, String string) {
		if (string != null && !string.isEmpty()) {
			if (!GameSounds.isKnownSound(string)) {
				DebugLog.log("WARNING: no such sound \"" + string + "\" in item " + item.getFullName());
			}
		}
	}
	...
	public boolean isListenerInRange(float float1, float float2, float float3) {
		if (GameServer.bServer) {
			...
			for (int int1 = 0; int1 < IsoPlayer.numPlayers; ++int1) {
				...
				if (player != null && !player.HasTrait("Deaf") && IsoUtils.DistanceToSquared(player.x, player.y, float1, float2) < float3 * float3) {
					return true;
				}
			}
	...
	public Audio PlayWorldSound(String string, boolean boolean1, IsoGridSquare square, float float1, float float2, float float3, boolean boolean2) {
		if (IsoPlayer.DemoMode) {
			return null;
		} else if (!GameServer.bServer && square != null) {
			if (GameClient.bClient) {
		...
		public AmbientSoundEffect(String string) {
			...
			if (gameSound != null && !gameSound.clips.isEmpty()) {
				...
				if (gameSoundClip.getEvent() != null) {
					this.eventDescription = javafmod.FMOD_Studio_System_GetEvent("event:/" + gameSoundClip.getEvent());
