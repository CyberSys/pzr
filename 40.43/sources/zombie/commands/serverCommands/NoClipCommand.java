...
public class NoClipCommand extends CommandBase {
	...
	protected String Command() {
		...
		if (player != null) {
			...
			if (boolean2) {
				LoggerManager.getLogger("admin").write(this.getExecutorUsername() + " enabled noclip on " + string);
				return "User " + string + " won\'t collide.";
			} else {
				LoggerManager.getLogger("admin").write(this.getExecutorUsername() + " disabled noclip on " + string);
