			...
			if (this.airCounter < 0.0F) {
				...
				if (radioLine == null) {
					...
					if (ChatServer.isInited()) {
						chatMessage = ChatServer.getInstance().createRadiostationMessage(radioLine.getText(), this.frequency);
					} else {
						chatMessage = ChatManager.getInstance().createRadiostationMessage(radioLine.getText(), this.frequency);
					}

					ZomboidRadio.getInstance().SendTransmission(0, 0, this.frequency, chatMessage, radioLine.getEffectsString(), radioLine.getR(), radioLine.getG(), radioLine.getB(), -1, this.isTv);
