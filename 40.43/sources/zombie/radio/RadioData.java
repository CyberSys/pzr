...
public class RadioData {
	...
	private static ArrayList fetchRadioData(boolean boolean1) {
		return fetchRadioData(boolean1, true);
	}
	...
	private static ArrayList fetchRadioData(boolean boolean1, boolean boolean2) {
		...
		try {
			...
			searchForFiles(new File("media" + File.separator + "radio"), "xml", arrayList3);
			...
			while (true) {
				...
				arrayList3.clear();
				searchForFiles(new File("media" + File.separator + "radio"), "txt", arrayList3);
				if (boolean1) {
					for (int1 = 0; int1 < arrayList2.size(); ++int1) {
						string = ZomboidFileSystem.instance.getModDir((String)arrayList2.get(int1));
						if (string != null) {
	...
	public static ArrayList fetchAllRadioData() {
		...
		for (int int1 = arrayList.size() - 1; int1 >= 0; --int1) {
			...
			if (radioData.loadRadioScripts()) {
				System.out.println(" Adding" + (radioData.isVanilla ? " (vanilla)" : "") + " file: " + radioData.xmlFilePath);
				System.out.println(" - GUID: " + radioData.GUID);
				radioData.currentTranslation = null;
				radioData.translationDataList.clear();
			} else {
				System.out.println("[Failure] Failed to load radio scripts for GUID: " + radioData.GUID);
				System.out.println("		  File: " + radioData.xmlFilePath);
	...
	private static void searchForFiles(File file, String string, ArrayList arrayList) {
		if (file.isDirectory()) {
			...
			for (int int1 = 0; int1 < stringArray.length; ++int1) {
				searchForFiles(new File(file.getAbsolutePath() + File.separator + stringArray[int1]), string, arrayList);
			}
	...
	private static RadioData ReadFile(String string) {
		...
		try {
			System.out.println("Reading xml: " + string);
			...
			System.out.println("RadioData nodes len: " + nodeList.getLength());
			if (nodeList.getLength() > 0) {
				radioData.rootNode = nodeList.item(0);
				boolean1 = radioData.loadRootInfo();
				System.out.println("valid file: " + boolean1);
			}
		} catch (Exception exception) {
			exception.printStackTrace();
		}
