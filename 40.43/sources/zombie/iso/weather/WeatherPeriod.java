...
public class WeatherPeriod {
	...
	public static final int STAGE_MAX = 11;
	...
	private float maxTemperatureInfluence = 7.0F;
	...
	protected void init(ClimateManager.AirFront airFront, double double1, int int1, float float1) {
		...
		if (this.startInit(airFront, double1)) {
			if (int1 >= 0 && int1 < 11) {
				this.createSingleStage(int1, float1);
			} else {
	...
	private boolean startInit(ClimateManager.AirFront airFront, double double1) {
		if (!this.isRunning && !GameClient.bClient && !((double)airFront.getStrength() < 0.1)) {
	...
	private void createWeatherPattern() {
		...
		float float1 = this.climateManager.getDayMeanTemperature();
		...
		float float4 = 0.0F;
		float float5 = 1.0F;
		float float6 = Rand.Next(0.0F, 100.0F);
		...
		if (this.frontCache.getType() == 1) {
			...
			if (this.frontCache.getStrength() > 0.75F) {
				if (float3 > 0.0F && Rand.Next(0.0F, 100.0F) < float3) {
					this.print("WeatherPeriod: tropical storm triggered.");
					boolean3 = true;
				} else if (float4 > 0.0F && Rand.Next(0.0F, 100.0F) < float4) {
					this.print("WeatherPeriod: blizzard triggered.");
					boolean2 = true;
				}
			}

			if (!boolean2 && !boolean3 && this.frontCache.getStrength() > 0.5F && float2 > 0.0F && Rand.Next(0.0F, 100.0F) < float2) {
				this.print("WeatherPeriod: storm triggered.");
				boolean4 = true;
			}

			float float8 = Rand.Next(24.0F, 48.0F) * this.frontCache.getStrength();
			float float9 = 0.0F;
			if (boolean3) {
				arrayList.add(this.createStage(8, (double)(8.0F + Rand.Next(0.0F, 16.0F * this.frontCache.getStrength()))));
				if (Rand.Next(0.0F, 100.0F) < 60.0F * this.frontCache.getStrength()) {
					arrayList.add(this.createStage(3, (double)(5.0F + Rand.Next(0.0F, 5.0F * this.frontCache.getStrength()))));
				}

				if (Rand.Next(0.0F, 100.0F) < 30.0F * this.frontCache.getStrength()) {
					arrayList.add(this.createStage(3, (double)(5.0F + Rand.Next(0.0F, 5.0F * this.frontCache.getStrength()))));
				}
			} else if (boolean2) {
				arrayList.add(this.createStage(7, (double)(24.0F + Rand.Next(0.0F, 24.0F * this.frontCache.getStrength()))));
			} else if (boolean4) {
				arrayList.add(this.createStage(3, (double)(5.0F + Rand.Next(0.0F, 5.0F * this.frontCache.getStrength()))));
				if (Rand.Next(0.0F, 100.0F) < 70.0F * this.frontCache.getStrength()) {
					arrayList.add(this.createStage(3, (double)(4.0F + Rand.Next(0.0F, 4.0F * this.frontCache.getStrength()))));
				}

				if (Rand.Next(0.0F, 100.0F) < 50.0F * this.frontCache.getStrength()) {
					arrayList.add(this.createStage(3, (double)(4.0F + Rand.Next(0.0F, 4.0F * this.frontCache.getStrength()))));
				}

				if (Rand.Next(0.0F, 100.0F) < 25.0F * this.frontCache.getStrength()) {
					arrayList.add(this.createStage(3, (double)(4.0F + Rand.Next(0.0F, 3.0F * this.frontCache.getStrength()))));
				}

				if (Rand.Next(0.0F, 100.0F) < 12.5F * this.frontCache.getStrength()) {
					arrayList.add(this.createStage(3, (double)(4.0F + Rand.Next(0.0F, 2.0F * this.frontCache.getStrength()))));
				}
			}

			for (int int2 = 0; int2 < arrayList.size(); ++int2) {
				float9 = (float)((double)float9 + ((WeatherPeriod.WeatherStage)arrayList.get(int2)).getStageDuration());
			}

			while (float9 < float8) {
				switch (Rand.Next(0, 10)) {
				case 0: 
					weatherStage = this.createStage(5, (double)(1.0F + Rand.Next(0.0F, 3.0F * this.frontCache.getStrength())));
					break;
				...
				case 3: 
					weatherStage = this.createStage(1, (double)(2.0F + Rand.Next(0.0F, 4.0F * this.frontCache.getStrength())));
					break;
				
				default: 
					weatherStage = this.createStage(2, (double)(2.0F + Rand.Next(0.0F, 4.0F * this.frontCache.getStrength())));
				
				}

				float9 = (float)((double)float9 + weatherStage.getStageDuration());
				arrayList.add(weatherStage);
			}
		} else {
			this.print("WeatherPeriod: Cold to warm front selected.");
			if (this.cloudColor == this.cloudColorReddish) {
				float6 = Rand.Next(0.0F, 100.0F);
				if (float6 < 50.0F) {
					this.cloudColor = this.cloudColorBlueish;
				} else {
					this.cloudColor = this.cloudColorPurplish;
			...
			float7 = Rand.Next(12.0F, 24.0F) * this.frontCache.getStrength();
			float float10 = 0.0F;
			while (float10 < float7) {
				switch (Rand.Next(0, 10)) {
				case 0: 
					weatherStage = this.createStage(1, (double)(2.0F + Rand.Next(0.0F, 3.0F * this.frontCache.getStrength())));
					break;
				...
				case 4: 
					weatherStage = this.createStage(6, (double)(2.0F + Rand.Next(0.0F, 3.0F * this.frontCache.getStrength())));
					break;
				
				default: 
					weatherStage = this.createStage(5, (double)(2.0F + Rand.Next(0.0F, 3.0F * this.frontCache.getStrength())));
		...
		float7 = Rand.Next(30.0F, 60.0F);
		this.weatherStages.add(this.createStage(0, (double)(1.0F + Rand.Next(0.0F, 2.0F * this.frontCache.getStrength()))));
		int int3;
		for (int3 = 0; int3 < arrayList.size(); ++int3) {
			this.weatherStages.add(arrayList.get(int3));
			if (int3 < arrayList.size() - 1 && Rand.Next(0.0F, 100.0F) < float7) {
				this.weatherStages.add(this.createStage(4, (double)(1.0F + Rand.Next(0.0F, 2.0F * this.frontCache.getStrength()))));
				this.weatherStages.add(this.createStage(9, (double)(1.0F + Rand.Next(0.0F, 3.0F * this.frontCache.getStrength()))));
				float7 = Rand.Next(30.0F, 60.0F);
			}
		}

		if (((WeatherPeriod.WeatherStage)this.weatherStages.get(this.weatherStages.size() - 1)).getStageID() != 9) {
			this.weatherStages.add(this.createStage(4, (double)(2.0F + Rand.Next(0.0F, 3.0F * this.frontCache.getStrength()))));
		}
		...
		for (int3 = 0; int3 < this.weatherStages.size(); ++int3) {
			...
			weatherStage2.stageDuration = weatherStage2.stageDuration * (double)float5;
		}
		...
		case 3: 
			...
			if (!this.currentStage.hasStartedCloud) {
				...
				for (int int2 = 0; int2 < int1; ++int2) {
					this.thunderStorm.startThunderCloud(float4, float3, float5, float6, float7, double1, boolean1);
					boolean1 = false;
				}
	...
	public void update(double double1) {
		if (!GameClient.bClient) {
			if (this.isRunning && this.currentStage != null && this.weatherStageIndex >= 0 && this.weatherStages.size() != 0) {
				...
				float float1 = ClimateManager.clamp(-1.0F, 1.0F, this.currentStrength * 2.0F) * this.maxTemperatureInfluence;
				if (this.frontCache.getType() == 1) {
				...
				if (this.isRunning) {
					...
					this.climateManager.ambient.setOverride(0.0F, float2);
					this.climateManager.dayLightStrength.setOverride(0.0F, float2);
