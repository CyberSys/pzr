...
public class SafeHouse {
	...
	public static SafeHouse getSafeHouse(IsoGridSquare square) {
		return square.getBuilding() != null && square.getBuilding().def != null ? isSafeHouse(square, (String)null, false) : null;
	}
	...
	public void kickOutOfSafehouse(IsoPlayer player) {
		if (player.getAccessLevel().equals("None")) {
			GameClient.sendTeleport(player, (float)(this.x - 1), (float)(this.y - 1), 0.0F);
		}
	}
	...
	public void updateSafehouse(IsoPlayer player) {
		if (player == null || !this.getPlayers().contains(player.getUsername()) && !this.getOwner().equals(player.getUsername())) {
			if (ServerOptions.instance.SafeHouseRemovalTime.getValue() > 0 && Calendar.getInstance().getTimeInMillis() - this.getLastVisited() > (long)(3600000 * ServerOptions.instance.SafeHouseRemovalTime.getValue())) {
				...
				for (int int1 = this.getX(); int1 < this.getX2(); ++int1) {
					for (int int2 = this.getY(); int2 < this.getY2(); ++int2) {
						...
						if (square != null) {
							for (int int3 = 0; int3 < square.getMovingObjects().size(); ++int3) {
								...
								if (movingObject instanceof IsoPlayer && (this.getPlayers().contains(((IsoPlayer)movingObject).getUsername()) || this.getOwner().equals(((IsoPlayer)movingObject).getUsername()))) {
									boolean1 = true;
									break;
								}
							}
