...
public class IsoBuilding extends IsoArea {
	...
	public float getNeedsScore(SurvivorGroup survivorGroup) {
		...
		for (int int1 = 0; int1 < this.container.size(); ++int1) {
			ItemContainer itemContainer = (ItemContainer)this.container.get(int1);
			for (int int2 = 0; int2 < survivorGroup.GroupNeeds.size(); ++int2) {
				Need need = (Need)survivorGroup.GroupNeeds.get(int2);
				float1 += (float)(itemContainer.getNumItems(need.item) * 20);
			}
		}
	...
	public float ScoreBuildingGroupSpecific(SurvivorGroup survivorGroup) {
		float float1 = 0.0F;
		float1 += (float)(this.Rooms.size() * 5);
		float1 -= (float)(this.transparentWalls * 20);
		float1 -= (float)(this.Windows.size() * 10);
		int int1;
		for (int int2 = 0; int2 < this.container.size(); ++int2) {
			ItemContainer itemContainer = (ItemContainer)this.container.get(int2);
			for (int1 = 0; int1 < survivorGroup.GroupNeeds.size(); ++int1) {
				Need need = (Need)survivorGroup.GroupNeeds.get(int1);
				float1 += (float)(itemContainer.getNumItems(need.item) * 100);
			}
	...
	public void FillContainers() {
		...
		if (!boolean1 || !Core.GameMode.equals("KateBaldspot")) {
