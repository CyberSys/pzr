...
public class WorldStreamer {
	...
	private void threadLoop() throws DataFormatException, InterruptedException, IOException {
		...
		if (this.jobList.isEmpty()) {
			...
			for (int int1 = this.jobList.size() - 1; int1 >= 0; --int1) {
	...
	public void create() {
		if (this.worldStreamer == null) {
			if (!GameServer.bServer) {
				...
				this.worldStreamer = new Thread(new Runnable(){
					
					public void run() {
						while (!WorldStreamer.this.bFinished) {
							try {
								WorldStreamer.this.threadLoop();
	...
	private void compare(WorldStreamer.ChunkRequest chunkRequest, ByteBuffer byteBuffer, File file) throws IOException {
		...
		DebugLog.log("downloaded crc=" + this.crc32.getValue() + " on-disk crc=" + ChunkChecksum.getChecksumIfExists(chunkRequest.chunk.wx, chunkRequest.chunk.wy));
		chunk2.LoadFromDisk();
	...
	private void compareErosion(IsoChunk chunk, IsoChunk chunk2) {
		if (chunk.getErosionData().init != chunk2.getErosionData().init) {
			DebugLog.log("init " + chunk.getErosionData().init + " != " + chunk2.getErosionData().init);
		}

		if (chunk.getErosionData().eTickStamp != chunk2.getErosionData().eTickStamp) {
			DebugLog.log("eTickStamp " + chunk.getErosionData().eTickStamp + " != " + chunk2.getErosionData().eTickStamp);
		}

		if (chunk.getErosionData().moisture != chunk2.getErosionData().moisture) {
			DebugLog.log("moisture " + chunk.getErosionData().moisture + " != " + chunk2.getErosionData().moisture);
		}

		if (chunk.getErosionData().minerals != chunk2.getErosionData().minerals) {
			DebugLog.log("minerals " + chunk.getErosionData().minerals + " != " + chunk2.getErosionData().minerals);
		}

		if (chunk.getErosionData().epoch != chunk2.getErosionData().epoch) {
			DebugLog.log("epoch " + chunk.getErosionData().epoch + " != " + chunk2.getErosionData().epoch);
		}

		if (chunk.getErosionData().soil != chunk2.getErosionData().soil) {
			DebugLog.log("soil " + chunk.getErosionData().soil + " != " + chunk2.getErosionData().soil);
		}
	}
				...
				if (this.bb1.compareTo(this.bb2) != 0) {
					...
					if (this.bb1.limit() == this.bb2.limit()) {
						...
						for (int2 = 0; int2 < square.getErosionData().regions.size(); ++int2) {
							if (((ErosionCategory.Data)square.getErosionData().regions.get(int2)).dispSeason != ((ErosionCategory.Data)square2.getErosionData().regions.get(int2)).dispSeason) {
								DebugLog.log("season1=" + ((ErosionCategory.Data)square.getErosionData().regions.get(int2)).dispSeason + " season2=" + ((ErosionCategory.Data)square2.getErosionData().regions.get(int2)).dispSeason);
								boolean1 = false;
							}
						}
					}
					...
					if (square.getObjects().size() == square2.getObjects().size()) {
						for (int2 = 0; int2 < square.getObjects().size(); ++int2) {
							...
							this.bb2.flip();
							if (this.bb1.compareTo(this.bb2) != 0) {
								DebugLog.log("  1: " + object.getClass().getName() + " " + object.getName() + " " + (object.sprite == null ? "no sprite" : object.sprite.name));
								DebugLog.log("  2: " + object2.getClass().getName() + " " + object2.getName() + " " + (object2.sprite == null ? "no sprite" : object2.sprite.name));
							}
						}
					} else {
						for (int2 = 0; int2 < square.getObjects().size(); ++int2) {
							object = (IsoObject)square.getObjects().get(int2);
							DebugLog.log("  " + object.getClass().getName() + " " + object.getName() + " " + (object.sprite == null ? "no sprite" : object.sprite.name));
						}
					}
				}
			} catch (Exception exception) {
