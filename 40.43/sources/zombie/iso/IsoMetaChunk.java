...
public class IsoMetaChunk {
	...
	public float getZombieIntensity() {
		...
		if (Rand.Next(3) == 0) {
			...
			if ((float)Rand.Next(int1) < float4 && SandboxOptions.instance.Zombies.getValue() != 5) {
				float1 = 120.0F;
				if (float1 > 12.0F) {
					float1 = 12.0F;
				}
	...
	public float getLootZombieIntensity() {
		...
		return SandboxOptions.instance.Zombies.getValue() == 5 ? 400.0F : float1;
	}
	...
	public IsoMetaGrid.Zone getZoneAt(int int1, int int2, int int3) {
		if (this.zones == null) {
			...
			for (int int4 = 0; int4 < this.zonesSize; ++int4) {
				...
				if (zone.x <= int1 && zone.y <= int2 && int1 < zone.x + zone.w && int2 < zone.y + zone.h) {
					return zone;
				}
			}
