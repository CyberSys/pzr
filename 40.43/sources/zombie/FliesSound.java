...
public class FliesSound {
	public static FliesSound instance = new FliesSound();
				...
				if (chunk != null) {
					...
					if (chunkData != null) {
						...
						for (int int4 = 0; int4 < chunkLevelData.emitters.length; ++int4) {
							if (chunkLevelData.emitters[int4] != null && chunkLevelData.emitters[int4].emitter != null) {
								this.paintSquare(chunkLevelData.emitters[int4].sq.x, chunkLevelData.emitters[int4].sq.y, chunkLevelData.emitters[int4].sq.z, 0.0F, 1.0F, 0.0F, 1.0F);
							}
		...
		boolean update() {
			if (this.emitter == null) {
				...
				if (this.volume < this.targetVolume) {
					this.volume += 0.01F * (30.0F / (float)PerformanceSettings.LockFPS);
					if (this.volume >= this.targetVolume) {
	...
	public class ChunkData {
		...
		ChunkData(int int1, int int2, Object object) {
			this(int1, int2);
		}
	}
	...
	private class ChunkLevelData {
		...
		void update(int int1, int int2, int int3, IsoPlayer player) {
			...
			if ((double)BodyDamage.getSicknessFromCorpsesRate(int5) > ZomboidGlobals.FoodSicknessDecrease) {
				...
				float float1 = Math.min(0.7692308F * (0.1F + (float)int5 * 0.002F), 1.0F);
				if (fadeEmitter.emitter == null) {
					...
					if (fadeEmitter.targetVolume != float1 && !FliesSound.this.fadeEmitters.contains(fadeEmitter)) {
						FliesSound.this.fadeEmitters.add(fadeEmitter);
					}
				}

				fadeEmitter.targetVolume = float1;
