	...
	synchronized void UpdateVMClient() throws InterruptedException {
		...
		if (serverVOIPEnable) {
			if (IsoPlayer.instance != null) {
				IsoPlayer.instance.isSpeek = System.currentTimeMillis() - this.indicator_is_voice <= 300L;
			}
				...
				if (recBuf != null) {
					label183: while (true) {
						while (true) {
							...
							if (!is3D || !IsoPlayer.instance.isDead()) {
								if (this.isModePPT && GameKeyboard.isKeyDown(Core.getInstance().getKey("Enable voice transmit"))) {
									rakVoice = this.voice;
									gameClient = GameClient.instance;
									RakVoice.SendFrame(GameClient.connection.connectedGUID, (long)IsoPlayer.instance.OnlineID, recBuf.buf(), recBuf.get_size());
			...
			if (long1 >= (long)period) {
				...
				label148: for (int1 = 0; int1 < arrayList.size(); ++int1) {
						...
						while (true) {
							while (true) {
								...
								if (player2.invisible) {
									if (!IsoPlayer.instance.invisible) {
										continue;
									}

									javafmod.FMOD_Channel_Set3DAttributes(voiceManagerData2.userplaychannel, IsoPlayer.instance.x, IsoPlayer.instance.y, IsoPlayer.instance.z, 0.0F, 0.0F, 0.0F);
									break;
								}
							...
							if (this.isDebugLoopbackLong) {
								rakVoice = this.voice;
								gameClient = GameClient.instance;
								RakVoice.SendFrame(GameClient.connection.connectedGUID, (long)IsoPlayer.instance.OnlineID, this.buf, (long)this.buf.length);
							}
						}
					}
				}
