...
public class TextureFBO {
	...
	public TextureFBO(ITexture iTexture) throws Exception {
		try {
	...
	public TextureFBO(ITexture iTexture, boolean boolean1) throws Exception {
		try {
			RenderThread.borrowContext();
			TextureID.USE_MIPMAP = false;
			this.texture = iTexture;
	...
	public void swapTexture(ITexture iTexture) {
		if (iTexture != null && iTexture != this.texture) {
			if (iTexture.getWidth() == this.texture.getWidth() && iTexture.getHeight() == this.texture.getHeight()) {
				funcs.glFramebufferTexture2D(funcs.GL_FRAMEBUFFER(), funcs.GL_COLOR_ATTACHMENT0(), 3553, iTexture.getID(), 0);
				Util.checkGLError();
				this.texture = iTexture;
			}
		}
