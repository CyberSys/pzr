...
public class VehiclePart implements ChatElementOwner,WaveSignalDevice {
	...
	public void setRandomCondition(InventoryItem inventoryItem) {
		...
		if (this.getVehicle().isGoodCar()) {
			...
			if (vehicleType != null) {
				int1 = (int)((float)int1 * vehicleType.getBaseVehicleQuality());
			}
			...
			if (inventoryItem != null) {
				...
				if (int2 > 0 && Rand.Next(100) < int2) {
					float1 = (float)Rand.Next(int1 - int1 / 3, int1);
				}
			} else {
				int2 = 30;
				if (vehicleType != null) {
			...
			case 1: 
				float1 -= Rand.Next(float1 * 0.1F, float1 * 0.35F);
				break;
			
			case 2: 
				float1 -= Rand.Next(float1 * 0.05F, float1 * 0.15F);
	...
	public void save(ByteBuffer byteBuffer) throws IOException {
		...
		if (this.getItemContainer() == null) {
			...
			byteBuffer.put((byte)1);
			this.getItemContainer().save(byteBuffer, false);
		}
	...
	public void AddDeviceText(String string, float float1, float float2, float float3, String string2, int int1) {
		if (this.deviceData != null && this.deviceData.getIsTurnedOn()) {
			...
			if (this.deviceData.getDeviceVolume() > 0.0F) {
				...
				if (string2 != null) {
					LuaEventManager.triggerEvent("OnDeviceText", string2, this.getX(), this.getY(), this.getZ());
				}
			}
		}
	}
