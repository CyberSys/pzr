...
public class ChatManager {
	...
	public void init(boolean boolean1, IsoPlayer player) {
		LoggerManager.init();
		LoggerManager.createLogger("client chat " + player.getDisplayName(), Core.bDebug);
		logger = LoggerManager.getLogger("client chat " + player.getDisplayName());
	...
	public void showRadioMessage(String string) {
		...
		if (chatBase.getType() == ChatType.radio) {
			...
			if (chatBase.isEnabled()) {
				if (!this.isSinglePlayerMode() && !chatMessage.isLocal()) {
					...
					if (this.isPlayerCanUseRadioChat() && chatBase.isSendingToRadio()) {
						ChatMessage chatMessage2 = this.radioChat.createMessage(chatMessage.getText());
						chatMessage2.setRadioChannel(this.player.getEquipedRadio().getDeviceData().getChannel());
						this.radioChat.sendToServer(chatMessage2);
					}
				}
			} else {
	...
	private WhisperChat getWhisperChat(String string) throws TimeoutException {
		if (this.whisperChats.containsKey(string)) {
			...
			PacketTypes.doPacket((short)187, byteBufferWriter);
			byteBufferWriter.putUTF(this.myNickname);
			byteBufferWriter.putUTF(string);
			this.serverConnection.endPacketImmediate();
