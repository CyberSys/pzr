...
public class RadarPanel extends UIElement {
		...
		if (this.isVisible()) {
			if (IsoPlayer.players[this.playerIndex] != null) {
				if (!GameClient.bClient) {
					...
					this.zoom = 3.0F;
					this.stencilOn();
					SpriteRenderer.instance.render((Texture)null, this.offx, this.offy, (float)this.getWidth().intValue(), this.drawh, 0.0F, 0.2F, 0.0F, 0.66F);
					this.renderBuildings();
					this.renderRect(this.xPos - 0.5F, this.yPos - 0.5F, 1.0F, 1.0F, 1.0F, 1.0F, 1.0F, 1.0F);
					this.stencilOff();
					this.renderZombies();
					SpriteRenderer.instance.render(this.border, this.offx - 4.0F, this.offy - 4.0F, this.draww + 8.0F, this.drawh + 8.0F, 1.0F, 1.0F, 1.0F, 0.25F);
				}
			}
		}
	}
	...
	private void stencilOn() {
		...
		SpriteRenderer.instance.render(this.mask, (int)this.x, (int)this.y, (int)this.width, (int)this.height, 1.0F, 1.0F, 1.0F, 1.0F);
	...
	private void renderZombies() {
		...
		int int1;
		if (++this.zombiePosFrameCount >= PerformanceSettings.LockFPS / 5) {
		...
		for (int1 = 0; int1 < this.zombiePosOccupied.length; ++int1) {
			if (this.zombiePosOccupied[int1]) {
				...
				SpriteRenderer.instance.render((Texture)null, float1 + (float3 + 1.0F) * (float)Math.cos(double5) - float4, float2 + (float3 + 1.0F) * (float)Math.sin(double5) - float4, 1.0F * this.zoom, 1.0F * this.zoom, 1.0F, 1.0F, 0.0F, 1.0F);
			}
		}
	}
