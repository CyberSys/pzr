		...
		if (this.isVisible()) {
			...
			if (!this.slices.isEmpty()) {
				...
				for (int int1 = 0; int1 < 48; ++int1) {
					...
					float8 = Math.min(float8 * UIManager.FBO_ALPHA_MULT, 1.0F);
					SpriteRenderer.instance.renderPoly((int)double10, (int)double1, (int)double4, (int)double5, (int)double6, (int)double7, (int)double2, (int)double3, float5, float6, float7, float8);
				}
				...
				int int3;
				int int4;
				for (int int5 = 0; int5 < this.slices.size(); ++int5) {
					int4 = Math.max(6, 48 / Math.max(this.slices.size(), 2));
					for (int3 = 0; int3 < int4; ++int3) {
						double1 = Math.toRadians((double)(float10 + (float)int5 * float9 + (float)int3 * float9 / (float)int4 + (int3 == 0 ? float4 : 0.0F)));
						double2 = Math.toRadians((double)(float10 + (float)int5 * float9 + (float)(int3 + 1) * float9 / (float)int4 - (int3 == int4 - 1 ? float4 : 0.0F)));
						double3 = Math.toRadians((double)(float10 + (float)int5 * float9 + (float)int3 * float9 / (float)int4 + (int3 == 0 ? float4 / 2.0F : 0.0F)));
						double4 = Math.toRadians((double)(float10 + (float)int5 * float9 + (float)(int3 + 1) * float9 / (float)int4) - (int3 == int4 - 1 ? (double)float4 / 1.5 : 0.0));
						...
						} else if (int5 == this.deselectIndex) {
						...
						if (UIManager.useUIFBO) {
							...
							float11 = 0.5F;
							if (int5 == int2) {
								float14 = 0.7F + 0.25F * this.select.fraction();
							} else if (int5 == this.deselectIndex) {
								float14 = 0.025F + 0.675F * this.deselect.fraction();
							}
						}

						SpriteRenderer.instance.renderPoly((int)double5, (int)double6, (int)double12, (int)double13, (int)double14, (int)double15, (int)double7, (int)double11, float11, float12, float13, float14);
					}

					Texture texture = ((RadialMenu.Slice)this.slices.get(int5)).texture;
					if (texture != null) {
						double1 = Math.toRadians((double)(float10 + (float)int5 * float9 + float9 / 2.0F));
				...
				if (slice != null) {
					String string = slice.text;
					int4 = TextManager.instance.MeasureStringX(UIFont.Medium, string);
					...
					int3 *= int6;
					this.DrawTextCentre(UIFont.Medium, string, (double)(this.width / 2.0F), (double)(this.height / 2.0F - (float)(int3 / 2)), 1.0, 1.0, 1.0, 1.0);
				}
			}
		}
	}
