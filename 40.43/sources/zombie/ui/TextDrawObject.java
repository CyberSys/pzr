...
public class TextDrawObject {
	...
	public TextDrawObject(int int1, int int2, int int3, boolean boolean1, boolean boolean2, boolean boolean3, boolean boolean4, boolean boolean5, boolean boolean6) {
		this.validImages = new String[]{"Icon_music_notes", "media/ui/CarKey.png"};
	...
	private void addText(String string) {
		...
		drawLine.charW = drawLine.charW + string.length();
	}
	...
	public void calculateDimensions() {
		...
		while (iterator.hasNext()) {
			drawLine = (TextDrawObject.DrawLine)iterator.next();
			drawLine.h = 0;
			drawLine.w = 0;
			Iterator iterator2 = drawLine.elements.iterator();
			while (iterator2.hasNext()) {
				...
				drawLine.w = drawLine.w + drawElement.w;
		...
		if (this.equalizeLineHeights) {
			this.height = 0;
			for (iterator = this.lines.iterator(); iterator.hasNext(); this.height += int1) {
				drawLine = (TextDrawObject.DrawLine)iterator.next();
				drawLine.h = int1;
			}
		}
	...
	public void DrawRaw(TextDrawHorizontal textDrawHorizontal, double double1, double double2, float float1, float float2, float float3, float float4, boolean boolean1) {
		...
		double double5 = 0.0;
		int int1 = Core.getInstance().getOffscreenTrueWidth();
		int int2 = Core.getInstance().getOffscreenTrueWidth();
		byte byte1 = 20;
		...
		if (!(double3 - (double)byte1 >= (double)int1) && !(double3 + (double)this.getWidth() + (double)byte1 <= 0.0) && !(double2 - (double)byte1 >= (double)int2) && !(double2 + (double)this.getHeight() + (double)byte1 <= 0.0)) {
			...
			TextDrawObject.DrawLine drawLine;
			for (Iterator iterator = this.lines.iterator(); iterator.hasNext(); double4 += (double)drawLine.h) {
				drawLine = (TextDrawObject.DrawLine)iterator.next();
				double3 = double1;
				if (textDrawHorizontal == TextDrawHorizontal.Center) {
				...
				TextDrawObject.DrawElement drawElement;
				for (Iterator iterator2 = drawLine.elements.iterator(); iterator2.hasNext(); double3 += (double)drawElement.w) {
					drawElement = (TextDrawObject.DrawElement)iterator2.next();
					...
					if (drawElement.isImage && drawElement.tex != null) {
						if (boolean1 && drawElement.isTextImage) {
							SpriteRenderer.instance.render(drawElement.tex, (int)(double3 - 1.0), (int)(double4 + double5 - 1.0), drawElement.w, drawElement.h, this.outlineR, this.outlineG, this.outlineB, float5);
							SpriteRenderer.instance.render(drawElement.tex, (int)(double3 + 1.0), (int)(double4 + double5 + 1.0), drawElement.w, drawElement.h, this.outlineR, this.outlineG, this.outlineB, float5);
							SpriteRenderer.instance.render(drawElement.tex, (int)(double3 - 1.0), (int)(double4 + double5 + 1.0), drawElement.w, drawElement.h, this.outlineR, this.outlineG, this.outlineB, float5);
							SpriteRenderer.instance.render(drawElement.tex, (int)(double3 + 1.0), (int)(double4 + double5 - 1.0), drawElement.w, drawElement.h, this.outlineR, this.outlineG, this.outlineB, float5);
						}
						...
						if (drawElement.useColor) {
							SpriteRenderer.instance.render(drawElement.tex, (int)double3, (int)(double4 + double5), drawElement.w, drawElement.h, drawElement.R, drawElement.G, drawElement.B, float4);
						} else if (drawElement.isTextImage) {
							SpriteRenderer.instance.render(drawElement.tex, (int)double3, (int)(double4 + double5), drawElement.w, drawElement.h, float1, float2, float3, float4);
						} else {
							SpriteRenderer.instance.render(drawElement.tex, (int)double3, (int)(double4 + double5), drawElement.w, drawElement.h, 1.0F, 1.0F, 1.0F, float4);
						}
					} else if (drawElement.useFont && drawElement.font != null) {
						if (boolean1) {
	...
	private class DrawElement {
			...
			if (float1 != this.currentScrambleVal) {
				...
				for (int int3 = 0; int3 < int2; ++int3) {
					...
					if (int4 > int1) {
						...
						this.scrambleText = this.scrambleText + new String(charArray) + " ";
					}
				}
			}
		}
		...
		private TextDrawObject.DrawElement softclone() {
			TextDrawObject.DrawElement drawElement = TextDrawObject.this.new DrawElement();
			if (this.useColor) {
