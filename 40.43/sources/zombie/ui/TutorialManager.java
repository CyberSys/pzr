...
public class TutorialManager {
	...
	public TutorialManager() {
		...
		this.radio = null;
		this.SpottedGunNut = false;
		this.bDoneWifeDeath = false;
		this.bDoneWifeDeathMourn = false;
	}
	...
	public void TutorialMessage(String string, String string2) {
		if (string == null && string2 == null) {
			UIManager.getTutorial().hide();
		} else {
			UIManager.getTutorial().ShowMessage(string, string2);
		}
	}

	public void update() {
		if (!FrameLoader.bDedicated) {
			if (!GameServer.bServer && IsoPlayer.getInstance().getCurrentSquare() != null) {
			}
		}
	}
