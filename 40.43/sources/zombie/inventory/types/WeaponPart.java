...
public class WeaponPart extends InventoryItem {
	...
	private ArrayList mountOn = new ArrayList();
	...
	public void DoTooltip(ObjectTooltip objectTooltip, ObjectTooltip.Layout layout) {
		...
		layoutItem.setValue(this.partType, 1.0F, 1.0F, 0.8F, 1.0F);
		layoutItem = layout.addItem();
		String string = Translator.getText("Tooltip_weapon_CanBeMountOn") + this.mountOn.toString().replaceAll("\\[", "").replaceAll("\\]", "");
	...
	public void setMountOn(ArrayList arrayList) {
		...
		for (int int1 = 0; int1 < arrayList.size(); ++int1) {
			...
			if (item != null) {
				this.mountOn.add(item.getDisplayName());
			}
		}
	}
	...
	public void setPartType(String string) {
		this.partType = Translator.getText("Tooltip_weapon_" + string);
	}
