...
public class AlarmClock extends InventoryItem {
	...
	public AlarmClock(String string, String string2, String string3, String string4) {
		super(string, string2, string3, string4);
		this.cat = ItemType.AlarmClock;
		this.randomizeAlarm();
	}

	public AlarmClock(String string, String string2, String string3, Item item) {
			...
			if (this.isRinging()) {
				...
				if (square != null && !(this.ringSince + 0.5 < double1)) {
					if (!GameClient.bClient && square != null) {
						WorldSoundManager.instance.addSound((IsoObject)null, square.getX(), square.getY(), square.getZ(), this.getSoundRadius(), 3);
					}
				} else {
					this.stopRinging();
				}
	...
	public void updateSound(BaseSoundEmitter baseSoundEmitter) {
		...
		if (square != null) {
			...
			if (GameClient.bClient && sendEvery.Check() && this.isInLocalPlayerInventory()) {
				GameClient.instance.sendWorldSound((IsoObject)null, square.x, square.y, square.z, this.getSoundRadius(), 3, false, 0.0F, 1.0F);
			}
	...
	private void wakeUpPlayers(IsoGridSquare square) {
		if (!GameServer.bServer) {
			...
			for (int int4 = 0; int4 < IsoPlayer.numPlayers; ++int4) {
				...
				if (player != null && !player.isDead() && player.getCurrentSquare() != null && !player.HasTrait("Deaf")) {
	...
	public void syncAlarmClock_Player(IsoPlayer player) {
		if (GameClient.bClient) {
			...
			byteBufferWriter.putLong(this.id);
