	...
	public IsoObject createObject(IsoGridSquare square, int int1, boolean boolean1, int int2) {
		...
		if (boolean1) {
			if (CellLoader.isoTreeCache.isEmpty()) {
				object = new IsoTree(square, string);
			} else {
				object = (IsoObject)CellLoader.isoTreeCache.pop();
				((IsoObject)object).sprite = (IsoSprite)square.getCell().SpriteManager.NamedMap.get(string);
