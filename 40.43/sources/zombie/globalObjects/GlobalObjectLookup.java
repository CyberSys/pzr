...
public final class GlobalObjectLookup {
	...
	private static GlobalObjectLookup.Cell[] cells;
	private final SGlobalObjectSystem system;

	public GlobalObjectLookup(SGlobalObjectSystem sGlobalObjectSystem) {
	...
	private GlobalObjectLookup.Cell getCellAt(int int1, int int2, boolean boolean1) {
		...
		if (int3 >= 0 && int4 >= 0 && int3 < metaGrid.getWidth() * 300 && int4 < metaGrid.getHeight() * 300) {
			...
			if (cells[int7] == null && boolean1) {
				cells[int7] = new GlobalObjectLookup.Cell(metaGrid.minX + int5, metaGrid.minY + int6);
			}
	...
	public static void init(IsoMetaGrid metaGrid) {
		metaGrid = metaGrid;
		cells = new GlobalObjectLookup.Cell[metaGrid.getWidth() * metaGrid.getHeight()];
	}

	public static void Reset() {
		for (int int1 = 0; int1 < cells.length; ++int1) {
			GlobalObjectLookup.Cell cell = cells[int1];
			if (cell != null) {
				cell.Reset();
			}
		}
