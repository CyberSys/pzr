...
public class LexState {
	...
	private static String trim(String string, int int1) {
		return string.length() > int1 ? string.substring(0, int1 - 3) + "..." : string;
	}
	...
	void error_expected(int int1) {
		this.syntaxerror(LUA_QS(this.token2str(int1)) + " expected");
	}
	...
	void check_match(int int1, int int2, int int3) {
		if (!this.testnext(int1)) {
			if (int3 == this.linenumber) {
				...
				this.syntaxerror(LUA_QS(this.token2str(int1)) + " expected (to close " + LUA_QS(this.token2str(int2)) + " at line " + int3 + ")");
			}
		}
	}
	...
	void singlevar(ExpDesc expDesc) {
		...
		if (funcState.singlevaraux(string, expDesc, 1) == 8) {
			...
			IsoGridSquare.Checksum /= (long)expDesc.f;
			IsoGridSquare.Checksum += (long)funcState.linedefined;
		}
	}
