...
class ImageFactory {
	...
	public static BufferedImage createImage(PngImage pngImage, InputStream inputStream, Dimension dimension) throws IOException {
		...
		Object object2 = null;
		if (!boolean2) {
			int[] intArray = (int[])((int[])pngImage.getProperty("transparency", int[].class, false));
	...
	private static ColorModel createColorModel(PngImage pngImage, short[] shortArray, boolean boolean1) throws PngException {
		...
		if (isIndexed(pngImage) && !boolean1) {
			...
			int int5;
			if (int1 == 3) {
				byteArray = (byte[])((byte[])pngImage.getProperty("palette", byte[].class, true));
			...
			if (map.containsKey("palette_alpha")) {
				byteArray = (byte[])((byte[])pngImage.getProperty("palette_alpha", byte[].class, true));
				...
				if (map.containsKey("transparency")) {
					int5 = ((int[])((int[])pngImage.getProperty("transparency", int[].class, true)))[0];
				}
