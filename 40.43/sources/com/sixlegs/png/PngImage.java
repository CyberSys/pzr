...
public class PngImage implements Transparency {
				...
				for (HashSet hashSet = new HashSet(); stateMachine.getState() != 6; pngInputStream.endChunk(int2)) {
					...
					try {
						...
						if (!this.isMultipleOK(int2) && !hashSet.add(Integers.valueOf(int2))) {
							throw new PngException("Multiple " + PngConstants.getChunkName(int2) + " chunks are not allowed", !PngConstants.isAncillary(int2));
						}
						...
						try {
							...
							throw new PngException("Malformed " + PngConstants.getChunkName(int2) + " chunk", ioException, !PngConstants.isAncillary(int2));
						}
	...
	public Color getBackground() {
		int[] intArray = (int[])((int[])this.getProperty("background_rgb", int[].class, false));
		if (intArray == null) {
			return null;
		} else {
			switch (this.getColorType()) {
			...
			case 3: 
				byte[] byteArray = (byte[])((byte[])this.getProperty("palette", byte[].class, true));
