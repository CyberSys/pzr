...
xpUpdate.onPlayerMove = function()
	...
	if player:IsRunning() and player:getStats():getEndurance() > player:getStats():getEndurancewarn() then
		-- you may gain 1 xp in sprinting or pacing
	...
	-- sneaking, if nobody saw you, you can gain sneaking xp
	if player:IsSneaking() and not player:isbCouldBeSeenThisFrame() and not player:isbSeenThisFrame() and xpUpdate.randXp() then
...
xpUpdate.onWeaponSwingHitPoint = function(owner, weapon)
...
xpUpdate.onNewGame = function(playerObj, square)
    ...
    if getWorld():getDifficulty() == "Easy" then
        local bag =  playerObj:getInventory():FindAndReturn("Base.Schoolbag");
        if not bag then
            bag = playerObj:getInventory():AddItem("Base.Schoolbag");
            playerObj:getInventory():AddItem("Base.WaterBottleFull");
...
Events.OnWeaponSwingHitPoint.Add(xpUpdate.onWeaponSwingHitPoint);
