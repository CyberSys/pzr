...
function Vehicles.Create.Engine(vehicle, part)
	...
	vehicle:setEngineFeature(ZombRand(engineQuality - 5, engineQuality + 5), engineLoudness , enginePower);
end
...
function Vehicles.Update.Engine(vehicle, part, elapsedMinutes)
	...
	if not part:getModData().temperature then
		part:getModData().temperature = 0
	end
	local previousTemp = part:getModData().temperature;
	if vehicle:isEngineRunning() then
		...
		part:getModData().temperature = math.min(part:getModData().temperature + increment * elapsedMinutes, max)
	elseif part:getModData().temperature > 0 then
		part:getModData().temperature = math.max(part:getModData().temperature - 2 * elapsedMinutes, 0)
	end
	Vehicles.elaspedMinutesForEngine[vehicle:getId()] = Vehicles.elaspedMinutesForEngine[vehicle:getId()] + elapsedMinutes;
	if isServer() and VehicleUtils.compareFloats(previousTemp, part:getModData().temperature, 2) and Vehicles.elaspedMinutesForEngine[vehicle:getId()] > 2 then
		Vehicles.elaspedMinutesForEngine[vehicle:getId()] = 0;
		vehicle:transmitPartModData(part);
	end
	-- stop updating parts when T° reach 0
	if part:getModData().temperature <= 0 and not vehicle:isEngineRunning() and not vehicle:getDriver() then
...
function Vehicles.Update.PassengerCompartment(vehicle, part, elapsedMinutes)
	...
	if not pc:getModData().windowtemperature then pc:getModData().windowtemperature = 0.0; end
	if not pc:getModData().temperature then pc:getModData().temperature = 0.0; end
	if vehicle:windowsOpen() > 0 then
		pc:getModData().temperature = math.max(pc:getModData().temperature - (0.1*vehicle:windowsOpen()) * elapsedMinutes, -5);
		pc:getModData().windowtemperature = math.max(pc:getModData().windowtemperature - (0.1*vehicle:windowsOpen()) * elapsedMinutes, 0);
	else
		if not heater:getModData().active then
			pc:getModData().windowtemperature = math.min(pc:getModData().windowtemperature + 0.1 * elapsedMinutes, 5);
		else
			pc:getModData().windowtemperature = math.max(pc:getModData().windowtemperature - 0.1 * elapsedMinutes, 0);
		end
	end
...
function VehicleUtils.testRecipes(chr, recipes)
	...
	for _,recipe in ipairs(recipes:split(";")) do
		if not chr:getKnownRecipes():contains(recipe) then return false end
	end
	return true
end
...
--		if not part:isSpecificItem() then
			local itemType = v:getChoosenParts():get(part:getItemType());
			...
			if not itemType then
				for i=0, part:getItemType():size() - 1 do
					if ZombRand(100) > v:getEngineQuality() or i == part:getItemType():size() - 1 then
						...
						v:getChoosenParts():put(part:getItemType(), itemType);
						break;
					end
				end
			end
...
--	end
	if perks and perks ~= "" then
		for _,perk in ipairs(perks:split(";")) do
			...
			if perkLvl < tonumber(level) then
				success = success - (20 + tonumber((tonumber(level) - perkLvl)) * 15);
				failure = failure + (20 + tonumber((tonumber(level) - perkLvl) * 15));
			end
		end
	end
	return tonumber(success), tonumber(failure);
end
