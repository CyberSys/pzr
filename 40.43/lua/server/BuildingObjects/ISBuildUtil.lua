...
function buildUtil.consumeMaterial(ISItem)
	...
	for index, value in pairs(modData) do
		if luautils.stringStarts(index, "need:") then
			...
			local itemType = luautils.split(itemToConsume, "%.")[2]
			...
			if itemCount > 0 then
				...
				for x=ISItem.sq:getX()-1,ISItem.sq:getX()+1 do
					for y=ISItem.sq:getY()-1,ISItem.sq:getY()+1 do
						local square = getCell():getGridSquare(x,y,ISItem.sq:getZ());
						...
						if wobs ~= nil then
							...
							for m=0, wobs:size()-1 do
								local o = wobs:get(m);
								if instanceof(o, "IsoWorldInventoryObject") and o:getItem():getType() == itemType then
									if not instanceof(o:getItem(), "InventoryContainer") or o:getItem():getInventory():getItems():isEmpty() then
							...
							for i,v in pairs(itemToRemove) do
								table.insert(consumedItems, v:getItem())
