...
function ISDestroyCursor:create(x, y, z, north, sprite)
	...
	local doIt = false
	if instanceof(destroy, "IsoWindow") or instanceof(destroy, "IsoDoor") or self:_isDoorFrame(destroy) or self:_isWall(destroy) then
		if luautils.walkAdjWindowOrDoor(player, sq, destroy, true) then
			doIt = true
		end
	elseif luautils.walkAdj(player, sq, true) then
		doIt = true
	end
	if doIt then
		if self.dismantle then
			...
			ISWorldObjectContextMenu.equip(player, player:getPrimaryHandItem(), "Sledgehammer", true)
...
function ISDestroyCursor:new(character, dismantle)
	local o = {}
	setmetatable(o, self)
	self.__index = self
	o:init()
