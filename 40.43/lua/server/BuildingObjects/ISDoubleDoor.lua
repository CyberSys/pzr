...
function ISDoubleDoor:addDoorPart(x,y,z,north,sprite,index)
	...
	self.modData = {}
	self.modData["xp:Woodwork"] = 6
	self.modData["need:Base.Plank"] = "3"
	self.modData["need:Base.Nails"] = "3"
	if index == 1 or index == 4 then
		self.modData["need:Base.Hinge"] = "2";
	else
		self.modData["need:Base.Doorknob"] = "1";
	end
...
function ISDoubleDoor:isValid(square)
	...
	if not self:partExists(square, 1) and not square:isFreeOrMidair(true, true) then
		return false
	end
	...
	-- don't allow dooble doors inside buildings to avoid weird conflicts
