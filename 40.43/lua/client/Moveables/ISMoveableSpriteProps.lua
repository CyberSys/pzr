...
function ISMoveableSpriteProps:instanceItem(_spriteNameOverride)
	if self.isMoveable then
		...
		if item and item:ReadFromWorldSprite(spriteName) then
    ...
    if tool and _player then
        ...
        if toolDef then
            for _,v in ipairs(toolDef.items) do
                local item = inventory:FindAndReturn(v);
                if item then
                    return item;
                end
            end
...
function ISMoveableSpriteProps:getInfoPanelDescription( _square, _object, _player, _mode )
    ...
    elseif _mode ~= "scrap" and not self.isMoveable then
        ...
        if InfoPanelFlags.mustPlaceRoomRoof then infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, "- "..getText("IGUI_NeedToBeStandingInside"), 255, 0, 0 ); end
        --##########################################
        return infoTable;
    end
end
...
function ISMoveableSpriteProps:getInfoPanelFlagsGeneral( _square, _object, _player, _mode )
    ...
    InfoPanelFlags.weight = tostring(self.weight);
    if _mode and _mode == "rotate" then
        InfoPanelFlags.canRotate = self:hasFaces();
    end
...
--[[
function ISMoveableSpriteProps:getInfoPanelDescriptionOLD2( _square, _object, _player, _mode )
    local infoTable = {};
    if not self.isMoveable then
        return ISMoveableSpriteProps.addLineToInfoTable( infoTable, getText("IGUI_NoMovable"), 255, 0, 0 );
    else
        if ISMoveableSpriteProps.debug then
            infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, "DEBUG", 128, 128, 128 );
        ...
        infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, getText("IGUI_Name")..":", 255, 255, 255, self.name, 0, 255, 0 );
        infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, getText("Tooltip_item_Weight")..":", 255, 255, 255, tostring(self.weight), 0, 255, 0 );

        if _mode and _mode == "rotate" then
            if self:hasFaces() then
                if _object and instanceof(_object,"IsoObject") and not self:objectNoContainerOrEmpty( _object ) then
                    return ISMoveableSpriteProps.addLineToInfoTable( infoTable, "- "..getText("IGUI_ItemsInContainer"), 255, 0, 0 );
                end
                infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, getText("IGUI_CanRotate"), 0, 255, 0 );
            else
                infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, getText("IGUI_CanNotRotate"), 255, 0, 0 );
            end
            return infoTable;
        end

        local hasSkill, nameSkill = self:hasRequiredSkill( _player, _mode );
        if nameSkill then
            infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, getText("IGUI_Skill")..":", 255, 255, 255, nameSkill, getColorValues(hasSkill) );
        end

        local tool = (_mode and _mode =="pickup") and self.pickUpTool or self.placeTool;
        if tool then
            if ISMoveableDefinitions:getInstance().getToolDefinition( tool ) then
                local toolDef = ISMoveableDefinitions:getInstance().getToolDefinition( tool );
                local str = "";
                local toolpref = getText("IGUI_Tool")..": ";
                local i = 0;
                for k,v in ipairs(toolDef.items) do
                    str = str .. toolDef.itemNames[v];
                    if i<2 and k<#toolDef.items then
                        str = str..", ";
                    end

                    i = i+1;
                    if i>=3 then
                        infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, toolpref, 255, 255, 255, str, getColorValues(self:hasTool( _player, (_mode and _mode ~= "pickup") )) );
                        i=0;
                        str="";
                        toolpref = "";
                    end
                ...
                infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, getText("IGUI_Tool")..":", 255, 255, 255, tool, getColorValues(self:hasTool( _player, (_mode and _mode ~= "pickup") )) );
            end
        else
            infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, getText("IGUI_Tool")..":", 255, 255, 255, getText("IGUI_None"), getColorValues(true) );
        end
        ...
        if _mode and _mode =="pickup" then
            ...
            if breakChance > 0 then
                ...
                if breakChance > 75 then
                    r,g,b = 255,0,0;
                elseif breakChance > 25 then
                    r,g,b = 255,255,0;
                end
                infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, getText("IGUI_ChanceToBreak")..":", 255, 255, 255, tostring(breakChance), r, g, b );
            end
            if not _player:getInventory():hasRoomFor(_player, self.weight) then
                infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, "- "..getText("IGUI_ToHeavy"), 255, 0, 0 );
            end
            if self.isTable and _square:Is("IsTableTop") then
                infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, "- "..getText("IGUI_ItemsSurface"), 255, 0, 0 );
            elseif self.isTable and _object and _object ~= self:getTopTable(_square) then
                infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, "- "..getText("IGUI_ItemsSurface"), 255, 0, 0 );
            end
            if self.isWaterCollector then
                if _object and _object:hasWater() then
                    infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, "- "..getText("IGUI_RainCollectorHasWater"), 255, 0, 0 );
                end
            end
            if self.type == "Window" then
                if _object and instanceof(_object,"IsoWindow") and _object:IsOpen() then
                    infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, "- "..getText("IGUI_WindowOpen"), 255, 0, 0 );
                end
                if _object and instanceof(_object,"IsoWindow") and _object:isBarricaded() then
                    infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, "- "..getText("IGUI_WindowBarricaded"), 255, 0, 0 );
                end
            end
            if _object and instanceof(_object,"IsoObject") and not self:objectNoContainerOrEmpty( _object ) then
                infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, "- "..getText("IGUI_ItemsInContainer"), 255, 0, 0 );
            ...
            if self.type=="WindowObject" then
                if _player:getSquare() and _player:getSquare():Is(IsoFlagType.exterior) then
                    infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, "- "..getText("IGUI_NeedToBeStandingInside"), 255, 0, 0 );
                end
            end
        end
        if _mode and _mode =="place" then
            if self.type=="WindowObject" then
                if _square:Is(IsoFlagType.exterior) then
                    infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, "- "..getText("IGUI_MustPlaceRoomRoof"), 255, 0, 0 );
                end
                if _player:getSquare() and _player:getSquare():Is(IsoFlagType.exterior) then
                    infoTable = ISMoveableSpriteProps.addLineToInfoTable( infoTable, "- "..getText("IGUI_NeedToBeStandingInside"), 255, 0, 0 );
                end
...
--[[
function ISMoveableSpriteProps:getInfoPanelDescriptionOLD( _square, _object, _player, _mode )
    --<RGB:1,0,0>
    if not self.isMoveable then
        return "[col=255,0,0]Not a movable[/]";
    else
        local description = "[col=255,255,255]Name:[/] [col=0,255,0]" .. self.name .. "[/][br/]";
        description = description .. "[col=255,255,255]Weight:[/] [col=0,255,0]" .. tostring(self.weight) .. "[/][br/]";

        if _mode and _mode == "rotate" then
            if self:hasFaces() then
                if _object and instanceof(_object,"IsoObject") and not self:objectNoContainerOrEmpty( _object ) then
                    description = description .. "[col=255,0,0]- Has items in container[/][br/]";
                    return description;
                end
                description = description .. "[col=0,255,0]Can rotate[/][br/]";
            else
                description = description .. "[col=255,0,0]Can not rotate[/][br/]";
            end
            return description;
        end
        ...
        local hasSkill, nameSkill = self:hasRequiredSkill( _player, _mode );
        if nameSkill then
            local color = hasSkill and "[col=0,255,0]" or "[col=255,0,0]";
            description = description .. "[col=255,255,255]Skill:[/] " .. color .. nameSkill .. "[/][br/]";
        end
        local tool = (_mode and _mode =="pickup") and self.pickUpTool or self.placeTool;
        if tool then
            local color = self:hasTool( _player, (_mode and _mode ~= "pickup") ) and "[col=0,255,0]" or "[col=255,0,0]";
            description = description .. "[col=255,255,255]Tool:[/] " .. color .. tool .. "[/][br/]";
        else
            description = description .. "[col=255,255,255]Tool:[/] [col=0,255,0]None[/][br/]";
        end
        if _mode and _mode =="pickup" then
            local breakChance = self:getBreakChance( _player );
            if breakChance > 0 then
                local color = breakChance > 75 and "[col=255,0,0]" or breakChance > 25 and "[col=255,255,0]" or "[col=0,255,0]";
                description = description .. "[col=255,255,255]Chance to break:[/] " .. color .. tostring(breakChance) .. "%[/][br/]";
            end
            if self.isTable and _square:Is("IsTableTop") then
                description = description .. "[col=255,0,0]- Has item on surface[/][br/]";
            end
            if self.type == "Window" then
                if _object and instanceof(_object,"IsoWindow") and _object:IsOpen() then
                    description = description .. "[col=255,0,0]- Window is open[/][br/]";
                end
            end
            if _object and instanceof(_object,"IsoObject") and not self:objectNoContainerOrEmpty( _object ) then
                description = description .. "[col=255,0,0]- Has items in container[/][br/]";
...
--[[
function ISMoveableSpriteProps:canPickUpMoveableOLD( _character, _square, _object )
    if self.isMoveable and self.isMultiSprite then
        --print("MultiSprite movable CanPickUp test")
        local spriteGrid = self.sprite:getSpriteGrid();
        --local curX = spriteGrid:getSpriteGridPosX(self.sprite);
        --local curY = spriteGrid:getSpriteGridPosY(self.sprite);
        --local curWx = _square:getX();
        --local curWy = _square:getY();
        local sX = _square:getX() - spriteGrid:getSpriteGridPosX(self.sprite); --curWx - curX;
        local sY = _square:getY() - spriteGrid:getSpriteGridPosY(self.sprite); --curWy - curY;
        local sZ = _square:getZ();
        ...
        for x=0,spriteGrid:getWidth()-1 do
            for y=0,spriteGrid:getHeight()-1 do
                ...
                for i = square:getObjects():size(),1,-1 do
                    ...
                    if obj:getSprite() == spriteForPos then
                        found = true;
                        if not self:canPickUpMoveableInternal( _character, square, obj, true ) then
                            canPickUp = false;
                        end
                    end
                end
    ...
    if self.isMoveable and instanceof(_character,"IsoGameCharacter") and instanceof(_square,"IsoGridSquare") then
        ...
        if obj and (_forceAllow or ISMoveableDefinitions.cheat or self:canPickUpMoveable( _character, _square, not sprInstance and obj or nil )) then
            if self.isMultiSprite then
                ...
                for _,gridMember in ipairs(sgrid) do
                    table.insert(items, self:pickUpMoveableInternal( _character, gridMember.square, gridMember.object, gridMember.sprInstance, gridMember.sprite:getName(), createItem ));
                end
                ...
                --local obj, sprInstance = self:findOnSquare( _square, self.spriteName );
                self:pickUpMoveableInternal( _character, _square, obj, sprInstance, self.spriteName, _createItem );
            end
...
function ISMoveableSpriteProps:pickUpMoveableInternal( _character, _square, _object, _sprInstance, _spriteName, _createItem )
    ...
    if item or (objIsIsoWindow and _object:isDestroyed()) then      -- destroyed windows return nil for instanceItem()
        ...
        if not objIsIsoWindow or not _object:isDestroyed() then     -- when its a destroyed window skip this
            if self:doBreakTest( _character ) then
                if self.type ~= "Window" then
...
--[[
function ISMoveableSpriteProps:pickUpMoveableInternalOldII( _character, _square )
    if self.isMoveable and instanceof(_character,"IsoGameCharacter") and instanceof(_square,"IsoGridSquare") then
        local obj, sprInstance = self:findOnSquare( _square, self.spriteName );
        if obj and self:canPickUpMoveable( _character, _square, not sprInstance and obj or nil ) then
            local objIsIsoWindow = self.type == "Window" and instanceof(obj,"IsoWindow");
            local item 	= self:instanceItem();

            if item or (objIsIsoWindow and obj:isDestroyed()) then      -- destroyed windows return nil for instanceItem()
                local windowGotSmashed = false;
                if not objIsIsoWindow or not obj:isDestroyed() then     -- when its a destroyed window skip this
                    if self:doBreakTest( _character ) then
                        if self.type ~= "Window" then
                            self:addBreakDebris( _square );
                        elseif objIsIsoWindow then
                            if not obj:isDestroyed() then               -- in case of a window, when it breaks and isnt broken yet smash it, leaves no debris.
                                obj:smashWindow();
                                windowGotSmashed = true;
                            end
                        end
                    elseif item then
                        if instanceof(obj, "IsoThumpable") then
                            item:getModData().name = obj:getName() or ""
                            item:getModData().health = obj:getHealth()
                            item:getModData().maxHealth = obj:getMaxHealth()
                            if obj:hasModData() then
                                item:getModData().modData = copyTable(obj:getModData())
                            end
                        end
                        _character:getInventory():AddItem(item);        -- add the item if it aint got broken
                    end
                ...
                -- custom/modified light info (custom bulb, use battery etc) for the various lamps can by copied to movable item and retrieved uppon placing;
                if instanceof(obj,"IsoLightSwitch") and sprInstance==nil then
                ...
                if self.type == "WallOverlay" then
                    if sprInstance then
                        local sprList = obj:getChildSprites();
                        local sprIndex = sprList and sprList:indexOf(sprInstance) or -1
                        if sprIndex ~= -1 then
                            obj:RemoveAttachedAnim(sprIndex)
                ...
                elseif self.type == "Window" then
                    if objIsIsoWindow and not windowGotSmashed then
                        if isClient() then _square:transmitRemoveItemFromSquare(obj) end
                        _square:RemoveTileObject(obj);
                    end
                elseif not sprInstance then --Objects, Vegitation, WallObjects etc
                    if self.isoType == "IsoRadio" or self.isoType == "IsoTelevision" then
...
--[[
function ISMoveableSpriteProps:pickUpMoveableOLD( _character, _square )
    if self.isMoveable and instanceof(_character,"IsoGameCharacter") and instanceof(_square,"IsoGridSquare") then
        local obj, sprInstance = self:findOnSquare( _square, self.spriteName );
        if obj and self:canPickUpMoveable( _character, _square, not sprInstance and obj or nil ) then
            local objIsIsoWindow = self.type == "Window" and instanceof(obj,"IsoWindow");
...
function ISMoveableSpriteProps:canPlaceMoveable( _character, _square, _item )
    if self.isMoveable and self.isMultiSprite then
        ...
        for x=1,spriteGrid:getWidth() do
            for y=1,spriteGrid:getHeight() do
                square = getCell():getGridSquare(sX+x-1, sY+y-1, sZ)
                if square and spriteGrid:getSprite(x-1, y-1) then
                    if x < spriteGrid:getWidth() then
                        local sqE = getCell():getGridSquare(square:getX()+1, square:getY(), square:getZ())
                        if sqE and square:isSomethingTo(sqE) then return false end
                    end
                    if y < spriteGrid:getHeight() then
                        local sqS = getCell():getGridSquare(square:getX(), square:getY()+1, square:getZ())
                        if sqS and square:isSomethingTo(sqS) then return false end
                    end
                end
            end
        end
...
function ISMoveableSpriteProps:placeMoveableInternal( _square, _item, _spriteName )
    ...
    elseif self.type=="WindowObject" then
        ...
        if self.isoType == "IsoBarbecue" then
            ...
            if bbqSprite and bbqSprite:getProperties():Is("propaneTank") then
                bbqSprite = getSprite( self.sheetName .. tostring(self.spriteID-8) );
                obj = IsoBarbecue.new( getCell(), _square, bbqSprite );
                obj:removePropaneTank();
            else
                obj = IsoBarbecue.new( getCell(), _square, bbqSprite );
            end
...
--[[
function ISMoveableSpriteProps:placeMoveableInternalOLD( _character, _square, _origSpriteName )
    if self.isMoveable and instanceof(_character,"IsoGameCharacter") and instanceof(_square,"IsoGridSquare") then
        local item = self:findInInventory( _character, _origSpriteName );
        if item  and self:canPlaceMoveable( _character, _square, item ) then
            local obj;
            local north         = self.facing and (self.facing=="N" or self.facing=="S");
            --local hasTileFloor  = _square and _square:getFloor();
            local insertIndex   = _square:getObjects() and _square:getObjects():size();
            ...
            if self.type=="WallOverlay" then
                local wall = self:getWallForFacing( _square, self.facing );
                if wall then
                    wall:AttachExistingAnim(getSprite(self.spriteName), 0, 0, false, 0, false, 0)

                    if isClient() then wall:transmitUpdatedSpriteToServer() end
                end
            elseif self.type=="FloorTile" then
                local spr = getSprite( item:getWorldSprite() );
                local floor = _square:getFloor();
                if floor and spr then
                    floor:setSprite(spr);
                    if isClient() then floor:transmitUpdatedSpriteToServer(); end
            ...
            elseif self.type=="Window" then
                obj = IsoWindow.new( getCell(), _square, getSprite( self.spriteName ), north );
                obj:setIsLocked(false)
            ...
            elseif self.type=="WindowObject" then
                obj = IsoCurtain.new( getCell(), _square, self.spriteName, north );
                if self:hasFaces() and (self.facing == "S" or self.facing == "E") then          -- If facing south or east, make sure window object is placed directly after window or windowFrame
                    local wallFrame = self:getWallForFacing( _square, north and "S" or "E", "WindowFrame" );
                    local window    = _square:getWindow(north);
                ...
                --local isGenericThump = false;
                local itemSprite = self.spriteName;
                local sprite = getSprite( itemSprite );
                local props = sprite and sprite:getProperties();
                local currentSurface = self:getTotalTableHeight(_square);
                if self.isMoveable and self.isTableTop then -- and self.facing ~= nil then		-- face correction when possible for items that are supposed to be on tables (like sinks)
                    local faces = self:getFaces();
                ...
                elseif self.isoType == "IsoMultiMedia" then
                    ...
                    elseif props and props:Is("lightR") then
                        ...
                        --item:setLightSettings(obj); --copies stored light settings to object
                    elseif self.type == "FloorRug" or self.type == "WallObject" or self.type == "Vegitation" or self.isTableTop then
                        obj = IsoObject.new( getCell(), _square, itemSprite );
                    else
                        obj = IsoThumpable.new(getCell(), _square, itemSprite, false, {}); --IsoObject.new( getCell(), self.square, itemSprite );
                ...
                --local sprite = getSprite( itemSprite );
                if props then --sprite and obj then
                    --local props = sprite:getProperties();
                    obj:createContainersFromSpriteProperties()
                    for i=1,obj:getContainerCount() do
                        obj:getContainerByIndex(i-1):setExplored(true)
...
--[[
function ISMoveableSpriteProps:canRotateMoveableOLD( _square, _object )
    if self.isMoveable and self:hasFaces() and not self.isTableTop then
        if _object and not self:objectNoContainerOrEmpty( _object ) then
            return false;
        end
...
function ISMoveableSpriteProps:rotateMoveableInternal( _character, _square, _origSpriteName )
    if self.isMoveable and self:hasFaces() and instanceof(_character,"IsoGameCharacter") and instanceof(_square,"IsoGridSquare") then
...
--[[
function ISMoveableSpriteProps:rotateMoveableOLD( _character, _square, _origSpriteName )
    if self.isMoveable and self:hasFaces() and instanceof(_character,"IsoGameCharacter") and instanceof(_square,"IsoGridSquare") then
        local obj, sprInstance = self:findOnSquare( _square, _origSpriteName );
...
function ISMoveableSpriteProps:scrapObject(_character)
    ...
    if added == 0 then
        _character:setHaloNote(getText("IGUI_Moveable_Fail"), 160,160,160,300);
    end
end
...
--[[
function ISMoveableSpriteProps:scrapObjectOLD(_character)
    local added = 0;
    local scrapResult, chance, perkName = self:canScrapObject(_character);
    local canScrap = scrapResult.canScrap;
    if canScrap then
        ...
        local square = object and self.object:getSquare();
        if scrapDef and object and square then
            if isClient() then square:transmitRemoveItemFromSquare(object) end
            square:RemoveTileObject(object);
            square:RecalcProperties();
...
--[[
function ISMoveableSpriteProps:canScrapObjectOLD(_character)
    local canScrap = false;
    local chance = 0;
    local perkName;
    local result = {};
    ...
    if self.canScrap and self.isFromObject and self.object:getSquare() and _character and instanceof(_character,"IsoGameCharacter") then
        canScrap = not self.object or self:objectNoContainerOrEmpty( self.object );
        result.containerFull = not self:objectNoContainerOrEmpty( self.object );

        if self.material then
            canScrap = ISMoveableDefinitions:getInstance().isScrapDefinitionValid( self.material );
            result.craftValid = canScrap;
        end

        if canScrap and self.isTable then
            canScrap = not self.object:getSquare():Is("IsTableTop") and self.object == self:getTopTable(self.object:getSquare());
        end

        if canScrap and self.isWaterCollector then
            if self.object and self.object:hasWater() then
                canScrap = false
            end
        end

        --if canScrap then --and _character and instanceof(_character,"IsoGameCharacter") then
            --print(" Scrap, tool= "..tostring(self.scrapUseTool)..", skill= "..tostring(self.scrapUseSkill));
            if self.scrapUseTool then
                canScrap = self:hasScrapTool( _character );
                result.haveTool = canScrap;
                --                if canScrap then
                    --local tool2 = self:hasScrapTool( _character, true );
                    result.haveTool2 = self:hasScrapTool( _character, true );
                    if not result.haveTool2 then canScrap = false; end
--                end
                --print("tool canscrap: "..tostring(canScrap))
            end
    ...
    result.canScrap = canScrap;
    --print("endresults: canscrap = "..tostring(canScrap)..", chance= "..tostring(chance)..", perknam= "..tostring(perkName))
    return result, chance, perkName;
end
--]]
...
function ISThumpableSpriteProps:getInfoPanelDescription( _square, _object, _player, _mode )
    ...
    local hasTool = _player:getInventory():contains("Saw")
    ...
    scriptItem = ScriptManager.instance:FindItem("Base.Screwdriver")
    hasTool = _player:getInventory():contains("Screwdriver")
...
function ISThumpableSpriteProps:walkToAndEquip( _character, _square, _mode )
    ...
    if luautils.walkAdj(_character, _square, false) then
        local tool = _character:getInventory():FindAndReturn("Saw")
        if not tool then return false end
        ISWorldObjectContextMenu.equip(_character, _character:getPrimaryHandItem(), tool:getType(), true)

        tool = _character:getInventory():FindAndReturn("Screwdriver")
        if not tool then return false end
