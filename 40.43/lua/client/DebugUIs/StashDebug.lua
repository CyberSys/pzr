...
function StashDebug:prerender()
    ...
    self:drawText("Stash Debug UI", self.width/2 - (getTextManager():MeasureStringX(UIFont.Medium, "Stash Debug UI") / 2), z, 1,1,1,1, UIFont.Medium);
    z = z + 30;
end
...
--        self:setVisible(false);
    ...
    if button.internal == "SPAWN" then
        if self.selectedStash then
            ...
            getPlayer():setX(self.selectedStash:getBuildingX() - 75);
            getPlayer():setY(self.selectedStash:getBuildingY() - 75);
            getPlayer():setLx(self.selectedStash:getBuildingX() - 75);
            getPlayer():setLy(self.selectedStash:getBuildingY() - 75);
            self:populateList();
        end
    end
