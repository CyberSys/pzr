...
function ISDestroyStuffAction:update()
	self.character:PlayAnim("Attack_" .. self.sledge:getSwingAnim());
	self.character:getSpriteDef():setFrameSpeedPerFrame(0.08 + (self.character:getPerkLevel(Perks.Strength) * 0.01));
		...
		for i=0, self.item:getSquare():getObjects():size() - 1 do
			...
			if itemAbove:getSprite() then
				if itemAbove:getSprite():getName() == "carpentry_01_17" then
					itemAbove:setSprite(getCell():getSpriteManager():getSprite("carpentry_01_16"));
				end
				if itemAbove:getSprite():getName() == "carpentry_01_18" then
					itemAbove:setSprite(getCell():getSpriteManager():getSprite("carpentry_01_17"));
				end
			end
		end
]]--
