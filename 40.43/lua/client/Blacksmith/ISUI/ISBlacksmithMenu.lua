...
ISBlacksmithMenu.doBuildMenu = function(player, context, worldobjects, test)
    ...
    if not disableFurnaceAnvil then
        ...
        if playerObj:getInventory():getItemCount("Base.Stone") < 1 then
            toolTip.description = toolTip.description .. " <LINE> <RGB:1,0,0> " .. getItemText("Stone") .. " " .. playerObj:getInventory():getItemCount("Base.Stone") .. "/50" ;
            furnaceOption.notAvailable = true;
        else
            toolTip.description = toolTip.description .. " <LINE> <RGB:1,1,1> " .. getItemText("Stone") .. " " .. playerObj:getInventory():getItemCount("Base.Stone") .. "/50" ;
        end
        ...
        if canBeCrafted then
            ...
            if not ingots then
                toolTip.description = toolTip.description .. " <LINE> <RGB:1,0,0> " .. getItemText("Iron Ingot") .. " " .. (metalAmount/100) .. " / " .. (ISBlacksmithMenu.metalForAnvil/100);
                anvilOption.notAvailable = true;
            else
                toolTip.description = toolTip.description .. " <LINE> <RGB:1,1,1> " .. getItemText("Iron Ingot") .. " " .. (metalAmount/100) .. " / " .. (ISBlacksmithMenu.metalForAnvil/100);
            end
        else
            anvilOption.notAvailable = true;
        end
...
--        ISBlacksmithMenu.checkMetalWeldingFurnitures = function(metalPipes, smallMetalSheet, metalSheet, hinge, scrapMetal, torchUse, skill, player, toolTip)
    ...
    --    end
        ...
        if not ISBuildMenu.cheat and not playerObj:getKnownRecipes():contains("Make Metal Walls") and not playerObj:getKnownRecipes():contains("Make Metal Fences") then
            subMenuMW:removeLastOption();
        end
        ...
        if playerObj:getKnownRecipes():contains("Make Metal Fences") or ISBuildMenu.cheat then
            ...
            local canDo, toolTip = ISBlacksmithMenu.checkMetalWeldingFurnitures(2,0,0,0,1,4,4,playerObj,toolTip);
            ...
            local canDo, toolTip = ISBlacksmithMenu.checkMetalWeldingFurnitures(3,0,0,0,4,6,5,playerObj,toolTip);
            ...
            if not canDo or not canDo2 then doubleDoorOption.notAvailable = true; end
            ...
            local canDo, toolTip = ISBlacksmithMenu.checkMetalWeldingFurnitures(8,0,0,2,2,10,7,playerObj,toolTip);
    ...
    --        end
        ...
        if playerObj:getKnownRecipes():contains("Make Metal Roof") or ISBuildMenu.cheat then
            ...
            local canDo, toolTip = ISBlacksmithMenu.checkMetalWeldingFurnitures(0,1,0,0,1,0,0,playerObj,toolTip);
    ...
    if metalDrumLuaObj and playerObj:DistToSquared(metalDrumIsoObj:getX() + 0.5, metalDrumIsoObj:getY() + 0.5) < 2 * 2 then
        ...
        if metalDrumIsoObj:getWaterAmount() > 0 then
            ...
            if not metalDrumLuaObj.haveLogs and not metalDrumLuaObj.haveCharcoal then
                ...
                if playerObj:getInventory():getItemCount("Base.Log") < 5 then
...
ISBlacksmithMenu.checkWire = function(wireUses, player, toolTip)
    ...
    local wires = player:getInventory():getItemsFromType("Wire");
    local canDo = true;
    local totalUse = 0;
    for i=0,wires:size()-1 do
        totalUse = totalUse + wires:get(i):getRemainingUses();
    end
    if totalUse > wireUses then
        toolTip.description = toolTip.description .. " <LINE> <RGB:1,1,1> " .. getItemText("Wire") .. " " .. wireUses .. "/" .. wireUses .. " " .. getText("ContextMenu_Uses");
    else
        toolTip.description = toolTip.description .. " <LINE> <RGB:1,0,0> " .. getItemText("Wire") .. " " .. totalUse .. "/" .. wireUses .. " " .. getText("ContextMenu_Uses");
        canDo = false;
    end
    return canDo, toolTip;
end

ISBlacksmithMenu.checkMetalWeldingFurnitures = function(metalPipes, smallMetalSheet, metalSheet, hinge, scrapMetal, torchUse, skill, player, toolTip, metalBar)
    if ISBuildMenu.cheat then
        return true, toolTip;
    end
...
--        toolTip.description = toolTip.description .. " <LINE> <RGB:1,1,1> " .. getItemText("Blow Torch") .. " 1/1" ;
    ...
    if metalPipes > 0 then
        if inv:getNumberOfItem("MetalPipe", false, true) < metalPipes then
            toolTip.description = toolTip.description .. " <LINE> <RGB:1,0,0> " .. getItemText("Metal Pipe") .. " " .. inv:getNumberOfItem("MetalPipe", false, true) .. "/" .. metalPipes;
            isOk = false;
        else
            toolTip.description = toolTip.description .. " <LINE> <RGB:1,1,1> " .. getItemText("Metal Pipe") .. " " .. metalPipes .. "/" .. metalPipes ;
        end
    end
...
ISBlacksmithMenu.onDoubleMetalDoor = function(worldobjects, player, torchUse)
...
ISBlacksmithMenu.onDoublePoleDoor = function(worldobjects, player, torchUse)
...
ISBlacksmithMenu.onBigMetalFence = function(worldobjects, player, torchUse)
...
ISBlacksmithMenu.onBigMetalFenceGate = function(worldobjects, player, torchUse)
