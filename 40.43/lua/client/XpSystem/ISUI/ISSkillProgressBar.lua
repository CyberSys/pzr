...
function ISSkillProgressBar:onMouseUp(x, y)
	...
	if self.char:getNumberOfPerksToPick() > 0 and self.xp >= self.xpForLvl and self.level == lvlSelected and not self.perk:isPassiv() then
...
function ISSkillProgressBar:activate()
	if self.char:getNumberOfPerksToPick() > 0 and self.xp >= self.xpForLvl and not self.perk:isPassiv() then
		self.char:LevelPerk(self.perk:getType())
		self.level = self.char:getPerkLevel(self.perk:getType())
		self.xpForLvl = ISSkillProgressBar.getXpForLvl(self.perk, self.level)
		self.message = nil
...
--~ 	end
	...
	-- passive perks level up automatically, update the UI
	...
	if self.level < 10 then
		...
		if percentProgress == 100 then
			if self.perk:isPassiv() then
				self:drawTexture(self.UnlockedSkill, x, y, 1,1,1,1);
			-- if we got skill pts available, we draw another rect over the square and make it glow
			elseif self.char:getNumberOfPerksToPick() > 0 then
				self:drawTexture(self.AddSkillBtn, x, y, 1,1,1,1);
				self:drawTexture(self.SkillBtnEmptyBig, x, y, ISSkillProgressBar.alpha,1,1,1);
