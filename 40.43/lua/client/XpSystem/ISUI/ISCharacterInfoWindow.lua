...
function ISCharacterInfoWindow:RestoreLayout(name, layout)
	...
	local floating = { info = true, skills = true, health = true }
...
function ISCharacterInfoWindow.OnClothingUpdated(chr)
	if instanceof(chr, 'IsoPlayer') and chr:isLocalPlayer() then
		getPlayerInfoPanel(chr:getPlayerNum()).charScreen.avatar = nil
	end
end
