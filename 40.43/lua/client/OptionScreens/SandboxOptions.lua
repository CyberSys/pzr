...
function SandboxOptionsScreen:create()
    ...
    local titleHgt = getTextManager():getFontFromEnum(UIFont.Cred2):getLineHeight()
    ...
    self:loadPresets();
    self.presetList.selected = 3 -- Survival
    self:onPresetChange()
...
function SandboxOptionsScreen:loadPresets()
    ...
    table.insert(self.presets, copyPreset(self.defaultPreset));
...
--    self.zombDisPanel:bringToTop();
    ISPanelJoypad.prerender(self);
    self:drawTextCentre(getText("UI_optionscreen_SandboxOptions"), self.width / 2, 10, 1, 1, 1, 1, UIFont.Cred2);
...
function SandboxOptionsScreen:render()
    ...
    elseif self.presetPanel.joyfocus then
        ...
        self:drawRectBorder(ui:getX() - 4, ui:getY() - 4, ui:getWidth() + 4 + 3, ui:getHeight() + 4 - 6, 0.4, 0.2, 1.0, 1.0)
        self:drawRectBorder(ui:getX() - 3, ui:getY() - 3, ui:getWidth() + 3 + 2, ui:getHeight() + 3 - 6, 0.4, 0.2, 1.0, 1.0)
    end
end
...
function SandboxOptionsScreen:onOptionMouseDown(button, x, y)
    ...
    if button.internal == "PLAY" then
        ...
        MainScreen.instance.charCreationMain.previousScreen = "SandboxOptionsScreen"
...
function SandboxOptionsScreen:deletePresetStep1(preset)
    local screenW = getCore():getScreenWidth()
    local screenH = getCore():getScreenHeight()
    local modal = ISModalDialog:new((screenW - 230) / 2, (screenH - 120) / 2, 230, 120, "Delete '" .. preset.name .. "'?", true, self, self.deletePresetStep2);
    modal.backgroundColor.a = 0.9
    modal:initialise()
    modal:setCapture(true)
    modal:setAlwaysOnTop(true)
