...
function CoopOptionsScreen:create()
    ...
    Events.OnResetLua.Add(CoopOptionsScreen.onResetLua)
end
...
function CoopOptionsScreen.onCoopServerMessage(tag, cookie, payload)
     ...
     if tag == "status" and CoopOptionsScreen.instance ~= nil then
         CoopOptionsScreen.instance.serverStatus = payload;
     end
 
    if tag == "status" and luautils.stringStarts(payload, "Server Started") then
...
function CoopOptionsScreen.onResetLua(reason)
