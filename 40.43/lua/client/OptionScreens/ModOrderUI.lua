...
function ModOrderUI:initialise()
	...
	self.datas = ISScrollingListBox:new(10, 50, self.width - 10 - btnWid2 - 10 - 10, self.height - 180);
...
function ModOrderUI:onClick(button)
	...
	if button.internal == "SAVE" then
		...
		for i=1,#self.datas.items do
			list:add(self.datas.items[i].text);
		end
		getCore():setMapOrder(list);
		getCore():setWarnMapConflict(false);
		getCore():saveOptions();
		self:setVisible(false);
		self:removeFromUIManager();
	end
