...
function NewGameScreen:create()
    self.mainPanel = ISPanel:new(0, 48, self:getWidth() / 2, self:getHeight() - (48*2));
    ...
    self.mainPanel.render = NewGameScreen.subPanelRender;
    self.mainPanel.prerender = NewGameScreen.subPanelPreRender;
    self.mainPanel.onMouseWheel = NewGameScreen.MainPanel_onMouseWheel
...
--        self.difficultyNormal:setX(right + xoffset)
    ...
    self.mainPanel:setScrollWidth(self.difficulty:getRight() + 20)
    self.mainPanel:setScrollHeight(self:getScrollHeight() + y);
    ...
    for _,child in pairs(self.mainPanel:getChildren()) do
        ...
        if child ~= self.mainPanel.vscroll then
            width = math.max(width, child:getRight())
        end
    end
    width = width + self.mainPanel.vscroll:getWidth() + 4
    self.mainPanelReqWidth = width
    width = math.min(width, self.width / 2)
    self.mainPanel:setX((self.width / 2 - width) / 2)
...
--[[
NewGameScreen.onVehicleEnable = function(changeOptionTarget, mouseOverOption, selected, changeOptionArgs1, changeOptionArgs2)
	getCore():setVehiclesEnable(selected)
	getCore():saveOptions()
	if not getCore():isVehiclesWarningShow() then
		local modal = ISVehicleConfirmBox:new();
		modal:initialise();
		modal:addToUIManager();
		modal.parent = changeOptionTarget
		modal:setVisible(true, changeOptionTarget.joyfocus)
	end
end
]]--
...
function NewGameScreen:prerender()
	...
	self:drawTextCentre(getText("UI_NewGameScreen_title"), self.width / 2, 10, 1, 1, 1, 1, UIFont.Cred2);
	self:ensureVisible()
end
...
function NewGameScreen:clickPlay()
    ...
    MainScreen.instance.charCreationMain.previousScreen = "NewGameScreen";
...
function NewGameScreen:onResolutionChange(oldw, oldh, neww, newh)
    local width = math.min(self.mainPanelReqWidth, self.width / 2)
    self.mainPanel:setX((self.width / 2 - width) / 2)
    self.mainPanel:setWidth(width)
end

function NewGameScreen:subPanelPreRender()
...
function NewGameScreen:subPanelRender()
    ISPanel.render(self);
    self:clearStencilRect();
end

function NewGameScreen:MainPanel_onMouseWheel(del)
