...
--[[
    ...
    self.whitelistServer.selected[1] = true;
    ...
    self.pwdProtected.selected[1] = true;
...
function PublicServerList.onResetLua(reason)
    if false and reason == "ConnectedToServer" then
        MainScreen.instance.bottomPanel:setVisible(false);
    end
end

Events.OnResetLua.Add(PublicServerList.onResetLua)
...
function PublicServerList.OnSteamRefreshInternetServers()
    table.sort(PublicServerList.instance.listbox.items, function(a,b)
        if string.find(a.item.server:getName(), "SpiffoSpace") then
            if not string.find(b.item.server:getName(), "SpiffoSpace") then
                return true
            else
                return tonumber(a.item.server:getPlayers())>tonumber(b.item.server:getPlayers());
            end
        elseif string.find(b.item.server:getName(), "SpiffoSpace") then
            if not string.find(a.item.server:getName(), "SpiffoSpace") then
                return false
            else
                return tonumber(a.item.server:getPlayers())>tonumber(b.item.server:getPlayers());
            end
...
--    table.sort(PublicServerList.instance.listbox.items, function(a,b) return tonumber(a.item.server:getPlayers())>tonumber(b.item.server:getPlayers()) end);
    steamReleaseInternetServersRequest()
end
