...
function MapSpawnSelect:getSpawnRegions()
	return self:getSafehouseSpawnRegion() or self:getFixedSpawnRegion() or SpawnRegionMgr.getSpawnRegions()
end
...
function MapSpawnSelect:clickNext()
	...
	MainScreen.instance.charCreationProfession.doSpawnPoint = true
	if getWorld():getGameMode() == "Sandbox" and not checkSaveFileExists("map_sand.bin") then
		...
		MainScreen.instance.charCreationMain.previousScreen = "MapSpawnSelect";
...
function MapSpawnSelect:prerender()
	...
	if self.textEntry and self.textEntry:isVisible() then
		local worldName = sanitizeWorldName(self.textEntry:getText())
		if worldName == "" or luautils.stringStarts(worldName, ".") or luautils.stringEnds(worldName, ".") then
			enable = false
		end
	end
...
function MapSpawnSelect:render()
	...
	self:drawTextCentre(getText("UI_mapspawn_title"), self.width / 2, 10, 1, 1, 1, 1, UIFont.Cred2)
