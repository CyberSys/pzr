...
--	self:addChild(MainScreen.instance.charCreationHeader);
    ...
    self.savedBuilds:addOption(getText("UI_characreation_SelectToLoad"))
...
function CharacterCreationProfession:checkXPBoost()
    ...
    for perk,level in pairs(levels) do
        ...
        if perk == Perks.Axe then
            addText = "(" .. getText("IGUI_perks_Blade") .. ") "
        end
        if perk == Perks.Blunt then
            addText = "(" .. getText("IGUI_perks_Blunt") .. ") "
        end
        if perk == Perks.BluntMaintenance or perk == Perks.BluntGuard then
            addText = "(" .. getText("IGUI_perks_Blunt") .. ") "
        end
        if perk == Perks.BladeMaintenance or perk == Perks.BladeGuard  then
            addText = "(" .. getText("IGUI_perks_Blade") .. ") "
        end
        if level < 0 then level = 0 end
        if level > 10 then level = 10 end
        self.listboxXpBoost:addItem(addText .. PerkFactory.getPerkName(perk), { perk = perk, level = level })
    end
...
if true then return end
	...
	if profession then
		-- The profession type as a string.
		local type = profession:getType();
		local avatar = MainScreen.instance.avatar;
		local desc = MainScreen.instance.desc;

		-- We'll use the sex of the character to index the table.
		local sex;
		if avatar:isFemale() then
			sex = "female";
		else
			sex = "male";
		end

		-- Load the clothing colours. If we can't find them in the
		-- table we simply replace them with standard values.
		local colorTop;
		local colorBottom;
		do
			local r = 1;
            local g = 1;
            local b = 1;
            if ProfessionClothing[type] and ProfessionClothing[type][sex] then
                r = ProfessionClothing[type][sex].topCol.r or 1.0;
                g = ProfessionClothing[type][sex].topCol.g or 1.0;
                b = ProfessionClothing[type][sex].topCol.b or 1.0;
            end
			colorTop = ColorInfo.new(r, g, b, 1);

            r = 1;
            g = 1;
            b = 1;
            if ProfessionClothing[type] and ProfessionClothing[type][sex] then
                r = ProfessionClothing[type][sex].bottomCol.r or 0.1;
                g = ProfessionClothing[type][sex].bottomCol.g or 0.1;
                b = ProfessionClothing[type][sex].bottomCol.b or 0.1;
            end
            colorBottom = ColorInfo.new(r, g, b, 1);
		end

		-- Now we load the clothing sprites && their palettes or the
		-- the standard variables.
		local topPal = "Vest_White";
        local top = "Vest";
        local bottomPal = "Trousers_White";
        local bottom = "Trousers";
        if ProfessionClothing[type] and ProfessionClothing[type][sex] then
            topPal = ProfessionClothing[type][sex].topPal or "Vest_White";
            top = ProfessionClothing[type][sex].top or "Vest";
            bottomPal = ProfessionClothing[type][sex].bottomPal or "Trousers_White";
            bottom = ProfessionClothing[type][sex].bottom or "Trousers";
        end
		...
		desc:setTopColor(colorTop:toColor());
		desc:setTrouserColor(colorBottom:toColor());
...
function CharacterCreationProfession:onOptionMouseDown(button, x, y)
    if button.internal == "INFO" then
        if not self.infoRichText then
            self.infoRichText = ISModalRichText:new(getCore():getScreenWidth()/2-400,getCore():getScreenHeight()/2-300,600,600,getText("UI_CharacterCreation"), false);
...
--		MainScreen.instance.charCreationProfession:removeChild(MainScreen.instance.charCreationHeader)
--		MainScreen.instance.charCreationMain:addChild(MainScreen.instance.charCreationHeader)
...
function CharacterCreationProfession:update()
	...
	self.deleteBuildButton:setEnable(self.savedBuilds.selected ~= 1)
...
function CharacterCreationProfession.initWorld()
	if isDemo() then
		if getCore():getGameMode() == "Tutorial" then
			getWorld():setLuaPlayerDesc(SurvivorFactory.CreateSurvivor())
			getWorld():getLuaTraits():clear()
		end
		return
	end
	if not getCore():isDedicated() ~= nil and MainScreen.instance ~= nil then
		...
		if MainScreen.instance.charCreationProfession.doSpawnPoint then
			...
			if not spawnRegion then
				print("ERROR: no spawn region was chosen, don't know where to spawn the player")
				return
			end
...
function CharacterCreationProfession:saveBuildStep1() -- {{{
    self.inputModal = BCRC.inputModal(true, nil, nil, nil, nil, CharacterCreationProfession.saveBuildStep2, self);
...
function BCRC.inputModal(_centered, _width, _height, _posX, _posY, _onclick, target, param1, param2) -- {{{
