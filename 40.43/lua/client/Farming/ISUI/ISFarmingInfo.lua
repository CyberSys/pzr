...
--		if not square then self:getParent():setVisible(false); return end
	...
	self:drawText(getText("Farming_Plant_Information") , 32, - 14, 1,1,1,1);
end
...
function ISFarmingInfo.getDiseaseName(info)
	...
	if(info.plant.mildewLvl > 0) then
		if(farmingLevel >= 4) then
			...
			if(farmingLevel >= 6) then
				result = result .. info.plant.mildewLvl .. "/100";
			else
				result = result .. getText("UI_FriendState_Unknown") .. "/100";
			end
		else
	...
	if(info.plant.aphidLvl > 0) then
		if(farmingLevel >= 4) then
			...
			if(farmingLevel >= 6) then
				result = result .. info.plant.aphidLvl .. "/100";
			else
				result = result .. getText("UI_FriendState_Unknown") .. "/100";
			end
		else
	...
	if(info.plant.fliesLvl > 0) then
		if(farmingLevel >= 4) then
			...
			if(farmingLevel >= 6) then
				result = result .. info.plant.fliesLvl .. "/100";
			else
				result = result .. getText("UI_FriendState_Unknown") .. "/100";
			end
		else
	...
	if(disease[1] == nil) then
		...
		ISFarmingInfo:getGreen(disease_rgb, "1");
	end
	return disease;
end
