...
ISFarmingMenu.onMildewCure = function(worldobjets, uses, sq, player)
	...
	if not AdjacentFreeTileFinder.isTileOrAdjacent(player:getCurrentSquare(), sq) then
		...
		if adjacent ~= nil then
			ISTimedActionQueue.add(ISWalkToTimedAction:new(player, adjacent));
			ISTimedActionQueue.add(ISCureMildewAction:new(player, ISFarmingMenu.GardeningSprayMilk, uses, CFarmingSystem.instance:getLuaObjectOnSquare(sq), 10 * (uses * 5)));
		return;
		else
			return;
		end
...
ISFarmingMenu.onFliesCure = function(worldobjets, uses, sq, player)
	...
	if not AdjacentFreeTileFinder.isTileOrAdjacent(player:getCurrentSquare(), sq) then
		...
		if adjacent ~= nil then
			ISTimedActionQueue.add(ISWalkToTimedAction:new(player, adjacent));
			ISTimedActionQueue.add(ISCureFliesAction:new(player, ISFarmingMenu.GardeningSprayCigarettes, uses, CFarmingSystem.instance:getLuaObjectOnSquare(sq), 10 * (uses * 5)));
		return;
		else
			return;
		end
