		...
		if i == #self.lines or (self.lineY[c+1] + self.marginTop + self:getYScroll() > 0) then
			...
			if string.trim(v) ~= "" then
				if orient == "centre" then
					self:drawTextCentre( string.trim(v), self.width / 2 , self.lineY[c] + self.marginTop, r, g, b,self.contentTransparency, self.font);
				elseif orient == "right" then
					self:drawTextLeft( string.trim(v), self.lineX[c] + self.marginLeft, self.lineY[c] + self.marginTop, r, g, b,self.contentTransparency, self.font);
				else
