...
--~ 	context:addOption("Information", items, ISInventoryPaneContextMenu.onInformationItems);
    ...
    if #moveItems > 0 and playerObj:getJoypadBind() ~= -1 then
        ...
        if isInPlayerInventory then
            context:addOption(getText("IGUI_invpage_Transfer_all"), getPlayerInventory(player), ISInventoryPane.transferAll)
        else
            context:addOption(getText("IGUI_invpage_Loot_all"), loot, ISInventoryPane.lootAll)
        end
    end
	...
	if not self.contents then
		...
		for _,source in ipairs(allSources) do
			...
			for k=1,source:getItems():size() do
				...
				local itemTexture = ""
				if item then
					itemTexture = item:getTex():getName()
					if sourceFullType == "Water" then
						if source:getCount() == 1 then
							itemName = getText("IGUI_CraftUI_CountOneUnit", getText("ContextMenu_WaterName"))
						else
							itemName = getText("IGUI_CraftUI_CountUnits", getText("ContextMenu_WaterName"), source:getCount())
						end
					elseif source:getItems():size() > 1 then -- no units
						itemName = item:getDisplayName()
					elseif not source:isDestroy() and item:IsDrainable() then
						if source:getCount() == 1 then
							itemName = getText("IGUI_CraftUI_CountOneUnit", item:getDisplayName())
						else
							itemName = getText("IGUI_CraftUI_CountUnits", item:getDisplayName(), source:getCount())
						end
					elseif not source:isDestroy() and source:getUse() > 0 then -- food
						if source:getUse() == 1 then
							itemName = getText("IGUI_CraftUI_CountOneUnit", item:getDisplayName())
						else
							itemName = getText("IGUI_CraftUI_CountUnits", item:getDisplayName(), source:getUse())
						end
					elseif source:getCount() > 1 then
						itemName = getText("IGUI_CraftUI_CountNumber", item:getDisplayName(), source:getCount())
					else
						itemName = item:getDisplayName()
					end
				else
					itemName = sourceFullType
				end
				local x2 = x1
				if source:getItems():size() > 1 then
					x2 = x2 + 20
				...
				if itemTexture ~= "" then
					self:addImage(x2, y1 + imageDY, itemTexture)
					x2 = x2 + IMAGE_SIZE + 6
				end
				self:addText(x2, y1 + textDY, itemName)
				y1 = y1 + lineHeight
			end
		end
...
ISInventoryPaneContextMenu.addDynamicalContextMenu = function(selectedItem, context, recipe, player, containerList)
	...
	for i=0,recipe:size() -1 do
        ...
        if recipe:get(i):getNumberOfNeededItem() > 0 then
			local tooltip = ISInventoryPaneContextMenu.addToolTip();
			setmetatable(tooltip, CraftTooltip)
			tooltip.recipe = recipe:get(i)
