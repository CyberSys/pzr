...
function ISRichTextLayout:render(x, y, ui)
	...
	for i,v in ipairs(self.lines) do
		...
		if i == #self.lines or (y + self.marginTop + self.lineY[c+1] + ui:getYScroll() > 0) then
			local r = self.r
			local b = self.b
			local g = self.g
			...
			if string.trim(v) ~= "" then
				...
				elseif orient == "right" then
					ui:drawTextLeft(string.trim(v), x + self.marginLeft + self.lineX[c], y + self.marginTop + self.lineY[c], r, g, b,1, self.font)
				else
