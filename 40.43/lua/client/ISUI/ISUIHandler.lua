...
ISUIHandler.setVisibleAllUI = function(visible)
	...
	if not visible then
		...
		for k in pairs (ISUIHandler.visibleUI) do
            ISUIHandler.visibleUI [k] = nil
        end
...
ISUIHandler.onKeyPressed = function(key)
	...
	if key == getCore():getKey("Toggle UI") and playerObj then
		ISUIHandler.toggleUI();
...
--        getAmbientStreamManager():doGunEvent();
	...
	if key == getCore():getKey("Take screenshot") then
		takeScreenshot();
    end
