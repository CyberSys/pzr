...
--    y = y + getPlayerData(player).y1top;
	...
	if (JoypadState.players[player+1] or ISEmptyGraves.canDigHere(worldobjects)) and not playerObj:getVehicle() and
			getSpecificPlayer(player):getInventory():contains("Shovel") then
		if test == true then return true; end
	...
	if graves and playerInv:contains("Shovel") then
		if test == true then return true; end
		context:addOption(getText("ContextMenu_FillGrave", graves:getModData()["corpses"]), graves, ISWorldObjectContextMenu.onFillGrave, player)
	end
    ...
    if door and not door:IsOpen() and doorKeyId then
        if getSpecificPlayer(player):getInventory():haveThisKeyId(doorKeyId) or not getSpecificPlayer(player):getCurrentSquare():Is(IsoFlagType.exterior) then
            if test == true then return true; end
	...
	if canBeWaterPiped then
		...
		if not playerObj:getInventory():FindAndReturn("Spanner") then
			option.notAvailable = true;
			local tooltip = ISWorldObjectContextMenu.addToolTip()
			tooltip:setName(getText("ContextMenu_PlumbItem", name));
			local usedItem = InventoryItemFactory.CreateItem("Base.Spanner");
...
--    end
	...
	if thump ~= nil and not invincibleWindow then
		...
		if thump:getCanBarricade() then
            ...
            if not ignoreObject then
                ...
                if not thump:haveSheetRope() and (not barricade or barricade:canAddPlank()) and (playerObj:getInventory():contains("Hammer", true) or playerObj:getInventory():contains("HammerStone", true)) and
                        playerObj:getInventory():contains("Plank", true) and playerObj:getInventory():getItemCount("Base.Nails", true) >= 2  then
                    if test == true then return true; end
                ...
                if not thump:haveSheetRope() and not barricade and ISWorldObjectContextMenu.checkBlowTorchForBarricade(playerObj) and playerObj:getInventory():contains("SheetMetal", true) then
                    if test == true then return true; end
                    context:addOption(getText("ContextMenu_MetalBarricade"), worldobjects, ISWorldObjectContextMenu.onMetalBarricade, thump, player);
                end
                if not thump:haveSheetRope() and not barricade and ISWorldObjectContextMenu.checkBlowTorchForBarricade(playerObj) and playerObj:getInventory():getItemCount("Base.MetalBar", true) >= 3 then
                    if test == true then return true; end
	...
	if window ~= nil and not invincibleWindow then
        ...
        if not window:haveSheetRope() and (not barricade or barricade:canAddPlank()) and (playerObj:getInventory():contains("Hammer", true) or playerObj:getInventory():contains("HammerStone", true)) and
                playerObj:getInventory():contains("Plank", true) and playerObj:getInventory():getItemCount("Base.Nails", true) >= 2 then
            if test == true then return true; end
        ...
        if not window:haveSheetRope() and not barricade and ISWorldObjectContextMenu.checkBlowTorchForBarricade(playerObj) and playerObj:getInventory():contains("SheetMetal", true) then
            if test == true then return true; end
        ...
        if not window:haveSheetRope() and not barricade and ISWorldObjectContextMenu.checkBlowTorchForBarricade(playerObj) and playerObj:getInventory():getItemCount("Base.MetalBar", true) >= 3 then
            if test == true then return true; end
	...
	if door ~= nil then
		...
		local canBarricade = door:getSprite() and not door:getSprite():getProperties():Is("DoubleDoor")
		local barricade = door:getBarricadeForCharacter(playerObj)
		-- barricade (hammer on 1st hand, plank on 2nd hand)
		if canBarricade and (not barricade or barricade:canAddPlank()) and (playerObj:getInventory():contains("Hammer", true) or playerObj:getInventory():contains("HammerStone", true)) and
                playerObj:getInventory():contains("Plank", true) and playerObj:getInventory():getItemCount("Base.Nails", true) >= 2 then
            if test == true then return true; end
        ...
        if canBarricade and not barricade and ISWorldObjectContextMenu.checkBlowTorchForBarricade(playerObj) and playerObj:getInventory():contains("SheetMetal", true) then
            if test == true then return true; end
            context:addOption(getText("ContextMenu_MetalBarricade"), worldobjects, ISWorldObjectContextMenu.onMetalBarricade, door, player);
        end
        if canBarricade and not barricade and ISWorldObjectContextMenu.checkBlowTorchForBarricade(playerObj) and playerObj:getInventory():getItemCount("Base.MetalBar", true) >= 3 then
            if test == true then return true; end
...
--    end
    ...
    if compost then
        ...
        option.notAvailable = compost:getCompost() < 10 or playerObj:getInventory():getItemsFromType("EmptySandbag", true):isEmpty()
        ...
        if playerObj:getInventory():getItemsFromType("EmptySandbag", true):isEmpty() then
...
function ISWorldObjectContextMenu.handleCarBatteryCharger(test, context, worldobjects, playerObj, playerInv)
	...
	if carBatteryCharger:getBattery() then
		...
		local batteryList = playerInv:getItemsFromType("CarBattery1")
		batteryList:addAll(playerInv:getItemsFromType("CarBattery2"))
		batteryList:addAll(playerInv:getItemsFromType("CarBattery3"))
...
ISWorldObjectContextMenu.onTakeFuel = function(worldobjects, player, square)
	...
	if not petrolCan then
		petrolCan = player:getInventory():FindAndReturn("EmptyPetrolCan")
	end
	if petrolCan and luautils.walkAdj(player, square) then
		ISInventoryPaneContextMenu.equipWeapon(petrolCan, true, false, player:getPlayerNum())
		ISTimedActionQueue.add(ISTakeFuel:new(player, square, petrolCan, 100))
	end
end
...
ISWorldObjectContextMenu.onPlumbItem = function(worldobjects, player, itemToPipe)
	local playerObj = getSpecificPlayer(player)
	local wrench = playerObj:getInventory():FindAndReturn("Spanner");
	ISTimedActionQueue.add(ISEquipWeaponAction:new(playerObj, wrench, 20, true, false));
...
ISWorldObjectContextMenu.onTakeWater = function(worldobjects, waterObject, waterContainer, player)
    ...
    if waterContainer:canStoreWater() and not waterContainer:isWaterSource() then
		...
		ISTimedActionQueue.add(ISEquipWeaponAction:new(playerObj, newItem, 40, true, false))
    ...
    elseif waterContainer:canStoreWater() and waterContainer:isWaterSource() then -- second case, a bottle contain some water, we just fill it
		...
		if playerObj:getPrimaryHandItem() ~= waterContainer and playerObj:getSecondaryHandItem() ~= waterContainer then
			ISTimedActionQueue.add(ISEquipWeaponAction:new(playerObj, waterContainer, 40, true, false))
		end
...
ISWorldObjectContextMenu.onAddWaterFromItem = function(worldobjects, waterObject, waterItem, playerObj)
	...
	if waterItem:canStoreWater() and waterItem:isWaterSource() then
		ISTimedActionQueue.add(ISEquipWeaponAction:new(playerObj, waterItem, 40, true, false))
...
ISWorldObjectContextMenu.onBarricade = function(worldobjects, window, player)
	...
	if playerObj:getInventory():getItemCount("Base.Nails", true) < 2 then return end
	local parent = window:getSquare();
	if not AdjacentFreeTileFinder.isTileOrAdjacent(playerObj:getCurrentSquare(), parent) then
		local adjacent = nil;
		if ISWorldObjectContextMenu.isThumpDoor(window) then
...
ISWorldObjectContextMenu.isTrappedAdjacentToWindow = function(player, window)
	...
	local north = IsoWindowFrame.isWindowFrame(window) and window:getProperties():Is(IsoFlagType.WindowN) or window:getNorth()
...
ISWorldObjectContextMenu.onFillGrave = function(grave, player)
	if luautils.walkAdj(getSpecificPlayer(player), grave:getSquare()) then
		ISTimedActionQueue.add(ISFillGrave:new(player, grave, 100));
	end
end
