	...
	if ISMouseDrag.dragging ~= nil and ISMouseDrag.draggingFocus ~= self and ISMouseDrag.draggingFocus ~= nil then
		if self:canPutIn() then
			...
			for i,v in ipairs(dragging) do
				...
				if transfer then
					ISTimedActionQueue.add(ISInventoryTransferAction:new(playerObj, v, v:getContainer(), self.inventory))
				end
			end
			self.selected = {};
			getPlayerLoot(self.player).inventoryPane.selected = {};
...
function ISInventoryPane:updateSmoothScrolling()
	...
	local frameRateFrac = 30 / getPerformance():getUIRenderFPS()
            ...
            for i,v in ipairs(dragging) do
                if not self.inventory:isInside(v) and not v:isFavorite() then
                    if not instanceof(v, "Moveable") or v:CanBeDroppedOnFloor() then
                        ISTimedActionQueue.add(ISInventoryTransferAction:new(playerObj, v, v:getContainer(), ISInventoryPage.floorContainer[self.player+1]))
...
--                    if self.joyselection >= #self.itemslist then self.joyselection = 0; end
                    ...
                    if count == 1 then -- normal background (no selected, no dragging..)
                        ...
                        if self.highlightItem and self.highlightItem == item:getType() then
                            ...
                            if not self.blinkAlphaIncrease then
                                self.blinkAlpha = self.blinkAlpha - 0.05 * (30 / getPerformance():getUIRenderFPS());
               ...
               -- print("trace:h");
                ...
                if (self.collapsed[v.name] or k2 == #v.items) and k < #self.itemslist and not self.itemslist[k+1].equipped and self.itemslist[k].equipped then
                    self:drawRect(1, ((y+1)*self.itemHgt)+self.headerHgt-1, self.column4, 1, 0.2, 1, 1, 1);
                end
