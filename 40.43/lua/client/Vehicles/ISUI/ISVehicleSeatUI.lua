...
function ISVehicleSeatUI:prerender()
	...
	if getText("IGUI_" .. seatname) ~= "IGUI_" .. seatname then
	...
	if str then
		...
		self:drawTextCentre(str, self:getWidth() / 2, 6 + fontHgt, 1, 0, 0, 1, UIFont.Small)
	end
end
...
function ISVehicleSeatUI:render()
	...
	local scale = height / extents:z() * script:getModelScale()
	...
	for seat=1,self.vehicle:getMaxPassengers() do
		...
		if posn then
			...
			if self.vehicle:isSeatOccupied(seat-1) then
				if self.vehicle:getCharacter(seat-1) then
					...
					if ISVehicleMenu.moveItemsFromSeat(self.character, self.vehicle, seat-1, false, false) then
						canSwitch = true;
						fillR, fillG, fillB = 0.0, 0.0, 0.0
						outlineR, outlineG, outlineB = 0.0, 1.0, 0.0
						texName = "icon_vehicle_empty.png"
					else
			...
			if not shiftKey and canSwitch and not self.joyfocus then
				self:drawTextCentre(tostring(seat), x + sizeX / 2, y + sizeY / 2 - fontHgtLarge / 2, 1, 1, 1, 1, UIFont.Large)
			end
			...
			if canSwitch and self.joyfocus and self.joypadSeat == seat then
				local tex = getTexture("media/ui/abutton.png")
		...
		if playerSeat ~= -1 and not self.joyfocus then
			...
			if canSwitch and posn then
				...
				if shiftKey then
					self:drawRect(x + texW / 2 - 8, y + texH / 2 - fontHgtLarge / 2, 16, fontHgtLarge, 1, 0.1, 0.1, 0.1)
					self:drawTextCentre(tostring(seat), x + texW / 2, y + texH / 2 - fontHgtLarge / 2, 1, 1, 1, 1, UIFont.Large)
				end
			end
		end
		if playerSeat ~= -1 and self.joyfocus and seat == self.joypadSeat then
			...
			if canSwitch and posn then
				local offset = posn:getOffset()
				local tex = getTexture("media/ui/xbutton.png")
				local texW,texH = tex:getWidthOrig(),tex:getHeightOrig()
				local x = self:getWidth() / 2 - offset:get(0) * scale - texW / 2
				local y = self:getHeight() / 2 - offset:get(2) * scale - texH / 2
				y = y + (SeatOffsetY[scriptName] or 0.0)
...
function ISVehicleSeatUI:closeSelf()
	Events.OnCustomUIKeyPressed.Remove(ISVehicleSeatUI.OnKeyPressed)
	Events.OnCustomUIKeyReleased.Remove(ISVehicleSeatUI.OnKeyReleased)
...
function ISVehicleSeatUI:setVehicle(vehicle)
	...
	Events.OnCustomUIKeyPressed.Add(ISVehicleSeatUI.OnKeyPressed)
	Events.OnCustomUIKeyReleased.Add(ISVehicleSeatUI.OnKeyReleased)
end
...
function ISVehicleSeatUI.OnKeyPressed(key)
	if not ISVehicleMenu.seatUI then return end
	local ui = ISVehicleMenu.seatUI[0]
...
function ISVehicleSeatUI.OnKeyReleased(key)
	if not ISVehicleMenu.seatUI then return end
	local ui = ISVehicleMenu.seatUI[0]
	if not ui or not ui:isReallyVisible() then return; end
	if not ui.vehicle or not ui.vehicle:getScript() then return end
	local numSeats = ui.vehicle:getMaxPassengers()
	if key >= Keyboard.KEY_1 and key < Keyboard.KEY_1 + numSeats then
		if isKeyDown(Keyboard.KEY_LSHIFT) or isKeyDown(Keyboard.KEY_RSHIFT) then
			ui:exitSeat(key - Keyboard.KEY_1)
		else
			ui:useSeat(key - Keyboard.KEY_1)
		end
	end
end
