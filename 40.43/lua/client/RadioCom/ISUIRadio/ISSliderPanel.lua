...
function ISSliderPanel:onMouseDown(_x, _y)
    ...
    if self.doButtons then
        local stepVal = self.shiftDown and self.shiftValue or self.stepValue;
...
function ISSliderPanel:update()
    ...
    if self.doButtons then
        if ISSliderPanel.shiftPressed > 0 then
            ISSliderPanel.shiftPressed = ISSliderPanel.shiftPressed - 1;
            self.shiftDown = true;
        else
...
ISSliderPanel.shiftPressed = 0;
function ISSliderPanel.onKeyKeepPressed( _key )
    if _key == getCore():getKey("Run") then
        ISSliderPanel.shiftPressed = 2;
    end
end


Events.OnKeyKeepPressed.Add( ISSliderPanel.onKeyKeepPressed );
