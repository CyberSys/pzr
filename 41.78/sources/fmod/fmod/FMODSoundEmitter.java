...
public final class FMODSoundEmitter extends BaseSoundEmitter {
	...
	private void limitSound(GameSound gameSound, int int1) {
		int int2 = this.countToStart(gameSound) + this.countInstances(gameSound);
		if (int2 > int1) {
			int int3;
			FMODSoundEmitter.Sound sound;
			for (int3 = 0; int3 < this.ToStart.size(); ++int3) {
				sound = (FMODSoundEmitter.Sound)this.ToStart.get(int3);
				if (gameSound.clips.contains(sound.clip)) {
					this.ToStart.remove(int3--);
					sound.release();
					--int2;
					if (int2 <= int1) {
						return;
			...
			for (int3 = 0; int3 < this.Instances.size(); ++int3) {
				sound = (FMODSoundEmitter.Sound)this.Instances.get(int3);
				if (gameSound.clips.contains(sound.clip)) {
					if (sound.clip.hasSustainPoints()) {
	...
	private int countToStart(GameSound gameSound) {
		...
		for (int int2 = 0; int2 < this.ToStart.size(); ++int2) {
			FMODSoundEmitter.Sound sound = (FMODSoundEmitter.Sound)this.ToStart.get(int2);
			if (gameSound.clips.contains(sound.clip)) {
				++int1;
			}
		}
	...
	private int countInstances(GameSound gameSound) {
		...
		for (int int2 = 0; int2 < this.Instances.size(); ++int2) {
			FMODSoundEmitter.Sound sound = (FMODSoundEmitter.Sound)this.Instances.get(int2);
			if (gameSound.clips.contains(sound.clip)) {
				++int1;
			}
		}
	...
	private FMODSoundEmitter.Sound addSound(GameSoundClip gameSoundClip, float float1, IsoObject object) {
		if (gameSoundClip == null) {
			...
			if (gameSoundClip.event != null && !gameSoundClip.event.isEmpty()) {
				...
				long long2 = FMODManager.instance.loadSound(gameSoundClip.file);
				if (long2 == 0L) {
					return null;
				} else {
					long long3 = javafmod.FMOD_System_PlaySound(long2, true);
					javafmod.FMOD_Channel_SetVolume(long3, 0.0F);
					javafmod.FMOD_Channel_SetPriority(long3, 9 - gameSoundClip.priority);
					javafmod.FMOD_Channel_SetChannelGroup(long3, FMODManager.instance.channelGroupInGameNonBankSounds);
					if (gameSoundClip.distanceMax == 0.0F || this.x == 0.0F && this.y == 0.0F) {
						javafmod.FMOD_Channel_SetMode(long3, (long)FMODManager.FMOD_2D);
					}
					...
					fileSound.sound = long2;
					fileSound.pitch = gameSoundClip.pitch;
					fileSound.channel = long3;
