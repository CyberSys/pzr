...
public final class TextureID extends Asset implements IDestroyable,Serializable {
	...
	public void setImageData(ImageData imageData) {
		imageData = this.limitMaxSize(imageData);
		this.data = imageData;
	...
	private ImageData limitMaxSize(ImageData imageData) {
		if (this.assetParams == null) {
			...
			int int1 = this.assetParams.flags;
			short short1 = 384;
			if ((int1 & short1) == 0) {
				return imageData;
			} else {
				int int2 = Core.getInstance().getMaxTextureSizeFromFlags(int1);
				if (imageData.getWidth() <= int2 && imageData.getHeight() <= int2) {
					return imageData;
				} else {
					imageData.bPreserveTransparentColor = true;
					MipMapLevel mipMapLevel = imageData.getData();
					int int3;
					for (int3 = 0; mipMapLevel.width > int2 || mipMapLevel.height > int2; mipMapLevel = imageData.getMipMapData(int3)) {
						++int3;
					}
					...
					ImageData imageData2 = new ImageData(imageData.getWidth() >> int3, imageData.getHeight() >> int3, wrappedBuffer);
