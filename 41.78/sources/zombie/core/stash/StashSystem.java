	...
	public static void checkStashItem(InventoryItem inventoryItem) {
		if (!GameClient.bClient && !possibleStashes.isEmpty()) {
			...
			if (inventoryItem.getStashChance() > 0) {
				int1 = inventoryItem.getStashChance();
			}
	...
	public static void doStashItem(Stash stash, InventoryItem inventoryItem) {
		...
		if ("Map".equals(stash.type)) {
			...
			if (stash.annotations != null) {
				for (int int1 = 0; int1 < stash.annotations.size(); ++int1) {
					StashAnnotation stashAnnotation = (StashAnnotation)stash.annotations.get(int1);
									...
									for (int int4 = 0; int4 < 8; ++int4) {
										...
										if (square != null) {
											for (int int5 = 0; int5 < square.getObjects().size(); ++int5) {
												...
												if (object.getContainer() != null && square.getRoom() != null && square.getRoom().getBuilding().getDef() == buildingDef && square.getRoom().getName() != null && itemPickerRoom.Containers.containsKey(object.getContainer().getType())) {
													ItemPickerJava.ItemPickerRoom itemPickerRoom2 = (ItemPickerJava.ItemPickerRoom)ItemPickerJava.rooms.get(square.getRoom().getName());
													boolean boolean1 = false;
													if (itemPickerRoom2 == null || !itemPickerRoom2.Containers.containsKey(object.getContainer().getType())) {
			...
			for (int int2 = 0; int2 < stash.containers.size(); ++int2) {
				...
				if (square != null) {
					if (stashContainer.containerItem != null && !stashContainer.containerItem.isEmpty()) {
						...
						ItemPickerJava.ItemPickerContainer itemPickerContainer = (ItemPickerJava.ItemPickerContainer)itemPickerRoom.Containers.get(inventoryItem.getType());
						ItemPickerJava.rollContainerItem((InventoryContainer)inventoryItem, (IsoGameCharacter)null, itemPickerContainer);
						...
						thumpable.container = new ItemContainer(stashContainer.containerType, square, thumpable);
		...
		if (stash.zombies > -1) {
			for (int3 = 0; int3 < buildingDef.rooms.size(); ++int3) {
				...
				if (IsoWorld.getZombiesEnabled()) {
					...
					ArrayList arrayList2 = VirtualZombieManager.instance.addZombiesToMap(int4, roomDef3, false);
					ZombieSpawnRecorder.instance.record(arrayList2, "StashSystem");
				}
			}
		}
	}
