...
public final class HumanVisual extends BaseVisual {
	...
	public void load(ByteBuffer byteBuffer, int int1) throws IOException {
		...
		if (int1 >= 187) {
			...
			if ((int4 & 4) != 0) {
				int5 = byteBuffer.get() & 255;
				int6 = byteBuffer.get() & 255;
				int7 = byteBuffer.get() & 255;
				this.naturalHairColor = new ImmutableColor(int5, int6, int7);
			}
			...
			if ((int4 & 2) != 0) {
				...
				int6 = byteBuffer.get() & 255;
				int7 = byteBuffer.get() & 255;
				this.naturalBeardColor = new ImmutableColor(int5, int6, int7);
			}
		}
	}
	...
	public ItemVisual removeBodyVisualFromItemType(String string) {
		for (int int1 = 0; int1 < this.bodyVisuals.size(); ++int1) {
			ItemVisual itemVisual = (ItemVisual)this.bodyVisuals.get(int1);
			if (itemVisual.getItemType().equals(string)) {
				this.bodyVisuals.remove(int1);
				return itemVisual;
			}
		}
		...
		if (!StringUtils.isNullOrWhitespace(string) && string.startsWith("version=")) {
			...
			for (int int1 = 0; int1 < stringArray.length; ++int1) {
				...
				if (int2 != -1) {
					...
					case -1034075528: 
						if (string2.equals("naturalHairColor")) {
							byte1 = 6;
						}
					...
					case 394260252: 
						if (string2.equals("naturalBeardColor")) {
							byte1 = 2;
						}
