...
public class UdpEngine {
	...
	public UdpEngine(int int1, int int2, int int3, String string, boolean boolean1) throws ConnectException {
		...
		this.peer.SetMaximumIncomingConnections(int3);
		...
		if (this.bServer) {
			...
			this.peer.SetServerPort(int1, int2);
		...
		this.maxConnections = int3;
		int int4 = this.peer.Startup(int3);
		DebugLog.Network.println("RakNet.Startup() return code: %s (0 means success)", int4);
		if (int4 != 0) {
			throw new ConnectException("Connection Startup Failed. Code: " + int4);
		} else {
			if (boolean1) {
	...
	private void decode(ByteBuffer byteBuffer) {
		...
		case 16: 
			...
			if (GameClient.bClient) {
				GameClient.connection = this.addConnection(int2, long1);
				ConnectionManager.log("RakNet", "connection-request-accepted", this.connectionArray[int2]);
		...
		case 19: 
			...
			ConnectionManager.log("RakNet", "new-incoming-connection", this.connectionArray[int2]);
			break;
		
		case 20: 
			int2 = byteBuffer.get() & 255;
			ConnectionManager.log("RakNet", "no-free-incoming-connections", this.connectionArray[int2]);
			break;
		
		case 21: 
			int2 = byteBuffer.get() & 255;
			long1 = this.peer.getGuidOfPacket();
			ConnectionManager.log("RakNet", "disconnection-notification", this.connectionArray[int2]);
		...
		case 23: 
			int2 = byteBuffer.get() & 255;
			ConnectionManager.log("RakNet", "connection-banned", this.connectionArray[int2]);
			if (GameClient.bClient) {
				GameClient.instance.addDisconnectPacket(int1);
			}
		...
		case 24: 
			int2 = byteBuffer.get() & 255;
			ConnectionManager.log("RakNet", "invalid-password", this.connectionArray[int2]);
			if (GameClient.bClient) {
				GameClient.instance.addDisconnectPacket(int1);
			}
		...
		case 31: 
			int2 = byteBuffer.get() & 255;
			ConnectionManager.log("RakNet", "remote-disconnection-notification", this.connectionArray[int2]);
			break;
		
		case 32: 
			int2 = byteBuffer.get() & 255;
			ConnectionManager.log("RakNet", "remote-connection-lost", this.connectionArray[int2]);
			if (GameClient.bClient) {
				GameClient.instance.addDisconnectPacket(int1);
			}
		...
		case 33: 
			int2 = byteBuffer.get() & 255;
			ConnectionManager.log("RakNet", "remote-new-incoming-connection", this.connectionArray[int2]);
			break;
		...
		case 46: 
			long2 = this.peer.getGuidOfPacket();
			UdpConnection udpConnection = (UdpConnection)this.connectionMap.get(long2);
			break;
		...
		default: 
			...
			if (GameServer.bServer) {
				long2 = this.peer.getGuidOfPacket();
				UdpConnection udpConnection3 = (UdpConnection)this.connectionMap.get(long2);
				try {
					if (ServerOptions.instance.AntiCheatProtectionType10.getValue() && PacketValidator.checkUser(udpConnection3)) {
						PacketValidator.doKickUser(udpConnection3, String.valueOf(int1), "Type10", (String)null);
					}
				} catch (Exception exception) {
	...
	public void Connect(String string, int int1, String string2, boolean boolean1) {
		if (int1 == 0 && SteamUtils.isSteamModeEnabled()) {
			...
			this.peer.ConnectToSteamServer(long1, this.hashServerPassword(string2), boolean1);
		} else {
			String string3;
			try {
			...
			this.peer.Connect(string3, int1, this.hashServerPassword(string2), boolean1);
		}
	}

	public void forceDisconnect(long long1, String string) {
		this.peer.disconnect(long1, string);
