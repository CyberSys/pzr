...
public class VoiceManager {
	...
	public void ResetRecSound() {
		...
		if (this.initialisedRecDev && this.FMODRecordSound != 0L) {
			int1 = javafmod.FMOD_System_RecordStop(this.FMODVoiceRecordDriverId);
								...
								for (int int4 = (int)player2.getZ() - 1; (float)int4 < player2.getZ() + 1.0F; ++int4) {
									...
									if (square != null) {
										...
										if (square.getVehicleContainer() != null && square == square.getVehicleContainer().getSquare()) {
											VehiclePart vehiclePart = square.getVehicleContainer().getPartById("Radio");
											if (vehiclePart != null) {
												DeviceData deviceData3 = vehiclePart.getDeviceData();
												if (deviceData3 != null && deviceData3.getIsTurnedOn()) {
													voiceManagerData.radioData.add(new VoiceManagerData.RadioData(deviceData3, (float)square.x, (float)square.y));
												}
											}
										}
									}
