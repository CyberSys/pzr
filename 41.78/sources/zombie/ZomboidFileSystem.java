...
public final class ZomboidFileSystem {
	...
	public File getCanonicalFile(File file, String string) {
		if (!file.isDirectory()) {
			...
			File[] fileArray = file.listFiles((filex,stringx)->{
				return stringx.equalsIgnoreCase(string);
			});

			return fileArray != null && fileArray.length != 0 ? fileArray[0] : new File(file, string);
		}
	}
	...
	public String getCurrentSaveDir() {
		String string = this.getGameModeCacheDir();
		return string + File.separator + Core.GameSaveWorld;
	}

	public String getFileNameInCurrentSave(String string) {
		String string2 = this.getCurrentSaveDir();
	...
	public URI getCanonicalURI(String string) {
		URI uRI = (URI)this.CanonicalURIMap.get(string);
		if (uRI == null) {
	...
	public void loadMod(String string) {
		if (this.getModDir(string) != null) {
			...
			for (int int1 = 0; int1 < this.loadList.size(); ++int1) {
				...
				String string3 = (new File((String)this.loadList.get(int1))).getAbsolutePath();
