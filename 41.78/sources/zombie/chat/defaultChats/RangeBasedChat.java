...
public abstract class RangeBasedChat extends ChatBase {
	...
	public void sendMessageToChatMembers(ChatMessage chatMessage) {
		...
		if (this.getRange() == -1.0F) {
			...
			while (iterator.hasNext()) {
				...
				if (player2 != null && player.getOnlineID() != short1 && ChatUtility.getDistance(player, player2) < this.getRange()) {
	...
	protected UIFont selectFont(String string) {
		...
		for (int int1 = 0; int1 < charArray.length; ++int1) {
			if (charArray[int1] > angelCodeFont.chars.length) {
	...
	protected void showInSpeechBubble(ChatMessage chatMessage) {
		...
		if (string != null && !"".equalsIgnoreCase(string) && !string.equalsIgnoreCase(currentPlayerName)) {
			...
			if (player2 != null) {
				...
				player2.getChatElement().addChatLine(string2, float1, float2, float3, this.selectFont(string2), this.getRange(), this.customTag, this.isAllowBBcode(), this.isAllowImages(), this.isAllowChatIcons(), this.isAllowColors(), this.isAllowFonts(), this.isEqualizeLineHeights());
			}
		} else {
			overHeadChat.addChatLine(string2, float1, float2, float3, this.selectFont(string2), this.getRange(), this.customTag, this.isAllowBBcode(), this.isAllowImages(), this.isAllowChatIcons(), this.isAllowColors(), this.isAllowFonts(), this.isEqualizeLineHeights());
		}
	}
