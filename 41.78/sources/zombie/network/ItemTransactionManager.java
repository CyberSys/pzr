		...
		try {
			...
			if (0 == byte1) {
				if (isConsistent(int1, int2, int3)) {
					...
					DebugLog.Multiplayer.trace("set accepted [ %d : %d => %d ]", int1, int2, int3);
				} else {
					sendItemTransaction(udpConnection, (byte)1, int1, int2, int3);
					DebugLog.Multiplayer.trace("set rejected [ %d : %d => %d ]", int1, int2, int3);
				}
			} else {
				...
				DebugLog.Multiplayer.trace("remove processed [ %d : %d => %d ]", int1, int2, int3);
			}
		} catch (Exception exception) {
			DebugLog.Multiplayer.printException(exception, "ReceiveOnClient: failed", LogSeverity.Error);
		}
