	...
	public void tick() {
		...
		for (int1 = 0; int1 < this.Instances.size(); ++int1) {
			...
			if (!javafmod.FMOD_Channel_IsPlaying(sound.channel)) {
				...
				float float1 = IsoUtils.DistanceManhatten(sound.x, sound.y, IsoPlayer.getInstance().x, IsoPlayer.getInstance().y, sound.z, IsoPlayer.getInstance().z) / sound.maxRange;
				if (float1 > 1.0F) {
					float1 = 1.0F;
				}
				...
				float float5 = (float)Math.toDegrees((double)IsoPlayer.getInstance().getForwardDirection().getDirectionNeg());
