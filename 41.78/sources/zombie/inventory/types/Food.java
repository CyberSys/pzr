...
public final class Food extends InventoryItem {
	...
	public void DoTooltip(ObjectTooltip objectTooltip, ObjectTooltip.Layout layout) {
		...
		if (this.isIsCookable() && !this.isFrozen() && !this.Burnt && (double)this.getHeat() > 1.6) {
			...
			float float8 = 1.0F;
			float float9 = colorInfo.getR();
			float float10 = colorInfo.getG();
			float float11 = colorInfo.getB();
			...
			if (float7 > float1) {
				...
				float9 = colorInfo.getR();
				float10 = colorInfo.getG();
				float11 = colorInfo.getB();
				float3 = (float7 - float1) / (float2 - float1);
				float4 = colorInfo.getR();
				float5 = colorInfo.getG();
				float6 = colorInfo.getB();
			}
			...
			layoutItem.setProgress(float3, float4, float5, float6, float8);
		}

		if (this.getFreezingTime() < 100.0F && this.getFreezingTime() > 0.0F) {
			float7 = this.getFreezingTime() / 100.0F;
			float1 = 0.0F;
			float2 = 0.6F;
			float3 = 0.0F;
			float float12 = 0.7F;
			...
			float6 = 0.8F;
			layoutItem = layout.addItem();
			layoutItem.setLabel(Translator.getText("IGUI_invpanel_FreezingTime") + ": ", float4, float5, float6, 1.0F);
			layoutItem.setProgress(float7, float1, float2, float3, float12);
		}
	...
	public float getUnhappyChange() {
		...
		Boolean Boolean1 = "Icecream".equals(this.getType()) || this.hasTag("GoodFrozen");
		if (this.isFrozen() && !Boolean1) {
			float1 += 30.0F;
		}
