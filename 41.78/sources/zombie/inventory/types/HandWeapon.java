...
public final class HandWeapon extends InventoryItem {
	...
	public float getActualWeight() {
		float float1 = this.getScriptItem().getActualWeight();
		float1 += this.getWeaponPartWeightModifier(this.canon);
		float1 += this.getWeaponPartWeightModifier(this.clip);
		float1 += this.getWeaponPartWeightModifier(this.recoilpad);
		float1 += this.getWeaponPartWeightModifier(this.scope);
		float1 += this.getWeaponPartWeightModifier(this.sling);
		float1 += this.getWeaponPartWeightModifier(this.stock);
		return float1;
	}
	...
	public void DoTooltip(ObjectTooltip objectTooltip, ObjectTooltip.Layout layout) {
		...
		float float5 = (float)this.Condition / (float)this.ConditionMax;
		Core.getInstance().getBadHighlitedColor().interp(Core.getInstance().getGoodHighlitedColor(), float5, colorInfo);
		layoutItem.setProgress(float5, colorInfo.getR(), colorInfo.getG(), colorInfo.getB(), 1.0F);
		float float6;
		float float7;
		if (this.getMaxDamage() > 0.0F) {
			...
			float5 = this.getMaxDamage() + this.getMinDamage();
			float6 = 5.0F;
			float7 = float5 / float6;
			Core.getInstance().getBadHighlitedColor().interp(Core.getInstance().getGoodHighlitedColor(), float7, colorInfo);
			layoutItem.setProgress(float7, colorInfo.getR(), colorInfo.getG(), colorInfo.getB(), 1.0F);
		}

		if (this.isRanged()) {
			layoutItem = layout.addItem();
			layoutItem.setLabel(Translator.getText("Tooltip_weapon_Range") + ":", float1, float2, float3, 1.0F);
			float5 = this.getMaxRange(IsoPlayer.getInstance());
			float6 = 40.0F;
			float7 = float5 / float6;
			Core.getInstance().getBadHighlitedColor().interp(Core.getInstance().getGoodHighlitedColor(), float7, colorInfo);
			layoutItem.setProgress(float7, colorInfo.getR(), colorInfo.getG(), colorInfo.getB(), 1.0F);
