...
public class InventoryItem {
	...
	public void DoTooltip(ObjectTooltip objectTooltip) {
		...
		if (!(this instanceof HandWeapon) && this.getAmmoType() != null) {
			...
			String string4 = InventoryItemFactory.CreateItem(this.getAmmoType()).getDisplayName();
			layoutItem.setValue(Translator.getText(string4), 1.0F, 1.0F, 1.0F, 1.0F);
		}
		...
		if (this instanceof DrainableComboItem) {
			...
			Core.getInstance().getBadHighlitedColor().interp(Core.getInstance().getGoodHighlitedColor(), float1, colorInfo);
			layoutItem.setProgress(float1, colorInfo.getR(), colorInfo.getG(), colorInfo.getB(), 1.0F);
		}
		...
		if (this.isWet()) {
			...
			colorInfo = new ColorInfo();
			Core.getInstance().getGoodHighlitedColor().interp(Core.getInstance().getBadHighlitedColor(), float1, colorInfo);
			layoutItem.setProgress(float1, colorInfo.getR(), colorInfo.getG(), colorInfo.getB(), 1.0F);
		}
	...
	public void update() {
		...
		if (!GameClient.bClient && !this.rainFactorZero && this.getWorldItem() != null && this.canStoreWater() && this.hasReplaceType("WaterSource") && RainManager.isRaining()) {
			...
			if (square2 != null && square2.isOutside()) {
				InventoryItem inventoryItem2 = InventoryItemFactory.CreateItem(this.getReplaceType("WaterSource"));
