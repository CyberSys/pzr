...
public final class ItemContainer {
		...
		if (inventoryItem == null) {
			...
			if (string != null && !string.equalsIgnoreCase(inventoryItem.getCategory())) {
				...
				if (this.parent != null && !this.parent.isItemAllowedInContainer(this, inventoryItem)) {
					return false;
				} else if (this.getType().equals("clothingrack") && !(inventoryItem instanceof Clothing)) {
					return false;
				} else if (this.getParent() != null && this.getParent().getProperties() != null && this.getParent().getProperties().Val("CustomName") != null && this.getParent().getProperties().Val("CustomName").equals("Toaster") && !inventoryItem.hasTag("FitsToaster")) {
					return false;
				} else {
					if (this.getParent() != null && this.getParent().getProperties() != null && this.getParent().getProperties().Val("GroupName") != null) {
						boolean boolean1 = this.getParent().getProperties().Val("GroupName").equals("Coffee") || this.getParent().getProperties().Val("GroupName").equals("Espresso");
						if (boolean1 && !inventoryItem.hasTag("CoffeeMaker")) {
							return false;
						}
					}
	...
	public InventoryItem getBestConditionEvalArg(LuaClosure luaClosure, Object object) {
		ItemContainer.EvalArgPredicate evalArgPredicate = ((ItemContainer.EvalArgPredicate)((ItemContainer.Predicates)TL_predicates.get()).evalArg.alloc()).init(luaClosure, object);
		InventoryItem inventoryItem = this.getBestCondition((Predicate)evalArgPredicate);
		((ItemContainer.Predicates)TL_predicates.get()).evalArg.release((Object)evalArgPredicate);
		return inventoryItem;
	}
	...
	public InventoryItem getBestConditionEvalArgRecurse(LuaClosure luaClosure, Object object) {
		ItemContainer.EvalArgPredicate evalArgPredicate = ((ItemContainer.EvalArgPredicate)((ItemContainer.Predicates)TL_predicates.get()).evalArg.alloc()).init(luaClosure, object);
		InventoryItem inventoryItem = this.getBestConditionRecurse((Predicate)evalArgPredicate);
		((ItemContainer.Predicates)TL_predicates.get()).evalArg.release((Object)evalArgPredicate);
		return inventoryItem;
	}
