...
public final class RecipeManager {
	...
	public static boolean IsRecipeValid(Recipe recipe, IsoGameCharacter gameCharacter, InventoryItem inventoryItem, ArrayList arrayList) {
		if (recipe.Result == null) {
			...
			return false;
		} else if (!isAllItemsUsableRotten(recipe, gameCharacter, inventoryItem, arrayList)) {
			return false;
	...
	public static boolean isAllItemsUsableRotten(Recipe recipe, IsoGameCharacter gameCharacter, InventoryItem inventoryItem, ArrayList arrayList) {
		if (gameCharacter.getPerkLevel(PerkFactory.Perks.Cooking) >= 7) {
			return true;
			...
			}	 while (!(inventoryItem2 instanceof Food) || !((Food)inventoryItem2).isRotten());
	...
	private static final class RMRecipe {
		...
		void getItemsFromContainer(IsoGameCharacter gameCharacter, ItemContainer itemContainer, InventoryItem inventoryItem) {
			for (int int1 = 0; int1 < itemContainer.getItems().size(); ++int1) {
				...
				if ((inventoryItem != null && inventoryItem == inventoryItem2 || !gameCharacter.isEquippedClothing(inventoryItem2) || this.isKeep(inventoryItem2.getFullType())) && (!this.recipe.InSameInventory || this.isKeep(inventoryItem2.getFullType()) || inventoryItem == null || itemContainer == inventoryItem.getContainer())) {
			...
			if (boolean1) {
				...
				for (int int2 = 0; int2 < this.itemLists.size(); ++int2) {
					...
					if (rMRecipeItemList2.hasItems()) {
						InventoryItem inventoryItem = ((RecipeManager.RMRecipeItem)rMRecipeItemList2.items.get(0)).item;
