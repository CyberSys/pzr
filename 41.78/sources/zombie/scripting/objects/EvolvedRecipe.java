...
public final class EvolvedRecipe extends BaseScriptObject {
	...
	public InventoryItem addItem(InventoryItem inventoryItem, InventoryItem inventoryItem2, IsoGameCharacter gameCharacter) {
		...
		if (this.itemsList.get(inventoryItem2.getType()) != null && ((ItemRecipe)this.itemsList.get(inventoryItem2.getType())).use > -1) {
			if (!(inventoryItem2 instanceof Food)) {
				...
				boolean boolean1 = food2.hasTag("HerbalTea") && food.hasTag("HerbalTea");
				if (food.isSpice() && inventoryItem instanceof Food) {
					if (inventoryItem instanceof Food && boolean1) {
				...
				if (inventoryItem instanceof Food) {
					...
					food2.setHungChange(food2.getHungChange() - float2);
					...
					inventoryItem.setUnhappyChange(((Food)inventoryItem).getUnhappyChangeUnmodified() - (float)(5 - int2 * 5));
					...
					float float6 = food.getThirstChangeUnmodified() * float5 * float4;
					if (!food.hasTag("DriedFood")) {
						food2.setThirstChange(food2.getThirstChangeUnmodified() + float6);
					}
					...
					food.setThirstChange(food.getThirstChange() - float6);
					...
					if ((double)food.getHungerChange() >= -0.02 || boolean2) {
						inventoryItem2.Use();
					}
					...
					if (food.getPoisonPower() > 0) {
						food.setPoisonPower((int)((double)((float)food.getPoisonPower() - (float)food.getPoisonPower() * float4) + 0.999));
						((Food)inventoryItem).setPoisonPower((int)((double)((float)food.getPoisonPower() * float4) + 0.999));
					}
				}
			}
	...
	private void useSpice(Food food, Food food2, float float1, int int1) {
		if (!this.isSpiceAdded(food2, food)) {
			...
			food2.setUnhappyChange(food2.getUnhappyChangeUnmodified() - float1 * 200.0F);
			food2.setBoredomChange(food2.getBoredomChangeUnmodified() - float1 * 200.0F);
