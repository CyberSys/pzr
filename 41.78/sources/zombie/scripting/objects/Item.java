...
public final class Item extends BaseScriptObject {
	...
	public String getSoundByID(String string) {
		return this.SoundMap == null ? null : (String)this.SoundMap.getOrDefault(string, (Object)null);
	}
	...
	public String getReplaceType(String string) {
		return this.ReplaceTypesMap == null ? null : (String)this.ReplaceTypesMap.get(string);
	}
	...
	public InventoryItem InstanceItem(String string) {
		...
		if (this.type == Item.Type.Literature) {
			...
			if (this.type == Item.Type.AlarmClockClothing) {
				...
				if (!StringUtils.isNullOrWhitespace(this.worldObjectSprite) && !radio.ReadFromWorldSprite(this.worldObjectSprite)) {
					string4 = this.moduleDotType != null ? this.moduleDotType : "unknown";
					DebugLog.log("Item -> Radio item = " + string4);
				}
		...
		if ((thread == GameWindow.GameThread || thread == GameLoadingState.loader || thread == GameServer.MainThread) && !((InventoryItem)object).isInitialised()) {
	...
	public void DoParam(String string) {
		if (string.trim().length() != 0) {
			try {
					...
					if (string2.trim().equalsIgnoreCase("Palettes")) {
						...
						if (string2.trim().equalsIgnoreCase("Icon")) {
							...
							if (this.type == Item.Type.Food) {
								...
								string5 = this.WorldTextureName.replace(".png", "Rotten.png");
								if (texture == null) {
							...
							if (string2.trim().equalsIgnoreCase("ReplaceOnUseOn")) {
								...
								if (string3.contains("-")) {
									...
									if (!string5.isEmpty() && !string6.isEmpty()) {
										...
										this.ReplaceTypesMap.put(string5, string6);
									}
								}
							} else {
								String string7;
								...
								if (string2.trim().equalsIgnoreCase("ReplaceTypes")) {
									...
									int2 = stringArray2.length;
									for (int int3 = 0; int3 < int2; ++int3) {
										String string8 = stringArray3[int3];
										String[] stringArray4 = string8.trim().split("\\s+");
										if (stringArray4.length == 2) {
											string7 = stringArray4[0].trim();
											String string9 = stringArray4[1].trim();
											if (!string7.isEmpty() && !string9.isEmpty()) {
												if (this.ReplaceTypesMap == null) {
													this.ReplaceTypesMap = new HashMap();
												}

												this.ReplaceTypesMap.put(string7, string9);
											}
										}
									}
									...
									if (stringArray2.length >= 2 && stringArray2.length <= 4) {
										...
										for (int2 = 0; int2 < this.modelWeaponPart.size(); ++int2) {
								...
								} else if (string2.trim().equalsIgnoreCase("SoundMap")) {
									stringArray2 = string3.split("\\s+");
									if (stringArray2.length == 2 && !stringArray2[0].trim().isEmpty()) {
										if (this.SoundMap == null) {
											this.SoundMap = new HashMap();
										}

										this.SoundMap.put(stringArray2[0].trim(), stringArray2[1].trim());
									}
								...
								} else if (string2.trim().equalsIgnoreCase("PlaceOneSound")) {
									this.PlaceOneSound = StringUtils.discardNullOrWhitespace(string3.trim());
								} else if (string2.trim().equalsIgnoreCase("PlaceMultipleSound")) {
								...
								} else if (string2.trim().equalsIgnoreCase("FillFromDispenserSound")) {
									this.fillFromDispenserSound = StringUtils.discardNullOrWhitespace(string3.trim());
								} else if (string2.trim().equalsIgnoreCase("FillFromTapSound")) {
								...
								} else if (string2.trim().equalsIgnoreCase("TeachedRecipes")) {
									...
									for (int1 = 0; int1 < stringArray2.length; ++int1) {
										...
										if (!string6.contains(":")) {
											...
											string7 = string6.split(":")[1];
											if (!string7.contains("|")) {
												int4 = Integer.parseInt(string6.split(":")[1]);
											} else {
												String[] stringArray5 = string7.split("\\|");
												for (int int5 = 0; int5 < stringArray5.length; ++int5) {
													if ("Cooked".equals(stringArray5[int5])) {
														boolean1 = true;
													}
												}

												int4 = Integer.parseInt(stringArray5[0]);
											}
										}

										ItemRecipe itemRecipe = new ItemRecipe(this.name, this.module.getName(), int4);
								...
								} else if (string2.trim().equalsIgnoreCase("WorldRender")) {
									this.worldRender = Boolean.parseBoolean(string3.trim());
								} else if (string2.trim().equalsIgnoreCase("CantEat")) {
								...
								} else if (string2.trim().equalsIgnoreCase("OnCreate")) {
									this.LuaCreate = string3.trim();
								} else if (string2.trim().equalsIgnoreCase("SoundParameter")) {
									stringArray2 = string3.split("\\s+");
									if (this.soundParameterMap == null) {
										this.soundParameterMap = new HashMap();
									}

									this.soundParameterMap.put(stringArray2[0].trim(), stringArray2[1].trim());
								} else {
									String string10 = string2.trim();
									DebugLog.log("adding unknown item param \"" + string10 + "\" = \"" + string3.trim() + "\"");
				...
				throw new InvalidParameterException("Error: " + string11 + " is not a valid parameter in item: " + this.name);
			}
		}
	}
	...
	public void resolveItemTypes() {
		...
		if (this.RequireInHandOrInventory != null) {
			for (int int1 = 0; int1 < this.RequireInHandOrInventory.size(); ++int1) {
				string = (String)this.RequireInHandOrInventory.get(int1);
		...
		if (this.ReplaceTypesMap != null) {
			Iterator iterator = this.ReplaceTypesMap.keySet().iterator();
			while (iterator.hasNext()) {
				string = (String)iterator.next();
				String string2 = (String)this.ReplaceTypesMap.get(string);
	...
	public void setRegistry_id(short short1) {
		if (this.registry_id != -1) {
			...
			short short2 = this.registry_id;
			throw new RuntimeException("Cannot override existing registry id (" + short2 + ") to new id (" + short1 + "), item: " + (this.getFullName() != null ? this.getFullName() : "unknown"));
		} else {
	...
	public String getSoundParameter(String string) {
		return this.soundParameterMap == null ? null : (String)this.soundParameterMap.get(string);
	}
