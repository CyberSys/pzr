		...
		if (!this.bClient) {
			if (worldSound.radius >= 50) {
				if (!worldSound.sourceIsZombie) {
					...
					float float1 = WorldSoundManager.instance.getHearingMultiplier(int1);
					n_worldSound(worldSound.x, worldSound.y, (int)PZMath.ceil((float)worldSound.radius * float1), worldSound.volume);
				}
			}
		}
	}
