...
public class SafeHouse {
	...
	public static void updateSafehousePlayersConnected() {
		...
		label51: for (int int1 = 0; int1 < safehouseList.size(); ++int1) {
			safeHouse = (SafeHouse)safehouseList.get(int1);
			safeHouse.setPlayerConnected(0);
			Iterator iterator;
			IsoPlayer player;
			if (GameClient.bClient) {
				iterator = GameClient.IDToPlayerMap.values().iterator();
				while (true) {
					do {
						if (!iterator.hasNext()) {
							continue label51;
		...
		if (GameClient.bClient) {
			...
			while (true) {
				do {
					...
					player = (IsoPlayer)iterator.next();
				}		 while (!this.getPlayers().contains(player.getUsername()) && !this.getOwner().equals(player.getUsername()));
	...
	public static String canBeSafehouse(IsoGridSquare square, IsoPlayer player) {
		if (!GameClient.bClient && !GameServer.bServer) {
			...
			if (int1 > 0 && player.getHoursSurvived() < (double)(int1 * 24)) {
				string = string + Translator.getText("IGUI_Safehouse_DaysSurvivedToClaim", int1) + System.lineSeparator();
			}
			...
			if (GameClient.bClient) {
				...
				while (kahluaTableIterator.advance()) {
					...
					while (kahluaTableIterator2.advance()) {
						...
						KahluaTableIterator kahluaTableIterator3 = kahluaTable2.iterator();
						while (kahluaTableIterator3.advance()) {
							KahluaTable kahluaTable3 = (KahluaTable)kahluaTableIterator3.getValue();
							Double Double1 = (Double)kahluaTable3.rawget("worldX");
							Double Double2 = (Double)kahluaTable3.rawget("worldY");
							Double Double3 = (Double)kahluaTable3.rawget("posX");
							Double Double4 = (Double)kahluaTable3.rawget("posY");
			...
			for (int int2 = 0; int2 < cell.getObjectList().size(); ++int2) {
				IsoMovingObject movingObject = (IsoMovingObject)cell.getObjectList().get(int2);
				if (movingObject != player && movingObject instanceof IsoGameCharacter && movingObject.getX() >= (float)(buildingDef2.getX() - diffError) && movingObject.getX() < (float)(buildingDef2.getX2() + diffError) && movingObject.getY() >= (float)(buildingDef2.getY() - diffError) && movingObject.getY() < (float)(buildingDef2.getY2() + diffError)) {
					boolean1 = false;
					break;
				}
			}

			if (player.getX() >= (float)(buildingDef2.getX() - diffError) && player.getX() < (float)(buildingDef2.getX2() + diffError) && player.getY() >= (float)(buildingDef2.getY() - diffError) && player.getY() < (float)(buildingDef2.getY2() + diffError) && player.getCurrentSquare() != null && !player.getCurrentSquare().Is(IsoFlagType.exterior)) {
				boolean2 = true;
			}
			...
			if (!boolean4 && !ServerOptions.instance.SafehouseAllowNonResidential.getValue()) {
		...
		if (GameServer.bServer && !ServerOptions.instance.SafehouseAllowTrepass.getValue() && player.getVehicle() == null && !player.isAccessLevel("admin")) {
			...
			if (safeHouse != null) {
				...
				if (player.isAsleep()) {
					...
					GameServer.sendWakeUpPlayer(player, (UdpConnection)null);
				}
			}
		}
	}
	...
	public static boolean isPlayerAllowedOnSquare(IsoPlayer player, IsoGridSquare square) {
		if (!ServerOptions.instance.SafehouseAllowTrepass.getValue()) {
			return isSafeHouse(square, player.getUsername(), true) == null;
		} else {
			return true;
		}
	}
