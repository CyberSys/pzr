...
public final class WorldStreamer {
	...
	public void updateMain() {
		...
		if (!this.tempRequests.isEmpty()) {
			...
			PacketTypes.PacketType.RequestZipList.doPacket(byteBufferWriter);
		...
		for (int int7 = 0; int7 < this.pendingRequests.size(); ++int7) {
			...
			if ((chunkRequest2.flagsWS & 1) != 0) {
				...
				if (chunkRequest2.partsReceived == null) {
					chunkRequest2.partsReceived = new boolean[int2];
				}

				chunkRequest2.partsReceived[int3] = true;
	...
	private static final class ChunkRequest {
		...
		boolean isReceived() {
			if (this.partsReceived == null) {
				...
				for (int int1 = 0; int1 < this.partsReceived.length; ++int1) {
					if (!this.partsReceived[int1]) {
						return false;
					}
				}
