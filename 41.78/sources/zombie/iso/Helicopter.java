		...
		if (this.bActive) {
			if (GameClient.bClient) {
				...
				if (GameServer.bServer) {
					...
					float1 = GameTime.getInstance().getTrueMultiplier();
				}
				...
				case Hovering: 
					if (this.target != null && !this.target.isDead()) {
						this.hoverTime += GameTime.getInstance().getRealworldSecondsSinceLastUpdate() * float1;
						if (this.hoverTime + this.searchTime > MAX_BOTHER_SECONDS) {
							this.changeState(Helicopter.State.Leaving);
						} else {
							if (!this.isTargetVisible()) {
								this.timeSinceChopperSawPlayer += GameTime.getInstance().getRealworldSecondsSinceLastUpdate() * float1;
				...
				case Searching: 
					if (this.target != null && !this.target.isDead()) {
						this.searchTime += GameTime.getInstance().getRealworldSecondsSinceLastUpdate() * float1;
						if (this.hoverTime + this.searchTime > MAX_BOTHER_SECONDS) {
							this.changeState(Helicopter.State.Leaving);
				...
				float float2 = this.move.x * (GameTime.getInstance().getMultiplier() / 1.6F);
				float float3 = this.move.y * (GameTime.getInstance().getMultiplier() / 1.6F);
				if (this.state != Helicopter.State.Leaving && IsoUtils.DistanceToSquared(this.x + float2, this.y + float3, this.targetX, this.targetY) > IsoUtils.DistanceToSquared(this.x, this.y, this.targetX, this.targetY)) {
					this.x = this.targetX;
					this.y = this.targetY;
				} else {
					this.x += float2;
