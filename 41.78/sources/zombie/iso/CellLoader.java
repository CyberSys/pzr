	...
	public static void DoTileObjectCreation(IsoSprite sprite, IsoObjectType objectType, IsoGridSquare square, IsoCell cell, int int1, int int2, int int3, String string) throws NumberFormatException {
		...
		if (square != null) {
			...
			if (smashedWindowSpriteMap.containsKey(sprite)) {
				sprite = (IsoSprite)smashedWindowSpriteMap.get(sprite);
				...
				if (objectType == IsoObjectType.lightswitch) {
					...
					if (((IsoObject)object).sprite.getProperties().Is("lightR")) {
						...
						if (((IsoObject)object).sprite.getProperties().Is("LightRadius") && Integer.parseInt(((IsoObject)object).sprite.getProperties().Val("LightRadius")) > 0) {
							int4 = Integer.parseInt(((IsoObject)object).sprite.getProperties().Val("LightRadius"));
						}

						IsoLightSource lightSource = new IsoLightSource(((IsoObject)object).square.getX(), ((IsoObject)object).square.getY(), ((IsoObject)object).square.getZ(), float2, float3, float1, int4);
					...
					if (!sprite.getProperties().Is(IsoFlagType.windowW) && !sprite.getProperties().Is(IsoFlagType.windowN)) {
								...
								if (sprite.getProperties().Is("signal")) {
									...
									if (sprite.getProperties().Is(IsoFlagType.WallOverlay)) {
										...
										if (sprite.getProperties().Is(IsoFlagType.attachedSE)) {
											...
											for (int int5 = square.getObjects().size() - 1; int5 >= 0; --int5) {
												IsoObject object3 = (IsoObject)square.getObjects().get(int5);
									...
									if (sprite.getProperties().Is(IsoFlagType.FloorOverlay)) {
										...
										for (int int6 = 0; int6 < square.getObjects().size(); ++int6) {
											IsoObject object5 = (IsoObject)square.getObjects().get(int6);
											if (object5 instanceof IsoTree) {
												square.getObjects().remove(int6);
												object5.reset();
						...
						if (boolean1) {
							((IsoWindow)object).setSmashed(true);
						}
					...
					boolean boolean3 = Integer.parseInt(string.substring(string.lastIndexOf("_") + 1)) % 8 <= 3;
					object = new IsoCurtain(cell, square, sprite, objectType == IsoObjectType.curtainN || objectType == IsoObjectType.curtainS, boolean3);
					AddSpecialObject(square, (IsoObject)object);
					GameClient.instance.objectSyncReq.putRequest(square, (IsoObject)object);
				}
			} else {
