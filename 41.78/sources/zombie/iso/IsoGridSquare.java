...
public final class IsoGridSquare {
	...
	public boolean hasDoorOnEdge(IsoDirections directions, boolean boolean1) {
		for (int int1 = 0; int1 < this.SpecialObjects.size(); ++int1) {
			IsoDoor door = (IsoDoor)Type.tryCastTo((IsoObject)this.SpecialObjects.get(int1), IsoDoor.class);
			if (door != null && door.getSpriteEdge(boolean1) == directions) {
				return true;
			}

			IsoThumpable thumpable = (IsoThumpable)Type.tryCastTo((IsoObject)this.SpecialObjects.get(int1), IsoThumpable.class);
	...
	public boolean hasClosedDoorOnEdge(IsoDirections directions) {
		...
		for (int int1 = 0; int1 < this.SpecialObjects.size(); ++int1) {
			IsoDoor door = (IsoDoor)Type.tryCastTo((IsoObject)this.SpecialObjects.get(int1), IsoDoor.class);
			if (door != null && !door.IsOpen() && door.getSpriteEdge(boolean1) == directions) {
				return true;
			}

			IsoThumpable thumpable = (IsoThumpable)Type.tryCastTo((IsoObject)this.SpecialObjects.get(int1), IsoThumpable.class);
			if (thumpable != null && !thumpable.IsOpen() && thumpable.getSpriteEdge(boolean1) == directions) {
				return true;
			}
		}
	...
	public IsoCurtain getCurtain(IsoObjectType objectType) {
		for (int int1 = 0; int1 < this.getSpecialObjects().size(); ++int1) {
			IsoCurtain curtain = (IsoCurtain)Type.tryCastTo((IsoObject)this.getSpecialObjects().get(int1), IsoCurtain.class);
			if (curtain != null && curtain.getType() == objectType) {
				return curtain;
			}
		}
	...
	public void DoCutawayShader(IsoObject object, IsoDirections directions, boolean boolean1, boolean boolean2, boolean boolean3, boolean boolean4, boolean boolean5, boolean boolean6, boolean boolean7, WallShaperWhole wallShaperWhole) {
		...
		if (texture != null && texture.getID() != -1) {
			...
			int int2 = 2 / Core.TileScale;
			try {
				Texture texture2 = object.getCurrentFrameTex();
				float float1 = 0.0F;
				float float2 = object.getCurrentFrameTex().getOffsetY();
				int int3 = 0;
				int int4 = 226 - texture2.getHeight() * int2;
				if (directions != IsoDirections.NW) {
					int3 = 66 - texture2.getWidth() * int2;
				}

				if (object.sprite.getProperties().Is(IsoFlagType.WallSE)) {
					int3 = 6 - texture2.getWidth() * int2;
					int4 = 196 - texture2.getHeight() * int2;
				}
				...
				if (object.sprite.name.contains("fencing_01_11")) {
					...
					String string;
					int int5;
					if (object.sprite.name.contains("location_community_medical")) {
						string = object.sprite.name.replaceAll("(.*)_", "");
						int5 = Integer.parseInt(string);
						switch (int5) {
						case 45: 
						...
						if (int5 == 4) {
							...
							float1 = -46.0F;
						} else if (int5 == 28 && !object.sprite.name.contains("03")) {
							float1 = -60.0F;
						} else if (int5 == 41) {
							float1 = -46.0F;
						}
				...
				if (directions == IsoDirections.N || directions == IsoDirections.NW) {
					...
					if (boolean4) {
						...
						if (object.sprite.name.contains("garage") || object.sprite.name.contains("industry_trucks")) {
							int6 = object.sprite.tileSheetIndex;
							if (int6 % 8 == 5) {
								short2 = 1356;
							} else if (int6 % 8 == 4) {
								short2 = 1582;
					...
					if (object.sprite.getProperties().Is(IsoFlagType.WallSE)) {
						SpriteRenderer.instance.setCutawayTexture(texture, short1 + (int)float1, short2 + (int)float2, 6 - int3, 196 - int4);
					} else {
						SpriteRenderer.instance.setCutawayTexture(texture, short3 + (int)float1, short2 + (int)float2, 66 - int3, 226 - int4);
					}
	...
	public void DoCutawayShaderSprite(IsoSprite sprite, IsoDirections directions, boolean boolean1, boolean boolean2, boolean boolean3) {
		...
		if (texture != null && texture.getID() != -1) {
			...
			try {
				...
				float float1 = 0.0F;
				float float2 = texture2.getOffsetY();
				int int3 = 0;
				int int4 = 226 - texture2.getHeight() * int2;
				if (directions != IsoDirections.NW) {
					int3 = 66 - texture2.getWidth() * int2;
				...
				if (sprite.name.contains("fencing_01_11")) {
					...
					String string;
					int int5;
					if (sprite.name.contains("location_community_medical")) {
						string = sprite.name.replaceAll("(.*)_", "");
						int5 = Integer.parseInt(string);
						switch (int5) {
						case 45: 
				...
				if (directions == IsoDirections.N || directions == IsoDirections.NW) {
					...
					coll2 = this.getVertLight(5, int1);
					if (sprite.getProperties().Is(IsoFlagType.WallSE)) {
						SpriteRenderer.instance.setCutawayTexture(texture, short2 + (int)float1, 0 + (int)float2, 6 - int3, 196 - int4);
					} else {
						SpriteRenderer.instance.setCutawayTexture(texture, short1 + (int)float1, 0 + (int)float2, 66 - int3, 226 - int4);
					}
	...
	public void removeCorpse(IsoDeadBody deadBody, boolean boolean1) {
		if (GameClient.bClient && !boolean1) {
			...
			GameClient.sendRemoveCorpseFromMap(deadBody);
		}
	...
	private IsoObject getSpecialSolid() {
		...
		for (int1 = 0; int1 < this.SpecialObjects.size(); ++int1) {
			object = (IsoObject)this.SpecialObjects.get(int1);
			if (object instanceof IsoThumpable) {
				IsoThumpable thumpable = (IsoThumpable)object;
		...
		for (int1 = 0; int1 < this.Objects.size(); ++int1) {
			object = (IsoObject)this.Objects.get(int1);
	...
	public Boolean getWallFull() {
		for (int int1 = 0; int1 < this.Objects.size(); ++int1) {
			IsoObject object = (IsoObject)this.Objects.get(int1);
			if (object != null && object.sprite != null && (object.sprite.cutN || object.sprite.cutW || object.sprite.getProperties().Is(IsoFlagType.WallN) || object.sprite.getProperties().Is(IsoFlagType.WallW))) {
				return true;
			}
		}
	...
	public void DoSplat(String string, boolean boolean1, IsoFlagType flagType, float float1, float float2, float float3) {
		for (int int1 = 0; int1 < this.getObjects().size(); ++int1) {
			IsoObject object = (IsoObject)this.getObjects().get(int1);
			if (object.sprite != null && object.sprite.getProperties().Is(flagType) && (!(object instanceof IsoWindow) || !object.isDestroyed())) {
	...
	public void AddWorldInventoryItem(String string, float float1, float float2, float float3, int int1) {
		for (int int2 = 0; int2 < int1; ++int2) {
			this.AddWorldInventoryItem(string, float1, float2, float3);
		}
	}
	...
	boolean renderMinusFloor(int int1, boolean boolean1, boolean boolean2, boolean boolean3, boolean boolean4, boolean boolean5, Shader shader) {
		...
		if (!this.localTemporaryObjects.isEmpty()) {
			boolean6 = this.renderMinusFloor(this.localTemporaryObjects, int1, boolean1, boolean2, boolean3, boolean4, boolean5, shader);
		}

		return this.renderMinusFloor(this.Objects, int1, boolean1, boolean2, boolean3, boolean4, boolean5, shader) || boolean6;
	}

	boolean renderMinusFloor(PZArrayList pZArrayList, int int1, boolean boolean1, boolean boolean2, boolean boolean3, boolean boolean4, boolean boolean5, Shader shader) {
		if (!DebugOptions.instance.Terrain.RenderTiles.IsoGridSquare.RenderMinusFloor.getValue()) {
			...
			boolean boolean9 = false;
			int int4 = pZArrayList.size();
			IsoObject[] objectArray = (IsoObject[])pZArrayList.getElements();
					...
					if (boolean14) {
						...
						if (object2.sprite == null || !boolean16 && !boolean17) {
							if (DebugOptions.instance.Terrain.RenderTiles.IsoGridSquare.Objects.getValue()) {
								...
								float float4;
								if ((objectType == IsoObjectType.WestRoofB || objectType == IsoObjectType.WestRoofM || objectType == IsoObjectType.WestRoofT) && (this.getRoomID() == -1 && (boolean5 || boolean3) && (IsoPlayer.players[int3].getX() < (float)this.x && IsoPlayer.players[int3].getY() < (float)this.y || IsoPlayer.players[int3].getZ() < (float)this.z) || (this.getRoomID() != -1 || int7 != -1) && IsoPlayer.players[int3].getX() + IsoPlayer.players[int3].getY() <= (float)(this.x + this.y + 6) && IsoPlayer.players[int3].getZ() < (float)(this.z + 1))) {
									object2.setTargetAlpha(int3, 0.0F);
								} else if (square != null && !boolean6 && this.getRoomID() != int7 && float1 < 0.5F) {
									...
									if (IsoPlayer.getInstance() != null && object2.getProperties() != null && (object2.getProperties().Is(IsoFlagType.solid) || object2.getProperties().Is(IsoFlagType.solidtrans) || object2.getProperties().Is(IsoFlagType.attachedCeiling) || object2.getSprite().getProperties().Is(IsoFlagType.attachedE) || object2.getSprite().getProperties().Is(IsoFlagType.attachedS)) || objectType.index() > 2 && objectType.index() < 9 && IsoCamera.frameState.CamCharacterZ <= object2.getZ()) {
										byte byte1 = 2;
										float4 = 0.75F;
										if (objectType.index() > 2 && objectType.index() < 9 || object2.getSprite().getProperties().Is(IsoFlagType.attachedE) || object2.getSprite().getProperties().Is(IsoFlagType.attachedS)) {
											byte1 = 4;
								...
								if (object2 instanceof IsoWindow) {
									...
									if (object2.getTargetAlpha(int3) > 0.4F && boolean4 && (boolean5 && object2.sprite.getProperties().Is(IsoFlagType.windowN) || boolean3 && object2.sprite.getProperties().Is(IsoFlagType.windowW))) {
										float4 = 0.4F;
										float float5 = 0.1F;
										IsoPlayer player = IsoPlayer.players[int3];
										if (player != null) {
											float float6 = 5.0F;
											float float7 = Math.abs(player.x - (float)this.x) * Math.abs(player.x - (float)this.x) + Math.abs(player.y - (float)this.y) * Math.abs(player.y - (float)this.y);
											float float8 = float4 * (float)(1.0 - Math.sqrt((double)(float7 / float6)));
											object2.setTargetAlpha(int3, Math.max(float8, float5));
										} else {
											object2.setTargetAlpha(int3, float5);
										}
								...
								IsoWorldInventoryObject worldInventoryObject = (IsoWorldInventoryObject)Type.tryCastTo(object2, IsoWorldInventoryObject.class);
								if (worldInventoryObject != null) {
									tempWorldInventoryObjects.add(worldInventoryObject);
								}
							...
							if (DebugOptions.instance.Terrain.RenderTiles.IsoGridSquare.DoorsAndWalls.getValue()) {
								...
								} else if (this.getRoomID() != int7 && !boolean6 && (object2.getProperties().Is(IsoFlagType.transparentN) || object2.getProperties().Is(IsoFlagType.transparentW))) {
									object2.setTargetAlpha(int3, 0.0F);
									object2.setAlpha(int3, 0.0F);
								} else if (!boolean18) {
									object2.setTargetAlpha(int3, 1.0F);
			...
			Arrays.sort((IsoWorldInventoryObject[])tempWorldInventoryObjects.getElements(), 0, tempWorldInventoryObjects.size(), (var0,pZArrayListx)->{
				float int1 = var0.xoff * var0.xoff + var0.yoff * var0.yoff;
				float boolean1 = pZArrayListx.xoff * pZArrayListx.xoff + pZArrayListx.yoff * pZArrayListx.yoff;
				if (int1 == boolean1) {
					return 0;
				} else {
					return int1 > boolean1 ? 1 : -1;
				}
			});

			for (int8 = 0; int8 < tempWorldInventoryObjects.size(); ++int8) {
				IsoWorldInventoryObject worldInventoryObject2 = (IsoWorldInventoryObject)tempWorldInventoryObjects.get(int8);
				worldInventoryObject2.render((float)this.x, (float)this.y, (float)this.z, colorInfo, true, false, (Shader)null);
			}
			...
			for (int int1 = 0; int1 < this.getMovingObjects().size(); ++int1) {
				...
				if (movingObject instanceof IsoGameCharacter) {
					...
					if (GameClient.bClient && movingObject instanceof IsoZombie && ((IsoZombie)movingObject).isRemoteZombie()) {
						movingObject.Hit((HandWeapon)InventoryItemFactory.CreateItem("Base.Axe"), IsoWorld.instance.CurrentCell.getFakeZombieForHit(), 0.0F, true, 0.0F);
					}
				}
			}
		}
	...
	public void fixPlacedItemRenderOffsets() {
		...
		for (int3 = 0; int3 < int1; ++int3) {
			object = objectArray[int3];
			int int4 = PZMath.roundToInt(object.getSurfaceOffsetNoTable());
			if (!((float)int4 <= 0.0F) && !PZArrayUtil.contains(SURFACE_OFFSETS, int2, int4)) {
				SURFACE_OFFSETS[int2++] = int4;
			}
		}

		if (int2 == 0) {
			SURFACE_OFFSETS[int2++] = 0;
		}

		for (int3 = 0; int3 < int1; ++int3) {
			object = objectArray[int3];
			IsoWorldInventoryObject worldInventoryObject = (IsoWorldInventoryObject)Type.tryCastTo(object, IsoWorldInventoryObject.class);
			if (worldInventoryObject != null) {
				...
				for (int int7 = 0; int7 < int2; ++int7) {
					...
					int6 = SURFACE_OFFSETS[int7];
					if (int7 < int2 - 1 && int5 < SURFACE_OFFSETS[int7 + 1]) {
						break;
					}
				}

				worldInventoryObject.zoff = (float)int6 / 96.0F;
			}
		}
	}
		...
		if (room != null && room.def != null && !room.def.bExplored) {
			...
			if (player != null) {
				if (this.z == (int)player.z) {
					...
					if (IsoUtils.DistanceToSquared(player.x, player.y, (float)this.x + 0.5F, (float)this.y + 0.5F) < (float)(byte1 * byte1)) {
	...
	public float getLightLevel(int int1) {
		return (this.lighting[int1].lightInfo().r + this.lighting[int1].lightInfo().g + this.lighting[int1].lightInfo().b) / 3.0F;
	}
