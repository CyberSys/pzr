...
public class IsoWindow extends IsoObject implements BarricadeAble,Thumpable {
	...
	public boolean isExterior() {
		...
		if (square2 == null) {
			...
			return square.isInARoom() != square2.isInARoom();
		}
	}
		...
		if (GameClient.bClient) {
			...
			if (thumpable != null) {
				if (thumpable instanceof IsoBarricade) {
					...
					if (handWeapon == ((IsoLivingCharacter)gameCharacter).bareHands) {
						if (player != null) {
	...
	public void smashWindow(boolean boolean1, boolean boolean2) {
		if (!this.destroyed) {
			...
			if (this.getAttachedAnimSprite() != null) {
				...
				for (int int1 = 0; int1 < this.getAttachedAnimSprite().size(); ++int1) {
					IsoSprite sprite = ((IsoSpriteInstance)this.getAttachedAnimSprite().get(int1)).parentSprite;
					if (sprite != null && sprite.getProperties().Is("AttachedToGlass")) {
						this.getAttachedAnimSprite().remove(int1);
						--int1;
					}
				}
			}
