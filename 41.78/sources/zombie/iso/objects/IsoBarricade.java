...
public class IsoBarricade extends IsoObject implements Thumpable {
	...
	private void chooseSprite() {
		...
		if (this.metalHealth > 0) {
			...
			switch (this.dir) {
			case W: 
				this.sprite = spriteManager.getSprite(string2 + "_55");
				break;
			
			case N: 
				this.sprite = spriteManager.getSprite(string2 + "_53");
				break;
			
			case E: 
				this.sprite = spriteManager.getSprite(string2 + "_52");
				break;
			
			case S: 
				this.sprite = spriteManager.getSprite(string2 + "_54");
				break;
		...
		if (!this.isDestroyed()) {
			if (movingObject instanceof IsoZombie) {
				...
				if (int1 != this.getNumPlanks()) {
					((IsoGameCharacter)movingObject).getEmitter().playSound("BreakBarricadePlank");
					if (GameServer.bServer) {
	...
	public void WeaponHit(IsoGameCharacter gameCharacter, HandWeapon handWeapon) {
		if (!this.isDestroyed()) {
			...
			if (GameClient.bClient) {
				...
				if (player != null) {
					player.setMeleeHitSurface(!this.isMetal() && !this.isMetalBar() ? ParameterMeleeHitSurface.Material.Wood : ParameterMeleeHitSurface.Material.Metal);
				}
				...
				if (this.isDestroyed()) {
					if (gameCharacter != null) {
						String string2 = string == "HitBarricadeMetal" ? "BreakBarricadeMetal" : "BreakBarricadePlank";
	...
	public void DamageBarricade(int int1) {
		this.Damage(int1);
	}
	...
	public float getThumpCondition() {
		if (this.metalHealth > 0) {
			...
			for (int int1 = 3; int1 >= 0; --int1) {
				if (this.plankHealth[int1] > 0) {
					return (float)PZMath.clamp(this.plankHealth[int1], 0, 1000) / 1000.0F;
				}
			}
	...
	public void render(float float1, float float2, float float3, ColorInfo colorInfo, boolean boolean1, boolean boolean2, Shader shader) {
		...
		if (barricadeAble != null && this.square.lighting[int1].targetDarkMulti() <= barricadeAble.getSquare().lighting[int1].targetDarkMulti()) {
			...
			this.setTargetAlpha(int1, ((IsoObject)barricadeAble).getTargetAlpha(int1));
		}
