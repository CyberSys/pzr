...
public class IsoDoor extends IsoObject implements BarricadeAble,Thumpable {
	...
	public IsoDoor(IsoCell cell, IsoGridSquare square, IsoSprite sprite, boolean boolean1) {
		...
		this.open = false;
		this.destroyed = false;
		this.open = sprite.getProperties().Is(IsoFlagType.open);
		...
		this.closedSprite = this.open ? IsoSprite.getSprite(IsoSpriteManager.instance, sprite, -byte1) : sprite;
		this.openSprite = this.open ? sprite : IsoSprite.getSprite(IsoSpriteManager.instance, (IsoSprite)sprite, byte1);
		...
		if (byte2 > -1) {
			this.Locked = Rand.Next(100) < byte2;
		...
		if (!this.isDestroyed()) {
			...
			if (this.Health <= 0) {
				...
				if (GameServer.bServer) {
					GameServer.PlayWorldSoundServer((IsoGameCharacter)movingObject, "BreakDoor", false, movingObject.getCurrentSquare(), 0.2F, 20.0F, 1.1F, true);
				}
		...
		if (GameClient.bClient) {
			...
			if (thumpable != null) {
				if (thumpable instanceof IsoBarricade) {
					...
					LuaEventManager.triggerEvent("OnWeaponHitThumpable", gameCharacter, handWeapon, this);
					if (!this.open) {
						if (!this.isDestroyed()) {
							...
							if (!this.IsStrengthenedByPushedItems() && this.Health <= 0 || this.IsStrengthenedByPushedItems() && this.Health <= -this.PushedMaxStrength) {
								...
								if (GameServer.bServer) {
									GameServer.PlayWorldSoundServer(gameCharacter, "BreakDoor", false, this.getSquare(), 0.2F, 20.0F, 1.1F, true);
								}
	...
	public void ToggleDoorActual(IsoGameCharacter gameCharacter) {
		...
		if (this.isBarricaded()) {
			...
			if (this.isLockedByKey() && gameCharacter != null && gameCharacter instanceof IsoPlayer && (gameCharacter.getCurrentSquare().Is(IsoFlagType.exterior) || this.getProperties().Is("forceLocked") || this.getModData().rawget("CustomLock") != null && this.getModData().rawget("CustomLock") instanceof Boolean && (Boolean)this.getModData().rawget("CustomLock")) && !this.open) {
	...
	public boolean isAdjacentToSquare(IsoGridSquare square) {
		...
		if (square2 != null && square != null) {
			boolean boolean1 = !this.IsOpen();
			IsoGridSquare square3 = square2.getAdjacentSquare(IsoDirections.NW);
			IsoGridSquare square4 = square2.getAdjacentSquare(IsoDirections.N);
			IsoGridSquare square5 = square2.getAdjacentSquare(IsoDirections.NE);
			IsoGridSquare square6 = square2.getAdjacentSquare(IsoDirections.W);
			IsoGridSquare square7 = square2.getAdjacentSquare(IsoDirections.E);
			IsoGridSquare square8 = square2.getAdjacentSquare(IsoDirections.SW);
			IsoGridSquare square9 = square2.getAdjacentSquare(IsoDirections.S);
			IsoGridSquare square10 = square2.getAdjacentSquare(IsoDirections.SE);
			switch (this.getSpriteEdge(false)) {
			case N: 
				if (square == square3) {
					if (!square3.isWallTo(square4) && !square3.isWindowTo(square4) && !square3.hasDoorOnEdge(IsoDirections.E, false) && !square4.hasDoorOnEdge(IsoDirections.W, false)) {
						if (square4.hasDoorOnEdge(IsoDirections.S, false)) {
							return false;
						}

						if (this.IsOpen() && square2.hasClosedDoorOnEdge(IsoDirections.N)) {
							return false;
						}
				...
				if (square == square5) {
					if (!square5.isWallTo(square4) && !square5.isWindowTo(square4) && !square5.hasDoorOnEdge(IsoDirections.W, false) && !square4.hasDoorOnEdge(IsoDirections.E, false)) {
						if (square4.hasDoorOnEdge(IsoDirections.S, false)) {
							return false;
						}
				...
				if (square == square6) {
					if (!square6.isWallTo(square2) && !square6.isWindowTo(square2) && !square6.hasDoorOnEdge(IsoDirections.E, false) && !square2.hasDoorOnEdge(IsoDirections.W, false)) {
						if (boolean1 && square2.hasOpenDoorOnEdge(IsoDirections.N)) {
							return false;
						}
				...
				if (square == square7) {
					if (!square7.isWallTo(square2) && !square7.isWindowTo(square2) && !square7.hasDoorOnEdge(IsoDirections.W, false) && !square2.hasDoorOnEdge(IsoDirections.E, false)) {
						if (boolean1 && square2.hasOpenDoorOnEdge(IsoDirections.N)) {
							return false;
						}
			...
			case S: 
				if (square == square6) {
					if (!square6.isWallTo(square2) && !square6.isWindowTo(square2) && !square6.hasDoorOnEdge(IsoDirections.E, false) && !square2.hasDoorOnEdge(IsoDirections.W, false)) {
						if (boolean1 && square2.hasOpenDoorOnEdge(IsoDirections.S)) {
							return false;
						}
				...
				if (square == square8) {
					if (!square8.isWallTo(square9) && !square8.isWindowTo(square9) && !square8.hasDoorOnEdge(IsoDirections.E, false) && !square9.hasDoorOnEdge(IsoDirections.W, false)) {
						if (square9.hasDoorOnEdge(IsoDirections.N, false)) {
							return false;
						}
				...
				if (square == square10) {
					if (!square10.isWallTo(square9) && !square10.isWindowTo(square9) && !square10.hasDoorOnEdge(IsoDirections.W, false) && !square9.hasDoorOnEdge(IsoDirections.E, false)) {
						if (square9.hasDoorOnEdge(IsoDirections.N, false)) {
							return false;
						}
			...
			case W: 
				if (square == square3) {
					if (!square3.isWallTo(square6) && !square3.isWindowTo(square6) && !square3.hasDoorOnEdge(IsoDirections.S, false) && !square6.hasDoorOnEdge(IsoDirections.N, false)) {
						if (boolean1 && square6.hasDoorOnEdge(IsoDirections.E, false)) {
							return false;
						}

						if (this.IsOpen() && square2.hasClosedDoorOnEdge(IsoDirections.W)) {
							return false;
						}
				...
				if (square == square8) {
					if (!square8.isWallTo(square6) && !square8.isWindowTo(square6) && !square8.hasDoorOnEdge(IsoDirections.N, false) && !square6.hasDoorOnEdge(IsoDirections.S, false)) {
						if (boolean1 && square6.hasDoorOnEdge(IsoDirections.E, false)) {
							return false;
						}
				...
				if (square == square4) {
					if (!square4.isWallTo(square2) && !square4.isWindowTo(square2) && !square4.hasDoorOnEdge(IsoDirections.S, false) && !square2.hasDoorOnEdge(IsoDirections.N, false)) {
						if (boolean1 && square2.hasOpenDoorOnEdge(IsoDirections.W)) {
							return false;
						}
				...
				if (square == square9) {
					if (!square9.isWallTo(square2) && !square9.isWindowTo(square2) && !square9.hasDoorOnEdge(IsoDirections.N, false) && !square2.hasDoorOnEdge(IsoDirections.S, false)) {
						if (boolean1 && square2.hasOpenDoorOnEdge(IsoDirections.W)) {
							return false;
						}
			...
			case E: 
				if (square == square4) {
					if (!square4.isWallTo(square2) && !square4.isWindowTo(square2) && !square4.hasDoorOnEdge(IsoDirections.S, false) && !square2.hasDoorOnEdge(IsoDirections.N, false)) {
						if (boolean1 && square2.hasOpenDoorOnEdge(IsoDirections.E)) {
							return false;
						}
				...
				if (square == square5) {
					if (!square5.isWallTo(square7) && !square5.isWindowTo(square7) && !square5.hasDoorOnEdge(IsoDirections.S, false) && !square6.hasDoorOnEdge(IsoDirections.N, false)) {
						if (square7.hasDoorOnEdge(IsoDirections.W, false)) {
							return false;
						}
				...
				if (square == square10) {
					if (!square10.isWallTo(square7) && !square10.isWindowTo(square7) && !square10.hasDoorOnEdge(IsoDirections.N, false) && !square7.hasDoorOnEdge(IsoDirections.S, false)) {
						if (square7.hasDoorOnEdge(IsoDirections.E, false)) {
							return false;
						}
	...
	public static void toggleDoubleDoor(IsoObject object, boolean boolean1) {
		...
		if (int1 != -1) {
			...
			boolean boolean3 = door == null ? thumpable.open : door.open;
			...
			if (boolean1) {
				if (object2 != null) {
					object2.syncIsoObject(false, (byte)(boolean3 ? 1 : 0), (UdpConnection)null, (ByteBuffer)null);
				} else if (object5 != null) {
					object5.syncIsoObject(false, (byte)(boolean3 ? 1 : 0), (UdpConnection)null, (ByteBuffer)null);
				}
			}
		}
	}
	...
	private static void toggleDoubleDoorObject(IsoObject object) {
		...
		if (int1 != -1) {
			...
			if (door != null) {
				...
				int2 = door.checkKeyId();
			}

			if (thumpable != null) {
				thumpable.open = !boolean2;
				thumpable.setLockedByKey(false);
				int2 = thumpable.getKeyId();
			}

			IsoSprite sprite = object.getSprite();
			int int3 = boolean1 ? DoubleDoorNorthSpriteOffset[int1 - 1] : DoubleDoorWestSpriteOffset[int1 - 1];
			if (boolean2) {
				int3 *= -1;
			}

			object.sprite = IsoSprite.getSprite(IsoSpriteManager.instance, sprite.getName(), int3);
			...
			if (int1 != 2 && int1 != 3) {
				...
				int int4 = square.getX() - intArray[int1 - 1];
				int int5 = square.getY() - intArray2[int1 - 1];
				int int6 = int4 + intArray3[int1 - 1];
				int int7 = int5 + intArray4[int1 - 1];
				square.RemoveTileObject(object);
				PolygonalMap2.instance.squareChanged(square);
				square = IsoWorld.instance.CurrentCell.getGridSquare(int6, int7, square.getZ());
				if (square == null) {
					return;
				}
	...
	public static boolean isDoubleDoorObstructed(IsoObject object) {
		...
		if (int1 == -1) {
			...
			if (boolean1) {
				if (hasSomething4x4(int2, int3, int2 + 1, int3 + 1, int8)) {
					return true;
				}

				if (hasSomething4x4(int2 + 2, int3, int2 + 3, int3 + 1, int8)) {
					return true;
				}
			} else {
				if (hasSomething4x4(int2, int3 - 1, int2 + 1, int3, int8)) {
					return true;
				}

				if (hasSomething4x4(int2, int3 - 3, int2 + 1, int3 - 2, int8)) {
					return true;
				}
			}
	...
	private static boolean isSomethingTo(IsoGridSquare square, IsoGridSquare square2) {
		if (square != null && square2 != null) {
			if (square2 != square.getAdjacentSquare(IsoDirections.E) || !square.hasDoorOnEdge(IsoDirections.E, false) && !square2.hasDoorOnEdge(IsoDirections.W, false) && !square2.getProperties().Is(IsoFlagType.DoorWallW)) {
				if (square2 != square.getAdjacentSquare(IsoDirections.SE) || !square.hasDoorOnEdge(IsoDirections.E, false) && !square.hasDoorOnEdge(IsoDirections.S, false) && !square2.hasDoorOnEdge(IsoDirections.W, false) && !square2.hasDoorOnEdge(IsoDirections.N, false) && !square2.getProperties().Is(IsoFlagType.DoorWallN) && !square2.getProperties().Is(IsoFlagType.DoorWallW)) {
					if (square2 == square.getAdjacentSquare(IsoDirections.S) && (square.hasDoorOnEdge(IsoDirections.S, false) || square2.hasDoorOnEdge(IsoDirections.N, false) || square2.getProperties().Is(IsoFlagType.DoorWallN))) {
						return true;
					} else {
						return square.isWallTo(square2) || square.isWindowTo(square2);
					}
				} else {
					return true;
				}
	...
	private static boolean hasSomething4x4(int int1, int int2, int int3, int int4, int int5) {
		IsoGridSquare square = IsoWorld.instance.CurrentCell.getGridSquare(int1, int2, int5);
		IsoGridSquare square2 = IsoWorld.instance.CurrentCell.getGridSquare(int3, int2, int5);
		IsoGridSquare square3 = IsoWorld.instance.CurrentCell.getGridSquare(int3, int4, int5);
		IsoGridSquare square4 = IsoWorld.instance.CurrentCell.getGridSquare(int1, int4, int5);
		if (isSomethingTo(square, square2)) {
			return true;
		} else if (isSomethingTo(square, square3)) {
			return true;
		} else {
			return isSomethingTo(square, square4);
		}
	}
	...
	public String getThumpSound() {
		...
		case -1326475798: 
			if (string.equals("PrisonMetalDoor")) {
				byte1 = 3;
			}
		...
		case 228116188: 
			if (string.equals("SlidingGlassDoor")) {
				byte1 = 4;
			}
