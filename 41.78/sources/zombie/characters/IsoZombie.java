...
public final class IsoZombie extends IsoGameCharacter implements IHumanVisual {
	...
	private void registerVariableCallbacks() {
		...
		this.setVariable("bthump", ()->{
			if (this.getThumpTarget() instanceof IsoObject && !(this.getThumpTarget() instanceof BaseVehicle)) {
				...
				if (baseVehicle.getSquare() == null || this.DistToSquared(baseVehicle.getX() + 0.5F, baseVehicle.getY() + 0.5F) > 9.0F) {
	...
	protected boolean renderTextureInsteadOfModel(float float1, float float2) {
		...
		DeadBodyAtlas.BodyTexture bodyTexture = DeadBodyAtlas.instance.getBodyTexture(this.isFemale(), string, string2, this.getDir(), int1, float3);
		if (bodyTexture != null) {
			float float4 = IsoUtils.XToScreen(float1, float2, this.getZ(), 0);
			float float5 = IsoUtils.YToScreen(float1, float2, this.getZ(), 0);
			float4 -= IsoCamera.getOffX();
			float5 -= IsoCamera.getOffY();
			int int2 = IsoCamera.frameState.playerIndex;
			bodyTexture.render(float4, float5, 0.0F, 0.0F, 0.0F, this.getAlpha(int2));
		}
	...
	private void updateInternal() {
		...
		if (baseVehicle != null) {
			if (this.target == null && baseVehicle.hasLightbar() && baseVehicle.lightbarSirenMode.get() > 0) {
	...
	public void updateVocalProperties() {
		if (!GameServer.bServer) {
			...
			if (!this.isDead() && !this.isFakeDead() && boolean1) {
			...
			if (!GameServer.bServer || boolean1) {
				...
				if (building != null && building.getDef() != null && building.getDef().getKeyId() != -1 && Rand.Next(4) == 0) {
					...
					if (Rand.Next(10) != 1) {
						...
						InventoryItem inventoryItem2 = this.inventory.AddItem("Base.KeyRing");
						if (inventoryItem2 instanceof InventoryContainer) {
							InventoryContainer inventoryContainer = (InventoryContainer)inventoryItem2;
							int2 = Rand.Next(5);
							String string2 = "Base.Key" + (int2 + 1);
							InventoryItem inventoryItem3 = inventoryContainer.getInventory().AddItem(string2);
							inventoryItem3.setKeyId(building.getDef().getKeyId());
						}
					}
				}
	...
	public void DoZombieStats() {
		...
		if (this.memory == -1 && int1 == 1 || int2 == 0) {
			this.memory = 1250;
		}

		if (this.memory == -1 && int1 == 2 || int2 == 1) {
			this.memory = 800;
		}

		if (this.memory == -1 && int1 == 3 || int2 == 2) {
			this.memory = 500;
		}

		if (this.memory == -1 && int1 == 4 || int2 == 3) {
			this.memory = 25;
		}
