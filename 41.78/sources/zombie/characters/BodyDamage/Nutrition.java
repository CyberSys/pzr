...
public final class Nutrition {
	...
	private void updateCalories() {
		...
		float float3 = (float)(this.getWeight() / 80.0);
	...
	private void updateWeight() {
		...
		float float4 = (float)((this.getWeight() - 80.0) * 40.0);
		float1 = 1600.0F + float4;
		float3 = (float)((this.getWeight() - 70.0) * 30.0);
		if (float3 > 0.0F) {
			float3 = 0.0F;
		}
		...
		if (this.getCalories() > float1) {
			...
			this.setWeight(this.getWeight() + (double)(float6 * float5 * GameTime.getInstance().getGameWorldSecondsSinceLastUpdate()));
			...
			this.setWeight(this.getWeight() - (double)(this.weightLoss * float5 * GameTime.getInstance().getGameWorldSecondsSinceLastUpdate()));
		}
	...
	public void load(ByteBuffer byteBuffer) {
		...
		this.setWeight((double)byteBuffer.getFloat());
	}
	...
	public void setWeight(double double1) {
		if (double1 < 35.0) {
			double1 = 35.0;
			float float1 = this.parent.getBodyDamage().getHealthReductionFromSevereBadMoodles() * GameTime.instance.getMultiplier();
			this.parent.getBodyDamage().ReduceGeneralHealth(float1);
			LuaEventManager.triggerEvent("OnPlayerGetDamage", this.parent, "LOWWEIGHT", float1);
		}
