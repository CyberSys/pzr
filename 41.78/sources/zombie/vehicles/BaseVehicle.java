...
public final class BaseVehicle extends IsoMovingObject implements Thumpable,IFMODParameterUpdater {
	...
	private static void LoadVehicleTextures(VehicleScript.Skin skin) {
		...
		if (skin.textureMask != null) {
			byte byte1 = 0;
			int int1 = byte1 | 256;
			skin.textureDataMask = LoadVehicleTexture(skin.textureMask, int1);
		}
	...
	public void setNetPlayerAuthorization(BaseVehicle.Authorization authorization, int int1) {
		this.netPlayerAuthorization = authorization;
		this.netPlayerId = (short)int1;
		this.netPlayerTimeout = int1 == -1 ? 0 : 30;
		if (GameClient.bClient) {
			boolean boolean1 = BaseVehicle.Authorization.Local.equals(authorization) || BaseVehicle.Authorization.LocalCollide.equals(authorization);
			if (this.getVehicleTowing() != null) {
				Bullet.setVehicleStatic(this, !boolean1);
				Bullet.setVehicleActive(this, boolean1);
				Bullet.setVehicleStatic(this.getVehicleTowing(), !boolean1);
				Bullet.setVehicleActive(this.getVehicleTowing(), boolean1);
		...
		DebugLog.Vehicle.trace("vid%s=%d pid=%d %s", this.getVehicleTowing() != null ? "-a" : (this.getVehicleTowedBy() != null ? "-b" : ""), this.getId(), int1, authorization.name());
	}

	public boolean isNetPlayerAuthorization(BaseVehicle.Authorization authorization) {
		return this.netPlayerAuthorization.equals(authorization);
	}

	public boolean isNetPlayerId(short short1) {
		return this.netPlayerId == short1;
	}
	...
	public void setPreviouslyEntered(boolean boolean1) {
		this.previouslyEntered = boolean1;
	}
	...
	public void putKeyToZombie(IsoZombie zombie) {
		...
		if (Rand.Next(10) != 1) {
			inventoryItem = this.createVehicleKey();
			zombie.getInventory().AddItem(inventoryItem);
		} else {
			inventoryItem = zombie.getInventory().AddItem("Base.KeyRing");
		...
		for (int int1 = 0; int1 < this.models.size(); ++int1) {
			...
			if (modelInfo.part == vehiclePart && modelInfo.scriptModel == model) {
				...
				if (modelInfo.m_animPlayer != null) {
					modelInfo.m_animPlayer = (AnimationPlayer)Pool.tryRelease((IPooledObject)modelInfo.m_animPlayer);
				}
	...
	public void playPartSound(VehiclePart vehiclePart, IsoPlayer player, String string) {
		if (this.parts.contains(vehiclePart)) {
			VehicleScript.Anim anim = vehiclePart.getAnimById(string);
			if (anim != null && anim.sound != null) {
	...
	public boolean enter(int int1, IsoGameCharacter gameCharacter, Vector3f vector3f) {
		...
		if (gameCharacter == null) {
			...
			if (GameClient.bClient) {
				VehicleManager.instance.sendEnter(GameClient.connection, this, gameCharacter, int1);
			}
	...
	public boolean exit(IsoGameCharacter gameCharacter) {
		...
		if (gameCharacter == null) {
			...
			if (int1 == -1) {
				...
				if (GameClient.bClient) {
					VehicleManager.instance.sendExit(GameClient.connection, this, gameCharacter, int1);
				}
		...
		if (this.removedFromWorld) {
			...
			if (this.getVehicleTowing() != null && this.getDriver() != null) {
				...
				if (this.getVehicleTowing().scriptName.equals("Base.Trailer")) {
					...
					if (this.getCurrentSpeedKmHour() > 30.0F && vehiclePart2.getCondition() < 50 && !vehiclePart2.container.Items.isEmpty()) {
						...
						int int3;
						for (int3 = 0; int3 < vehiclePart2.container.Items.size(); ++int3) {
							if ((double)((InventoryItem)vehiclePart2.container.Items.get(int3)).getWeight() >= 3.5) {
								arrayList.add((InventoryItem)vehiclePart2.container.Items.get(int3));
							}
						}

						if (!arrayList.isEmpty()) {
							int3 = vehiclePart2.getCondition();
							int int4 = 0;
							int1 = 0;
							for (int int5 = 0; int5 < this.getVehicleTowing().parts.size(); ++int5) {
								vehiclePart = this.getVehicleTowing().getPartByIndex(int5);
								if (vehiclePart != null && vehiclePart.item != null) {
									if (vehiclePart.partId != null && vehiclePart.partId.contains("Suspension")) {
										int4 += vehiclePart.getCondition();
									} else if (vehiclePart.partId != null && vehiclePart.partId.contains("Tire")) {
										int1 += vehiclePart.getCondition();
									}
								}
							}

							float2 = this.parameterVehicleSteer.getCurrentValue();
							int2 = (int)(Math.pow((double)(100 - int3 * 2), 2.0) * 0.3 * (1.0 + (double)(100 - int4 / 2) * 0.005) * (1.0 + (double)(100 - int4 / 2) * 0.005) * (double)(1.0F + float2 / 3.0F));
							if (Rand.Next(0, Math.max(10000 - int2, 1)) == 0) {
								...
								vehiclePart2.getSquare().AddWorldInventoryItem(inventoryItem, Rand.Next(0.0F, 0.5F), Rand.Next(0.0F, 0.5F), 0.0F);
								vehiclePart2.container.Items.remove(inventoryItem);
			...
			if (this.physics != null) {
				...
				if (!GameServer.bServer && vehicleImpulse != null && vehicleImpulse.enable) {
					...
					Vector3f vector3f = vehicleImpulse.rel_pos.cross(vehicleImpulse.impulse, (Vector3f)((BaseVehicle.Vector3fObjectPool)TL_vector3f_pool.get()).alloc());
				...
				short short1 = 1000;
				int int6;
				if (System.currentTimeMillis() - this.engineCheckTime > (long)short1 && !GameClient.bClient) {
					this.engineCheckTime = System.currentTimeMillis();
					if (!GameClient.bClient) {
						if (this.engineState != BaseVehicle.engineStateTypes.Idle) {
							int2 = (int)((double)this.engineLoudness * this.engineSpeed / 2500.0);
							double double1 = Math.min(this.getEngineSpeed(), 2000.0);
							int2 = (int)((double)int2 * (1.0 + double1 / 4000.0));
							int6 = 120;
							if (GameServer.bServer) {
								int6 = (int)((double)int6 * ServerOptions.getInstance().CarEngineAttractionModifier.getValue());
								int2 = (int)((double)int2 * ServerOptions.getInstance().CarEngineAttractionModifier.getValue());
							}

							if (Rand.Next((int)((float)int6 * GameTime.instance.getInvMultiplier())) == 0) {
								WorldSoundManager.instance.addSoundRepeating(this, (int)this.getX(), (int)this.getY(), (int)this.getZ(), Math.max(8, int2), int2 / 40, false);
							}

							if (Rand.Next((int)((float)(int6 - 85) * GameTime.instance.getInvMultiplier())) == 0) {
								WorldSoundManager.instance.addSoundRepeating(this, (int)this.getX(), (int)this.getY(), (int)this.getZ(), Math.max(8, int2 / 2), int2 / 40, false);
							}

							if (Rand.Next((int)((float)(int6 - 110) * GameTime.instance.getInvMultiplier())) == 0) {
								WorldSoundManager.instance.addSoundRepeating(this, (int)this.getX(), (int)this.getY(), (int)this.getZ(), Math.max(8, int2 / 4), int2 / 40, false);
							}

							WorldSoundManager.instance.addSoundRepeating(this, (int)this.getX(), (int)this.getY(), (int)this.getZ(), Math.max(8, int2 / 6), int2 / 40, false);
						}
					...
					if (this.engineState == BaseVehicle.engineStateTypes.Running) {
						...
						if (vehiclePart != null && vehiclePart.getCondition() < 50 && Rand.Next(Rand.AdjustForFramerate(vehiclePart.getCondition() * 12)) == 0) {
							this.shutOff();
						}
					}
				...
				if (square == null && !this.chunk.refs.isEmpty()) {
					...
					int int7 = this.chunk.wx * 10;
					int6 = this.chunk.wy * 10;
					int int8 = int7 + 10;
					int int9 = int6 + 10;
					float float4 = this.x;
					float float5 = this.y;
					this.x = Math.max(this.x, (float)int7 + float3);
					this.x = Math.min(this.x, (float)int8 - float3);
					this.y = Math.max(this.y, (float)int6 + float3);
					this.y = Math.min(this.y, (float)int9 - float3);
					this.z = 0.2F;
				...
				this.updateTransform();
				Vector3f vector3f2 = allocVector3f().set((Vector3fc)this.jniLinearVelocity);
				if (this.jniIsCollide && this.limitCrash.Check()) {
					this.jniIsCollide = false;
					this.limitCrash.Reset();
					Vector3f vector3f3 = allocVector3f();
					vector3f3.set((Vector3fc)vector3f2).sub(this.lastLinearVelocity);
					vector3f3.y = 0.0F;
					float float6 = vector3f3.length();
					DebugLog.Vehicle.debugln("Vehicle vid=%d velocity last=%s/%f current=%s/%f delta=%f", this.VehicleID, this.lastLinearVelocity, this.lastLinearVelocity.length(), vector3f2, vector3f2.length(), float6);
					if (vector3f2.lengthSquared() > this.lastLinearVelocity.lengthSquared() && float6 > 6.0F) {
						DebugLog.Vehicle.trace("Vehicle vid=%d got sharp speed increase delta=%f", this.VehicleID, float6);
						float6 = 6.0F;
					}
					...
					if (float6 > 1.0F) {
						...
						DebugLog.Vehicle.trace("Vehicle vid=%d crash delta=%f", this.VehicleID, float6);
						Vector3f vector3f4 = this.getForwardVector(allocVector3f());
						float float7 = vector3f3.dot(vector3f4);
						releaseVector3f(vector3f4);
						this.crash(float6 * 3.0F, float7 < 0.0F);
						this.damageObjects(float6 * 30.0F);
					}

					releaseVector3f(vector3f3);
				}

				this.lastLinearVelocity.set((Vector3fc)vector3f2);
			...
			int int10;
			for (int10 = 0; int10 < this.impulseFromSquishedZombie.length; ++int10) {
				BaseVehicle.VehicleImpulse vehicleImpulse2 = this.impulseFromSquishedZombie[int10];
			...
			for (int10 = 0; int10 < IsoPlayer.numPlayers; ++int10) {
				if (this.current == null || !this.current.lighting[int10].bCanSee()) {
					this.setTargetAlpha(int10, 0.0F);
				}

				IsoPlayer player = IsoPlayer.players[int10];
				if (player != null && this.DistToSquared(player) < 225.0F) {
					this.setTargetAlpha(int10, 1.0F);
				}
			}

			for (int10 = 0; int10 < this.getScript().getPassengerCount(); ++int10) {
				if (this.getCharacter(int10) != null) {
					Vector3f vector3f5 = this.getPassengerWorldPos(int10, (Vector3f)((BaseVehicle.Vector3fObjectPool)TL_vector3f_pool.get()).alloc());
					this.getCharacter(int10).setX(vector3f5.x);
					this.getCharacter(int10).setY(vector3f5.y);
					this.getCharacter(int10).setZ(vector3f5.z * 1.0F);
			...
			for (int1 = 0; int1 < this.getMaxPassengers(); ++int1) {
				BaseVehicle.Passenger passenger = this.getPassenger(int1);
	...
	private void applyImpulseFromHitZombies() {
		if (!this.impulseFromHitZombie.isEmpty()) {
			if ((!GameClient.bClient || this.hasAuthorization(GameClient.connection)) && !GameServer.bServer) {
				...
				int int3 = 0;
				for (int int4 = this.impulseFromHitZombie.size(); int3 < int4; ++int3) {
					BaseVehicle.VehicleImpulse vehicleImpulse2 = (BaseVehicle.VehicleImpulse)this.impulseFromHitZombie.get(int3);
	...
	public void authorizationClientCollide(IsoPlayer player) {
		if (player != null && this.getDriver() == null) {
			this.setNetPlayerAuthorization(BaseVehicle.Authorization.LocalCollide, player.getOnlineID());
			this.authSimulationTime = System.currentTimeMillis();
			this.interpolation.clear();
			if (this.getVehicleTowing() != null) {
				this.getVehicleTowing().setNetPlayerAuthorization(BaseVehicle.Authorization.LocalCollide, player.getOnlineID());
				this.getVehicleTowing().authSimulationTime = System.currentTimeMillis();
				this.getVehicleTowing().interpolation.clear();
			} else if (this.getVehicleTowedBy() != null) {
				this.getVehicleTowedBy().setNetPlayerAuthorization(BaseVehicle.Authorization.LocalCollide, player.getOnlineID());
	...
	public void authorizationServerCollide(short short1, boolean boolean1) {
		if (!this.isNetPlayerAuthorization(BaseVehicle.Authorization.Local)) {
			if (boolean1) {
				this.setNetPlayerAuthorization(BaseVehicle.Authorization.LocalCollide, short1);
				if (this.getVehicleTowing() != null) {
					this.getVehicleTowing().setNetPlayerAuthorization(BaseVehicle.Authorization.LocalCollide, short1);
				} else if (this.getVehicleTowedBy() != null) {
					this.getVehicleTowedBy().setNetPlayerAuthorization(BaseVehicle.Authorization.LocalCollide, short1);
				}
			} else {
				BaseVehicle.Authorization authorization = short1 == -1 ? BaseVehicle.Authorization.Server : BaseVehicle.Authorization.Local;
				this.setNetPlayerAuthorization(authorization, short1);
				if (this.getVehicleTowing() != null) {
					this.getVehicleTowing().setNetPlayerAuthorization(authorization, short1);
				} else if (this.getVehicleTowedBy() != null) {
					this.getVehicleTowedBy().setNetPlayerAuthorization(authorization, short1);
				}
			}
		}
	}

	public void authorizationServerOnSeat(IsoPlayer player, boolean boolean1) {
		BaseVehicle baseVehicle = this.getVehicleTowing();
		BaseVehicle baseVehicle2 = this.getVehicleTowedBy();
		if (this.isNetPlayerId((short)-1) && boolean1) {
			if (baseVehicle != null && baseVehicle.getDriver() == null) {
				this.addPointConstraint((IsoPlayer)null, baseVehicle, this.getTowAttachmentSelf(), baseVehicle.getTowAttachmentSelf());
			} else if (baseVehicle2 != null && baseVehicle2.getDriver() == null) {
				this.addPointConstraint((IsoPlayer)null, baseVehicle2, this.getTowAttachmentSelf(), baseVehicle2.getTowAttachmentSelf());
			} else {
				this.setNetPlayerAuthorization(BaseVehicle.Authorization.Local, player.getOnlineID());
			}
		} else if (this.isNetPlayerId(player.getOnlineID()) && !boolean1) {
			if (baseVehicle != null && baseVehicle.getDriver() != null) {
				baseVehicle.addPointConstraint((IsoPlayer)null, this, baseVehicle.getTowAttachmentSelf(), this.getTowAttachmentSelf());
			} else if (baseVehicle2 != null && baseVehicle2.getDriver() != null) {
				baseVehicle2.addPointConstraint((IsoPlayer)null, this, baseVehicle2.getTowAttachmentSelf(), this.getTowAttachmentSelf());
			} else {
				this.setNetPlayerAuthorization(BaseVehicle.Authorization.Server, -1);
				if (baseVehicle != null) {
					baseVehicle.setNetPlayerAuthorization(BaseVehicle.Authorization.Server, -1);
				} else if (baseVehicle2 != null) {
					baseVehicle2.setNetPlayerAuthorization(BaseVehicle.Authorization.Server, -1);
				}
			}
		}
	}
		...
		if (!this.isNetPlayerId((short)-1) && udpConnection != null) {
			if (GameServer.bServer) {
				for (int int1 = 0; int1 < udpConnection.players.length; ++int1) {
					if (udpConnection.players[int1] != null && this.isNetPlayerId(udpConnection.players[int1].OnlineID)) {
						return true;
					}
				}
	...
	public void netPlayerFromServerUpdate(BaseVehicle.Authorization authorization, short short1) {
		if (!this.isNetPlayerAuthorization(authorization) || !this.isNetPlayerId(short1)) {
			if (BaseVehicle.Authorization.Local.equals(authorization)) {
				if (IsoPlayer.getLocalPlayerByOnlineID(short1) != null) {
					this.setNetPlayerAuthorization(BaseVehicle.Authorization.Local, short1);
				} else {
					this.setNetPlayerAuthorization(BaseVehicle.Authorization.Remote, short1);
				}
			} else if (BaseVehicle.Authorization.LocalCollide.equals(authorization)) {
				if (IsoPlayer.getLocalPlayerByOnlineID(short1) != null) {
					this.setNetPlayerAuthorization(BaseVehicle.Authorization.LocalCollide, short1);
				} else {
					this.setNetPlayerAuthorization(BaseVehicle.Authorization.RemoteCollide, short1);
				}
			} else {
				this.setNetPlayerAuthorization(BaseVehicle.Authorization.Server, -1);
			}
	...
	public float getSpeed2D() {
		float float1 = this.jniLinearVelocity.x;
		float float2 = this.jniLinearVelocity.z;
	...
	public boolean isAtRest() {
		if (this.physics == null) {
			...
			float float1 = this.jniLinearVelocity.y;
	...
	public void checkPhysicsValidWithServer() {
		...
		if (this.limitPhysicValid.Check() && Bullet.getOwnVehiclePhysics(this.VehicleID, physicsParams) == 0) {
			float float2 = Math.abs(physicsParams[0] - this.x);
			float float3 = Math.abs(physicsParams[1] - this.y);
			if (float2 > float1 || float3 > float1) {
				VehicleManager.instance.sendRequestGetPosition(this.VehicleID, PacketTypes.PacketType.Vehicles);
				DebugLog.Vehicle.trace("diff-x=%f diff-y=%f delta=%f", float2, float3, float1);
			}
		}
	}
	...
	public void crash(float float1, boolean boolean1) {
		if (GameClient.bClient) {
			SoundManager.instance.PlayWorldSound(this.getCrashSound(float1), this.square, 1.0F, 20.0F, 1.0F, true);
			...
			SoundManager.instance.PlayWorldSound(this.getCrashSound(float3), this.square, 1.0F, 20.0F, 1.0F, true);
		}
	}

	private String getCrashSound(float float1) {
		if (float1 < 5.0F) {
			return "VehicleCrash1";
		} else {
			return float1 < 30.0F ? "VehicleCrash2" : "VehicleCrash";
		}
	}
	...
	public boolean isCharacterAdjacentTo(IsoGameCharacter gameCharacter) {
		if ((int)gameCharacter.z != (int)this.z) {
			...
			float float3 = vector3f3.z - vector3f2.z / 2.0F;
			float float4 = vector3f3.z + vector3f2.z / 2.0F;
			if (vector3f.x >= float1 - 0.5F && vector3f.x < float2 + 0.5F && vector3f.z >= float3 - 0.5F && vector3f.z < float4 + 0.5F) {
				((BaseVehicle.Vector3fObjectPool)TL_vector3f_pool.get()).release(vector3f);
				return true;
			} else {
				((BaseVehicle.Vector3fObjectPool)TL_vector3f_pool.get()).release(vector3f);
	...
	public boolean testCollisionWithVehicle(BaseVehicle baseVehicle) {
		...
		if (vehicleScript != null && vehicleScript2 != null) {
			Vector2[] vector2Array = BaseVehicle.L_testCollisionWithVehicle.testVecs1;
			Vector2[] vector2Array2 = BaseVehicle.L_testCollisionWithVehicle.testVecs2;
			if (vector2Array[0] == null) {
				for (int int1 = 0; int1 < vector2Array.length; ++int1) {
					vector2Array[int1] = new Vector2();
					vector2Array2[int1] = new Vector2();
				}
			}
			...
			Vector3f vector3f5 = BaseVehicle.L_testCollisionWithVehicle.worldPos;
			float float1 = 0.5F;
	...
	public void hitCharacter(IsoZombie zombie) {
		IsoPlayer player = (IsoPlayer)Type.tryCastTo(zombie, IsoPlayer.class);
		IsoZombie zombie2 = (IsoZombie)Type.tryCastTo(zombie, IsoZombie.class);
		if (zombie.getCurrentState() != StaggerBackState.instance() && zombie.getCurrentState() != ZombieFallDownState.instance()) {
			if (!(Math.abs(zombie.x - this.x) < 0.01F) && !(Math.abs(zombie.y - this.y) < 0.01F)) {
				float float1 = 15.0F;
				Vector3f vector3f = this.getLinearVelocity((Vector3f)((BaseVehicle.Vector3fObjectPool)TL_vector3f_pool.get()).alloc());
				vector3f.y = 0.0F;
				...
				if (float2 < 0.05F) {
					...
					Vector3f vector3f2 = (Vector3f)((BaseVehicle.Vector3fObjectPool)TL_vector3f_pool.get()).alloc();
					vector3f2.set(this.x - zombie.x, 0.0F, this.y - zombie.y);
					vector3f2.normalize();
					vector3f.normalize();
					float float3 = vector3f.dot(vector3f2);
					((BaseVehicle.Vector3fObjectPool)TL_vector3f_pool.get()).release(vector3f);
					if (float3 < 0.0F && !GameServer.bServer) {
						this.ApplyImpulse(zombie, this.getFudgedMass() * 7.0F * float2 / float1 * Math.abs(float3));
					}
					...
					if (player != null) {
						...
						zombie2.setVehicleHitLocation(this);
					}

					BaseSoundEmitter baseSoundEmitter = IsoWorld.instance.getFreeEmitter(zombie.x, zombie.y, zombie.z);
					...
					zombie.Hit(this, float4, float3 > 0.0F, vector2.set(-vector3f2.x, -vector3f2.z));
					((BaseVehicle.Vector2ObjectPool)TL_vector2_pool.get()).release(vector2);
					((BaseVehicle.Vector3fObjectPool)TL_vector3f_pool.get()).release(vector3f2);
					...
					if (float2 >= 5.0F || this.zombiesHits > 10) {
						...
						Vector3f vector3f3 = (Vector3f)((BaseVehicle.Vector3fObjectPool)TL_vector3f_pool.get()).alloc();
						this.getLocalPos(zombie.x, zombie.y, zombie.z, vector3f3);
						int int1;
						if (vector3f3.z > 0.0F) {
	...
	public int calculateDamageWithCharacter(IsoGameCharacter gameCharacter) {
		Vector3f vector3f = (Vector3f)((BaseVehicle.Vector3fObjectPool)TL_vector3f_pool.get()).alloc();
		this.getLocalPos(gameCharacter.x, gameCharacter.y, gameCharacter.z, vector3f);
		int int1;
		if (vector3f.z > 0.0F) {
			int1 = this.caclulateDamageWithBodies(true);
		} else {
			int1 = -1 * this.caclulateDamageWithBodies(false);
		}
	...
	public boolean isEnterBlocked(IsoGameCharacter gameCharacter, int int1) {
		return this.isExitBlocked(gameCharacter, int1);
	}
	...
	public boolean isExitBlocked(IsoGameCharacter gameCharacter, int int1) {
		VehicleScript.Position position = this.getPassengerPosition(int1, "inside");
		VehicleScript.Position position2 = this.getPassengerPosition(int1, "outside");
		if (position != null && position2 != null) {
			Vector3f vector3f = this.getPassengerPositionWorldPos(position2, (Vector3f)((BaseVehicle.Vector3fObjectPool)TL_vector3f_pool.get()).alloc());
			...
			if (!boolean1 && GameClient.bClient) {
				IsoGridSquare square = IsoWorld.instance.CurrentCell.getGridSquare((double)vector3f.x, (double)vector3f.y, (double)vector3f.z);
				if (square != null && gameCharacter instanceof IsoPlayer && !SafeHouse.isPlayerAllowedOnSquare((IsoPlayer)gameCharacter, square)) {
					boolean1 = true;
				}
			}
	...
	public boolean isExitBlocked2(int int1) {
		...
		if (position != null && position2 != null) {
			...
			IsoGridSquare square = IsoWorld.instance.CurrentCell.getGridSquare((double)vector3f.x, (double)vector3f.y, (double)vector3f.z);
			IsoGameCharacter gameCharacter = this.getCharacter(int1);
			if (gameCharacter instanceof IsoPlayer && !SafeHouse.isPlayerAllowedOnSquare((IsoPlayer)gameCharacter, square)) {
				boolean1 = true;
			}
	...
	private void renderInterpolateBuffer() {
		if (this.netPlayerAuthorization == BaseVehicle.Authorization.Remote) {
			float float1 = IsoUtils.XToScreenExact(this.x, this.y, 0.0F, 0);
			float float2 = IsoUtils.YToScreenExact(this.x, this.y, 0.0F, 0);
			float float3 = float1 - 310.0F;
			float float4 = float2 + 22.0F;
			float float5 = 300.0F;
			float float6 = 150.0F;
			float float7 = 4.0F;
			...
			LineDrawer.drawLine(float3, float4, float3 + float5, float4, color.r, color.g, color.b, color.a, 1);
			LineDrawer.drawLine(float3, float4 + float6, float3 + float5, float4 + float6, color.r, color.g, color.b, color.a, 1);
			long long1 = GameTime.getServerTimeMills();
			long long2 = long1 - 150L - (long)this.interpolation.history;
			long long3 = long1 + 150L;
			this.renderInterpolateBuffer_drawVertLine(long2, color, float3, float4, float5, float6, long2, long3, true);
			this.renderInterpolateBuffer_drawVertLine(long3, color, float3, float4, float5, float6, long2, long3, true);
			this.renderInterpolateBuffer_drawVertLine(long1, color2, float3, float4, float5, float6, long2, long3, true);
			this.renderInterpolateBuffer_drawVertLine(long1 - (long)this.interpolation.delay, color3, float3, float4, float5, float6, long2, long3, true);
			this.renderInterpolateBuffer_drawPoint(long1 - (long)this.interpolation.delay, this.x, color5, 5, float3, float4, float5, float6, long2, long3, this.x - float7, this.x + float7);
			this.renderInterpolateBuffer_drawPoint(long1 - (long)this.interpolation.delay, this.y, color6, 5, float3, float4, float5, float6, long2, long3, this.y - float7, this.y + float7);
			long long4 = 0L;
			float float8 = Float.NaN;
			float float9 = Float.NaN;
			VehicleInterpolationData vehicleInterpolationData = new VehicleInterpolationData();
			vehicleInterpolationData.time = long1 - (long)this.interpolation.delay;
			VehicleInterpolationData vehicleInterpolationData2 = (VehicleInterpolationData)this.interpolation.buffer.higher(vehicleInterpolationData);
			VehicleInterpolationData vehicleInterpolationData3 = (VehicleInterpolationData)this.interpolation.buffer.floor(vehicleInterpolationData);
			VehicleInterpolationData vehicleInterpolationData4;
			for (Iterator iterator = this.interpolation.buffer.iterator(); iterator.hasNext(); float9 = vehicleInterpolationData4.y) {
				vehicleInterpolationData4 = (VehicleInterpolationData)iterator.next();
				boolean boolean1 = (vehicleInterpolationData4.hashCode() & 1) == 0;
				this.renderInterpolateBuffer_drawVertLine(vehicleInterpolationData4.time, color4, float3, float4, float5, float6, long2, long3, boolean1);
				if (vehicleInterpolationData4 == vehicleInterpolationData2) {
					this.renderInterpolateBuffer_drawTextHL(vehicleInterpolationData4.time, "H", color3, float3, float4, float5, float6, long2, long3);
				}

				if (vehicleInterpolationData4 == vehicleInterpolationData3) {
					this.renderInterpolateBuffer_drawTextHL(vehicleInterpolationData4.time, "L", color3, float3, float4, float5, float6, long2, long3);
				}

				this.renderInterpolateBuffer_drawPoint(vehicleInterpolationData4.time, vehicleInterpolationData4.x, color5, 5, float3, float4, float5, float6, long2, long3, this.x - float7, this.x + float7);
				this.renderInterpolateBuffer_drawPoint(vehicleInterpolationData4.time, vehicleInterpolationData4.y, color6, 5, float3, float4, float5, float6, long2, long3, this.y - float7, this.y + float7);
				if (!Float.isNaN(float8)) {
					this.renderInterpolateBuffer_drawLine(long4, float8, vehicleInterpolationData4.time, vehicleInterpolationData4.x, color5, float3, float4, float5, float6, long2, long3, this.x - float7, this.x + float7);
					this.renderInterpolateBuffer_drawLine(long4, float9, vehicleInterpolationData4.time, vehicleInterpolationData4.y, color6, float3, float4, float5, float6, long2, long3, this.y - float7, this.y + float7);
				}

				long4 = vehicleInterpolationData4.time;
				float8 = vehicleInterpolationData4.x;
			}

			float[] floatArray = new float[27];
			float[] floatArray2 = new float[2];
			boolean boolean2 = this.interpolation.interpolationDataGet(floatArray, floatArray2, long1 - (long)this.interpolation.delay);
			TextManager.instance.DrawString((double)float3, (double)(float4 + float6 + 20.0F), String.format("interpolationDataGet=%s", boolean2 ? "True" : "False"), (double)color3.r, (double)color3.g, (double)color3.b, (double)color3.a);
			TextManager.instance.DrawString((double)float3, (double)(float4 + float6 + 30.0F), String.format("buffer.size=%d buffering=%s", this.interpolation.buffer.size(), String.valueOf(this.interpolation.buffering)), (double)color3.r, (double)color3.g, (double)color3.b, (double)color3.a);
			if (this.interpolation.buffer.size() >= 2) {
				TextManager.instance.DrawString((double)float3, (double)(float4 + float6 + 40.0F), String.format("last=%d first=%d", ((VehicleInterpolationData)this.interpolation.buffer.last()).time, ((VehicleInterpolationData)this.interpolation.buffer.first()).time), (double)color3.r, (double)color3.g, (double)color3.b, (double)color3.a);
				TextManager.instance.DrawString((double)float3, (double)(float4 + float6 + 50.0F), String.format("(last-first).time=%d delay=%d", ((VehicleInterpolationData)this.interpolation.buffer.last()).time - ((VehicleInterpolationData)this.interpolation.buffer.first()).time, this.interpolation.delay), (double)color3.r, (double)color3.g, (double)color3.b, (double)color3.a);
			}
		}
	}

	private void renderInterpolateBuffer_drawTextHL(long long1, String string, Color color, float float1, float float2, float float3, float float4, long long2, long long3) {
		float float5 = float3 / (float)(long3 - long2);
		float float6 = (float)(long1 - long2) * float5;
		TextManager.instance.DrawString((double)(float6 + float1), (double)float2, string, (double)color.r, (double)color.g, (double)color.b, (double)color.a);
	}

	private void renderInterpolateBuffer_drawVertLine(long long1, Color color, float float1, float float2, float float3, float float4, long long2, long long3, boolean boolean1) {
		float float5 = float3 / (float)(long3 - long2);
		float float6 = (float)(long1 - long2) * float5;
		LineDrawer.drawLine(float6 + float1, float2, float6 + float1, float2 + float4, color.r, color.g, color.b, color.a, 1);
		TextManager.instance.DrawString((double)(float6 + float1), (double)(float2 + float4 + (boolean1 ? 0.0F : 10.0F)), String.format("%.1f", (float)(long1 - long1 / 100000L * 100000L) / 1000.0F), (double)color.r, (double)color.g, (double)color.b, (double)color.a);
	}

	private void renderInterpolateBuffer_drawLine(long long1, float float1, long long2, float float2, Color color, float float3, float float4, float float5, float float6, long long3, long long4, float float7, float float8) {
		float float9 = float5 / (float)(long4 - long3);
		float float10 = (float)(long1 - long3) * float9;
		float float11 = (float)(long2 - long3) * float9;
		float float12 = float6 / (float8 - float7);
		float float13 = (float1 - float7) * float12;
		float float14 = (float2 - float7) * float12;
		LineDrawer.drawLine(float10 + float3, float13 + float4, float11 + float3, float14 + float4, color.r, color.g, color.b, color.a, 1);
	}

	private void renderInterpolateBuffer_drawPoint(long long1, float float1, Color color, int int1, float float2, float float3, float float4, float float5, long long2, long long3, float float6, float float7) {
		float float8 = float4 / (float)(long3 - long2);
		float float9 = (float)(long1 - long2) * float8;
		float float10 = float5 / (float7 - float6);
		float float11 = (float1 - float6) * float10;
		LineDrawer.drawCircle(float9 + float2, float11 + float3, (float)int1, 10, color.r, color.g, color.b);
	}
	...
	private void renderAuthorizations() {
		...
		LineDrawer.drawLine(IsoUtils.XToScreenExact(this.poly.x1, this.poly.y1, 0.0F, 0), IsoUtils.YToScreenExact(this.poly.x1, this.poly.y1, 0.0F, 0), IsoUtils.XToScreenExact(this.poly.x2, this.poly.y2, 0.0F, 0), IsoUtils.YToScreenExact(this.poly.x2, this.poly.y2, 0.0F, 0), float1, float2, float3, float4, 1);
		LineDrawer.drawLine(IsoUtils.XToScreenExact(this.poly.x2, this.poly.y2, 0.0F, 0), IsoUtils.YToScreenExact(this.poly.x2, this.poly.y2, 0.0F, 0), IsoUtils.XToScreenExact(this.poly.x3, this.poly.y3, 0.0F, 0), IsoUtils.YToScreenExact(this.poly.x3, this.poly.y3, 0.0F, 0), float1, float2, float3, float4, 1);
		LineDrawer.drawLine(IsoUtils.XToScreenExact(this.poly.x3, this.poly.y3, 0.0F, 0), IsoUtils.YToScreenExact(this.poly.x3, this.poly.y3, 0.0F, 0), IsoUtils.XToScreenExact(this.poly.x4, this.poly.y4, 0.0F, 0), IsoUtils.YToScreenExact(this.poly.x4, this.poly.y4, 0.0F, 0), float1, float2, float3, float4, 1);
		LineDrawer.drawLine(IsoUtils.XToScreenExact(this.poly.x4, this.poly.y4, 0.0F, 0), IsoUtils.YToScreenExact(this.poly.x4, this.poly.y4, 0.0F, 0), IsoUtils.XToScreenExact(this.poly.x1, this.poly.y1, 0.0F, 0), IsoUtils.YToScreenExact(this.poly.x1, this.poly.y1, 0.0F, 0), float1, float2, float3, float4, 1);
		float float5 = 0.0F;
		if (this.getVehicleTowing() != null) {
			...
			Vector3f vector3f3 = (Vector3f)vector3fObjectPool.alloc();
			Vector3f vector3f4 = this.getVehicleTowing().getTowingWorldPos(this.getVehicleTowing().getTowAttachmentSelf(), vector3f3);
			if (vector3f2 != null && vector3f4 != null) {
				LineDrawer.DrawIsoLine(vector3f2.x, vector3f2.y, vector3f2.z, vector3f4.x, vector3f4.y, vector3f4.z, float1, float2, float3, float4, 1);
				LineDrawer.DrawIsoCircle(vector3f2.x, vector3f2.y, vector3f2.z, 0.2F, 16, float1, float2, float3, float4);
				float5 = IsoUtils.DistanceTo(vector3f2.x, vector3f2.y, vector3f2.z, vector3f4.x, vector3f4.y, vector3f4.z);
			}

			vector3fObjectPool.release(vector3f);
			vector3fObjectPool.release(vector3f3);
		}
		...
		float4 = 1.0F;
		float float6 = 10.0F;
		float float7 = IsoUtils.XToScreenExact(this.x, this.y, 0.0F, 0);
		float float8 = IsoUtils.YToScreenExact(this.x, this.y, 0.0F, 0);
		IsoPlayer player = (IsoPlayer)GameClient.IDToPlayerMap.get(this.netPlayerId);
		String string = player == null ? "@server" : player.getUsername();
		String string2 = string + " ( " + this.netPlayerId + " )";
		TextManager textManager = TextManager.instance;
		double double1 = (double)float7;
		double double2 = (double)(float8 + (float6 += 12.0F));
		String string3 = this.getScriptName();
		textManager.DrawString(double1, double2, "VID: " + string3 + " ( " + this.getId() + " )", (double)float1, (double)float2, (double)float3, (double)float4);
		TextManager.instance.DrawString((double)float7, (double)(float8 + (float6 += 12.0F)), "PID: " + string2, (double)float1, (double)float2, (double)float3, (double)float4);
		textManager = TextManager.instance;
		double1 = (double)float7;
		double2 = (double)(float8 + (float6 += 12.0F));
		string3 = this.netPlayerAuthorization.name();
		textManager.DrawString(double1, double2, "Auth: " + string3, (double)float1, (double)float2, (double)float3, (double)float4);
		double1 = (double)float7;
		double2 = (double)(float8 + (float6 += 12.0F));
		boolean boolean1 = this.isStatic;
		TextManager.instance.DrawString(double1, double2, "Static/active: " + boolean1 + "/" + this.isActive, (double)float1, (double)float2, (double)float3, (double)float4);
		double1 = (double)float7;
		double2 = (double)(float8 + (float6 += 12.0F));
		float float9 = this.x;
		TextManager.instance.DrawString(double1, double2, "x=" + float9 + " / y=" + this.y, (double)float1, (double)float2, (double)float3, (double)float4);
		TextManager.instance.DrawString((double)float7, (double)(float8 + (float6 += 14.0F)), String.format("Passengers: %d/%d", Arrays.stream(this.passengers).filter((var0)->{
			return var0.character != null;
		}).count(), this.passengers.length), (double)float1, (double)float2, (double)float3, (double)float4);
		TextManager.instance.DrawString((double)float7, (double)(float8 + (float6 += 12.0F)), String.format("Speed: %s%.3f kmph", this.getCurrentSpeedKmHour() >= 0.0F ? "+" : "", this.getCurrentSpeedKmHour()), (double)float1, (double)float2, (double)float3, (double)float4);
		TextManager.instance.DrawString((double)float7, (double)(float8 + (float6 += 12.0F)), String.format("Engine speed: %.3f", this.engineSpeed), (double)float1, (double)float2, (double)float3, (double)float4);
		TextManager.instance.DrawString((double)float7, (double)(float8 + (float6 += 12.0F)), String.format("Mass: %.3f/%.3f", this.getMass(), this.getFudgedMass()), (double)float1, (double)float2, (double)float3, (double)float4);
		if (float5 > 1.5F) {
			float2 = 0.75F;
		}

		if (this.getVehicleTowing() != null) {
			TextManager.instance.DrawString((double)float7, (double)(float8 + (float6 += 14.0F)), "Towing: " + this.getVehicleTowing().getId(), (double)float1, (double)float2, (double)float3, (double)float4);
			TextManager.instance.DrawString((double)float7, (double)(float8 + (float6 += 12.0F)), String.format("Distance: %.3f", float5), (double)float1, (double)float2, (double)float3, (double)float4);
		}

		if (this.getVehicleTowedBy() != null) {
			TextManager.instance.DrawString((double)float7, (double)(float8 + (float6 += 14.0F)), "TowedBy: " + this.getVehicleTowedBy().getId(), (double)float1, (double)float2, (double)float3, (double)float4);
			TextManager.instance.DrawString((double)float7, (double)(float8 + (float6 += 12.0F)), String.format("Distance: %.3f", float5), (double)float1, (double)float2, (double)float3, (double)float4);
		}
	}
			...
			if (vehiclePart != null) {
				...
				if (area != null) {
					...
					if (vector22 == null) {
						...
						Vector3f vector3f = this.getForwardVector((Vector3f)((BaseVehicle.Vector3fObjectPool)TL_vector3f_pool.get()).alloc());
						float float1 = Core.getInstance().getGoodHighlitedColor().getR();
						float float2 = Core.getInstance().getGoodHighlitedColor().getG();
						float float3 = Core.getInstance().getGoodHighlitedColor().getB();
	...
	public void updateSounds() {
		...
		if (player == null) {
			...
			float float5;
			if (!GameServer.bServer) {
				float float6 = ClimateManager.getInstance().isRaining() ? ClimateManager.getInstance().getPrecipitationIntensity() : 0.0F;
				...
				} else if (float6 > 0.0F && float1 < 100.0F) {
			...
			if (this.getController() != null) {
				if (!GameServer.bServer) {
					...
					float5 = Math.abs(this.getCurrentSpeedKmHour());
	...
	public void tryHotwire(int int1) {
		...
		boolean boolean1 = false;
		String string = null;
		if (Rand.Next(100) <= 11 - int1 && this.alarmed) {
			this.triggerAlarm();
		}
		...
		if (!GameClient.bClient) {
			...
			if (vehicleDistribution == null) {
				...
				for (int2 = 0; int2 < this.parts.size(); ++int2) {
					...
					if (vehiclePart2.getItemContainer() != null) {
						if (GameServer.bServer && GameServer.bSoftReset) {
							vehiclePart2.getItemContainer().setExplored(false);
						}

						if (!vehiclePart2.getItemContainer().bExplored) {
							vehiclePart2.getItemContainer().clear();
							if (Rand.Next(100) <= 100) {
								this.randomizeContainer(vehiclePart2, itemPickerRoom);
								LuaEventManager.triggerEvent("OnFillContainer", string, vehiclePart2.getItemContainer().getType(), vehiclePart2.getItemContainer());
							}
				...
				for (int int1 = 0; int1 < this.passengers.length; ++int1) {
					if (this.getPassenger(int1).character != null) {
						...
						if (GameServer.bServer && gameCharacter instanceof IsoPlayer) {
							...
							LuaEventManager.triggerEvent("OnPlayerGetDamage", gameCharacter, "CARCRASHDAMAGE", float1);
						}
					}
				}
			}
	...
	public void addPointConstraint(IsoPlayer player, BaseVehicle baseVehicle, String string, String string2) {
		this.addPointConstraint(player, baseVehicle, string, string2, false);
	}

	public void addPointConstraint(IsoPlayer player, BaseVehicle baseVehicle, String string, String string2, Boolean Boolean1) {
		if (baseVehicle == null || player != null && (IsoUtils.DistanceToSquared(player.x, player.y, this.x, this.y) > 100.0F || IsoUtils.DistanceToSquared(player.x, player.y, baseVehicle.x, baseVehicle.y) > 100.0F)) {
			DebugLog.General.warn("The " + player.getUsername() + " user attached vehicles at a long distance");
		}

		this.breakConstraint(true, Boolean1);
		baseVehicle.breakConstraint(true, Boolean1);
		BaseVehicle.Vector3fObjectPool vector3fObjectPool = (BaseVehicle.Vector3fObjectPool)TL_vector3f_pool.get();
		...
		if (vector3f != null && vector3f2 != null) {
			if (!GameServer.bServer) {
				if (!this.getScriptName().contains("Trailer") && !baseVehicle.getScriptName().contains("Trailer")) {
					this.constraintTowing = Bullet.addRopeConstraint(this.VehicleID, baseVehicle.VehicleID, vector3f.x, vector3f.y, vector3f.z, vector3f2.x, vector3f2.y, vector3f2.z, 1.5F);
				} else {
					this.constraintTowing = Bullet.addPointConstraint(this.VehicleID, baseVehicle.VehicleID, vector3f.x, vector3f.y, vector3f.z, vector3f2.x, vector3f2.y, vector3f2.z);
				}
			}

			baseVehicle.constraintTowing = this.constraintTowing;
			this.setVehicleTowing(baseVehicle, string, string2);
			baseVehicle.setVehicleTowedBy(this, string, string2);
			vector3fObjectPool.release(vector3f);
			vector3fObjectPool.release(vector3f2);
			this.constraintChanged();
			baseVehicle.constraintChanged();
			if (GameServer.bServer && player != null && this.netPlayerAuthorization == BaseVehicle.Authorization.Server && baseVehicle.netPlayerAuthorization == BaseVehicle.Authorization.Server) {
				this.setNetPlayerAuthorization(BaseVehicle.Authorization.LocalCollide, player.OnlineID);
				this.authSimulationTime = System.currentTimeMillis();
				baseVehicle.setNetPlayerAuthorization(BaseVehicle.Authorization.LocalCollide, player.OnlineID);
				baseVehicle.authSimulationTime = System.currentTimeMillis();
			}

			if (GameServer.bServer && !Boolean1) {
				VehicleManager.instance.sendTowing(this, baseVehicle, string, string2);
			}
		} else {
			if (vector3f != null) {
				vector3fObjectPool.release(vector3f);
	...
	public void authorizationChanged(IsoGameCharacter gameCharacter) {
		if (gameCharacter != null) {
			this.setNetPlayerAuthorization(BaseVehicle.Authorization.Local, gameCharacter.getOnlineID());
		} else {
			this.setNetPlayerAuthorization(BaseVehicle.Authorization.Server, -1);
		}
	}
	...
	public boolean canAttachTrailer(BaseVehicle baseVehicle, String string, String string2, boolean boolean1) {
		if (this != baseVehicle && this.physics != null && this.constraintTowing == -1) {
			if (baseVehicle != null && baseVehicle.physics != null && baseVehicle.constraintTowing == -1) {
				...
				if (vector3f != null && vector3f2 != null) {
					float float1 = IsoUtils.DistanceToSquared(vector3f.x, vector3f.y, 0.0F, vector3f2.x, vector3f2.y, 0.0F);
					vector3fObjectPool.release(vector3f);
					vector3fObjectPool.release(vector3f2);
					...
					if (modelAttachment != null && modelAttachment.getCanAttach() != null && !modelAttachment.getCanAttach().contains(string2)) {
						...
						DebugLog.Vehicle.trace("vidA=%d (%s) vidB=%d (%s) dist: %f", this.getId(), string, baseVehicle.getId(), string2, float1);
						boolean boolean2 = this.getScriptName().contains("Trailer") || baseVehicle.getScriptName().contains("Trailer");
						return float1 < (boolean1 ? 10.0F : (boolean2 ? 1.0F : 4.0F));
					}
				} else {
					return false;
				}
	...
	private void tryReconnectToTowedVehicle() {
		if (GameClient.bClient) {
			short short1 = VehicleManager.instance.getTowedVehicleID(this.VehicleID);
			if (short1 != -1) {
				...
				if (baseVehicle != null) {
					if (this.canAttachTrailer(baseVehicle, this.towAttachmentSelf, this.towAttachmentOther, true)) {
						this.addPointConstraint((IsoPlayer)null, baseVehicle, this.towAttachmentSelf, this.towAttachmentOther, true);
					}
				}
			}
			...
			if (this.vehicleTowingID != -1) {
				...
				for (int int1 = 0; int1 < arrayList.size(); ++int1) {
					BaseVehicle baseVehicle3 = (BaseVehicle)arrayList.get(int1);
				...
				if (baseVehicle2 != null) {
					if (this.canAttachTrailer(baseVehicle2, this.towAttachmentSelf, this.towAttachmentOther, true)) {
						this.addPointConstraint((IsoPlayer)null, baseVehicle2, this.towAttachmentSelf, this.towAttachmentOther, false);
					}
				}
			}
		}
	...
	public void positionTrailer(BaseVehicle baseVehicle) {
		if (baseVehicle != null) {
			...
			if (vector3f != null && vector3f2 != null) {
				...
				this.addPointConstraint((IsoPlayer)null, baseVehicle, "trailer", "trailer");
				vector3fObjectPool.release(vector3f);
				vector3fObjectPool.release(vector3f2);
			}
		}
	...
	public BaseVehicle setSmashed(String string, boolean boolean1) {
		...
		if (string2 != null) {
			...
			baseVehicle.savedRot.set((Quaternionfc)this.savedRot);
			if (boolean1) {
				float float1 = this.getAngleY();
				baseVehicle.savedRot.rotationXYZ(0.0F, float1 * 0.017453292F, 3.1415927F);
			}
	...
	public boolean isCollided(IsoGameCharacter gameCharacter) {
		if (GameClient.bClient && this.getDriver() != null && !this.getDriver().isLocal()) {
			...
			Vector2 vector2 = this.testCollisionWithCharacter(gameCharacter, 0.20000002F, this.hitVars.collision);
			return vector2 != null && vector2.x != -1.0F;
		}
	}

	public BaseVehicle.HitVars checkCollision(IsoGameCharacter gameCharacter) {
		if (gameCharacter.isProne()) {
			int int1 = this.testCollisionWithProneCharacter(gameCharacter, true);
			if (int1 > 0) {
				this.hitVars.calc(gameCharacter, this);
				this.hitCharacter(gameCharacter, this.hitVars);
				return this.hitVars;
			} else {
				return null;
			}
	...
	public boolean updateHitByVehicle(IsoGameCharacter gameCharacter) {
		if (gameCharacter.isVehicleCollisionActive(this) && (this.isCollided(gameCharacter) || gameCharacter.isCollidedWithVehicle()) && this.physics != null) {
			BaseVehicle.HitVars hitVars = this.checkCollision(gameCharacter);
			if (hitVars != null) {
				gameCharacter.doHitByVehicle(this, hitVars);
				return true;
			}
		}
	...
	public void hitCharacter(IsoGameCharacter gameCharacter, BaseVehicle.HitVars hitVars) {
		if (hitVars.dot < 0.0F && !GameServer.bServer) {
			this.ApplyImpulse(gameCharacter, hitVars.vehicleImpulse);
		}
		...
		if (gameCharacter instanceof IsoPlayer) {
			((IsoPlayer)gameCharacter).setVehicleHitLocation(this);
		} else if (gameCharacter instanceof IsoZombie) {
			((IsoZombie)gameCharacter).setVehicleHitLocation(this);
		}
		...
		if (hitVars.vehicleSpeed >= 5.0F || this.zombiesHits > 10) {
			...
			if (vector3f.z > 0.0F) {
				...
				DebugLog.Vehicle.trace("Damage car front hits=%d damage=%d", this.zombiesHits, int1);
				hitVars.vehicleDamage = int1;
	...
	public static enum Authorization {
		...
		public static BaseVehicle.Authorization valueOf(byte byte1) {
			return (BaseVehicle.Authorization)authorizations.getOrDefault(byte1, Server);
		}
		public byte index() {
	...
	public static class HitVars {
		...
		public void calc(IsoGameCharacter gameCharacter, BaseVehicle baseVehicle) {
			...
			this.targetImpulse.set(baseVehicle.x - gameCharacter.x, 0.0F, baseVehicle.y - gameCharacter.y);
			...
			this.isTargetHitFromBehind = "BEHIND".equals(gameCharacter.testDotSide(baseVehicle));
		}
	}
