...
public final class PublicServerUtil {
				...
				for (int int2 = 0; int2 < int1; ++int2) {
					...
					if (string2.contains("ip")) {
						...
						if (GameServer.ip.contains(":")) {
							...
							DebugLog.log("The IP address (" + string3 + ") looks like the IPv6 address. Please make sure IPv4 server address is set to the " + ServerOptions.getInstance().server_browser_announced_ip.getName() + " server option.");
						}
					}
				}
	...
	private static void insertDatas() throws Exception {
		if (isEnabled) {
			...
			string2 = webSite;
			callUrl(string2 + "write.php?name=" + ServerOptions.instance.PublicName.getValue().replaceAll(" ", "%20") + string + "&port=" + ServerOptions.instance.DefaultPort.getValue() + "&UDPPort=" + ServerOptions.instance.UDPPort.getValue() + "&players=" + int1 + "&ip=" + string5 + "&open=" + (ServerOptions.instance.Open.getValue() ? "1" : "0") + "&password=" + ("".equals(ServerOptions.instance.Password.getValue()) ? "0" : "1") + "&maxPlayers=" + ServerOptions.getInstance().getMaxPlayers() + "&version=" + Core.getInstance().getVersion().replaceAll(" ", "%20") + string3 + "&mac=" + getMacAddress());
