...
local function predicateBlowTorch(item)
	return (item:hasTag("BlowTorch") or item:getType() == "BlowTorch") and item:getDrainableUsesInt() >= 20
end
...
function ISVehicleMenu.onExit(playerObj, seatFrom)
	...
	if (not playerObj:isBlockMovement()) then
		...
		if vehicle:isDriver(playerObj) then
			if math.abs(vehicle:getCurrentSpeedKmHour()) > 0.8f then ISTimedActionQueue.add(ISStopVehicle:new(playerObj)) end
		else 
			if not vehicle:isStopped() then
