...
--	self:drawTexture(self.HandSecondaryTexture, -1, 50, 1, 1, 1, 1);
    ...
    if isClient() then
        ...
        self:drawTextRight(statusData.svnRevision.."  "..statusData.version, maxX-50, maxY-100, 0.8, 0.8, 0.8, 1, UIFont.NewMedium);
        self:drawTextRight(statusData.serverTime, maxX-50, maxY-80, 0.8, 0.8, 0.8, 1, UIFont.NewMedium);

        if statusData.pingEnabled or self.chr:isShowMPInfos() then
            ...
            self:drawTextRight("Ping "..tostring(lastPing).." ms", maxX-50, maxY-60 , r, g, b, 1, UIFont.NewMedium);
        end

        if tonumber(getMaxPlayers()) > 32 then
            self:drawTextRight(getText("UI_MaxPlayers_Notification"), maxX-50, maxY-40 , 0.8, 0.8, 0.8, 1, UIFont.NewMedium);
        end
    end
