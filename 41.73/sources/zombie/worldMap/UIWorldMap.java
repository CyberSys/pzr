...
public class UIWorldMap extends UIElement {
			...
			if (this.m_renderer.getBoolean("Players")) {
				if (this.m_renderer.getBoolean("RemotePlayers")) {
					...
					while (iterator.hasNext()) {
						...
						if (this.shouldShowRemotePlayer(player)) {
							Vector2 vector2 = (Vector2)entry.getValue();
							this.renderPlayer(vector2.x, vector2.y);
							String string = player == null ? String.valueOf(entry.getKey()) : player.getUsername();
							this.renderPlayerName(vector2.x, vector2.y, string);
						}
					}
				}
			}
	...
	private boolean shouldShowRemotePlayer(IsoPlayer player) {
		if (ServerOptions.getInstance().MapRemotePlayerVisibility.getValue() == 3) {
			...
			for (int int1 = 0; int1 < IsoPlayer.numPlayers; ++int1) {
				...
				if (player2 != null) {
					if (this.isInSameFaction(player2, player)) {
						return true;
					}

					if (this.isInSameSafehouse(player2, player)) {
						return true;
					}
				}
			}
	...
	private boolean isInSameFaction(IsoPlayer player, IsoPlayer player2) {
	...
	private boolean isInSameSafehouse(IsoPlayer player, IsoPlayer player2) {
