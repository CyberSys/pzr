...
public final class RecipeManager {
	...
	private static RecipeManager.SourceItems getAvailableItems(Recipe recipe, IsoGameCharacter gameCharacter, ArrayList arrayList, InventoryItem inventoryItem, ArrayList arrayList2, boolean boolean1) {
		...
		if (inventoryItem != null && recipe.InSameInventory) {
			ItemContainer itemContainer = gameCharacter.getInventory();
			int int1;
			ItemContainer itemContainer2;
			if (inventoryItem.getOutermostContainer() == itemContainer) {
				for (int1 = arrayList.size() - 1; int1 >= 0; --int1) {
					itemContainer2 = (ItemContainer)arrayList.get(int1);
					if (itemContainer2 != inventoryItem.getContainer()) {
						arrayList.remove(itemContainer2);
					}
				}
			} else {
				for (int1 = arrayList.size() - 1; int1 >= 0; --int1) {
					itemContainer2 = (ItemContainer)arrayList.get(int1);
					if (itemContainer2 == itemContainer || itemContainer2.getCharacter() == gameCharacter) {
						arrayList.remove(itemContainer2);
					}
				}
			}
