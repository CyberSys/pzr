...
public class Clothing extends InventoryItem {
	...
	public void DoTooltip(ObjectTooltip objectTooltip, ObjectTooltip.Layout layout) {
		...
		for (int2 = 0; int2 < BloodBodyPartType.MAX.index(); ++int2) {
			if (itemVisual.getHole(BloodBodyPartType.FromIndex(int2)) > 0.0F) {
				++int1;
			}
		}
		...
		if (int1 > 0) {
			...
			layoutItem.setValueRightNoPlus(int1);
		}
	...
	private Clothing.WetDryState getWetDryState() {
		if (this.getWorldItem() == null) {
			if (this.container == null) {
				...
				IsoCombinationWasherDryer combinationWasherDryer = (IsoCombinationWasherDryer)Type.tryCastTo(this.container.parent, IsoCombinationWasherDryer.class);
	...
	public int getCondLossPerHole() {
		...
		int int2 = PZMath.max(1, this.getConditionMax() / int1);
		return int2;
	}
