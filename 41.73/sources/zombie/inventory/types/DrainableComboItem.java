...
public final class DrainableComboItem extends InventoryItem implements Drainable,IUpdater {
	...
	public void update() {
		...
		if (itemContainer != null) {
			...
			if (this.IsCookable) {
				if (this.Heat > 1.6F) {
					...
					if (int1 != this.LastCookMinute) {
						...
						float float3 = this.CookingTime;
						if (float3 < 1.0F) {
							float3 = 10.0F;
						}

						float3 += float2;
						if (this.isTaintedWater() && float3 > Math.min(this.MinutesToCook, 10.0F)) {
							this.setTaintedWater(false);
						}

						if (!this.isCooked() && float3 > this.MinutesToCook) {
							this.setCooked(true);
			...
			} else if (itemContainer != null && itemContainer.isMicrowave() && this.isTaintedWater() && this.Heat > 1.6F) {
				int1 = GameTime.getInstance().getMinutes();
				if (int1 != this.LastCookMinute) {
					this.LastCookMinute = int1;
					float2 = 1.0F;
