...
public final class MPDebugInfo {
		...
		if (GameServer.bServer) {
			if (udpConnection.accessLevel == 32) {
				...
				if (byte1 != 2) {
					...
					PacketTypes.PacketType.KeepAlive.doPacket(byteBufferWriter2);
	...
	public void render(ZombiePopulationRenderer zombiePopulationRenderer, float float1) {
		...
		if (GameClient.bClient && DebugOptions.instance.MultiplayerShowPosition.getValue()) {
			float float4 = (float)((IsoChunkMap.ChunkGridWidth / 2 + 2) * 10);
			Iterator iterator = GameClient.positions.entrySet().iterator();
			while (iterator.hasNext()) {
				Entry entry = (Entry)iterator.next();
				IsoPlayer player = (IsoPlayer)GameClient.IDToPlayerMap.get(entry.getKey());
				Color color2 = Color.white;
				if (player != null) {
					color2 = player.getSpeakColour();
				}

				Vector2 vector2 = (Vector2)entry.getValue();
				zombiePopulationRenderer.renderZombie(vector2.x, vector2.y, color2.r, color2.g, color2.b);
				zombiePopulationRenderer.renderCircle(vector2.x, vector2.y, float4, color2.r, color2.g, color2.b, color2.a);
				zombiePopulationRenderer.renderString(vector2.x, vector2.y, player == null ? String.valueOf(entry.getKey()) : player.getUsername(), (double)color2.r, (double)color2.g, (double)color2.b, (double)color2.a);
			}

			if (IsoPlayer.getInstance() != null) {
				IsoPlayer player2 = IsoPlayer.getInstance();
				color = player2.getSpeakColour();
				zombiePopulationRenderer.renderZombie(player2.x, player2.y, color.r, color.g, color.b);
				zombiePopulationRenderer.renderCircle(player2.x, player2.y, float4, color.r, color.g, color.b, color.a);
				zombiePopulationRenderer.renderString(player2.x, player2.y, player2.getUsername(), (double)color.r, (double)color.g, (double)color.b, (double)color.a);
				color = Colors.LightBlue;
				zombiePopulationRenderer.renderCircle(player2.x, player2.y, RakVoice.GetMinDistance(), color.r, color.g, color.b, color.a);
				zombiePopulationRenderer.renderCircle(player2.x, player2.y, RakVoice.GetMaxDistance(), color.r, color.g, color.b, color.a);
			}
		}
		...
		try {
			...
			Iterator iterator2 = debugSounds.entrySet().iterator();
			while (iterator2.hasNext()) {
				Entry entry2 = (Entry)iterator2.next();
				color = Colors.LightBlue;
				if (((MPDebugInfo.MPSoundDebugInfo)entry2.getValue()).sourceIsZombie) {
					color = Colors.GreenYellow;
				} else if (((MPDebugInfo.MPSoundDebugInfo)entry2.getValue()).bRepeating) {
					color = Colors.Coral;
				}

				float float5 = 1.0F - Math.max(0.0F, Math.min(1.0F, (float)(System.currentTimeMillis() - (Long)entry2.getKey()) / 1000.0F));
				zombiePopulationRenderer.renderCircle((float)((MPDebugInfo.MPSoundDebugInfo)entry2.getValue()).x, (float)((MPDebugInfo.MPSoundDebugInfo)entry2.getValue()).y, (float)((MPDebugInfo.MPSoundDebugInfo)entry2.getValue()).radius, color.r, color.g, color.b, float5);
			}
