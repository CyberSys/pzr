...
public final class VirtualZombieManager {
	...
	private int getBlockedBits(IsoGridSquare square) {
		...
		if (square == null) {
			...
			if (square.nav[IsoDirections.N.index()] == null) {
				int1 |= this.NO_SQUARE_N;
			} else if (IsoGridSquare.getMatrixBit(square.pathMatrix, (int)1, (int)0, (int)1)) {
				int1 |= this.BLOCKED_N;
			}

			if (square.nav[IsoDirections.S.index()] == null) {
				int1 |= this.NO_SQUARE_S;
			} else if (IsoGridSquare.getMatrixBit(square.pathMatrix, (int)1, (int)2, (int)1)) {
				int1 |= this.BLOCKED_S;
			}

			if (square.nav[IsoDirections.W.index()] == null) {
				int1 |= this.NO_SQUARE_W;
			} else if (IsoGridSquare.getMatrixBit(square.pathMatrix, (int)0, (int)1, (int)1)) {
				int1 |= this.BLOCKED_W;
			}

			if (square.nav[IsoDirections.E.index()] == null) {
				int1 |= this.NO_SQUARE_E;
			} else if (IsoGridSquare.getMatrixBit(square.pathMatrix, (int)2, (int)1, (int)1)) {
				int1 |= this.BLOCKED_E;
			}
	...
	private boolean canPathOnlyN(IsoGridSquare square) {
		while (true) {
			int int1 = this.getBlockedBits(square);
			if ((int1 & (this.BLOCKED_W | this.BLOCKED_E)) != (this.BLOCKED_W | this.BLOCKED_E)) {
				return false;
			}

			if ((int1 & this.NO_SQUARE_N) != 0) {
				return false;
			}

			if ((int1 & this.BLOCKED_N) != 0) {
				return true;
			}
	...
	private boolean canPathOnlyS(IsoGridSquare square) {
		while (true) {
			...
			if ((int1 & this.NO_SQUARE_S) != 0) {
				return false;
			}

			if ((int1 & this.BLOCKED_S) != 0) {
				return true;
			}
	...
	private boolean canPathOnlyW(IsoGridSquare square) {
		while (true) {
			int int1 = this.getBlockedBits(square);
			if ((int1 & (this.BLOCKED_N | this.BLOCKED_S)) != (this.BLOCKED_N | this.BLOCKED_S)) {
				return false;
			}

			if ((int1 & this.NO_SQUARE_W) != 0) {
				return false;
			}

			if ((int1 & this.BLOCKED_W) != 0) {
				return true;
			}
	...
	private boolean canPathOnlyE(IsoGridSquare square) {
		while (true) {
			...
			if ((int1 & this.NO_SQUARE_E) != 0) {
				return false;
			}

			if ((int1 & this.BLOCKED_E) != 0) {
				return true;
			}
	...
	public boolean canSpawnAt(int int1, int int2, int int3) {
		...
		if (square != null && square.isFree(false)) {
			int int4 = this.getBlockedBits(square);
			if (int4 == (this.BLOCKED_N | this.BLOCKED_S | this.BLOCKED_W | this.BLOCKED_E)) {
				return false;
			} else if ((int4 & (this.BLOCKED_N | this.BLOCKED_S)) == (this.BLOCKED_N | this.BLOCKED_S) && this.canPathOnlyW(square) && this.canPathOnlyE(square)) {
				return false;
			} else {
				return (int4 & (this.BLOCKED_W | this.BLOCKED_E)) != (this.BLOCKED_W | this.BLOCKED_E) || !this.canPathOnlyN(square) || !this.canPathOnlyS(square);
			}
		} else {
			return false;
		}
