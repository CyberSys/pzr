...
public class IsoCarBatteryCharger extends IsoObject {
	...
	public void update() {
		...
		if (this.battery == null) {
			...
			boolean boolean1 = this.square != null && (this.square.haveElectricity() || IsoWorld.instance.isHydroPowerOn() && this.square.getRoom() != null);
			if (!boolean1) {
				this.activated = false;
			}
	...
	public void syncIsoObject(boolean boolean1, byte byte1, UdpConnection udpConnection, ByteBuffer byteBuffer) {
		if (GameClient.bClient && !boolean1) {
			...
			if (GameServer.bServer && !boolean1) {
				...
				if (byteBuffer.get() == 1) {
					try {
						this.battery = InventoryItem.loadItem(byteBuffer, 194);
					} catch (Exception exception) {
						exception.printStackTrace();
					}
				} else {
