...
public class IsoRaindrop extends IsoObject {
	...
	public void update() {
		...
		if (this.AttachedAnimSprite != null && this.AttachedAnimSprite.size() > this.AnimSpriteIndex && this.AnimSpriteIndex >= 0) {
			float float1 = this.SplashY + (float)((int)this.OffsetY);
			((IsoSpriteInstance)this.AttachedAnimSprite.get(this.AnimSpriteIndex)).parentSprite.soffY = (short)((int)float1);
		}

		if (this.OffsetY < 0.0F) {
			this.OffsetY = RainManager.RaindropStartDistance;
			this.Vel_Y = 0.0F;
			float float2 = 1000000.0F / (float)Rand.Next(1000000) + 1.0E-5F;
