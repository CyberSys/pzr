...
public abstract class WeatherParticle {
	...
	public boolean isOnScreen(float float1, float float2) {
		...
		float float3 = float1 + (float)this.bounds.getX();
		float float4 = float2 + (float)this.bounds.getY();
		float float5 = float3 + this.oWidth;
		float float6 = float4 + this.oHeight;
		if (!(float3 >= (float)int1) && !(float5 <= 0.0F)) {
			return !(float4 >= (float)int2) && !(float6 <= 0.0F);
		} else {
			return false;
		}
	}
	...
	public void render(float float1, float float2) {
		SpriteRenderer.instance.render(this.texture, float1 + (float)this.bounds.getX(), float2 + (float)this.bounds.getY(), this.oWidth, this.oHeight, this.color.r, this.color.g, this.color.b, this.renderAlpha, (Consumer)null);
	}
}
