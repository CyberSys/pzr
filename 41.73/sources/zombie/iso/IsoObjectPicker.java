		...
		public int calculateScore() {
			...
			if (!(object instanceof IsoDoor) && (!(object instanceof IsoThumpable) || !((IsoThumpable)object).isDoor())) {
				if (object instanceof IsoWindow) {
					...
					if (player.getCurrentSquare() != null && square.getRoom() == player.getCurrentSquare().getRoom()) {
						++float1;
					} else {
						float1 -= 100000.0F;
					}
					...
					if (object instanceof IsoCurtain) {
						...
						float1 += 20.0F;
					} else if (object instanceof IsoThumpable && ((IsoThumpable)object).getLightSource() != null) {
						float1 += 3.0F;
			...
			float float3 = IsoUtils.DistanceManhatten((float)square.getX() + 0.5F, (float)square.getY() + 0.5F, player.getX(), player.getY());
			float1 -= float3 / 2.0F;
			return (int)float1;
		}
