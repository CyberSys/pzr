...
public final class BuildingDef {
	...
	public long calculateMetaID(int int1, int int2) {
		...
		for (int int6 = 0; int6 < arrayList.size(); ++int6) {
			RoomDef roomDef = (RoomDef)arrayList.get(int6);
			if (roomDef.level <= int5) {
				if (roomDef.level < int5) {
				...
				int5 = roomDef.level;
				for (int int7 = 0; int7 < roomDef.rects.size(); ++int7) {
					RoomDef.RoomRect roomRect = (RoomDef.RoomRect)roomDef.rects.get(int7);
		...
		int3 -= int1 * 300;
		int4 -= int2 * 300;
		return (long)int5 << 32 | (long)int4 << 16 | (long)int3;
	}
	...
	public float getClosestPoint(float float1, float float2, Vector2f vector2f) {
		...
		for (int int1 = 0; int1 < this.rooms.size(); ++int1) {
			RoomDef roomDef = (RoomDef)this.rooms.get(int1);
			float float4 = roomDef.getClosestPoint(float1, float2, vector2f2);
			if (float4 < float3) {
				float3 = float4;
