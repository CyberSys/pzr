...
public class NonBlockingSetInt extends AbstractSet implements Serializable {
	...
	public boolean add(int int1) {
		if (int1 < 0) {
			throw new IllegalArgumentException(int1.makeConcatWithConstants < invokedynamic > (int1));
		} else {
		...
		private void print(int int1) {
			...
			while (nBSI != null) {
				for (int int2 = 0; int2 < nBSI._bits.length; ++int2) {
					long long1 = nBSI._bits[int2];
					System.out.print(Long.toHexString(long1) + " ");
				}
			...
			if (this._copyIdx.get() != 0 || this._copyDone.get() != 0) {
				int int3 = this._copyIdx.get();
				this.print(int1, "_copyIdx=" + int3 + " _copyDone=" + this._copyDone.get() + " _words_to_cpy=" + this._sum_bits_length);
			}
