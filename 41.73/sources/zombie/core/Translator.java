...
public final class Translator {
							...
							if (!string.trim().isEmpty()) {
								...
								if (string.startsWith("[VERSION]")) {
									...
									if (hashMap.containsKey(string2)) {
										...
										hashMap.put(string2, news = new Translator.News(string2));
										arrayList = null;
									}
								}
								...
								if (news != null) {
									...
									} else if (string.startsWith("[BUG FIX]")) {
										arrayList = news.getOrCreateSectionList("[Bug Fix]");
									} else if (arrayList != null) {
										addNewsLine(string, arrayList);
									}
								}
							}
						}
	...
	private static void addNewsLine(String string, ArrayList arrayList) {
		if (string.startsWith("[BOLD]")) {
			...
			byte byte1 = 7;
			String string2 = " <INDENT:%d> ".formatted(new Object[]{21 - byte1});
			arrayList.add("<IMAGE:media/ui/dot.png> <SIZE:small> " + string2 + string + " <INDENT:0> <LINE> ");
		}
	}
	...
	private static final class News {
		...
		ArrayList getOrCreateSectionList(String string) {
			if (this.sectionNames.contains(string)) {
				return (ArrayList)this.sectionLists.get(string);
			} else {
		...
		String toRichText() {
			...
			Iterator iterator = this.sectionNames.iterator();
			while (true) {
				String string;
				ArrayList arrayList;
				do {
					...
					arrayList = (ArrayList)this.sectionLists.get(string);
				}	 while (arrayList.isEmpty());

				stringBuilder.append("<LINE> <LEFT> <SIZE:medium> %s <LINE> <LINE> ".formatted(new Object[]{string}));
