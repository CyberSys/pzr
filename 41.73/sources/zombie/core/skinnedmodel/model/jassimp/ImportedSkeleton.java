...
public final class ImportedSkeleton {
	...
	private void processAiScene(ImportedSkeletonParams importedSkeletonParams) {
		...
		for (int int2 = 0; int2 < arrayList.size(); ++int2) {
			AiNode aiNode4 = (AiNode)arrayList.get(int2);
			...
			if (!this.boneIndices.containsKey(string)) {
				...
				if (aiNode4 == this.rootBoneNode) {
					...
					AiNode aiNode5;
					for (aiNode5 = aiNode4.getParent(); aiNode5 != null && !this.boneIndices.containsKey(aiNode5.getName()); aiNode5 = aiNode5.getParent()) {
					}

					if (aiNode5 != null) {
						this.SkeletonHierarchy.add((Integer)this.boneIndices.get(aiNode5.getName()));
					} else {
		...
		for (int4 = 0; int4 < arrayList.size(); ++int4) {
			AiNode aiNode6 = (AiNode)arrayList.get(int4);
			String string2 = aiNode6.getName();
			...
			if (aiBone != null) {
				...
				if (aiMatrix4f != null) {
					...
					String string3 = aiNode6.getParent().getName();
						...
						for (int int5 = 0; int5 < arrayList3.size(); ++int5) {
							...
							if (boolean2) {
								if (boolean3) {
									...
									boolean boolean4 = StringUtils.startsWithIgnoreCase(keyframe.BoneName, "Translation_Data");
									if (!boolean4) {
