		...
		if (this.removedFromWorld) {
			...
			if (this.physics != null) {
				...
				if (System.currentTimeMillis() - this.engineCheckTime > (long)short1 && !GameClient.bClient) {
					...
					if (!GameClient.bClient) {
						if (this.engineState != BaseVehicle.engineStateTypes.Idle) {
							int int2 = (int)((double)this.engineLoudness * this.engineSpeed / 2500.0);
							double double1 = Math.min(this.getEngineSpeed(), 2000.0);
							int2 = (int)((double)int2 * (1.0 + double1 / 4000.0));
							int1 = 120;
							if (GameServer.bServer) {
								int1 = (int)((double)int1 * ServerOptions.getInstance().CarEngineAttractionModifier.getValue());
								int2 = (int)((double)int2 * ServerOptions.getInstance().CarEngineAttractionModifier.getValue());
							}

							if (Rand.Next((int)((float)int1 * GameTime.instance.getInvMultiplier())) == 0) {
							...
							if (Rand.Next((int)((float)(int1 - 85) * GameTime.instance.getInvMultiplier())) == 0) {
							...
							if (Rand.Next((int)((float)(int1 - 110) * GameTime.instance.getInvMultiplier())) == 0) {
					...
					if (this.engineState == BaseVehicle.engineStateTypes.Running) {
						VehiclePart vehiclePart = this.getPartById("Engine");
				...
				if (square == null && !this.chunk.refs.isEmpty()) {
					float float2 = 5.0E-4F;
					int int3 = this.chunk.wx * 10;
					int1 = this.chunk.wy * 10;
					int int4 = int3 + 10;
					int int5 = int1 + 10;
					float float3 = this.x;
					float float4 = this.y;
					this.x = Math.max(this.x, (float)int3 + float2);
					this.x = Math.min(this.x, (float)int4 - float2);
					this.y = Math.max(this.y, (float)int1 + float2);
					this.y = Math.min(this.y, (float)int5 - float2);
					this.z = 0.2F;
				...
				if (this.jniIsCollide && this.limitCrash.Check()) {
					...
					vector3f3.y = 0.0F;
					float float5 = vector3f3.length();
					DebugLog.Vehicle.debugln("Vehicle vid=%d velocity last=%s/%f current=%s/%f delta=%f", this.VehicleID, this.lastLinearVelocity, this.lastLinearVelocity.length(), vector3f2, vector3f2.length(), float5);
					if (vector3f2.lengthSquared() > this.lastLinearVelocity.lengthSquared() && float5 > 6.0F) {
						DebugLog.Vehicle.trace("Vehicle vid=%d got sharp speed increase delta=%f", this.VehicleID, float5);
						float5 = 6.0F;
					}
					...
					if (float5 > 1.0F) {
						...
						DebugLog.Vehicle.trace("Vehicle vid=%d crash delta=%f", this.VehicleID, float5);
						Vector3f vector3f4 = this.getForwardVector(allocVector3f());
						float float6 = vector3f3.dot(vector3f4);
						releaseVector3f(vector3f4);
						this.crash(float5 * 3.0F, float6 < 0.0F);
						this.damageObjects(float5 * 30.0F);
					}
			...
			for (int6 = 0; int6 < this.getScript().getPassengerCount(); ++int6) {
				if (this.getCharacter(int6) != null) {
					Vector3f vector3f5 = this.getPassengerWorldPos(int6, (Vector3f)((BaseVehicle.Vector3fObjectPool)TL_vector3f_pool.get()).alloc());
					this.getCharacter(int6).setX(vector3f5.x);
					this.getCharacter(int6).setY(vector3f5.y);
					this.getCharacter(int6).setZ(vector3f5.z * 1.0F);
					((BaseVehicle.Vector3fObjectPool)TL_vector3f_pool.get()).release(vector3f5);
				}
			}

			VehiclePart vehiclePart2 = this.getPartById("lightbar");
			if (vehiclePart2 != null && this.lightbarLightsMode.isEnable() && vehiclePart2.getCondition() == 0 && !GameClient.bClient) {
				this.setLightbarLightsMode(0);
			}

			if (vehiclePart2 != null && this.lightbarSirenMode.isEnable() && vehiclePart2.getCondition() == 0 && !GameClient.bClient) {
			...
			VehiclePart vehiclePart3 = this.getPartById("GasTank");
			if (vehiclePart3 != null && vehiclePart3.getContainerContentAmount() > (float)vehiclePart3.getContainerCapacity()) {
				vehiclePart3.setContainerContentAmount((float)vehiclePart3.getContainerCapacity());
			}
