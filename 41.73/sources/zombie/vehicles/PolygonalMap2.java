...
public final class PolygonalMap2 {
	...
	public Vector2f resolveCollision(IsoGameCharacter gameCharacter, float float1, float float2, Vector2f vector2f) {
		return GameClient.bClient && gameCharacter.isSkipResolveCollision() ? vector2f.set(float1, float2) : this.collideWithObstacles.resolveCollision(gameCharacter, float1, float2, vector2f);
	}
	...
	private static final class LineClearCollideMain {
		...
		Vector2 getCollidepoint(PolygonalMap2 polygonalMap2, float float1, float float2, float float3, float float4, int int1, BaseVehicle baseVehicle, int int2) {
			...
			polygonalMap2.supercover(float7, float8, float9, float10, int1, this.pointPool, this.pts);
			polygonalMap2.supercover(float11, float12, float13, float14, int1, this.pointPool, this.pts);
			this.pts.sort((float1x,float2x)->{
				return (int)(IsoUtils.DistanceManhatten(float1, float2, (float)float1x.x, (float)float1x.y) - IsoUtils.DistanceManhatten(float1, float2, (float)float2x.x, (float)float2x.y));
			});
			PolygonalMap2.Point point;
			if (boolean4) {
				for (int3 = 0; int3 < this.pts.size(); ++int3) {
					point = (PolygonalMap2.Point)this.pts.get(int3);
					LineDrawer.addLine((float)point.x, (float)point.y, (float)int1, (float)point.x + 1.0F, (float)point.y + 1.0F, (float)int1, 1.0F, 1.0F, 0.0F, (String)null, false);
				}
			}
			...
			for (int3 = 0; int3 < this.pts.size(); ++int3) {
				point = (PolygonalMap2.Point)this.pts.get(int3);
				IsoGridSquare square = IsoWorld.instance.CurrentCell.getGridSquare(point.x, point.y, int1);
				if (boolean3 && square != null && PolygonalMap2.SquareUpdateTask.getCost(square) > 0) {
					return PolygonalMap2.temp.set((float)point.x + 0.5F, (float)point.y + 0.5F);
				}
				...
				if (square != null && !square.isSolid() && (!square.isSolidTrans() || square.isAdjacentToWindow()) && !square.HasStairs()) {
					label249: {
						...
						if (square.Is(IsoFlagType.collideW) || !boolean1 && square.hasBlockedDoor(false)) {
							...
							if (this.LB.lineRectIntersect(float1, float2, float3 - float1, float4 - float2, (float)point.x - float15, (float)point.y - float16, (float)point.x + float17, (float)point.y + 1.0F + float18)) {
								if (boolean4) {
									LineDrawer.addLine((float)point.x - float15, (float)point.y - float16, (float)int1, (float)point.x + float17, (float)point.y + 1.0F + float18, (float)int1, 1.0F, 0.0F, 0.0F, (String)null, false);
								}

								return PolygonalMap2.temp.set((float)point.x + (float1 - float3 < 0.0F ? -0.5F : 0.5F), (float)point.y + 0.5F);
							}
						}
						...
						if (this.LB.lineRectIntersect(float1, float2, float3 - float1, float4 - float2, (float)point.x - float15, (float)point.y - float16, (float)point.x + 1.0F + float17, (float)point.y + float18)) {
							if (boolean4) {
								LineDrawer.addLine((float)point.x - float15, (float)point.y - float16, (float)int1, (float)point.x + 1.0F + float17, (float)point.y + float18, (float)int1, 1.0F, 0.0F, 0.0F, (String)null, false);
							}

							return PolygonalMap2.temp.set((float)point.x + 0.5F, (float)point.y + (float2 - float4 < 0.0F ? -0.5F : 0.5F));
						}
				...
				if (this.LB.lineRectIntersect(float1, float2, float3 - float1, float4 - float2, (float)point.x - float15, (float)point.y - float16, (float)point.x + 1.0F + float17, (float)point.y + 1.0F + float18)) {
					if (boolean4) {
						LineDrawer.addLine((float)point.x - float15, (float)point.y - float16, (float)int1, (float)point.x + 1.0F + float17, (float)point.y + 1.0F + float18, (float)int1, 1.0F, 0.0F, 0.0F, (String)null, false);
					}
			...
			return PolygonalMap2.temp.set(float3, float4);
		}
	...
	public static final class LiangBarsky {
		...
		public boolean lineRectIntersect(float float1, float float2, float float3, float float4, float float5, float float6, float float7, float float8) {
			return this.lineRectIntersect(float1, float2, float3, float4, float5, float6, float7, float8, (double[])null);
		}

		public boolean lineRectIntersect(float float1, float float2, float float3, float float4, float float5, float float6, float float7, float float8, double[] doubleArray) {
