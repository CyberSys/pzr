...
public final class CollideWithObstacles {
			...
			for (int9 = int6; int9 < int8 - 1; ++int9) {
				for (int10 = int4; int10 < int7 - 1; ++int10) {
					...
					if (square != null) {
						...
						if (boolean1 && square.Is(IsoFlagType.doorW)) {
							boolean1 = false;
						}

						boolean boolean2 = square.Is(IsoFlagType.collideN);
						if (square.Is(IsoFlagType.windowN) || square.Is(IsoFlagType.WindowN)) {
							boolean2 = true;
						}

						if (boolean2 && square.Is(IsoFlagType.doorN)) {
							boolean2 = false;
						}

						if (boolean1 || square.hasBlockedDoor(false) || square.Has(IsoObjectType.stairsBN)) {
							CollideWithObstacles.CCObjectOutline.setWest(int10 - int4, int9 - int6, int3, this.oo);
						}

						if (boolean2 || square.hasBlockedDoor(true) || square.Has(IsoObjectType.stairsBW)) {
						...
						} else if (square.Has(IsoObjectType.stairsBW) && int9 != int8 - 2) {
	...
	Vector2f resolveCollision(IsoGameCharacter gameCharacter, float float1, float float2, Vector2f vector2f) {
		...
		if (gameCharacter.getCurrentSquare() != null && gameCharacter.getCurrentSquare().HasStairs()) {
			...
			if (float3 == float1 && float4 == float2) {
				...
				for (int int3 = 0; int3 < this.obstacles.size(); ++int3) {
					CollideWithObstacles.CCObstacle cCObstacle2 = (CollideWithObstacles.CCObstacle)this.obstacles.get(int3);
					byte byte1 = 0;
					if (cCObstacle2.isPointInside(gameCharacter.x, gameCharacter.y, byte1)) {
						for (int int4 = 0; int4 < cCObstacle2.edges.size(); ++int4) {
							CollideWithObstacles.CCEdge cCEdge2 = (CollideWithObstacles.CCEdge)cCObstacle2.edges.get(int4);
							if (cCEdge2.node1.visible.contains(cCEdge2.node2)) {
								CollideWithObstacles.CCNode cCNode3 = cCEdge2.closestPoint(gameCharacter.x, gameCharacter.y, this.closest);
				...
				float float7;
				if (cCEdge != null) {
					float7 = cCEdge.normal.dot(this.move);
					if (float7 >= 0.01F) {
						cCEdge = null;
					}
				}
				...
				if (cCEdge == null) {
					...
					cCEdge = null;
					cCNode = null;
					for (int5 = 0; int5 < this.obstacles.size(); ++int5) {
						CollideWithObstacles.CCObstacle cCObstacle3 = (CollideWithObstacles.CCObstacle)this.obstacles.get(int5);
						for (int int6 = 0; int6 < cCObstacle3.edges.size(); ++int6) {
							CollideWithObstacles.CCEdge cCEdge3 = (CollideWithObstacles.CCEdge)cCObstacle3.edges.get(int6);
							if (cCEdge3.node1.visible.contains(cCEdge3.node2)) {
								float float8 = cCEdge3.node1.x;
								float float9 = cCEdge3.node1.y;
								float float10 = cCEdge3.node2.x;
								float float11 = cCEdge3.node2.y;
								...
								if (boolean1 && DebugOptions.instance.CollideWithObstaclesRenderNormals.getValue()) {
									LineDrawer.addLine(float12, float13, (float)cCEdge3.node1.z, float12 + cCEdge3.normal.x, float13 + cCEdge3.normal.y, (float)cCEdge3.node1.z, 0.0F, 0.0F, 1.0F, (String)null, true);
								}
								...
								if (double4 != 0.0) {
									...
									float float14 = cCEdge3.normal.dot(this.move);
									if (!(float14 >= 0.0F) && double5 >= 0.0 && double5 <= 1.0 && double6 >= 0.0 && double6 <= 1.0) {
										if (double6 < 0.01 || double6 > 0.99) {
											CollideWithObstacles.CCNode cCNode4 = double6 < 0.01 ? cCEdge3.node1 : cCEdge3.node2;
				...
				if (cCNode != null) {
					CollideWithObstacles.CCEdge cCEdge4 = cCEdge;
					CollideWithObstacles.CCEdge cCEdge5 = null;
					for (int5 = 0; int5 < cCNode.edges.size(); ++int5) {
						CollideWithObstacles.CCEdge cCEdge6 = (CollideWithObstacles.CCEdge)cCNode.edges.get(int5);
					...
					if (cCEdge4 != null && cCEdge5 != null) {
						...
						if (cCEdge == cCEdge4) {
							...
							cCNode5 = cCNode == cCEdge4.node1 ? cCEdge4.node2 : cCEdge4.node1;
							this.edgeVec.set(cCNode5.x - cCNode.x, cCNode5.y - cCNode.y);
							this.edgeVec.normalize();
							if (this.move.dot(this.edgeVec) >= 0.0F) {
								cCEdge = cCEdge4;
				...
				if (cCEdge != null) {
					...
					float float17 = cCEdge.node1.y;
					float float18 = cCEdge.node2.x;
					float float19 = cCEdge.node2.y;
					if (boolean1) {
						LineDrawer.addLine(float7, float17, (float)cCEdge.node1.z, float18, float19, (float)cCEdge.node1.z, 0.0F, 1.0F, 1.0F, (String)null, true);
					}

					cCEdge.closestPoint(float1, float2, this.closest);
	...
	private static final class CCObstacle {
		...
		CollideWithObstacles.EdgeRingHit isPointInPolygon_WindingNumber(float float1, float float2, int int1) {
			int int2 = 0;
			for (int int3 = 0; int3 < this.edges.size(); ++int3) {
				CollideWithObstacles.CCEdge cCEdge = (CollideWithObstacles.CCEdge)this.edges.get(int3);
			...
			return int2 == 0 ? CollideWithObstacles.EdgeRingHit.Outside : CollideWithObstacles.EdgeRingHit.Inside;
		}

		boolean isPointInside(float float1, float float2, int int1) {
			return this.isPointInPolygon_WindingNumber(float1, float2, int1) == CollideWithObstacles.EdgeRingHit.Inside;
		}
		...
		boolean isNodeInsideOf(CollideWithObstacles.CCNode cCNode) {
			if (this.hasNode(cCNode)) {
				...
				byte byte1 = 0;
				return this.isPointInside(cCNode.x, cCNode.y, byte1);
			}
		}
