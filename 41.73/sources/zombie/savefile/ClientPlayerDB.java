...
public final class ClientPlayerDB {
		...
		for (int int1 = 1; int1 < this.networkProfile.playerCount; ++int1) {
			...
			if (byteArray != null) {
				...
				try {
					...
					player.load(byteBuffer, this.networkProfile.worldVersion[int1]);
					if (this.networkProfile.isDead[int1]) {
	...
	public void clientLoadNetworkCharacter(ByteBuffer byteBuffer, UdpConnection udpConnection) {
		...
		if (boolean1) {
			...
			if (this.networkProfile != null) {
				...
				case 2: 
					this.networkProfile.worldVersion[1] = int2;
					this.networkProfile.character[1] = byteArray;
				...
				case 3: 
					this.networkProfile.worldVersion[2] = int2;
					this.networkProfile.character[2] = byteArray;
				...
				case 4: 
					this.networkProfile.worldVersion[3] = int2;
					this.networkProfile.character[3] = byteArray;
				...
				this.networkProfile.character[0] = byteArray;
				this.networkProfile.worldVersion[0] = int2;
	...
	public void clientSendNetworkPlayerInt(IsoPlayer player) {
		if (GameClient.connection != null) {
			try {
				...
				String string2 = string + " " + player.getDescriptor().getSurname();
	...
	public byte[] getClientLoadNetworkPlayerData(int int1) {
		if (this.networkProfile != null && this.networkProfile.isLoaded && this.networkProfile.username.equals(GameClient.username) && this.networkProfile.server.equals(GameClient.ip)) {
			...
			case 4: 
				return this.networkProfile.character[int1 - 1];
	...
	public boolean loadNetworkPlayer() {
		try {
			byte[] byteArray = this.getClientLoadNetworkPlayerData(1);
			if (byteArray != null) {
				ByteBuffer byteBuffer = ByteBuffer.allocate(byteArray.length);
				byteBuffer.rewind();
				byteBuffer.put(byteArray);
	...
	public void forgetPlayer(int int1) {
		if (this.networkProfile != null && int1 >= 1 && int1 <= 4) {
			this.networkProfile.character[int1 - 1] = null;
			this.networkProfile.isDead[int1 - 1] = true;
		}
	}
	...
	public int getNextServerPlayerIndex() {
		if (this.networkProfile != null && this.networkProfile.isLoaded && this.networkProfile.username.equals(GameClient.username) && this.networkProfile.server.equals(GameClient.ip)) {
			for (int int1 = 1; int1 < 4; ++int1) {
				if (this.networkProfile.character[int1] == null || this.networkProfile.isDead[int1]) {
					return int1 + 1;
				}
			}
		}
