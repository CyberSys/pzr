...
public final class ClimbThroughWindowState extends State {
	...
	public void animEvent(IsoGameCharacter gameCharacter, AnimEvent animEvent) {
		...
		if (animEvent.m_EventName.equalsIgnoreCase("PlayWindowSound")) {
			if (!SoundManager.instance.isListenerInRange(gameCharacter.getX(), gameCharacter.getY(), 10.0F)) {
				return;
			}

			long long1 = gameCharacter.getEmitter().playSoundImpl(animEvent.m_ParameterValue, (IsoObject)null);
			gameCharacter.getEmitter().setParameterValue(long1, FMODManager.instance.getParameterDescription("TripObstacleType"), 9.0F);
		}
