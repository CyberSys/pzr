...
public final class ZombieEatBodyState extends State {
	...
	public void enter(IsoGameCharacter gameCharacter) {
		...
		if (zombie.getEatBodyTarget() instanceof IsoDeadBody) {
			...
			if (GameClient.bClient && zombie.isLocal()) {
				GameClient.sendEatBody(zombie, zombie.getEatBodyTarget());
			}
		} else if (zombie.getEatBodyTarget() instanceof IsoPlayer && GameClient.bClient && zombie.isLocal()) {
			GameClient.sendEatBody(zombie, zombie.getEatBodyTarget());
		}
	}
	...
	public void exit(IsoGameCharacter gameCharacter) {
		...
		if (GameClient.bClient && zombie.isLocal()) {
			GameClient.sendEatBody(zombie, (IsoMovingObject)null);
		}
	}
}
