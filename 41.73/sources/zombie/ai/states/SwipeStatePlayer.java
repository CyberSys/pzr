...
public final class SwipeStatePlayer extends State {
	...
	private void doAttack(IsoPlayer player, float float1, boolean boolean1, String string, AttackVars attackVars) {
	...
	public void enter(IsoGameCharacter gameCharacter) {
		...
		this.CalcAttackVars((IsoLivingCharacter)gameCharacter, player.attackVars);
		this.doAttack(player, 2.0F, gameCharacter.isForceShove(), gameCharacter.getClickSound(), player.attackVars);
		HandWeapon handWeapon = gameCharacter.getUseHandWeapon();
		if (!GameClient.bClient || player.isLocalPlayer()) {
			gameCharacter.setVariable("AimFloorAnim", player.attackVars.bAimAtFloor);
		}
		...
		handWeapon = player.attackVars.getWeapon(player);
		...
		if (!player.bDoShove && !boolean1 && !handWeapon.isRanged() && player.isLocalPlayer()) {
			gameCharacter.clearVariable("PlayedSwingSound");
	...
	public void animEvent(IsoGameCharacter gameCharacter, AnimEvent animEvent) {
		...
		if (animEvent.m_EventName.equalsIgnoreCase("ActiveAnimFinishing") || animEvent.m_EventName.equalsIgnoreCase("NonLoopedAnimFadeOut")) {
			...
			if (boolean1 && !gameCharacter.isRangedWeaponEmpty()) {
				...
				} else if (Rand.NextBool(2) && !handWeapon.isRanged() && WeaponType.getWeaponType(gameCharacter) != WeaponType.barehand) {
		...
		if (animEvent.m_EventName.equalsIgnoreCase("AttackAnim")) {
			gameCharacter.setAttackAnim(Boolean.parseBoolean(animEvent.m_ParameterValue));
		}
		...
		if (animEvent.m_EventName.equalsIgnoreCase("AttackCollisionCheck") && hashMap.get(PARAM_ATTACKED) == Boolean.FALSE && gameCharacter instanceof IsoPlayer && ((IsoPlayer)gameCharacter).isLocalPlayer()) {
		...
		if (animEvent.m_EventName.equalsIgnoreCase("PlaySwingSound")) {
			...
			if (gameCharacter.getVariableBoolean("PlayedSwingSound")) {
				return;
			}

			gameCharacter.setVariable("PlayedSwingSound", true);
			if (!StringUtils.isNullOrWhitespace(animEvent.m_ParameterValue)) {
				String string = handWeapon2.getSoundByID(animEvent.m_ParameterValue);
				if (string != null) {
			...
			gameCharacter.playSound(handWeapon2.getSwingSound());
		}
	...
	public void exit(IsoGameCharacter gameCharacter) {
		...
		gameCharacter.setAttackAnim(false);
		...
		((IsoLivingCharacter)gameCharacter).setDoShove(false);
		...
		if (handWeapon != null && (handWeapon.getCondition() <= 0 || boolean1 && handWeapon.isUseSelf())) {
			...
			if (DebugOptions.instance.MultiplayerAutoEquip.getValue() && handWeapon.getPhysicsObject() != null) {
				gameCharacter.setPrimaryHandItem(gameCharacter.getInventory().getItemFromType(handWeapon.getType()));
			}
		...
		gameCharacter.hitList.clear();
		gameCharacter.attackVars.clear();
	}

	public void CalcAttackVars(IsoLivingCharacter livingCharacter, AttackVars attackVars) {
		...
		if (!GameClient.bClient || livingCharacter.isLocal()) {
			boolean boolean1 = livingCharacter.isAttackAnim() || livingCharacter.getVariableBoolean("ShoveAnim") || livingCharacter.getVariableBoolean("StompAnim");
			attackVars.setWeapon(handWeapon == null ? livingCharacter.bareHands : handWeapon);
			attackVars.targetOnGround.setMovingObject((IsoMovingObject)null);
			...
			if (attackVars.getWeapon(livingCharacter) == livingCharacter.bareHands || attackVars.bDoShove || livingCharacter.isForceShove()) {
				attackVars.bDoShove = true;
				attackVars.bAimAtFloor = false;
				attackVars.setWeapon(livingCharacter.bareHands);
			}

			this.calcValidTargets(livingCharacter, attackVars.getWeapon(livingCharacter), true, attackVars.targetsProne, attackVars.targetsStanding);
			HitInfo hitInfo = attackVars.targetsStanding.isEmpty() ? null : (HitInfo)attackVars.targetsStanding.get(0);
			HitInfo hitInfo2 = attackVars.targetsProne.isEmpty() ? null : (HitInfo)attackVars.targetsProne.get(0);
			...
			if (!(float1 >= attackVars.getWeapon(livingCharacter).getMinRange() * attackVars.getWeapon(livingCharacter).getMinRange()) && (hitInfo == null || !this.isWindowBetween(livingCharacter, hitInfo.getObject()))) {
			...
			boolean boolean2 = livingCharacter.getVariableBoolean("StartedAttackWhileSprinting");
			if (Keyboard.isKeyDown(int1) && (int1 != int2 || !boolean2)) {
				attackVars.bAimAtFloor = true;
				attackVars.bDoShove = false;
				livingCharacter.setDoShove(false);
			}

			if (attackVars.getWeapon(livingCharacter).isRanged()) {
				int int3 = attackVars.getWeapon(livingCharacter).getRecoilDelay();
	...
	public void calcValidTargets(IsoLivingCharacter livingCharacter, HandWeapon handWeapon, boolean boolean1, ArrayList arrayList, ArrayList arrayList2) {
		...
		for (int int1 = 0; int1 < arrayList3.size(); ++int1) {
			...
			HitInfo hitInfo = this.calcValidTarget(livingCharacter, handWeapon, movingObject, float3);
			if (hitInfo != null) {
	...
	private boolean shouldIgnoreProneZombies(IsoGameCharacter gameCharacter, ArrayList arrayList, float float1) {
		if (float1 <= 0.0F) {
			...
			for (int int1 = 0; int1 < arrayList.size(); ++int1) {
				...
				IsoZombie zombie = (IsoZombie)Type.tryCastTo(hitInfo.getObject(), IsoZombie.class);
				if ((zombie == null || zombie.target != null || boolean1) && !(hitInfo.distSq > float1 * float1)) {
					int int2 = (int)gameCharacter.z;
					boolean boolean2 = PolygonalMap2.instance.lineClearCollide(gameCharacter.x, gameCharacter.y, hitInfo.getObject().x, hitInfo.getObject().y, int2, gameCharacter, false, true);
					if (!boolean2) {
						return true;
					}
				}
	...
	private boolean isUnhittableTarget(IsoGameCharacter gameCharacter, HandWeapon handWeapon, float float1, float float2, HitInfo hitInfo, boolean boolean1) {
		if (!(hitInfo.dot < float1) && !(hitInfo.dot > float2)) {
			Vector3 vector3 = tempVectorBonePos.set(hitInfo.x, hitInfo.y, hitInfo.z);
			return !gameCharacter.IsAttackRange(handWeapon, hitInfo.getObject(), vector3, boolean1);
		} else {
			return true;
		}
	}
	...
	private HitInfo calcValidTarget(IsoLivingCharacter livingCharacter, HandWeapon handWeapon, IsoMovingObject movingObject, float float1) {
			...
			if (gameCharacter == null) {
				...
				if (!handWeapon.isRanged() && float2 >= 0.5F) {
					...
					if (float3 < 0.0F) {
						...
						return testResults != LosUtil.TestResults.Blocked && testResults != LosUtil.TestResults.ClearThroughClosedDoor ? ((HitInfo)this.hitInfoPool.alloc()).init(gameCharacter, float3, float4, vector4f.x, vector4f.y, vector4f.z) : null;
					}
				}
			}
		}
		...
		if (hitInfo != null && hitInfo.getObject() != null) {
			if (hitInfo2 != null && hitInfo2.getObject() != null) {
				if (hitInfo.distSq <= hitInfo2.distSq) {
					...
					int int1 = (int)gameCharacter.z;
					boolean boolean1 = PolygonalMap2.instance.lineClearCollide(gameCharacter.x, gameCharacter.y, hitInfo.getObject().x, hitInfo.getObject().y, int1, (IsoMovingObject)null, false, true);
					if (!boolean1) {
						return false;
					} else {
						int1 = (int)gameCharacter.z;
						boolean boolean2 = PolygonalMap2.instance.lineClearCollide(gameCharacter.x, gameCharacter.y, hitInfo2.getObject().x, hitInfo2.getObject().y, int1, (IsoMovingObject)null, false, true);
						return !boolean2;
					}
				}
			} else {
	...
	public static boolean checkPVP(IsoGameCharacter gameCharacter, IsoMovingObject movingObject) {
		...
		if (GameClient.bClient && player2 != null && gameCharacter != null) {
		...
		return GameClient.bClient || player2 == null || IsoPlayer.getCoopPVP();
	}

	private void CalcHitListShove(IsoGameCharacter gameCharacter, boolean boolean1, AttackVars attackVars, ArrayList arrayList) {
		HandWeapon handWeapon = attackVars.getWeapon((IsoLivingCharacter)gameCharacter);
		...
		for (int int1 = 0; int1 < arrayList2.size(); ++int1) {
			...
			if (movingObject != gameCharacter && !(movingObject instanceof BaseVehicle)) {
				...
				if (gameCharacter2 != null && !gameCharacter2.isGodMod() && !gameCharacter2.isDead()) {
					IsoZombie zombie = (IsoZombie)Type.tryCastTo(movingObject, IsoZombie.class);
					if ((zombie == null || !zombie.isCurrentState(FakeDeadZombieState.instance())) && checkPVP(gameCharacter, movingObject)) {
						boolean boolean2 = movingObject == attackVars.targetOnGround.getMovingObject() || movingObject.isShootable() && isStanding(movingObject) && !attackVars.bAimAtFloor || movingObject.isShootable() && isProne(movingObject) && attackVars.bAimAtFloor;
						if (boolean2) {
							Vector4f vector4f = this.tempVector4f;
							if (this.getNearestTargetPosAndDot(gameCharacter, handWeapon, movingObject, boolean1, vector4f)) {
								...
								if (testResults != LosUtil.TestResults.Blocked && testResults != LosUtil.TestResults.ClearThroughClosedDoor && (movingObject.getCurrentSquare() == null || gameCharacter.getCurrentSquare() == null || movingObject.getCurrentSquare() == gameCharacter.getCurrentSquare() || !movingObject.getCurrentSquare().isWindowBlockedTo(gameCharacter.getCurrentSquare())) && movingObject.getSquare().getTransparentWallTo(gameCharacter.getSquare()) == null) {
									HitInfo hitInfo = ((HitInfo)this.hitInfoPool.alloc()).init(movingObject, float1, float2, vector4f.x, vector4f.y, vector4f.z);
	...
	private void CalcHitListWeapon(IsoGameCharacter gameCharacter, boolean boolean1, AttackVars attackVars, ArrayList arrayList) {
		HandWeapon handWeapon = attackVars.getWeapon((IsoLivingCharacter)gameCharacter);
		ArrayList arrayList2 = IsoWorld.instance.CurrentCell.getObjectList();
		for (int int1 = 0; int1 < arrayList2.size(); ++int1) {
			IsoMovingObject movingObject = (IsoMovingObject)arrayList2.get(int1);
		...
		if (arrayList.isEmpty()) {
			this.CalcHitListWindow(gameCharacter, handWeapon, arrayList);
		}
	}

	private void CalcHitListWindow(IsoGameCharacter gameCharacter, HandWeapon handWeapon, ArrayList arrayList) {
		...
		ArrayList arrayList2 = IsoWorld.instance.CurrentCell.getWindowList();
		for (int int1 = 0; int1 < arrayList2.size(); ++int1) {
			IsoWindow window = (IsoWindow)arrayList2.get(int1);
				...
				if (Line2D.linesIntersect((double)gameCharacter.x, (double)gameCharacter.y, (double)(gameCharacter.x + vector2.x), (double)(gameCharacter.y + vector2.y), (double)float1, (double)float2, (double)float3, (double)float4)) {
					...
					if (square != null && !LosUtil.lineClearCollide((int)gameCharacter.x, (int)gameCharacter.y, (int)gameCharacter.z, square.x, square.y, square.z, false)) {
						...
						if (hitInfo == null || !(hitInfo.distSq < float5)) {
							float float6 = 1.0F;
							if (hitInfo == null) {
								hitInfo = (HitInfo)this.hitInfoPool.alloc();
							}
	...
	public void CalcHitList(IsoGameCharacter gameCharacter, boolean boolean1, AttackVars attackVars, ArrayList arrayList) {
		if (!GameClient.bClient || gameCharacter.isLocal()) {
			...
			if (handWeapon == ((IsoPlayer)gameCharacter).bareHands && attackVars.targetOnGround.getMovingObject() != null) {
				int1 = 1;
			}

			if (0 < int1) {
				if (attackVars.bDoShove) {
					this.CalcHitListShove(gameCharacter, boolean1, attackVars, arrayList);
				} else {
					this.CalcHitListWeapon(gameCharacter, boolean1, attackVars, arrayList);
				}

				if (arrayList.size() == 1 && ((HitInfo)arrayList.get(0)).getObject() == null) {
					return;
				}
				...
				if (handWeapon.isPiercingBullets()) {
					...
					for (int int2 = 0; int2 < arrayList.size(); ++int2) {
						HitInfo hitInfo = (HitInfo)arrayList.get(int2);
					...
					while (arrayList.size() > int1) {
						this.hitInfoPool.release((Object)((HitInfo)arrayList.remove(arrayList.size() - 1)));
					}
				}
			}

			for (int int3 = 0; int3 < arrayList.size(); ++int3) {
				HitInfo hitInfo2 = (HitInfo)arrayList.get(int3);
	...
	private void filterTargetsByZ(IsoGameCharacter gameCharacter) {
		...
		HitInfo hitInfo2;
		float float2;
		for (int1 = 0; int1 < gameCharacter.hitList.size(); ++int1) {
			hitInfo2 = (HitInfo)gameCharacter.hitList.get(int1);
		...
		if (hitInfo != null) {
			for (int1 = gameCharacter.hitList.size() - 1; int1 >= 0; --int1) {
				hitInfo2 = (HitInfo)gameCharacter.hitList.get(int1);
				if (hitInfo2 != hitInfo) {
					float2 = Math.abs(hitInfo2.z - hitInfo.z);
					if (float2 > 0.5F) {
						this.hitInfoPool.release((Object)hitInfo2);
						gameCharacter.hitList.remove(int1);
					}
				}
			}
		}
	...
	public int CalcHitChance(IsoGameCharacter gameCharacter, HandWeapon handWeapon, HitInfo hitInfo) {
		IsoMovingObject movingObject = hitInfo.getObject();
		if (movingObject == null) {
			return 0;
		} else {
	...
	public void ConnectSwing(IsoGameCharacter gameCharacter, HandWeapon handWeapon) {
		...
		gameCharacter.attackVars.setWeapon(handWeapon);
		gameCharacter.attackVars.targetOnGround.setMovingObject(livingCharacter.targetOnGround);
		gameCharacter.attackVars.bAimAtFloor = gameCharacter.isAimAtFloor();
		gameCharacter.attackVars.bDoShove = livingCharacter.bDoShove;
		if (gameCharacter.getVariableBoolean("ShoveAnim")) {
			gameCharacter.attackVars.bDoShove = true;
		}

		this.CalcHitList(gameCharacter, false, gameCharacter.attackVars, gameCharacter.hitList);
		int int1 = gameCharacter.hitList.size();
		...
		if (handWeapon.isUseEndurance()) {
			...
			if (int1 <= 0 && !gameCharacter.isForceShove()) {
				float float2 = (handWeapon.getWeight() * 0.18F * handWeapon.getFatigueMod(gameCharacter) * gameCharacter.getFatigueMod() * handWeapon.getEnduranceMod() * 0.3F + float1) * 0.04F;
				float float3 = 1.0F;
		...
		gameCharacter.setLastHitCount(gameCharacter.hitList.size());
		...
		int int2 = 1;
		this.dotList.clear();
		if (gameCharacter.hitList.isEmpty() && gameCharacter.getClickSound() != null && !livingCharacter.bDoShove) {
		...
		for (int int3 = 0; int3 < gameCharacter.hitList.size(); ++int3) {
			byte byte1 = 0;
			boolean boolean6 = false;
			HitInfo hitInfo = (HitInfo)gameCharacter.hitList.get(int3);
			IsoMovingObject movingObject = hitInfo.getObject();
			...
			if (hitInfo.getObject() == null && hitInfo.window.getObject() != null) {
				hitInfo.window.getObject().WeaponHit(gameCharacter, handWeapon);
			} else {
				this.smashWindowBetween(gameCharacter, movingObject, handWeapon);
				if (!this.isWindowBetween(gameCharacter, movingObject)) {
					int int4 = hitInfo.chance;
					boolean boolean7 = Rand.Next(100) <= int4;
					if (boolean7) {
						...
						float float5;
						for (int int5 = 0; int5 < this.dotList.size(); ++int5) {
							float5 = (Float)this.dotList.get(int5);
						...
						if (gameCharacter.isAimAtFloor() && !handWeapon.isRanged() && gameCharacter.isNPC()) {
							...
							if (player == null || player.isLocalPlayer()) {
								if (!StringUtils.isNullOrEmpty(handWeapon.getHitFloorSound())) {
									...
									long1 = gameCharacter.playSound(handWeapon.getZombieHitSound());
								}
							}

							int int6 = this.DoSwingCollisionBoneCheck(gameCharacter, GetWeapon(gameCharacter), (IsoGameCharacter)movingObject, ((IsoGameCharacter)movingObject).getAnimationPlayer().getSkinningBoneIndex("Bip01_Head", -1), 0.28F);
							if (int6 == -1) {
								int6 = this.DoSwingCollisionBoneCheck(gameCharacter, GetWeapon(gameCharacter), (IsoGameCharacter)movingObject, ((IsoGameCharacter)movingObject).getAnimationPlayer().getSkinningBoneIndex("Bip01_Spine", -1), 0.28F);
								if (int6 == -1) {
									int6 = this.DoSwingCollisionBoneCheck(gameCharacter, GetWeapon(gameCharacter), (IsoGameCharacter)movingObject, ((IsoGameCharacter)movingObject).getAnimationPlayer().getSkinningBoneIndex("Bip01_L_Calf", -1), 0.13F);
									if (int6 == -1) {
										int6 = this.DoSwingCollisionBoneCheck(gameCharacter, GetWeapon(gameCharacter), (IsoGameCharacter)movingObject, ((IsoGameCharacter)movingObject).getAnimationPlayer().getSkinningBoneIndex("Bip01_R_Calf", -1), 0.13F);
									}

									if (int6 == -1) {
										int6 = this.DoSwingCollisionBoneCheck(gameCharacter, GetWeapon(gameCharacter), (IsoGameCharacter)movingObject, ((IsoGameCharacter)movingObject).getAnimationPlayer().getSkinningBoneIndex("Bip01_L_Foot", -1), 0.23F);
									}

									if (int6 == -1) {
										int6 = this.DoSwingCollisionBoneCheck(gameCharacter, GetWeapon(gameCharacter), (IsoGameCharacter)movingObject, ((IsoGameCharacter)movingObject).getAnimationPlayer().getSkinningBoneIndex("Bip01_R_Foot", -1), 0.23F);
									}
								...
								splash(movingObject, handWeapon, gameCharacter);
								splash(movingObject, handWeapon, gameCharacter);
								byte1 = (byte)(Rand.Next(0, 3) + 1);
							}
						}

						if (!gameCharacter.attackVars.bAimAtFloor && (!gameCharacter.attackVars.bCloseKill || !gameCharacter.isCriticalHit()) && !livingCharacter.bDoShove && movingObject instanceof IsoGameCharacter && (player == null || player.isLocalPlayer())) {
							if (player != null) {
								player.setMeleeHitSurface(ParameterMeleeHitSurface.Material.Body);
							}

							if (handWeapon.isRanged()) {
								long1 = ((IsoGameCharacter)movingObject).playSound(handWeapon.getZombieHitSound());
							} else {
								gameCharacter.getEmitter().stopSoundByName(handWeapon.getSwingSound());
								long1 = gameCharacter.playSound(handWeapon.getZombieHitSound());
							}
						...
						if (gameCharacter.isAimAtFloor() && livingCharacter.bDoShove) {
							float7 = 0.0F;
							for (int7 = BodyPartType.ToIndex(BodyPartType.UpperLeg_L); int7 <= BodyPartType.ToIndex(BodyPartType.Foot_R); ++int7) {
								float7 += ((BodyPart)gameCharacter.getBodyDamage().getBodyParts().get(int7)).getPain();
							}
							...
							float7 = 0.0F;
							for (int7 = BodyPartType.ToIndex(BodyPartType.Hand_L); int7 <= BodyPartType.ToIndex(BodyPartType.UpperArm_R); ++int7) {
								float7 += ((BodyPart)gameCharacter.getBodyDamage().getBodyParts().get(int7)).getPain();
							}
						...
						if ("KnifeDeath".equals(gameCharacter.getVariableString("ZombieHitReaction"))) {
							...
							gameCharacter.attackVars.bCloseKill = true;
							movingObject.setCloseKilled(true);
						} else {
							gameCharacter.attackVars.bCloseKill = false;
						...
						if (zombie != null) {
							...
							zombie.setHitAngle(gameCharacter.getForwardDirection());
							zombie.setPlayerAttackPosition(zombie.testDotSide(gameCharacter));
							zombie.setHitHeadWhileOnFloor(byte1);
							...
							int int9;
							if (byte1 > 0) {
								int9 = Rand.Next(BodyPartType.ToIndex(BodyPartType.Head), BodyPartType.ToIndex(BodyPartType.Neck) + 1);
							} else if (boolean6) {
								int9 = Rand.Next(BodyPartType.ToIndex(BodyPartType.Groin), BodyPartType.ToIndex(BodyPartType.Foot_R) + 1);
							} else {
								int9 = Rand.Next(BodyPartType.ToIndex(BodyPartType.Hand_L), BodyPartType.ToIndex(BodyPartType.Neck) + 1);
							}

							float float12 = zombie.getBodyPartClothingDefense(int9, false, handWeapon.isRanged()) / 2.0F;
							float12 += zombie.getBodyPartClothingDefense(int9, true, handWeapon.isRanged());
							if (float12 > 70.0F) {
								float12 = 70.0F;
							}
							...
							if (!GameClient.bClient && !GameServer.bServer || GameClient.bClient && gameCharacter instanceof IsoPlayer && ((IsoPlayer)gameCharacter).isLocalPlayer()) {
								boolean3 = zombie.helmetFall(byte1 > 0);
							}
							...
							if ("KnifeDeath".equals(gameCharacter.getVariableString("ZombieHitReaction")) && !"Tutorial".equals(Core.GameMode)) {
								...
								int8 = gameCharacter.getPerkLevel(PerkFactory.Perks.SmallBlade) + 1;
								if (Rand.NextBool(byte2 + int8 * 2)) {
						...
						float float14 = 0.0F;
						boolean boolean11 = gameCharacter.isCriticalHit();
						if (baseVehicle == null && movingObject.getSquare() != null && gameCharacter.getSquare() != null) {
							movingObject.setCloseKilled(gameCharacter.attackVars.bCloseKill);
							if (((IsoPlayer)gameCharacter).isLocalPlayer() || gameCharacter.isNPC()) {
								...
								this.setParameterCharacterHitResult(gameCharacter, zombie, long1);
							}
							...
							if (vehiclePart != null) {
								...
								for (int8 = 0; int8 < vehiclePart.getChildCount(); ++int8) {
									VehiclePart vehiclePart2 = vehiclePart.getChild(int8);
								...
								if (vehicleWindow != null && vehicleWindow.isHittable()) {
									int8 = this.calcDamageToVehicle((int)float7 * 10, handWeapon.getDoorDamage(), true);
									vehicleWindow.damage(int8);
									gameCharacter.playSound("HitVehicleWindowWithWeapon");
								} else {
									int8 = this.calcDamageToVehicle((int)float7 * 10, handWeapon.getDoorDamage(), false);
									vehiclePart.setCondition(vehiclePart.getCondition() - int8);
						...
						if (GameClient.bClient && gameCharacter.isLocal()) {
							if (movingObject instanceof IsoGameCharacter) {
								HitReactionNetworkAI.CalcHitReactionWeapon(gameCharacter, (IsoGameCharacter)movingObject, handWeapon);
							}

							boolean5 = GameClient.sendHitCharacter(gameCharacter, movingObject, handWeapon, float14, boolean1, float11, boolean11, boolean3, byte1 > 0);
						}
					}
				}
			}
		...
		if (GameClient.bClient && ((IsoPlayer)gameCharacter).isLocalPlayer() && !boolean5) {
			GameClient.sendHitCharacter(gameCharacter, (IsoMovingObject)null, handWeapon, 0.0F, boolean1, 1.0F, gameCharacter.isCriticalHit(), false, false);
		}

		if (!boolean2 && boolean4) {
			boolean boolean12 = this.bHitOnlyTree && handWeapon.getScriptItem().Categories.contains("Axe");
			int int10 = boolean12 ? 2 : 1;
			if (Rand.Next(handWeapon.getConditionLowerChance() * int10 + gameCharacter.getMaintenanceMod() * 2) == 0) {
				boolean2 = true;
			} else if (Rand.NextBool(2) && WeaponType.getWeaponType(gameCharacter) != WeaponType.barehand && (!handWeapon.isTwoHandWeapon() || gameCharacter.getPrimaryHandItem() == handWeapon || gameCharacter.getSecondaryHandItem() == handWeapon || !Rand.NextBool(3))) {
				gameCharacter.getXp().AddXP(PerkFactory.Perks.Maintenance, 1.0F);
			}
		}
	...
	public static void splash(IsoMovingObject movingObject, HandWeapon handWeapon, IsoGameCharacter gameCharacter) {
		...
		if (SandboxOptions.instance.BloodLevel.getValue() > 1) {
			gameCharacter2.splatBloodFloorBig();
		}
	...
	private void setParameterCharacterHitResult(IsoGameCharacter gameCharacter, IsoZombie zombie, long long1) {
		if (long1 != 0L) {
			byte byte1 = 0;
			if (zombie != null) {
				if (zombie.isDead()) {
			...
			gameCharacter.getEmitter().setParameterValue(long1, FMODManager.instance.getParameterDescription("CharacterHitResult"), (float)byte1);
		}
	}
	...
	public static class CustomComparator implements Comparator {
		...
		public int compare(HitInfo hitInfo, HitInfo hitInfo2) {
			...
			IsoZombie zombie = (IsoZombie)Type.tryCastTo(hitInfo.getObject(), IsoZombie.class);
			IsoZombie zombie2 = (IsoZombie)Type.tryCastTo(hitInfo2.getObject(), IsoZombie.class);
