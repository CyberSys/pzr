	...
	public void enter() {
		...
		try {
			if (!GameClient.bClient && !GameServer.bServer && !Core.bTutorial && !Core.isLastStand() && !"Multiplayer".equals(Core.GameMode)) {
				...
				FileWriter fileWriter = new FileWriter(new File(string + File.separator + "latestSave.ini"));
		...
		loader = new Thread(ThreadGroups.Workers, new Runnable(){
			...
			private void runInner() throws Exception {
				...
				String file0002 = ZomboidFileSystem.instance.getGameModeCacheDir();
				boolean file = (new File(file0002 + File.separator)).mkdir();
				...
				if (GameWindow.bServerDisconnected) {
					...
					if (!GameLoadingState.build23Stop) {
						...
						if (!GameServer.bServer) {
							...
							if (ioException || IsoWorld.SavedWorldVersion != 194) {
								if (!ioException && IsoWorld.SavedWorldVersion != 194) {
