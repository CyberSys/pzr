			...
			if (GameServer.bServer && this.chr instanceof IsoPlayer) {
				if (System.currentTimeMillis() - this.lastXPTime > 60000L) {
					...
					if ((double)this.lastXPGrowthRate > 1000.0 * SandboxOptions.instance.XpMultiplier.getValue() * ServerOptions.instance.AntiCheatProtectionType9ThresholdMultiplier.getValue()) {
						...
						if (ServerOptions.instance.AntiCheatProtectionType9.getValue() && PacketValidator.doKickUser(udpConnection, this.getClass().getSimpleName(), "UI_ValidationFailed_Type9")) {
							LoggerManager.getLogger("kick").write(String.format("Kick: player=\"%s\" type=\"%s\" issuer=\"%s\"", udpConnection.username, "UI_ValidationFailed_Type9", this.getClass().getSimpleName()));
							...
							LoggerManager.getLogger("user").write(String.format("Warning: player=\"%s\" type=\"%s\" issuer=\"%s\"", udpConnection.username, "UI_ValidationFailed_Type9", this.getClass().getSimpleName()));
						}
					}
				}
			}
