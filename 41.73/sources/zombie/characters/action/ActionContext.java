...
public final class ActionContext {
	...
	public ActionState getNextState() {
		...
		for (int1 = 0; int1 < this.m_currentState.transitions.size(); ++int1) {
			ActionTransition actionTransition = (ActionTransition)this.m_currentState.transitions.get(int1);
			if (actionTransition.passes(this, 0) && !StringUtils.isNullOrWhitespace(actionTransition.transitionTo)) {
				actionState2 = this.m_stateGroup.get(actionTransition.transitionTo);
				if (actionState2 != null && !this.hasChildState(actionState2)) {
					if (!actionTransition.asSubstate || !this.currentStateSupportsChildState(actionState2)) {
						actionState = actionState2;
						break;
					}
		...
		for (int1 = 0; int1 < this.childStateCount(); ++int1) {
			...
			actionState2 = this.getChildStateAt(int1);
			for (int int2 = 0; int2 < actionState2.transitions.size(); ++int2) {
				ActionTransition actionTransition2 = (ActionTransition)actionState2.transitions.get(int2);
				if (actionTransition2.passes(this, 1)) {
					...
					if (!StringUtils.isNullOrWhitespace(actionTransition2.transitionTo)) {
						ActionState actionState4 = this.m_stateGroup.get(actionTransition2.transitionTo);
			...
			if (actionState3 != this.m_currentState && actionState3 != null) {
