...
public class DictionaryData {
	...
	protected void backupCurrentDataSet() throws IOException {
		...
		if (!Core.getInstance().isNoSave()) {
			...
			if (file.exists()) {
				...
				this.dataBackupPath = new File(ZomboidFileSystem.instance.getFileNameInCurrentSave("WorldDictionary_" + long1 + ".bak"));
	...
	protected void createErrorBackups() {
		if (!Core.getInstance().isNoSave()) {
			try {
				...
				long long1 = Instant.now().getEpochSecond();
				String string = ZomboidFileSystem.instance.getFileNameInCurrentSave("WD_ERROR_" + long1);
				...
				file2 = new File(ZomboidFileSystem.instance.getFileNameInCurrentSave("WorldDictionaryLog.lua"));
	...
	protected void loadFromByteBuffer(ByteBuffer byteBuffer) throws IOException {
		...
		for (int5 = 0; int5 < int4; ++int5) {
			...
			itemInfo.load(byteBuffer, 194, arrayList, arrayList2);
			this.itemIdToInfoMap.put(itemInfo.registryID, itemInfo);
			this.itemTypeToInfoMap.put(itemInfo.fullType, itemInfo);
		}
