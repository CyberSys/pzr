...
public final class PlayerDownloadServer {
	...
	private void removeOlderDuplicateRequests() {
		for (int int1 = this.ccrWaiting.size() - 1; int1 >= 0; --int1) {
			ClientChunkRequest clientChunkRequest = (ClientChunkRequest)this.ccrWaiting.get(int1);
			for (int int2 = 0; int2 < clientChunkRequest.chunks.size(); ++int2) {
				ClientChunkRequest.Chunk chunk = (ClientChunkRequest.Chunk)clientChunkRequest.chunks.get(int2);
				if (this.workerThread.isRequestCancelled(chunk)) {
					clientChunkRequest.chunks.remove(int2--);
					clientChunkRequest.releaseChunk(chunk);
				} else {
					for (int int3 = int1 - 1; int3 >= 0; --int3) {
						ClientChunkRequest clientChunkRequest2 = (ClientChunkRequest)this.ccrWaiting.get(int3);
						if (this.cancelDuplicateChunk(clientChunkRequest2, chunk.wx, chunk.wy)) {
						}
					}
				}
			}
			...
			if (clientChunkRequest.chunks.isEmpty()) {
				this.ccrWaiting.remove(int1);
				this.workerThread.freeRequests.add(clientChunkRequest);
			}
		}
	}

	private boolean cancelDuplicateChunk(ClientChunkRequest clientChunkRequest, int int1, int int2) {
		for (int int3 = 0; int3 < clientChunkRequest.chunks.size(); ++int3) {
			ClientChunkRequest.Chunk chunk = (ClientChunkRequest.Chunk)clientChunkRequest.chunks.get(int3);
			if (this.workerThread.isRequestCancelled(chunk)) {
				clientChunkRequest.chunks.remove(int3--);
				clientChunkRequest.releaseChunk(chunk);
			} else if (chunk.wx == int1 && chunk.wy == int2) {
				this.workerThread.sendNotRequired(chunk, false);
				clientChunkRequest.chunks.remove(int3);
				clientChunkRequest.releaseChunk(chunk);
				return true;
