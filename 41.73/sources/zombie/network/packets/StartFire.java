...
public class StartFire implements INetworkPacket {
	...
	public boolean validate(UdpConnection udpConnection) {
		if (GameServer.bServer && !this.smoke && ServerOptions.instance.NoFire.getValue()) {
			if (ServerOptions.instance.AntiCheatProtectionType16.getValue() && PacketValidator.doKickUser(udpConnection, this.getClass().getSimpleName(), "UI_ValidationFailed_Type16")) {
				LoggerManager.getLogger("kick").write(String.format("Kick: player=\"%s\" type=\"%s\" issuer=\"%s\" description=\"%s\"", udpConnection.username, "UI_ValidationFailed_Type16", this.getClass().getSimpleName(), this.getDescription()));
			}
			...
			if (ServerOptions.instance.AntiCheatProtectionType17.getValue() && PacketValidator.doKickUser(udpConnection, this.getClass().getSimpleName(), "UI_ValidationFailed_Type17")) {
				LoggerManager.getLogger("kick").write(String.format("Kick: player=\"%s\" type=\"%s\" issuer=\"%s\" description=\"%s\"", udpConnection.username, "UI_ValidationFailed_Type17", this.getClass().getSimpleName(), this.getDescription()));
			}

			return false;
		} else if (this.smoke && !IsoFire.CanAddSmoke(this.square.getSquare(), this.ignite)) {
			if (ServerOptions.instance.AntiCheatProtectionType18.getValue() && PacketValidator.doKickUser(udpConnection, this.getClass().getSimpleName(), "UI_ValidationFailed_Type18")) {
				LoggerManager.getLogger("kick").write(String.format("Kick: player=\"%s\" type=\"%s\" issuer=\"%s\" description=\"%s\"", udpConnection.username, "UI_ValidationFailed_Type18", this.getClass().getSimpleName(), this.getDescription()));
			}
