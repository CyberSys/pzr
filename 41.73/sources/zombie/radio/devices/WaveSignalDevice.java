...
public interface WaveSignalDevice {
	...
	void AddDeviceText(String string, float float1, float float2, float float3, String string2, String string3, int int1);

	boolean HasPlayerInRange();

	default void AddDeviceText(IsoPlayer player, String string, float float1, float float2, float float3, String string2, String string3, int int1) {
		if (this.getDeviceData() != null && this.getDeviceData().getDeviceVolume() > 0.0F) {
			...
			if (player != null && player.isLocalPlayer() && !player.Traits.Deaf.isSet()) {
				if (this.getDeviceData().getParent() instanceof InventoryItem && player.isEquipped((InventoryItem)this.getDeviceData().getParent())) {
					...
					((VehiclePart)this.getDeviceData().getParent()).getChatElement().addChatLine(string, float1, float2, float3, UIFont.Medium, (float)this.getDeviceData().getDeviceVolumeRange(), "default", true, true, true, true, true, true);
				}
