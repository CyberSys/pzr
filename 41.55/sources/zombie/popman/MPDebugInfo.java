...
public final class MPDebugInfo {
	...
	public void render(ZombiePopulationRenderer zombiePopulationRenderer, float float1) {
		...
		if (GameClient.bClient && DebugOptions.instance.MultiplayerShowPosition.getValue()) {
			float float6 = (float)((IsoChunkMap.ChunkGridWidth / 2 + 2) * 10);
			...
			while (iterator.hasNext()) {
				...
				if (player != null) {
					color = player.getSpeakColour();
				}

				Vector2 vector2 = (Vector2)entry.getValue();
				zombiePopulationRenderer.renderZombie(vector2.x, vector2.y, color.r, color.g, color.b);
				zombiePopulationRenderer.renderCircle(vector2.x, vector2.y, float6, color.r, color.g, color.b, color.a);
				zombiePopulationRenderer.renderString(vector2.x, vector2.y, player == null ? String.valueOf(entry.getKey()) : player.getUsername(), (double)color.r, (double)color.g, (double)color.b, (double)color.a);
			}

			if (IsoPlayer.getInstance() != null) {
				IsoPlayer player2 = IsoPlayer.getInstance();
				Color color2 = player2.getSpeakColour();
				zombiePopulationRenderer.renderZombie(player2.x, player2.y, color2.r, color2.g, color2.b);
				zombiePopulationRenderer.renderCircle(player2.x, player2.y, float6, color2.r, color2.g, color2.b, color2.a);
				zombiePopulationRenderer.renderString(player2.x, player2.y, player2.getUsername(), (double)color2.r, (double)color2.g, (double)color2.b, (double)color2.a);
			}
		}
