...
public final class WallVines extends ErosionCategory {
		...
		for (int int2 = int1 - 1; int2 >= 1; --int2) {
			...
			if (object.AttachedAnimSprite != null) {
				for (int int3 = 0; int3 < object.AttachedAnimSprite.size(); ++int3) {
					IsoSprite sprite = ((IsoSpriteInstance)object.AttachedAnimSprite.get(int3)).parentSprite;
	...
	public void getObjectNames(ArrayList arrayList) {
		for (int int1 = 0; int1 < this.objs.size(); ++int1) {
			if (((ErosionObjOverlay)this.objs.get(int1)).name != null && !arrayList.contains(((ErosionObjOverlay)this.objs.get(int1)).name)) {
				arrayList.add(((ErosionObjOverlay)this.objs.get(int1)).name);
			}
		}
	}
