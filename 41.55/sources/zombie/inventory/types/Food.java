...
public final class Food extends InventoryItem {
	...
	public void update() {
		...
		if (itemContainer != null) {
			float float1 = itemContainer.getTemprature();
			if (this.Heat > float1) {
	...
	public void updateSound(BaseSoundEmitter baseSoundEmitter) {
		if (this.shouldPlayCookingSound()) {
			...
			this.m_cookingSound = baseSoundEmitter.playSound(this.getCookingSound());
			this.setCookingParameter(baseSoundEmitter);
		} else {
	...
	public float getActualWeight() {
		...
		if (this.haveExtraItems()) {
			float float2 = this.getHungChange();
			float float3 = this.getBaseHunger();
			float1 = float3 == 0.0F ? 0.0F : float2 / float3;
			float float4 = 0.0F;
			...
			float float6 = (float5 - float4) * float1 + float4;
			return float6;
		} else {
			if (this.getReplaceOnUse() != null) {
				...
				if (item2 != null) {
					...
					return (this.getScriptItem().getActualWeight() - item2.getActualWeight()) * float1 + item2.getActualWeight();
				}
			}
