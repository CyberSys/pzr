...
public final class ItemPickerJava {
				...
				if ((float)Rand.Next(10000) <= float2 * 100.0F * float3 + float1 * 10.0F) {
					...
					if (inventoryItem instanceof Key) {
						...
						if (!key.getFullType().equals("Base.Padlock")) {
							String string2 = Translator.getText("IGUI_HouseKey");
							key.setName(string2 + " " + key.getKeyId());
						}
					...
					if (inventoryItem instanceof HandWeapon && !itemPickerContainer.dontSpawnAmmo && Rand.Next(100) < 90) {
						...
						if (!StringUtils.isNullOrEmpty(handWeapon.getAmmoBox()) && Rand.Next(100) < int3) {
	...
	private static void checkStashItem(InventoryItem inventoryItem, ItemPickerJava.ItemPickerContainer itemPickerContainer) {
		if (itemPickerContainer.stashChance > 0 && !StringUtils.isNullOrEmpty(inventoryItem.getMap())) {
			...
			for (int int1 = 0; (float)int1 < itemPickerContainer.rolls; ++int1) {
				...
				for (int int2 = 0; int2 < itemPickerItemArray.length; ++int2) {
					...
					if ((float)Rand.Next(10000) <= float2 * 100.0F * float3 + float1 * 10.0F) {
						...
						int int3;
						if (!StringUtils.isNullOrEmpty(inventoryItem.getMap()) && itemPickerContainer.maxMap > 0) {
							int int4 = 0;
							for (int3 = 0; int3 < itemContainer.getItems().size(); ++int3) {
								InventoryItem inventoryItem2 = (InventoryItem)itemContainer.getItems().get(int3);
								if (!StringUtils.isNullOrEmpty(inventoryItem2.getMap())) {
									++int4;
								}
							}
