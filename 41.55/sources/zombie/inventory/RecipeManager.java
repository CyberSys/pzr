...
public final class RecipeManager {
	...
	public static boolean IsRecipeValid(Recipe recipe, IsoGameCharacter gameCharacter, InventoryItem inventoryItem, ArrayList arrayList) {
		if (recipe.Result == null) {
			...
			return CanPerform(recipe, gameCharacter);
		}
	}
	...
	private static boolean CanPerform(Recipe recipe, IsoGameCharacter gameCharacter) {
		if (StringUtils.isNullOrWhitespace(recipe.getCanPerform())) {
			...
			if (object == null) {
				...
				Boolean Boolean1 = LuaManager.caller.protectedCallBoolean(LuaManager.thread, object, recipe, gameCharacter);
	...
	public static boolean HasAllRequiredItems(Recipe recipe, IsoGameCharacter gameCharacter, InventoryItem inventoryItem, ArrayList arrayList) {
		...
		for (int int1 = 0; int1 < arrayList2.size(); ++int1) {
			InventoryItem inventoryItem2 = (InventoryItem)arrayList2.get(int1);
			if (inventoryItem2 instanceof Food && ((Food)inventoryItem2).getFreezingTime() > 0.0F) {
				return false;
			}

			if (recipe.noBrokenItems() && inventoryItem2.isBroken()) {
				return false;
			}

			if ("Clothing".equals(inventoryItem2.getCategory()) && inventoryItem2.isFavorite()) {
				return false;
			}
		}
		...
		void getItemsFromContainer(IsoGameCharacter gameCharacter, ItemContainer itemContainer, InventoryItem inventoryItem) {
			for (int int1 = 0; int1 < itemContainer.getItems().size(); ++int1) {
				...
				if (inventoryItem != null && inventoryItem == inventoryItem2 || !gameCharacter.isEquippedClothing(inventoryItem2) || this.isKeep(inventoryItem2.getFullType())) {
					...
					} else if (this.allSourceTypes.contains(inventoryItem2.getFullType()) && inventoryItem2.getCondition() > 0 || this.recipe.AllowDestroyedItem) {
						this.allItems.add(RecipeManager.RMRecipeItem.alloc(inventoryItem2));
					}
				}
			}
	...
	private static final class RMRecipeItemList {
		...
		void getItemsFrom(ArrayList arrayList, RecipeManager.RMRecipe rMRecipe) {
			...
			for (int int1 = 0; int1 < arrayList.size(); ++int1) {
				...
				} else if (string.equals(rMRecipeItem.item.getFullType()) && (!(rMRecipe.recipe.getHeat() > 0.0F) || !(rMRecipeItem.item instanceof DrainableComboItem) || !rMRecipeItem.item.IsCookable || !(rMRecipeItem.item.getInvHeat() + 1.0F < rMRecipe.recipe.getHeat())) && (!(rMRecipe.recipe.getHeat() < 0.0F) || !(rMRecipeItem.item instanceof DrainableComboItem) || !rMRecipeItem.item.IsCookable || !(rMRecipeItem.item.getInvHeat() > rMRecipe.recipe.getHeat())) && rMRecipe.Test(rMRecipeItem.item)) {
