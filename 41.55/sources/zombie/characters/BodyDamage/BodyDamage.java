		...
		if (GameServer.bServer) {
			...
			if (boolean3) {
				...
				if (this.ParentChar instanceof IsoPlayer && GameClient.bClient && ((IsoPlayer)this.ParentChar).isLocalPlayer()) {
					...
					GameClient.sendInjuries(player);
				}
			}
		}
	}
		...
		if (GameServer.bServer) {
			...
			if (Rand.Next(100) > int1) {
				...
				if (boolean5) {
					...
					if ((float)Rand.Next(100) < Float1) {
						this.getParentChar().addHoleFromZombieAttacks(BloodBodyPartType.FromIndex(int5), boolean7);
						return false;
					}

					this.getParentChar().addHole(BloodBodyPartType.FromIndex(int5), true);
					boolean4 = true;
	...
	public void Update() {
		if (!(this.getParentChar() instanceof IsoZombie)) {
			if (GameServer.bServer) {
				...
				float float2;
				if (int2 > 0 && this.getHealth() < 60.0F || int2 > 3) {
					float2 = 1.0F / (float)int2 * 200.0F * GameTime.instance.getInvMultiplier();
				...
				if (this.getOverallBodyHealth() != 0.0F) {
					...
					if (this.ParentChar.getPainEffect() > 0.0F) {
						...
						for (int int4 = 0; int4 < BodyPartType.ToIndex(BodyPartType.MAX); ++int4) {
							float2 += ((BodyPart)this.getBodyParts().get(int4)).getPain() * BodyPartType.getPainModifyer(int4);
						}
