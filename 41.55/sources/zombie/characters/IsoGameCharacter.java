...
public abstract class IsoGameCharacter extends IsoMovingObject implements Talker,ChatElementOwner,IAnimatable,IAnimationVariableMap,IClothingItemListener,IActionStateChanged,IAnimEventCallback,IFMODParameterUpdater,ILuaVariableSource,ILuaGameCharacter {
	...
	private float bagRunSpeedModifier = 1.0F;
	...
	public float lastUpdateX = 0.0F;
	public float lastUpdateY = 0.0F;
	public float lastUpdateT = 0.0F;
	...
	public NetworkTeleport.NetworkTeleportDebug teleportDebug = null;
	public NetworkStrikeAI.Strike strike = null;
	private NetworkTeleport teleport = null;
	public HashMap debugData = new LinkedHashMap(){
    
    protected boolean removeEldestEntry(Entry var1) {
        return this.size() > 10;
    }
};
	public ArrayList invRadioFreq = new ArrayList();
	...
	public boolean movingOnNetwork = false;
	...
	public boolean isInTrees2(boolean boolean1) {
		if (this.isCurrentState(BumpedState.instance())) {
			...
			if (square != null) {
				...
				if (!boolean1 && square.getProperties().Is("Bush")) {
					return true;
				}
			}
		...
		if (animationPlayer != null) {
			if (this.getPath2() != null && !this.isCurrentState(ClimbOverFenceState.instance()) && !this.isCurrentState(ClimbThroughWindowState.instance())) {
				...
				if (GameClient.bClient) {
					...
					} else if (this instanceof IsoPlayer && !((IsoPlayer)this).isLocalPlayer()) {
						return;
					}
				}
		...
		if (!(this.fallTime < 20.0F) && !this.isClimbing()) {
			...
			if (this.isZombie()) {
				...
				if (!boolean1) {
					...
					float1 *= Math.max(0.1F, 1.0F - (float)this.getPerkLevel(PerkFactory.Perks.Fitness) * 0.1F);
					...
					if (this.fallTime > 70.0F) {
						...
						if (Rand.Next(100) >= int1) {
							...
							float float2 = (float)Rand.Next(50, 80);
							if (this.Traits.FastHealer.isSet()) {
								float2 = (float)Rand.Next(30, 50);
							} else if (this.Traits.SlowHealer.isSet()) {
								float2 = (float)Rand.Next(80, 150);
							}
							...
							this.getBodyDamage().getBodyPart(BodyPartType.FromIndex(Rand.Next(BodyPartType.ToIndex(BodyPartType.UpperLeg_L), BodyPartType.ToIndex(BodyPartType.Foot_R) + 1))).setFractureTime(float2);
	...
	public void setOnDeathDone(boolean boolean1) {
		this.bDead = boolean1;
		if (this instanceof IsoZombie) {
			((IsoZombie)this).networkAI.extraUpdate();
		}
	}
	...
	public void setWornItem(String string, InventoryItem inventoryItem, boolean boolean1) {
		...
		if (boolean1 && inventoryItem2 != null && this instanceof IsoPlayer && !this.getInventory().hasRoomFor(this, inventoryItem2)) {
			...
			if (square != null) {
				...
				float float3 = square.getApparentZ(float1, float2);
	...
	public void OnAnimEvent(AnimLayer animLayer, AnimEvent animEvent) {
		if (animEvent.m_EventName != null) {
			...
			if (animEvent.m_EventName.equalsIgnoreCase("PlaySound")) {
				this.getEmitter().playSound(animEvent.m_ParameterValue);
			}
		...
		if (perk == PerkFactory.Perks.MAX) {
			...
			if (perkInfo != null) {
				...
				if (this instanceof IsoPlayer && !perk.isPassiv() && !"Tutorial".equals(Core.GameMode) && this.getHoursSurvived() > 1.0) {
					HaloTextHelper.addTextWithArrow((IsoPlayer)this, "+1 " + perk.getName(), true, HaloTextHelper.getColorGreen());
				}
	...
	public boolean Eat(InventoryItem inventoryItem, float float1) {
		...
		if (food == null) {
			...
			if (food.getBaseHunger() != 0.0F && food.getHungChange() != 0.0F) {
				float float3 = food.getBaseHunger() * float1;
				float2 = float3 / food.getHungChange();
			...
			if (this.BodyDamage.getFoodSicknessLevel() > 0.0F && (float)food.getReduceFoodSickness() > 0.0F && this.effectiveEdibleBuffTimer <= 0.0F) {
				float2 = this.BodyDamage.getFoodSicknessLevel();
				...
				float4 = this.BodyDamage.getPoisonLevel();
			...
			if (float1 == 1.0F) {
				...
				float2 = food.getHungChange();
				float4 = food.getThirstChange();
				food.multiplyFoodValues(1.0F - float1);
				if (float2 < 0.0F && (double)food.getHungerChange() > -0.01) {
					food.setHungChange(0.0F);
					food.UseItem();
					return true;
				}

				if (float2 == 0.0F && float4 < 0.0F && food.getThirstChange() > -0.01F) {
					food.setHungChange(0.0F);
					food.UseItem();
					return true;
				}
			...
			if (!GameServer.bServer || !(this instanceof IsoPlayer)) {
				if (!GameClient.bClient || !this.isZombie()) {
					if (this.isZombie() && VirtualZombieManager.instance.isReused((IsoZombie)this)) {
						...
						if (this.square != null && !GameServer.bServer && (!GameClient.bClient || this instanceof IsoPlayer && ((IsoPlayer)this).isLocalPlayer()) && this.square.getProperties().Is(IsoFlagType.burning)) {
	...
	private float processHitDamage(HandWeapon handWeapon, IsoGameCharacter gameCharacter, float float1, boolean boolean1, float float2) {
		...
		if (gameCharacter.isCrit && !boolean1) {
	...
	public boolean isMaskClicked(int int1, int int2, boolean boolean1) {
		if (this.sprite == null) {
			...
			boolean boolean2 = this.legsSprite.isMaskClicked(this.dir, int1, int2, boolean1);
			return boolean2;
		}
	}
		...
		if (this.BodyDamage != null) {
			if (this.BodyDamage.IsSneezingCoughing() > 0) {
				...
				if (this.BodyDamage.IsSneezingCoughing() == 1) {
					string = "Ah-choo!";
					int1 = Rand.Next(2);
					this.setVariable("Ext", "Sneeze" + (int1 + 1));
				}
	...
	public void Say(String string) {
		if (!this.isZombie()) {
			this.ProcessSay(string, this.SpeakColour.r, this.SpeakColour.g, this.SpeakColour.b, UIFont.Dialogue, 30.0F, "default");
		}
	}

	public void Say(String string, float float1, float float2, float float3, UIFont uIFont, float float4, String string2) {
		this.ProcessSay(string, float1, float2, float3, uIFont, float4, string2);
	}

	public void SayWhisper(String string) {
		this.ProcessSay(string, this.SpeakColour.r, this.SpeakColour.g, this.SpeakColour.b, UIFont.Dialogue, 10.0F, "whisper");
	}

	public void SayShout(String string) {
		this.ProcessSay(string, this.SpeakColour.r, this.SpeakColour.g, this.SpeakColour.b, UIFont.Dialogue, 60.0F, "shout");
	}

	private void ProcessSay(String string, float float1, float float2, float float3, UIFont uIFont, float float4, String string2) {
	...
	protected void updateUserName() {
		if (this.userName != null && this.isoPlayer != null) {
			...
			if (!ServerOptions.instance.DisplayUserName.getValue() && !boolean3) {
				boolean1 = false;
			}
	...
	public void renderlast() {
			...
			if (this.isoPlayer != null && (this == IsoCamera.frameState.CamCharacter || this.getCurrentSquare() != null && this.getCurrentSquare().getCanSee(int1)) || IsoPlayer.getInstance().isCanSeeAll()) {
				...
				if (!GameClient.bClient && this.isoPlayer != null && this.isoPlayer.getVehicle() == null) {
					...
					if (this.getVehicle() == null && baseVehicle != null && baseVehicle.getPartById("Engine") != null && (this.isoPlayer.getInventory().haveThisKeyId(baseVehicle.getKeyId()) != null || baseVehicle.isHotwired() || SandboxOptions.getInstance().VehicleEasyUse.getValue()) && UIManager.VisibleAllUI) {
						Color color = Color.HSBtoRGB(baseVehicle.colorHue, baseVehicle.colorSaturation * 0.5F, baseVehicle.colorValue, IsoGameCharacter.L_renderLast.color);
		...
		if (Core.bDebug && DebugOptions.instance.MultiplayerShowPrediction.getValue()) {
			if (this instanceof IsoZombie) {
				IsoZombie zombie = (IsoZombie)this;
				...
				if (zombie.networkAI.DebugInterfaceActive) {
				...
				} else if (zombie.networkAI.getOwner() != -1) {
					...
					LineDrawer.DrawIsoCircle(this.x, this.y, this.z, 0.3F, 16, Colors.Magenta.r, Colors.Magenta.g, Colors.Magenta.b, 0.35F);
				}
				...
				if (IsoUtils.DistanceToSquared(this.x, this.y, this.realx, this.realy) > 25.0F) {
				...
				float4 = IsoUtils.XToScreenExact(zombie.networkAI.targetX + 0.4F, zombie.networkAI.targetY + 0.4F, (float)zombie.networkAI.targetZ, 0);
				float5 = IsoUtils.YToScreenExact(zombie.networkAI.targetX + 0.4F, zombie.networkAI.targetY + 0.4F, (float)zombie.networkAI.targetZ, 0);
				int int6 = (int)(GameTime.getServerTime() / 1000000L);
				TextManager.instance.DrawStringCentre(UIFont.DebugConsole, (double)float4, (double)float5, "ETA: " + (zombie.networkAI.targetT - int6), (double)Colors.LimeGreen.r, (double)Colors.LimeGreen.g, (double)Colors.LimeGreen.b, 0.6499999761581421);
				TextManager.instance.DrawStringCentre(UIFont.DebugConsole, (double)float4, (double)(float5 + 10.0F), "Type: " + zombie.networkAI.predictionType, (double)Colors.LimeGreen.r, (double)Colors.LimeGreen.g, (double)Colors.LimeGreen.b, 0.6499999761581421);
				TextManager.instance.DrawStringCentre(UIFont.DebugConsole, (double)float4, (double)(float5 + 20.0F), "Target: " + zombie.networkAI.moveToTarget, (double)Colors.LimeGreen.r, (double)Colors.LimeGreen.g, (double)Colors.LimeGreen.b, 0.6499999761581421);
				float4 = IsoUtils.XToScreenExact(this.x + 0.4F, this.y + 0.4F, this.z, 0);
				float5 = IsoUtils.YToScreenExact(this.x + 0.4F, this.y + 0.4F, this.z, 0);
				TextManager.instance.DrawStringCentre(UIFont.DebugConsole, (double)float4, (double)float5, "ID: " + this.getOnlineID(), (double)Colors.LightYellow.r, (double)Colors.LightYellow.g, (double)Colors.LightYellow.b, 0.75);
				TextManager.instance.DrawStringCentre(UIFont.DebugConsole, (double)float4, (double)(float5 + 10.0F), "State: " + this.getCurrentStateName(), (double)Colors.LightYellow.r, (double)Colors.LightYellow.g, (double)Colors.LightYellow.b, 0.75);
				TextManager.instance.DrawStringCentre(UIFont.DebugConsole, (double)float4, (double)(float5 + 20.0F), "Previous: " + this.getPreviousStateName(), (double)Colors.LightYellow.r, (double)Colors.LightYellow.g, (double)Colors.LightYellow.b, 0.75);
				TextManager.instance.DrawStringCentre(UIFont.DebugConsole, (double)float4, (double)(float5 + 30.0F), "Health: " + this.getHealth(), (double)Colors.LightYellow.r, (double)Colors.LightYellow.g, (double)Colors.LightYellow.b, 0.75);
			} else if (this instanceof IsoPlayer) {
				if (this.isoPlayer != null && this.isoPlayer.isLocalPlayer()) {
					LineDrawer.DrawIsoCircle(this.x, this.y, this.z, (float)((IsoChunkMap.ChunkGridWidth / 2 + 2) * 10), 32, this.isoPlayer.SpeakColour.r, this.isoPlayer.SpeakColour.g, this.isoPlayer.SpeakColour.b, 0.5F);
					LineDrawer.DrawIsoCircle(this.x, this.y, this.z, 40.0F, 32, Colors.Blue.r, Colors.Blue.g, Colors.Blue.b, 0.5F);
					LineDrawer.DrawIsoCircle(this.x, this.y, this.z, 5.0F, 32, Colors.Magenta.r, Colors.Magenta.g, Colors.Magenta.b, 0.5F);
					LineDrawer.DrawIsoCircle(this.x, this.y, this.z, 2.0F, 32, Colors.Magenta.r, Colors.Magenta.g, Colors.Magenta.b, 0.5F);
				}
		...
		if (Core.bDebug) {
			if (DebugOptions.instance.MultiplayerShowTeleport.getValue()) {
				if (this instanceof IsoZombie && this.teleportDebug != null) {
					LineDrawer.DrawIsoLine(this.teleportDebug.lx, this.teleportDebug.ly, this.teleportDebug.lz, this.teleportDebug.nx, this.teleportDebug.ny, this.teleportDebug.nz, Colors.NavajoWhite.r, Colors.NavajoWhite.g, Colors.NavajoWhite.b, 0.7F, 3);
					LineDrawer.DrawIsoCircle(this.teleportDebug.nx, this.teleportDebug.ny, this.teleportDebug.nz, 0.2F, 16, Colors.NavajoWhite.r, Colors.NavajoWhite.g, Colors.NavajoWhite.b, 0.7F);
					TextManager.instance.DrawStringCentre(UIFont.DebugConsole, (double)IsoUtils.XToScreenExact(this.teleportDebug.lx, this.teleportDebug.ly, this.teleportDebug.lz, 0), (double)IsoUtils.YToScreenExact(this.teleportDebug.lx, this.teleportDebug.ly, this.teleportDebug.lz, 0), String.format("Z_%d id=%d", this.teleportDebug.id, this.teleportDebug.time), (double)Colors.NavajoWhite.r, (double)Colors.NavajoWhite.g, (double)Colors.NavajoWhite.b, 0.699999988079071);
				}
			} else {
				this.teleportDebug = null;
			}

			if (DebugOptions.instance.MultiplayerShowStatus.getValue() && (this instanceof IsoZombie || this instanceof IsoPlayer)) {
				float3 = IsoUtils.XToScreenExact(this.x, this.y, this.z, 0);
				float4 = IsoUtils.YToScreenExact(this.x, this.y, this.z, 0);
				Color color2 = this.isDead() ? Colors.OrangeRed : Colors.NavajoWhite;
				TextManager.instance.DrawStringCentre(UIFont.DebugConsole, (double)float3, (double)float4, String.format("%d: %f", this.getOnlineID(), this.getHealth()), (double)color2.r, (double)color2.g, (double)color2.b, 0.699999988079071);
				TextManager.instance.DrawStringCentre(UIFont.DebugConsole, (double)float3, (double)(float4 + 10.0F), String.format("State: %s/%s", this.getPreviousActionContextStateName(), this.getCurrentActionContextStateName()), (double)color2.r, (double)color2.g, (double)color2.b, 0.699999988079071);
			}
		}

		if (this.inventory != null) {
			int int7;
			for (int7 = 0; int7 < this.inventory.Items.size(); ++int7) {
				InventoryItem inventoryItem = (InventoryItem)this.inventory.Items.get(int7);
				if (inventoryItem instanceof IUpdater) {
			...
			if (DebugOptions.instance.Animation.Debug.getValue() && this.hasActiveModel()) {
				int7 = (int)IsoUtils.XToScreenExact(this.x, this.y, this.z, 0);
				int int8 = (int)IsoUtils.YToScreenExact(this.x, this.y, this.z, 0);
				TextManager.instance.DrawString((double)int7, (double)int8, this.getAnimationDebug());
			}
	...
	private void debugAim() {
		if (this == IsoPlayer.getInstance()) {
			...
			if (player.IsAiming()) {
				...
				SwipeStatePlayer.HitInfo hitInfo;
				if (attackVars.targetOnGround != null) {
					SwipeStatePlayer.HitInfo hitInfo2 = (SwipeStatePlayer.HitInfo)attackVars.targetsProne.get(0);
					LineDrawer.DrawIsoCircle(hitInfo2.x, hitInfo2.y, hitInfo2.z, 0.1F, 8, 1.0F, 1.0F, 0.0F, 1.0F);
				} else {
					byte byte1 = 0;
					if (byte1 < attackVars.targetsStanding.size()) {
						hitInfo = (SwipeStatePlayer.HitInfo)attackVars.targetsStanding.get(byte1);
				...
				for (int int1 = 0; int1 < SwipeStatePlayer.HitList.size(); ++int1) {
					hitInfo = (SwipeStatePlayer.HitInfo)SwipeStatePlayer.HitList.get(int1);
					IsoMovingObject movingObject = hitInfo.object;
					if (movingObject != null) {
						int int2 = hitInfo.chance;
						...
						TextManager.instance.DrawStringCentre(uIFont, (double)float16, (double)float17, String.valueOf(hitInfo.dot), 1.0, 1.0, 1.0, 1.0);
						TextManager.instance.DrawStringCentre(uIFont, (double)float16, (double)(float17 + (float)TextManager.instance.getFontHeight(uIFont)), String.valueOf(hitInfo.chance) + "%", 1.0, 1.0, 1.0, 1.0);
						float7 = 1.0F;
						float8 = 1.0F;
						float float18 = 1.0F;
						float float19 = PZMath.sqrt(hitInfo.distSq);
		...
		if (this.current != null) {
			...
			if (!this.isDead() || this.current != null && this.current.getMovingObjects().contains(this)) {
				...
				if (this.getBodyDamage() != null && this.getCurrentBuilding() != null && this.getCurrentBuilding().isToxic()) {
					float1 = GameTime.getInstance().getMultiplier() / 1.6F;
				...
				if (!this.isZombie()) {
					...
					if (this.Traits.Claustophobic.isSet() && this.getCurrentSquare().getRoom() != null) {
						float1 = 1.0F;
						int1 = this.getCurrentSquare().getRoom().def.getH() * this.getCurrentSquare().getRoom().def.getW();
						float1 = 1.0F - (float)int1 / 70.0F;
						if (float1 < 0.0F) {
							float1 = 0.0F;
						}
					...
					int2 = this.Moodles.getMoodleLevel(MoodleType.Panic);
					if (int2 == 2) {
						stats = this.stats;
					...
					int1 = this.Moodles.getMoodleLevel(MoodleType.Tired);
					if (int1 == 4) {
						stats = this.stats;
	...
	protected void updateMovementMomentum() {
		...
		if (this.isPlayerMoving() && !this.isAiming()) {
			...
			float4 = float3 / 0.55F;
			float5 = PZMath.clamp(float4, 0.0F, 1.0F);
			this.m_momentumScalar = float5;
		} else {
	...
	protected float calculateIdleSpeed() {
		...
		this.idleSpeed = float1;
		return float1;
	}

	public float calculateBaseSpeed() {
		float float1 = 0.8F;
		this.bagRunSpeedModifier = 1.0F;
		...
		for (int int2 = 0; int2 < this.bagsWorn.size(); ++int2) {
			...
			this.bagRunSpeedModifier += this.calcRunSpeedModByBag(inventoryContainer);
		}

		if (this.getPrimaryHandItem() != null && this.getPrimaryHandItem() instanceof InventoryContainer) {
			this.bagRunSpeedModifier += this.calcRunSpeedModByBag((InventoryContainer)this.getPrimaryHandItem());
		}

		if (this.getSecondaryHandItem() != null && this.getSecondaryHandItem() instanceof InventoryContainer) {
			this.bagRunSpeedModifier += this.calcRunSpeedModByBag((InventoryContainer)this.getSecondaryHandItem());
		}

		this.fullSpeedMod = this.runSpeedModifier + (this.bagRunSpeedModifier - 1.0F);
		float float2 = float1 * (1.0F - Math.abs(1.0F - this.fullSpeedMod) / 2.0F);
		return float2;
	}
	...
	protected float calculateCombatSpeed() {
		...
		return float1 * GameTime.getAnimSpeedFix();
	}
	...
	private float getFootInjurySpeedModifier() {
		...
		float float2 = 0.0F;
		float float3 = 0.0F;
		for (int int1 = BodyPartType.ToIndex(BodyPartType.UpperLeg_L); int1 < BodyPartType.ToIndex(BodyPartType.MAX); ++int1) {
	...
	public void HitSilence(HandWeapon handWeapon, IsoGameCharacter gameCharacter, float float1, boolean boolean1, float float2) {
		if (gameCharacter != null && handWeapon != null) {
			if (this.getStateMachine().getCurrent() != StaggerBackState.instance()) {
				...
				if (handWeapon.getName().contains("Bare Hands") || gameCharacter instanceof IsoPlayer && ((IsoPlayer)gameCharacter).bDoShove) {
					boolean1 = true;
					this.noDamage = true;
				}
				...
				float8 /= handWeapon.getKnockdownMod();
				if (gameCharacter instanceof IsoPlayer && !handWeapon.isAimedHandWeapon()) {
					float8 *= 2.0F - ((IsoPlayer)gameCharacter).useChargeDelta;
				}
				...
				boolean boolean2 = Rand.Next((int)float8) == 0;
				if (this.isDead() || (handWeapon.isAlwaysKnockdown() || boolean2) && this.isZombie()) {
	...
	public void setMoving(boolean boolean1) {
		...
		if (GameClient.bClient && this instanceof IsoPlayer && ((IsoPlayer)this).bRemote) {
			((IsoPlayer)this).JustMoved = ((IsoPlayer)this).m_isPlayerMoving = boolean1;
		}
	}
	...
	public void setGodMod(boolean boolean1) {
		...
		if (this instanceof IsoPlayer && GameClient.bClient && ((IsoPlayer)this).isLocalPlayer()) {
			...
			GameClient.sendInjuries((IsoPlayer)this);
		}
	}
	...
	public void setAttackingZombie(IsoZombie zombie) {
		this.attackingZombie = zombie;
	}
	...
	public String testDotSide(IsoMovingObject movingObject) {
		Vector2 vector2 = IsoGameCharacter.l_testDotSide.v1.set(movingObject.getX(), movingObject.getY());
		Vector2 vector22 = IsoGameCharacter.l_testDotSide.v2.set(this.getX(), this.getY());
		Vector2 vector23 = IsoGameCharacter.l_testDotSide.v3.set(vector2.x - vector22.x, vector2.y - vector22.y);
		vector23.normalize();
		float float1 = Vector2.dot(vector23.x, vector23.y, this.m_forwardDirection.x, this.m_forwardDirection.y);
		if ((double)float1 > 0.7) {
			...
			float float6 = vector22.x + this.m_forwardDirection.x;
			float float7 = vector22.y + this.m_forwardDirection.y;
	...
	public void addHole(BloodBodyPartType bloodBodyPartType, boolean boolean1) {
		if (this instanceof IHumanVisual) {
			...
			HumanVisual humanVisual = ((IHumanVisual)this).getHumanVisual();
			this.getItemVisuals(tempItemVisuals);
			BloodClothingType.addHole(bloodBodyPartType, humanVisual, tempItemVisuals, boolean1);
			this.bUpdateModelTextures = true;
			if (!GameServer.bServer && this instanceof IsoPlayer && ((IsoPlayer)this).isLocalPlayer()) {
				LuaEventManager.triggerEvent("OnClothingUpdated", this);
				if (GameClient.bClient && this instanceof IsoPlayer) {
	...
	public void attackFromWindowsLunge(IsoZombie zombie) {
		if (!(this.lungeFallTimer > 0.0F) && (int)this.getZ() == (int)zombie.getZ() && !zombie.isDead() && !this.getCurrentSquare().isDoorBlockedTo(zombie.getCurrentSquare()) && !this.getCurrentSquare().isWallTo(zombie.getCurrentSquare()) && !this.getCurrentSquare().isWindowTo(zombie.getCurrentSquare())) {
			...
			for (int int5 = int2; int5 < int4; ++int5) {
				for (int int6 = int1; int6 < int3; ++int6) {
					...
					if (chunk != null) {
						for (int int7 = 0; int7 < chunk.vehicles.size(); ++int7) {
							BaseVehicle baseVehicle = (BaseVehicle)chunk.vehicles.get(int7);
							if ((int)this.getZ() == (int)baseVehicle.getZ() && (!(this instanceof IsoPlayer) || !((IsoPlayer)this).isLocalPlayer() || baseVehicle.getTargetAlpha(((IsoPlayer)this).PlayerIndex) != 0.0F) && !(this.DistToSquared((float)((int)baseVehicle.x), (float)((int)baseVehicle.y)) >= 16.0F)) {
								return baseVehicle;
							}
						}
					}
	...
	public boolean isHeavyItem(InventoryItem inventoryItem) {
		if (inventoryItem == null) {
			return false;
		} else if (inventoryItem.getItemReplacementSecondHand() != null) {
			return true;
		} else if (!inventoryItem.getType().equals("CorpseMale") && !inventoryItem.getType().equals("CorpseFemale")) {
	...
	public boolean shouldWaitToStartTimedAction() {
		if (this.isSitOnGround()) {
			...
			if (liveAnimNode == null || !liveAnimNode.isMainAnimActive()) {
				return true;
			}
		}
	...
	public void playBloodSplatterSound() {
		...
		this.getEmitter().playSound("BloodSplatter");
	}
	...
	public class XP {
		...
		public void AddXP(PerkFactory.Perk perk, float float1, boolean boolean1) {
			this.AddXP(perk, float1, boolean1, true);
		}

		public void AddXP(PerkFactory.Perk perk, float float1, boolean boolean1, boolean boolean2) {
			this.AddXP(perk, float1, boolean1, boolean2, true, false);
		}

		public void AddXP(PerkFactory.Perk perk, float float1, boolean boolean1, boolean boolean2, boolean boolean3, boolean boolean4) {
			if (!boolean4 && GameClient.bClient && this.chr instanceof IsoPlayer) {
				GameClient.instance.sendAddXpFromPlayerStatsUI((IsoPlayer)this.chr, perk, (int)float1, boolean2, false);
			}
					...
					if (boolean3) {
						...
						label184: while (true) {
							while (true) {
								...
								do {
									if (!iterator.hasNext()) {
										if (!boolean5 && !this.isSkillExcludedFromSpeedReduction(perk2.getType())) {
											float4 = 0.25F;
										}
										...
										if (IsoGameCharacter.this.Traits.Pacifist.isSet()) {
											if (perk2.getType() != PerkFactory.Perks.SmallBlade && perk2.getType() != PerkFactory.Perks.SmallBlunt && perk2.getType() != PerkFactory.Perks.Spear && perk2.getType() != PerkFactory.Perks.Maintenance && perk2.getType() != PerkFactory.Perks.Blunt && perk2.getType() != PerkFactory.Perks.Axe) {
