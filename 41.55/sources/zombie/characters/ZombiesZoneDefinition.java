	...
	public static ZombiesZoneDefinition.PickDefinition pickDefinition(int int1, int int2, int int3, boolean boolean1) {
		...
		if (square == null) {
			...
			for (int int4 = 0; int4 < arrayList.size(); ++int4) {
				...
				if ("ZombiesType".equalsIgnoreCase(zone2.type) || s_zoneMap.containsKey(zone2.type)) {
					zone = zone2;
					break;
				}
			}
			...
			if (zone == null) {
				...
				if (zZDZone == null) {
					...
					if (zZDZone.chanceToSpawn != -1) {
						int int5 = zZDZone.chanceToSpawn;
						int int6 = zZDZone.toSpawn;
						ArrayList arrayList2 = (ArrayList)IsoWorld.instance.getSpawnedZombieZone().get(zone.getName());
						if (arrayList2 == null) {
							arrayList2 = new ArrayList();
							IsoWorld.instance.getSpawnedZombieZone().put(zone.getName(), arrayList2);
						}

						if (arrayList2.contains(zone.id)) {
							zone.spawnSpecialZombies = true;
						}

						if (int6 == -1 || zone.spawnSpecialZombies == null && arrayList2.size() < int6) {
							if (Rand.Next(100) < int5) {
								zone.spawnSpecialZombies = true;
								arrayList2.add(zone.id);
							} else {
					...
					if (zone == null) {
						...
						int int7 = zZDZone.maleChance;
						int int8 = zZDZone.femaleChance;
						if (int7 > 0 && Rand.Next(100) < int7) {
							boolean1 = false;
						}

						if (int8 > 0 && Rand.Next(100) < int8) {
							boolean1 = true;
						}

						for (int int9 = 0; int9 < zZDZone.outfits.size(); ++int9) {
							ZombiesZoneDefinition.ZZDOutfit zZDOutfit = (ZombiesZoneDefinition.ZZDOutfit)zZDZone.outfits.get(int9);
						...
						if (!arrayList3.isEmpty()) {
							zZDOutfit2 = (ZombiesZoneDefinition.ZZDOutfit)PZArrayUtil.pickRandom((List)arrayList3);
						} else {
							zZDOutfit2 = getRandomOutfitInSetList(arrayList4, true);
						}
