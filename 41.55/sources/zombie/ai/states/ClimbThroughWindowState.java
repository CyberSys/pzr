...
public final class ClimbThroughWindowState extends State {
	...
	public void execute(IsoGameCharacter gameCharacter) {
		...
		if (!this.isWindowClosing(gameCharacter)) {
			...
			int int1;
			int int2;
			if (!gameCharacter.getVariableBoolean("ClimbWindowStarted") || !"back".equals(string) && !"fallback".equals(string)) {
				if (gameCharacter.getVariableBoolean("ClimbWindowStarted")) {
					int1 = (Integer)hashMap.get(PARAM_END_X);
					int2 = (Integer)hashMap.get(PARAM_END_Y);
					boolean1 = this.isPastInnerEdgeOfSquare(gameCharacter, int1, int2, directions);
				}
			} else {
				int1 = (Integer)hashMap.get(PARAM_START_X);
				int2 = (Integer)hashMap.get(PARAM_START_Y);
				boolean1 = this.isPastInnerEdgeOfSquare(gameCharacter, int1, int2, directions.RotLeft(4));
			}

			gameCharacter.setCollidable(boolean1);
			int int3;
			int int4;
			if (gameCharacter instanceof IsoZombie) {
				boolean boolean2 = hashMap.get(PARAM_ZOMBIE_ON_FLOOR) == Boolean.TRUE;
				if (!gameCharacter.isFallOnFront() && boolean2) {
					int2 = (Integer)hashMap.get(PARAM_OPPOSITE_X);
					int3 = (Integer)hashMap.get(PARAM_OPPOSITE_Y);
					int4 = (Integer)hashMap.get(PARAM_Z);
				...
				gameCharacter.setOnFloor(boolean2);
				((IsoZombie)gameCharacter).bKnockedDown = boolean2;
				gameCharacter.setFallOnFront(boolean2);
			}
			...
			if (gameCharacter instanceof IsoPlayer && !gameCharacter.getVariableBoolean("ClimbWindowEnd") && !"fallfront".equals(string) && !"back".equals(string) && !"fallback".equals(string)) {
				int3 = (Integer)hashMap.get(PARAM_OPPOSITE_X);
				int4 = (Integer)hashMap.get(PARAM_OPPOSITE_Y);
				int int5 = (Integer)hashMap.get(PARAM_Z);
				IsoGridSquare square2 = IsoWorld.instance.CurrentCell.getGridSquare(int3, int4, int5);
				if (square2 != null) {
	...
	public void exit(IsoGameCharacter gameCharacter) {
		...
		if (gameCharacter instanceof IsoZombie) {
			...
			((IsoZombie)gameCharacter).bKnockedDown = false;
		}
