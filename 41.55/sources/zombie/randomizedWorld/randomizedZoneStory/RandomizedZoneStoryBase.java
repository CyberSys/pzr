...
public class RandomizedZoneStoryBase extends RandomizedWorldBase {
	...
	public static void initAllRZSMapChance(IsoMetaGrid.Zone zone) {
		...
		for (int int1 = 0; int1 < IsoWorld.instance.getRandomizedZoneList().size(); ++int1) {
			...
			if (randomizedZoneStoryBase.isValid(zone, false) && randomizedZoneStoryBase.isTimeValid(false)) {
				totalChance += ((RandomizedZoneStoryBase)IsoWorld.instance.getRandomizedZoneList().get(int1)).chance;
				rzsMap.put((RandomizedZoneStoryBase)IsoWorld.instance.getRandomizedZoneList().get(int1), ((RandomizedZoneStoryBase)IsoWorld.instance.getRandomizedZoneList().get(int1)).chance);
			}
		}
	}
	...
	private static boolean doRandomStory(IsoMetaGrid.Zone zone) {
		...
		if (Rand.Next(100) < byte1) {
			...
			if (randomizedZoneStoryBase == null) {
				...
				int int1 = zone.x;
				int int2 = zone.y;
				int int3 = zone.x + zone.w - randomizedZoneStoryBase.minZoneWidth / 2;
				int int4 = zone.y + zone.h - randomizedZoneStoryBase.minZoneHeight / 2;
		...
		for (int int5 = int1; int5 < int3; ++int5) {
			for (int int6 = int2; int6 < int4; ++int6) {
				...
				if (square != null) {
					...
					for (int7 = 0; int7 < square.getSpecialObjects().size(); ++int7) {
						square.RemoveTileObject((IsoObject)square.getSpecialObjects().get(int7));
					}

					for (int7 = 0; int7 < square.getDeadBodys().size(); ++int7) {
						square.removeCorpse((IsoDeadBody)square.getDeadBodys().get(int7), false);
					}
	...
	public static enum ZoneType {
		...
		private static RandomizedZoneStoryBase.ZoneType[] $values() {
			return new RandomizedZoneStoryBase.ZoneType[]{Forest, Beach, Lake};
		}
	}
}
