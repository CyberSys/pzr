...
public final class PathFindBehavior2 implements PolygonalMap2.IPathfinder {
	...
	public void setTargetT(int int1) {
		this.targetT = int1;
	}
			...
			if (state != ClimbOverFenceState.instance() && state != ClimbThroughWindowState.instance()) {
				if (this.chr.getVehicle() != null) {
					...
					if (GameClient.bClient) {
						...
						if (this.targetT != -1 && this.targetT > int1) {
							long long1 = (long)int1 + (long)(this.getPathLength() / (float1 * 0.05F));
							float2 = (float)(long1 - (long)int1) / (float)(this.targetT - int1);
							float2 = Math.max(0.6F, Math.min(1.5F, float2));
						}

						NetworkPlayerAI networkPlayerAI = IsoPlayer.getInstance().networkAI;
						float float3 = float2 * NetworkPlayerAI.controlQuality + 1.0F;
						networkPlayerAI = IsoPlayer.getInstance().networkAI;
						float2 = float3 - NetworkPlayerAI.controlQuality;
						float1 *= float2;
						this.pathNextSpeedMul = float2;
					}
					...
					float float5 = IsoUtils.DistanceTo(this.pathNextX, this.pathNextY, this.chr.x, this.chr.y);
					if (float4 >= float5) {
					...
					if (zombie != null) {
						this.checkCrawlingTransition(pathNode, pathNode2, float5);
					}

					if (zombie == null && float5 >= 0.5F) {
	...
	public float calcSpeedMul(float float1, float float2, int int1, float float3) {
		if (int1 == -1) {
			return 1.0F;
		} else {
			float float4 = 1.0F;
			long long1 = GameTime.getServerTime() / 1000000L;
			if (int1 != 0 && (long)int1 > long1 + 10L && float3 > 1.0E-4F) {
				float float5 = IsoUtils.DistanceTo(float1, float2, this.chr.x, this.chr.y) / (float3 * 0.05F);
				float4 = float5 / (float)((long)int1 - long1);
			}

			float4 = Math.max(0.6F, Math.min(1.5F, float4));
			return float4;
		}
	}
