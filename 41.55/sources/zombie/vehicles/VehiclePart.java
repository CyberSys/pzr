...
public final class VehiclePart implements ChatElementOwner,WaveSignalDevice {
	...
	public void setInventoryItem(InventoryItem inventoryItem, int int1) {
		...
		if (this.scriptPart != null && this.scriptPart.itemType != null && !this.scriptPart.itemType.isEmpty() && this.scriptPart.models != null) {
			for (int int2 = 0; int2 < this.scriptPart.models.size(); ++int2) {
				VehicleScript.Model model = (VehicleScript.Model)this.scriptPart.models.get(int2);
				this.vehicle.setModelVisible(this, model, inventoryItem != null);
			}
		}
	...
	public void repair() {
		...
		if (this.getItemType() != null && !this.getItemType().isEmpty() && this.getInventoryItem() == null) {
			...
			if (string != null && !string.isEmpty()) {
				...
				if (inventoryItem != null) {
					...
					int1 = this.getWheelIndex();
					if (int1 != -1) {
						this.vehicle.setTireRemoved(int1, false);
		...
		if ("Engine".equalsIgnoreCase(this.getId())) {
			byte byte1 = 100;
			int int2 = (int)((double)vehicleScript.getEngineLoudness() * SandboxOptions.getInstance().ZombieAttractionMultiplier.getValue());
			int1 = (int)vehicleScript.getEngineForce();
			this.vehicle.setEngineFeature(byte1, int2, int1);
