...
public final class DeviceData implements Cloneable {
	...
	private void sendDeviceDataStatePacket(UdpConnection udpConnection, short short1) throws IOException {
			...
			case 10: 
				if (GameClient.bClient) {
					...
					if (mediaData != null && int1 >= 0 && int1 < mediaData.getLineCount()) {
						...
						String string3 = null;
						if (!recordedMedia.hasListenedLineAndAdd(mediaLineData.getTextGuid())) {
							string3 = mediaLineData.getCodes();
						}

						this.parent.AddDeviceText(string2, color.r, color.g, color.b, string3, 0);
					}
				}
						...
						if (this.lineCounter <= 0.0F) {
							...
							if (mediaLineData != null) {
								...
								if (GameServer.bServer) {
									...
									String string2 = null;
									if (!recordedMedia.hasListenedLineAndAdd(mediaLineData.getTextGuid())) {
										string2 = mediaLineData.getCodes();
									}

									this.parent.AddDeviceText(string, color.r, color.g, color.b, string2, 0);
								}
