			...
			if (this.airCounter < 0.0F) {
				...
				if (radioLine == null) {
					...
					if (!ZomboidRadio.DISABLE_BROADCASTING) {
						ZomboidRadio.getInstance().SendTransmission(0, 0, this.frequency, radioLine.getText(), radioLine.getEffectsString(), radioLine.getR(), radioLine.getG(), radioLine.getB(), -1, this.isTv);
					}
