...
public final class ZomboidRadio {
			...
			if (radio != null && radio.getDeviceData() != null && radio.getDeviceData().getIsPortable() && radio.getDeviceData().getIsTurnedOn() && radio.getDeviceData().getChannel() == int3) {
				...
				if (boolean2) {
					if (int4 > 0) {
						...
						string = this.doDeviceRangeDistortion(string, int4, int5);
					}

					if (!this.hasAppliedRangeDistortion) {
						radio.AddDeviceText(string, float1, float2, float3, string2, int5);
					} else {
						radio.AddDeviceText(string, 0.5F, 0.5F, 0.5F, string2, int5);
					}
				}
			}
		}
	...
	private void DistributeTransmission(int int1, int int2, int int3, String string, String string2, float float1, float float2, float float3, int int4, boolean boolean1) {
		...
		if (!boolean1) {
			if (!GameClient.bClient && !GameServer.bServer) {
				...
				while (iterator.hasNext()) {
					Entry entry = (Entry)iterator.next();
					this.DistributeToPlayer((IsoPlayer)entry.getValue(), int1, int2, int3, string, string2, float1, float2, float3, int4, boolean1);
				}
			}
		}
					...
					if (int3 == waveSignalDevice.getDeviceData().getChannel()) {
						...
						if (boolean2) {
							...
							if (int4 > 0) {
								...
								string = this.doDeviceRangeDistortion(string, int4, int6);
							}

							if (!this.hasAppliedRangeDistortion) {
								waveSignalDevice.AddDeviceText(string, float1, float2, float3, string2, int6);
							} else {
								waveSignalDevice.AddDeviceText(string, 0.5F, 0.5F, 0.5F, string2, int6);
							}
						}
					}
				}
	...
	public void SendTransmission(int int1, int int2, int int3, String string, String string2, float float1, float float2, float float3, int int4, boolean boolean1) {
		...
		if (gameMode.equals(GameMode.SinglePlayer)) {
			...
			GameServer.sendIsoWaveSignal(int1, int2, int3, string, string2, float1, float2, float3, int4, boolean1);
