...
public class IsoMannequin extends IsoObject implements IHumanVisual {
	...
	private void validateSkinTexture() {
		String string = this.humanVisual.getSkinTexture();
		if (this.bFemale) {
			if ("F_Mannequin_Black".equals(string)) {
				return;
			}

			if ("F_Mannequin_White".equals(string)) {
				return;
			}
		} else {
			if ("M_Mannequin_Black".equals(string)) {
				return;
			}

			if ("M_Mannequin_White".equals(string)) {
				return;
			}
		}
	...
	private void validatePose() {
		AnimationSet animationSet = AnimationSet.GetAnimationSet("mannequin", false);
		AnimState animState = animationSet.GetState(this.bFemale ? "female" : "male");
		Iterator iterator = animState.m_Nodes.iterator();
		AnimNode animNode;
		do {
			if (!iterator.hasNext()) {
				if (animState.m_Nodes == null) {
					DebugLog.log("ERROR: mannequin AnimSet \"" + animState.m_Name + "\" node \"" + this.pose + "\" doesn\'t exist");
					this.pose = "Invalid";
					return;
				}
	...
	public void render(float float1, float float2, float float3, ColorInfo colorInfo, boolean boolean1, boolean boolean2, Shader shader) {
		...
		if (this.bAnimate) {
			...
			if (perPlayer.atlasTex != null) {
				...
				Texture texture = perPlayer.atlasTex;
				texture.render((float)((int)this.screenX - texture.getWidth() / 2), (float)((int)this.screenY - texture.getHeight() / 2), (float)texture.getWidth(), (float)texture.getHeight(), colorInfo.r, colorInfo.g, colorInfo.b, this.getAlpha(int1), (Consumer)null);
				if (Core.bDebug) {
				}
			}
		}
	...
	private void initOutfit() {
		if (!this.bInit) {
			...
			String string = "White";
			String string2 = this.sprite.name;
			byte byte1 = -1;
			switch (string2.hashCode()) {
			case 1420407857: 
				if (string2.equals("location_shop_mall_01_65")) {
					byte1 = 0;
				}
			...
			case 1420407858: 
				if (string2.equals("location_shop_mall_01_66")) {
					byte1 = 1;
				}
			...
			case 1420407859: 
				if (string2.equals("location_shop_mall_01_67")) {
					byte1 = 2;
				}
			...
			case 1420407860: 
				if (string2.equals("location_shop_mall_01_68")) {
					byte1 = 3;
				}
			...
			case 1420407861: 
				if (string2.equals("location_shop_mall_01_69")) {
					byte1 = 4;
				}
			...
			case 1420407883: 
				if (string2.equals("location_shop_mall_01_70")) {
					byte1 = 5;
				}
			...
			case 1420407886: 
				if (string2.equals("location_shop_mall_01_73")) {
					byte1 = 6;
				}
			...
			case 1420407887: 
				if (string2.equals("location_shop_mall_01_74")) {
					byte1 = 7;
				}
			...
			case 1420407888: 
				if (string2.equals("location_shop_mall_01_75")) {
					byte1 = 8;
				}
			...
			case 1420407889: 
				if (string2.equals("location_shop_mall_01_76")) {
					byte1 = 9;
				}
			...
			case 1420407890: 
				if (string2.equals("location_shop_mall_01_77")) {
					byte1 = 10;
				}
			...
			case 1420407891: 
				if (string2.equals("location_shop_mall_01_78")) {
					byte1 = 11;
				}
			...
			ArrayList arrayList = metaCell == null ? null : metaCell.mannequinZones;
			if (arrayList != null) {
			...
			if (this.pose == null) {
				AnimationSet animationSet = AnimationSet.GetAnimationSet("mannequin", false);
				AnimState animState = animationSet.GetState(this.bFemale ? "female" : "male");
				AnimNode animNode = (AnimNode)PZArrayUtil.pickRandom(animState.m_Nodes);
			...
			String string3 = this.bFemale ? "F_Mannequin_" : "M_Mannequin_";
			this.humanVisual.setSkinTextureName(string3 + string);
