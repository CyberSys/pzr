		...
		if (this.item == null) {
			...
			if (!this.item.canStoreWater()) {
				...
				if (this.item.getReplaceOnUseOn() != null && this.item.getReplaceOnUseOnString() != null) {
					Item item = ScriptManager.instance.getItem(this.item.getReplaceOnUseOnString());
	...
	public void setWaterAmount(int int1) {
		if (this.isWaterSource()) {
			...
			if (drainableComboItem != null) {
				...
				inventoryItem = InventoryItemFactory.CreateItem(this.getItem().getReplaceOnUseOnString());
				if (inventoryItem != null) {
					inventoryItem.setCondition(this.getItem().getCondition());
					inventoryItem.setFavorite(this.getItem().isFavorite());
