...
public final class IsoDeadBody extends IsoMovingObject implements Talker,IHumanVisual {
	...
	public IsoDeadBody(IsoGameCharacter gameCharacter, boolean boolean1) {
		...
		if (square != null) {
			...
			if (gameCharacter instanceof IsoZombie) {
				if ((GameServer.bServer || GameClient.bClient) && zombie != null && zombie.networkAI != null && zombie.networkAI.deadZombie != null) {
					this.x = zombie.networkAI.deadZombie.x;
					this.y = zombie.networkAI.deadZombie.y;
					this.z = zombie.networkAI.deadZombie.z;
					this.dir = zombie.networkAI.deadZombie.direction;
					this.m_angle = zombie.networkAI.deadZombie.angle;
					this.fallOnFront = zombie.networkAI.deadZombie.isFallOnFront;
					this.bCrawling = zombie.networkAI.deadZombie.isCrawling;
					if (this.bCrawling) {
						this.fallOnFront = true;
					}
	...
	public int getOnlineId() {
		return this.player != null ? this.player.getOnlineID() : -1;
	}
				...
				for (int int1 = 0; int1 < AllBodies.size(); ++int1) {
					...
					if (!deadBody.updateFakeDead() && (ServerOptions.instance.RemovePlayerCorpsesOnCorpseRemoval.getValue() || deadBody.wasZombie)) {
						...
						if (!(float4 < float1 + (deadBody.isSkeleton() ? float2 : 0.0F))) {
							if (boolean1) {
								int int4 = (int)(float4 / float2);
								DebugLog.General.debugln("%s REMOVE %d -> %d age=%.2f stages=%d", deadBody, int2, int3, float4, int4);
							}
	...
	public void reanimate() {
		...
		zombie.bCrawling = this.bCrawling;
		...
		if (GameServer.bServer) {
			...
			GameServer.sendZombieUpdate(zombie);
			GameServer.removeCorpseFromMap(this);
		}
