...
public final class IsoPuddlesCompute {
	...
	private static Vector2f add = new Vector2f(1.0F, 0.0F);
	private static Vector3f add_xyy = new Vector3f(1.0F, 0.0F, 0.0F);
	private static Vector3f add_xxy = new Vector3f(1.0F, 1.0F, 0.0F);
	private static Vector3f add_xxx = new Vector3f(1.0F, 1.0F, 1.0F);
	private static Vector3f add_xyx = new Vector3f(1.0F, 0.0F, 1.0F);
	private static Vector3f add_yxy = new Vector3f(0.0F, 1.0F, 0.0F);
	private static Vector3f add_yyx = new Vector3f(0.0F, 0.0F, 1.0F);
	private static Vector3f add_yxx = new Vector3f(0.0F, 1.0F, 1.0F);
	private static Vector3f HashVector31 = new Vector3f(17.1F, 31.7F, 32.6F);
	private static Vector3f HashVector32 = new Vector3f(29.5F, 13.3F, 42.6F);

	private static Vector3f floor(Vector3f vector3f) {
		return new Vector3f((float)Math.floor((double)vector3f.x), (float)Math.floor((double)vector3f.y), (float)Math.floor((double)vector3f.z));
	}
	...
	private static float FuncHash(Vector3f vector3f) {
		Vector3f vector3f2 = new Vector3f(vector3f.dot(HashVector31), vector3f.dot(HashVector32), 0.0F);
	...
	private static float FuncNoise(Vector3f vector3f) {
		...
		Vector3f vector3f4 = new Vector3f(vector3f3.x * vector3f3.x * (4.5F - 3.5F * vector3f3.x), vector3f3.y * vector3f3.y * (4.5F - 3.5F * vector3f3.y), vector3f3.z * vector3f3.z * (4.5F - 3.5F * vector3f3.z));
		float float1 = mix(FuncHash(vector3f2), FuncHash((new Vector3f(vector3f2)).add(add_xyy)), vector3f4.x);
		float float2 = mix(FuncHash((new Vector3f(vector3f2)).add(add_yxy)), FuncHash((new Vector3f(vector3f2)).add(add_xxy)), vector3f4.x);
		float float3 = mix(FuncHash((new Vector3f(vector3f2)).add(add_yyx)), FuncHash((new Vector3f(vector3f2)).add(add_xyx)), vector3f4.x);
		float float4 = mix(FuncHash((new Vector3f(vector3f2)).add(add_yxx)), FuncHash((new Vector3f(vector3f2)).add(add_xxx)), vector3f4.x);
	...
	private static float PerlinNoise(Vector3f vector3f) {
		if (hd_quality) {
			...
			float1 = (float)((double)float1 * Math.min(1.0, 2.0 * (double)FuncNoise((new Vector3f(vector3f)).mul(0.02F)) * Math.min(1.0, 1.0 * (double)FuncNoise((new Vector3f(vector3f)).mul(0.1F)))));
			return float1;
		} else {
	...
	private static float getPuddles(Vector2f vector2f) {
		...
		float4 = (float)((double)float4 + (double)float1 * Math.sin(((double)vector2f.x * 1.0 + (double)vector2f.y * 2.0) * (double)Pi * 1.0) * Math.cos(((double)vector2f.x * 1.0 + (double)vector2f.y * 2.0) * (double)Pi * 1.0) * 2.0);
		float4 = (float)((double)float4 + (double)float2 * Math.sin(((double)vector2f.x * 1.0 - (double)vector2f.y * 2.0) * (double)Pi * 1.0) * Math.cos(((double)vector2f.x * 1.0 - (double)vector2f.y * 2.0) * (double)Pi * 1.0) * 2.0);
		float4 = (float)((double)float4 + (double)float3 * 0.3);
		float float5 = PerlinNoise(new Vector3f(vector2f.x * 1.0F, 0.0F, vector2f.y * 2.0F));
		float float6 = Math.min(0.7F, float4 * float5);
		float5 = Math.min(0.7F, PerlinNoise(new Vector3f(vector2f.x * 0.7F, 1.0F, vector2f.y * 0.7F)));
	...
	public static float computePuddle(IsoGridSquare square) {
		...
		if (!Core.getInstance().getUseShaders()) {
			...
			if (Core.getInstance().getPerfPuddles() > 0 && square.z > 0) {
				...
				Vector2f vector2f = new Vector2f((float1 * vector4f.z + vector4f.x) * 8.0E-4F + (float)square.z * 7.0F, (float2 * vector4f.w + vector4f.y) * 8.0E-4F + (float)square.z * 7.0F);
