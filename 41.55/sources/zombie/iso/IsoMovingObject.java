...
public class IsoMovingObject extends IsoObject implements Mover {
		...
		if (!this.destroyed) {
			...
			if (!GameClient.bClient && this.isZombie() && (int)this.z == 0 && this.getCurrentBuilding() == null && !this.isInLoadedArea((int)this.nx, (int)this.ny) && (((IsoZombie)this).getCurrentState() == PathFindState.instance() || ((IsoZombie)this).getCurrentState() == WalkTowardState.instance())) {
				...
				if (this instanceof IsoGameCharacter && !this.isOnFloor() && ((IsoGameCharacter)this).getVehicle() == null && this.isCollidable() && (!(this instanceof IsoPlayer) || !((IsoPlayer)this).isNoClip())) {
					Vector2f vector2f = PolygonalMap2.instance.resolveCollision((IsoGameCharacter)this, this.nx, this.ny, IsoMovingObject.L_postUpdate.vector2f);
					if (vector2f.x != this.nx || vector2f.y != this.ny) {
						if (this instanceof IsoPlayer && !((IsoPlayer)this).isLocalPlayer()) {
							float1 = GameTime.getInstance().getMultiplier() / 5.0F;
							this.nx += (vector2f.x - this.nx) * float1;
							this.ny += (vector2f.y - this.ny) * float1;
						} else {
							this.nx = vector2f.x;
							this.ny = vector2f.y;
						}
				...
				if (player != null && player.isLocalPlayer() && !player.isCurrentState(CollideWithWallState.instance()) && !this.collidedN && !this.collidedS && !this.collidedW && !this.collidedE) {
					this.setCollideType((String)null);
				}

				float4 = this.nx - this.x;
				float float6 = this.ny - this.y;
				float float7 = !(Math.abs(float4) > 0.0F) && !(Math.abs(float6) > 0.0F) ? 0.0F : this.getGlobalMovementMod();
				if (Math.abs(float4) > 0.01F || Math.abs(float6) > 0.01F) {
					float4 *= float7;
					float6 *= float7;
				}
		...
		if (GameClient.bClient && (this instanceof IsoZombie || this instanceof IsoPlayer)) {
			...
			if (!this.destroyed) {
				...
				if (!(this instanceof IsoZombie) || (int)this.z != 0 || this.getCurrentBuilding() != null || this.isInLoadedArea((int)this.nx, (int)this.ny) || ((IsoZombie)this).getCurrentState() != PathFindState.instance() && ((IsoZombie)this).getCurrentState() != WalkTowardState.instance()) {
					...
					this.checkHitWall();
					IsoPlayer player2 = (IsoPlayer)Type.tryCastTo(this, IsoPlayer.class);
					if (player2 != null && !player2.isCurrentState(CollideWithWallState.instance()) && !this.collidedN && !this.collidedS && !this.collidedW && !this.collidedE) {
						this.setCollideType((String)null);
					}

					float6 = this.nx - this.x;
					float float8 = this.ny - this.y;
					float float9 = !(Math.abs(float6) > 0.0F) && !(Math.abs(float8) > 0.0F) ? 0.0F : this.getGlobalMovementMod();
					if (Math.abs(float6) > 0.01F || Math.abs(float8) > 0.01F) {
						float6 *= float9;
						float8 *= float9;
					}
						...
						for (int int2 = 0; int2 < square.getMovingObjects().size(); ++int2) {
							...
							if (movingObject != this && movingObject.isSolidForSeparate() && !(Math.abs(this.z - movingObject.z) > 0.3F)) {
								...
								if (gameCharacter2 != null) {
									if (float2 < float1 + 16.0F && player != null && player.getBumpedChr() != movingObject && player.getBeenSprintingFor() >= 70.0F && WeaponType.getWeaponType((IsoGameCharacter)player) == WeaponType.spear) {
									...
									if (!(float2 >= float1)) {
										if (player != null && player.getVariableFloat("WalkSpeed", 0.0F) > 0.2F && player.runningTime > 0.5F && player.getBumpedChr() != movingObject && !"charge".equals(player.getAttackType())) {
											boolean boolean1 = !this.isOnFloor() && (gameCharacter.getBumpedChr() != null || (System.currentTimeMillis() - player.getLastBump()) / 100L < 15L || player.isSprinting()) && (player2 == null || !player2.isNPC());
											if (boolean1) {
												++gameCharacter.bumpNbr;
												int int3 = 10 - gameCharacter.bumpNbr * 3;
												int3 += gameCharacter.getPerkLevel(PerkFactory.Perks.Fitness);
												int3 += gameCharacter.getPerkLevel(PerkFactory.Perks.Strength);
												...
												if (Rand.Next(int3) == 0 || gameCharacter.isSprinting()) {
											...
											boolean boolean2 = gameCharacter.isBehind(gameCharacter2);
											...
											gameCharacter2.setHitFromBehind(boolean2);
											if (boolean1) {
	...
	private void slideAwayFromWalls() {
		if (this.current != null) {
			...
			if (zombie != null && (this.isOnFloor() || zombie.bKnockedDown)) {
				if (!zombie.isCrawling() || zombie.getPath2() == null && !zombie.isMoving()) {
					if (!zombie.isCurrentState(ClimbOverFenceState.instance()) && !zombie.isCurrentState(ClimbThroughWindowState.instance())) {
						if (zombie.hasAnimationPlayer() && zombie.getAnimationPlayer().isReady()) {
							...
							float float1;
							if (zombie.bKnockedDown && (zombie.isCurrentState(ZombieFallDownState.instance()) || zombie.isCurrentState(StaggerBackState.instance()))) {
	...
	private boolean DoCollide(int int1) {
		...
		if (this.current != this.last && this.last != null && this.current != null) {
			...
			IsoGameCharacter gameCharacter2 = (IsoGameCharacter)Type.tryCastTo(this, IsoGameCharacter.class);
			if (gameCharacter2 != null) {
				if (gameCharacter2.isClimbing()) {
					square2 = this.current;
				}

				if (square2 != null && square2 != this.current && gameCharacter2.getPath2() != null && !gameCharacter2.getPath2().crossesSquare(square2.x, square2.y, square2.z)) {
					square2 = this.current;
				}
			}

			if (square2 != null && square2 != this.current && this.current != null && this.current.testCollideAdjacent(this, square2.getX() - this.current.getX(), square2.getY() - this.current.getY(), square2.getZ() - this.current.getZ())) {
				if (this.last != null) {
			...
			if (this.current != null) {
				...
				if (player != null) {
					if (StringUtils.isNullOrEmpty(this.getCollideType())) {
						...
						if (boolean1 && player.isSprinting()) {
