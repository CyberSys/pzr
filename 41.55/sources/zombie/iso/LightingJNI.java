	...
	public static float calculateVisionCone(IsoGameCharacter gameCharacter) {
		...
		if (gameCharacter.getVehicle() == null) {
			...
			if (gameCharacter.getVehicle().getHeadlightsOn() && gameCharacter.getVehicle().getHeadlightCanEmmitLight()) {
				float1 = 1.5F - 3.0F * (1.0F - ClimateManager.getInstance().getDayLightStrength());
				if (float1 < -0.8F) {
					float1 = -0.8F;
				}
			} else {
	...
	public static final class JNILighting implements IsoGridSquare.ILighting {
		...
		private void checkRoomSeen() {
			...
			if (room != null && room.def != null && !room.def.bExplored) {
				IsoPlayer player = IsoPlayer.players[this.playerIndex];
				if (player != null) {
					if (this.square.z == (int)player.z) {
						byte byte1 = 10;
						if (player.getBuilding() == room.building) {
							byte1 = 50;
						}

						if (IsoUtils.DistanceToSquared(player.x, player.y, (float)this.square.x + 0.5F, (float)this.square.y + 0.5F) < (float)(byte1 * byte1)) {
