...
public class ServerGUI {
	...
	public static void init() {
		...
		try {
			...
			byte byte1 = 2;
			GameWindow.LoadTexturePack("UI", byte1);
			GameWindow.LoadTexturePack("UI2", byte1);
			GameWindow.LoadTexturePack("IconsMoveables", byte1);
			GameWindow.LoadTexturePack("RadioIcons", byte1);
			GameWindow.LoadTexturePack("ApComUI", byte1);
			GameWindow.LoadTexturePack("WeatherFx", byte1);
			TexturePackPage.bIgnoreWorldItemTextures = false;
			byte1 = 0;
			GameWindow.LoadTexturePack("Tiles2x", byte1);
			GameWindow.LoadTexturePack("JumboTrees2x", byte1);
			GameWindow.LoadTexturePack("Overlays2x", byte1);
	...
	public static void update() {
		if (created) {
			...
			int int1 = Mouse.getWheelState();
			if (int1 != 0) {
				int int2 = int1 - 0 < 0 ? 1 : -1;
				Core.getInstance().doZoomScroll(0, int2);
			}
			...
			if (player == null) {
				...
				IsoPlayer.players[byte1] = player;
	...
	private static void updateCamera(IsoPlayer player) {
		byte byte1 = 0;
		PlayerCamera playerCamera = IsoCamera.cameras[byte1];
		float float1 = IsoUtils.XToScreen(player.x + playerCamera.DeferedX, player.y + playerCamera.DeferedY, player.z, 0);
		float float2 = IsoUtils.YToScreen(player.x + playerCamera.DeferedX, player.y + playerCamera.DeferedY, player.z, 0);
		...
		playerCamera.OffX = float1;
		playerCamera.OffY = float2;
	...
	private static int renderFloor(IsoGridSquare square) {
		...
		for (int int2 = 0; int2 < square.getObjects().size(); ++int2) {
			...
			if (boolean1) {
				...
				object.render((float)square.x, (float)square.y, (float)square.z, defColorInfo, true, false, (Shader)null);
			...
			if ((!boolean2 || object.sprite == null || object.sprite.Properties.Is(IsoFlagType.canBeRemoved) || object.sprite.Properties.Is(IsoFlagType.attachedFloor)) && (boolean2 || object.sprite == null || !object.sprite.Properties.Is(IsoFlagType.canBeRemoved) && !object.sprite.Properties.Is(IsoFlagType.attachedFloor))) {
				...
				if (boolean5) {
					...
					if (object.sprite != null && !square.getProperties().Is(IsoFlagType.blueprint) && (object.sprite.getType() == IsoObjectType.doorFrW || object.sprite.getType() == IsoObjectType.doorFrN || object.sprite.getType() == IsoObjectType.doorW || object.sprite.getType() == IsoObjectType.doorN || object.sprite.getProperties().Is(IsoFlagType.cutW) || object.sprite.getProperties().Is(IsoFlagType.cutN)) && PerformanceSettings.LightingFrameSkip < 3) {
						if (object.getTargetAlpha(int3) < 1.0F) {
							...
							object.setAlphaAndTarget(int3, 1.0F);
						}
						...
						object.setTargetAlpha(int3, 1.0F);
						if (IsoCamera.CamCharacter != null && object.getProperties() != null && (object.getProperties().Is(IsoFlagType.solid) || object.getProperties().Is(IsoFlagType.solidtrans))) {
							...
							if (int8 > 0 && int8 < 3 && int9 >= 0 && int9 < 3 || int9 > 0 && int9 < 3 && int8 >= 0 && int8 < 3) {
								object.setTargetAlpha(int3, 0.99F);
							}
						}

						if (object instanceof IsoWindow && object.getTargetAlpha(int3) < 1.0E-4F) {
							...
							if (square3 != null && square3 != square && square3.lighting[int3].bSeen()) {
								object.setTargetAlpha(int3, square3.lighting[int3].darkMulti() * 2.0F);
							}
						}
	...
	private static void renderCharacters(IsoGridSquare square, boolean boolean1) {
		...
		for (int2 = 0; int2 < int1; ++int2) {
			...
			if (movingObject.sprite != null && (!boolean1 || movingObject instanceof IsoDeadBody) && (boolean1 || !(movingObject instanceof IsoDeadBody))) {
				movingObject.render(movingObject.getX(), movingObject.getY(), movingObject.getZ(), defColorInfo, true, false, (Shader)null);
