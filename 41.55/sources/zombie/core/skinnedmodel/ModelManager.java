...
public final class ModelManager {
	...
	public void create() {
		if (!this.m_bCreated) {
			...
			ModelMesh modelMesh = (ModelMesh)MeshAssetManager.instance.load(new AssetPath("skinned/malebody"), meshAssetParams);
			modelMesh.m_animationsMesh = modelMesh;
			this.m_modAnimations.put(this.m_gameAnimations.m_modID, this.m_gameAnimations);
			while (modelMesh.isEmpty()) {
			...
			try {
				this.loadAnimsFromDir("media/anims/", modelMesh);
				this.loadAnimsFromDir("media/anims_X/", modelMesh);
	...
	public Model getLoadedModel(String string) {
		...
		if (modelScript != null) {
			if (modelScript.loadedModel != null) {
				...
				Model model = modelScript.loadedModel = this.tryGetLoadedModel(modelScript.getMeshName(), modelScript.getTextureName(), modelScript.bStatic, modelScript.getShaderName(), false);
				if (model != null) {
					return model;
				} else {
					model = modelScript.bStatic ? this.loadModelInternal(modelScript.getMeshName(), modelScript.getTextureName(), modelScript.getShaderName(), (ModelMesh)null, true) : this.loadModelInternal(modelScript.getMeshName(), modelScript.getTextureName(), modelScript.getShaderName(), (ModelMesh)null, false);
					return model;
				}
			}
		} else {
		...
		for (int int1 = 0; int1 < arrayList.size(); ++int1) {
			...
			if (mod != null && mod.animsXFile.isDirectory()) {
				...
				if (modAnimations2 != null) {
					...
					this.loadAnimsFromDir(mod.baseFile.toURI(), mod.mediaFile.toURI(), mod.animsXFile, this.m_animModel, modAnimations2);
				}
			}
		}
	...
	class AnimDirReloader implements PredicatedFileWatcher.IPredicatedFileWatcherCallback {
		...
		private boolean IsInDir(String string) {
			...
			try {
				if (this.m_dirSecondary == null) {
					return string.startsWith(this.m_dir);
				} else {
					return string.startsWith(this.m_dir) || string.startsWith(this.m_dirSecondary);
				}
			} catch (Exception exception) {
				exception.printStackTrace();
				return false;
		...
		public void call(String string) {
			...
			if (string2.endsWith(".fbx") || string2.endsWith(".x") || string2.endsWith(".txt")) {
				String string3 = ZomboidFileSystem.instance.getAnimName(this.m_mediaURI, new File(new File(this.m_baseURI), string));
