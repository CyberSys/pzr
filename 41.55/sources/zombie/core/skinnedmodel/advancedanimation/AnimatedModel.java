...
public final class AnimatedModel extends AnimationVariableSource implements IAnimatable,IAnimEventCallback,IActionStateChanged,IHumanVisual {
	...
	private void initOutfit() {
		...
		if (!StringUtils.isNullOrWhitespace(string)) {
			...
			this.humanVisual.dressInNamedOutfit(string, this.itemVisuals);
	...
	private void populateCharacterModelSlot() {
		...
		for (int1 = this.humanVisual.getBodyVisuals().size() - 1; int1 >= 0; --int1) {
			itemVisual = (ItemVisual)this.humanVisual.getBodyVisuals().get(int1);
			clothingItem = itemVisual.getClothingItem();
		...
		Lambda.forEachFrom(PZArrayUtil::forEach, (List)this.modelInstance.sub, this.modelInstance, (var0,characterMaskx)->{
			var0.AnimPlayer = characterMaskx.AnimPlayer;
		});
	}

	private void addHeadHair(ItemVisual itemVisual) {
		ImmutableColor immutableColor = this.humanVisual.getHairColor();
		ImmutableColor immutableColor2 = this.humanVisual.getBeardColor();
		HairStyle hairStyle;
		if (this.isFemale()) {
			hairStyle = HairStyles.instance.FindFemaleStyle(this.humanVisual.getHairModel());
			...
			hairStyle = HairStyles.instance.FindMaleStyle(this.humanVisual.getHairModel());
			if (hairStyle != null && itemVisual != null && itemVisual.getClothingItem() != null) {
				hairStyle = HairStyles.instance.getAlternateForHat(hairStyle, itemVisual.getClothingItem().m_HatCategory);
			}
			...
			BeardStyle beardStyle = BeardStyles.instance.FindStyle(this.humanVisual.getBeardModel());
	...
	private void addHeadHairItem(String string, String string2, ImmutableColor immutableColor) {
		if (StringUtils.isNullOrWhitespace(string)) {
			...
			if (modelInstance != null) {
				this.postProcessNewItemInstance(modelInstance, immutableColor);
			}
		}
	}
	...
	private void postProcessNewItemInstance(ModelInstance modelInstance, ImmutableColor immutableColor) {
		...
		this.modelInstance.sub.add(modelInstance);
	...
	private void DoCharacterModelEquipped() {
		...
		if (!StringUtils.isNullOrWhitespace(this.primaryHandModelName)) {
			...
			this.postProcessNewItemInstance(modelInstance, ImmutableColor.white);
		}
		...
		for (int int1 = 0; int1 < this.attachedModelNames.size(); ++int1) {
			...
			ModelInstance modelInstance2 = ModelManager.instance.addStatic((ModelInstance)null, attachedModelName.modelName, attachedModelName.attachmentName, attachedModelName.attachmentName);
			this.postProcessNewItemInstance(modelInstance2, ImmutableColor.white);
	...
	public boolean isReadyToRender() {
		if (!this.animPlayer.isReady()) {
			...
			for (int int1 = 0; int1 < this.modelInstance.sub.size(); ++int1) {
				ModelInstance modelInstance = (ModelInstance)this.modelInstance.sub.get(int1);
				if (!this.isModelInstanceReady(modelInstance)) {
					return false;
				}
			}
	...
	public int renderMain() {
		...
		if (this.modelInstance != null) {
			...
			for (int1 = 0; int1 < this.modelInstance.sub.size(); ++int1) {
				ModelInstance modelInstance = (ModelInstance)this.modelInstance.sub.get(int1);
			...
			AnimatedModel.AnimatedModelInstanceRenderData animatedModelInstanceRenderData = ((AnimatedModel.AnimatedModelInstanceRenderData)instDataPool.alloc()).init(this.modelInstance);
			stateInfo.instData.add(animatedModelInstanceRenderData);
			for (int int2 = 0; int2 < this.modelInstance.sub.size(); ++int2) {
				ModelInstance modelInstance2 = (ModelInstance)this.modelInstance.sub.get(int2);
				AnimatedModel.AnimatedModelInstanceRenderData animatedModelInstanceRenderData2 = ((AnimatedModel.AnimatedModelInstanceRenderData)instDataPool.alloc()).init(modelInstance2);
		...
		for (int1 = 0; int1 < stateInfo.readyData.size(); ++int1) {
			AnimatedModel.AnimatedModelInstanceRenderData animatedModelInstanceRenderData3 = (AnimatedModel.AnimatedModelInstanceRenderData)stateInfo.readyData.get(int1);
			animatedModelInstanceRenderData3.init(animatedModelInstanceRenderData3.modelInstance);
			animatedModelInstanceRenderData3.transformToParent(stateInfo.getParentData(animatedModelInstanceRenderData3.modelInstance));
		}
	...
	public void DoRender(ModelCamera modelCamera) {
		...
		int int2;
		for (int2 = 0; int2 < this.modelInstance.sub.size(); ++int2) {
			ModelInstance modelInstance = (ModelInstance)this.modelInstance.sub.get(int2);
		...
		for (int2 = 0; int2 < stateInfo.instData.size(); ++int2) {
