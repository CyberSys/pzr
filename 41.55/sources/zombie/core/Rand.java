...
public final class Rand {
	...
	private static int Next(int int1, CellularAutomatonRNG cellularAutomatonRNG) {
		if (int1 <= 0) {
			return 0;
		} else {
			++id;
	...
	public static long Next(long long1, CellularAutomatonRNG cellularAutomatonRNG) {
		return (long)Next((int)long1, cellularAutomatonRNG);
	}
	...
	public static int AdjustForFramerate(int int1) {
		if (GameServer.bServer) {
			...
			int1 = (int)((float)int1 * ((float)PerformanceSettings.getLockFPS() / 30.0F));
		}
