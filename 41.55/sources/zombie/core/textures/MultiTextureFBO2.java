...
public final class MultiTextureFBO2 {
	...
	public void update() {
		...
		if (this.bAutoZoom[int1] && IsoCamera.CamCharacter != null && this.bZoomEnabled) {
			float1 = 1.0F;
			if (IsoCamera.CamCharacter.getCurrentSquare().getRoom() == null && ((!(IsoPlayer.getInstance().closestZombie < 6.0F) || !IsoPlayer.getInstance().isTargetedByZombie()) && !(IsoPlayer.getInstance().lastTargeted < (float)(PerformanceSettings.getLockFPS() * 4)) || IsoPlayer.getInstance().IsRunning())) {
				float1 = this.zoomedOutLevel;
			} else {
				float1 = this.zoomedInLevel;
			}

			float float2 = IsoUtils.DistanceTo(IsoCamera.getRightClickOffX(), IsoCamera.getRightClickOffY(), 0.0F, 0.0F);
