...
public final class PNGDecoder {
	...
	public void decode(ByteBuffer byteBuffer, int int1, PNGDecoder.Format format) throws IOException {
		...
		try {
			for (int int4 = 0; int4 < this.height; ++int4) {
				...
				label123: switch (this.colorType) {
				case 0: 
					switch (format) {
					case RGBA: 
						this.copyGREYtoRGBA(byteBuffer, byteArray);
						break label123;
	...
	private void copyGREYtoRGBA(ByteBuffer byteBuffer, byte[] byteArray) {
		int int1 = 1;
		for (int int2 = byteArray.length; int1 < int2; ++int1) {
