...
public class Texture extends Asset implements IDestroyable,ITexture,Serializable {
	...
	public void save(String string) {
		...
		byteBuffer = this.getData().getBuffer();
		int[] intArray = new int[int1 * int2];
		File file = new File(string);
		for (int int3 = 0; int3 < intArray.length; ++int3) {
			int int4 = int3 * 4;
			intArray[int3] = (byteBuffer.get(int4 + 3) & 255) << 24 | (byteBuffer.get(int4) & 255) << 16 | (byteBuffer.get(int4 + 1) & 255) << 8 | (byteBuffer.get(int4 + 2) & 255) << 0;
		}
