...
public class Matrix3dStack extends Matrix3d {
	...
	public Matrix3dStack(int int1) {
		if (int1 < 1) {
			throw new IllegalArgumentException("stackSize must be >= 1");
		} else {
			this.mats = new Matrix3d[int1 - 1];
			for (int int2 = 0; int2 < this.mats.length; ++int2) {
				this.mats[int2] = new Matrix3d();
			}
		}
	}
	...
	public boolean equals(Object object) {
		if (this == object) {
			...
			if (object instanceof Matrix3dStack) {
				Matrix3dStack matrix3dStack = (Matrix3dStack)object;
				if (this.curr != matrix3dStack.curr) {
					return false;
				}

				for (int int1 = 0; int1 < this.curr; ++int1) {
					if (!this.mats[int1].equals(matrix3dStack.mats[int1])) {
						return false;
					}
				}
			}
