...
public final class PathFindBehavior2 implements PolygonalMap2.IPathfinder {
   ...
   public void setTargetT(int var1) {
      this.targetT = var1;
   }
   ...
   public PathFindBehavior2.BehaviorResult update() {
      if (this.chr.getFinder().progress == AStarPathFinder.PathFindProgress.notrunning) {
         PolygonalMap2.PathFindRequest var11 = PolygonalMap2.instance.addRequest(this, this.chr, this.startX, this.startY, this.startZ, this.targetX, this.targetY, this.targetZ);
         ...
         if (var1 != ClimbOverFenceState.instance() && var1 != ClimbThroughWindowState.instance()) {
            if (this.chr.getVehicle() != null) {
               ...
               PolygonalMap2.PathNode var12;
               if (this.pathIndex == this.path.nodes.size() - 2) {
                  var12 = (PolygonalMap2.PathNode)this.path.nodes.get(this.path.nodes.size() - 1);
                  if (IsoUtils.DistanceToSquared(this.chr.x, this.chr.y, var12.x, var12.y) <= 0.0025000002F) {
               ...
               var12 = (PolygonalMap2.PathNode)this.path.nodes.get(this.pathIndex);
               ...
               if (GameClient.bClient) {
                  var7 = 1.0F;
                  int var8 = (int)(GameTime.getServerTime() / 1000000L);
                  if (this.targetT != -1 && this.targetT > var8) {
                     long var9 = (long)var8 + (long)(this.getPathLength() / (var6 * 0.05F));
                     var7 = (float)(var9 - (long)var8) / (float)(this.targetT - var8);
                     var7 = Math.max(0.6F, Math.min(1.5F, var7));
                  }

                  NetworkPlayerAI var10001 = IsoPlayer.getInstance().networkAI;
                  float var10000 = var7 * NetworkPlayerAI.controlQuality + 1.0F;
                  var10001 = IsoPlayer.getInstance().networkAI;
                  var7 = var10000 - NetworkPlayerAI.controlQuality;
                  var6 *= var7;
                  this.pathNextSpeedMul = var7;
               }
               ...
               float var14 = IsoUtils.DistanceTo(this.pathNextX, this.pathNextY, this.chr.x, this.chr.y);
               if (var13 >= var14) {
                  var6 *= var14 / var13;
                  ++this.pathIndex;
               }
               ...
               if (var2 != null) {
                  this.checkCrawlingTransition(var12, var4, var14);
               }

               if (var2 == null && var14 >= 0.5F) {
   ...
   private void updateWhileRunningPathfind() {
      if (this.pathNextIsSet) {
         this.moveToPoint(this.pathNextX, this.pathNextY, this.pathNextSpeedMul);
      }
   }

   public float calcSpeedMul(float var1, float var2, int var3, float var4) {
      if (var3 == -1) {
         return 1.0F;
      } else {
         float var5 = 1.0F;
         long var6 = GameTime.getServerTime() / 1000000L;
         if (var3 != 0 && (long)var3 > var6 + 10L && var4 > 1.0E-4F) {
            float var8 = IsoUtils.DistanceTo(var1, var2, this.chr.x, this.chr.y) / (var4 * 0.05F);
            var5 = var8 / (float)((long)var3 - var6);
         }

         var5 = Math.max(0.6F, Math.min(1.5F, var5));
         return var5;
      }
   }
