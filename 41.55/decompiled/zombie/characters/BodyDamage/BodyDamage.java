...
public final class BodyDamage {
   ...
   public boolean AddRandomDamageFromZombie(IsoZombie var1, String var2) {
      ...
      if (GameServer.bServer) {
         ...
         var5 -= (var11 - 1) * 10;
         int var20 = var6 - (var11 - 1) * 30;
         int var21 = var7 - (var11 - 1) * 15;
         byte var12 = 3;
         ...
         int var19;
         if (!var1.bCrawling) {
            var19 = Rand.Next(BodyPartType.ToIndex(BodyPartType.Hand_L), BodyPartType.ToIndex(BodyPartType.Groin) + 1);
            ...
            if (var9 && (float)Rand.Next(100) < var13) {
               var19 = BodyPartType.ToIndex(BodyPartType.Neck);
            }

            if (var19 == BodyPartType.ToIndex(BodyPartType.Head) || var19 == BodyPartType.ToIndex(BodyPartType.Neck)) {
               byte var14 = 70;
               if (var9) {
                  var14 = 90;
               }
               ...
               if (Rand.Next(100) > var14) {
                  ...
                  while(true) {
                     do {
                        ...
                        var15 = true;
                        var19 = Rand.Next(BodyPartType.ToIndex(BodyPartType.MAX));
                     } while(var19 != BodyPartType.ToIndex(BodyPartType.Head) && var19 != BodyPartType.ToIndex(BodyPartType.Neck));
            ...
            var19 = Rand.Next(BodyPartType.ToIndex(BodyPartType.UpperLeg_L), BodyPartType.ToIndex(BodyPartType.MAX));
         }
         ...
         if (Rand.Next(100) > var5) {
            var22 = true;
            this.getParentChar().helmetFall(var19 == BodyPartType.ToIndex(BodyPartType.Neck) || var19 == BodyPartType.ToIndex(BodyPartType.Head));
            if (Rand.Next(100) > var21) {
               var22 = false;
               var16 = true;
            }
            ...
            Float var18;
            if (var22) {
               var18 = this.getParentChar().getBodyPartClothingDefense(var19, false, false);
               ...
               if ((float)Rand.Next(100) < var18) {
                  this.getParentChar().addHoleFromZombieAttacks(BloodBodyPartType.FromIndex(var19), var17);
                  return false;
               }

               this.getParentChar().addHole(BloodBodyPartType.FromIndex(var19), true);
               var15 = true;
               this.AddDamage(var19, var13);
               this.SetScratched(var19, true);
               this.getParentChar().addBlood(BloodBodyPartType.FromIndex(var19), true, false, true);
               var3 = 1;
               if (GameServer.bServer && this.ParentChar instanceof IsoPlayer) {
               ...
               var18 = this.getParentChar().getBodyPartClothingDefense(var19, true, false);
               ...
               var15 = true;
               this.AddDamage(var19, var13);
               this.SetBitten(var19, true);
               if (var19 == BodyPartType.ToIndex(BodyPartType.Neck)) {
                  this.getParentChar().addBlood(BloodBodyPartType.FromIndex(var19), false, true, true);
                  this.getParentChar().addBlood(BloodBodyPartType.FromIndex(var19), false, true, true);
         ...
         case 0:
            var10000 = this.ParentChar.getStats();
            var10000.Pain += this.getInitialThumpPain() * BodyPartType.getPainModifyer(var19);
            break;
         case 1:
            var10000 = this.ParentChar.getStats();
            var10000.Pain += this.getInitialScratchPain() * BodyPartType.getPainModifyer(var19);
            break;
         case 2:
            var10000 = this.ParentChar.getStats();
            var10000.Pain += this.getInitialBitePain() * BodyPartType.getPainModifyer(var19);
         }
         ...
         if (this.ParentChar instanceof IsoPlayer && GameClient.bClient && ((IsoPlayer)this.ParentChar).isLocalPlayer()) {
            IsoPlayer var23 = (IsoPlayer)this.ParentChar;
            var23.updateMovementRates();
            GameClient.sendInjuries(var23);
         }
      ...
      if (!(this.getParentChar() instanceof IsoZombie)) {
         if (GameServer.bServer) {
            ...
            if (this.getOverallBodyHealth() != 0.0F) {
               ...
               int var9;
               if (!this.isInfected()) {
                  for(var9 = 0; var9 < BodyPartType.ToIndex(BodyPartType.MAX); ++var9) {
                     if (this.IsInfected(var9)) {
                        this.setInfected(true);
                        if (this.IsFakeInfected(var9)) {
                           this.DisableFakeInfection(var9);
               ...
               if (this.ParentChar.getPainEffect() > 0.0F) {
                  ...
                  for(int var11 = 0; var11 < BodyPartType.ToIndex(BodyPartType.MAX); ++var11) {
                     var3 += ((BodyPart)this.getBodyParts().get(var11)).getPain() * BodyPartType.getPainModifyer(var11);
                  }
               ...
               if (this.isInfected()) {
                  var9 = SandboxOptions.instance.Lore.Mortality.getValue();
                  if (var9 == 1) {
                     this.ReduceGeneralHealth(110.0F);
                     this.setInfectionLevel(100.0F);
                  } else if (var9 != 7) {
                     var10 = this.getCurrentTimeForInfection();
                     ...
                     if (this.InfectionTime < 0.0F) {
                        this.InfectionTime = var10;
                     }

                     if (this.InfectionTime > var10) {
                        this.InfectionTime = var10;
                     }

                     float var5 = (var10 - this.InfectionTime) / this.InfectionMortalityDuration;
               ...
               for(var9 = 0; var9 < BodyPartType.ToIndex(BodyPartType.MAX); ++var9) {
                  ((BodyPart)this.getBodyParts().get(var9)).DamageUpdate();
               }
               ...
               if (this.getOverallBodyHealth() <= 0.0F) {
                  if (this.isIsOnFire()) {
                     ...
                     for(var9 = 0; var9 < BodyPartType.ToIndex(BodyPartType.MAX); ++var9) {
                        ((BodyPart)this.getBodyParts().get(var9)).SetHealth((float)Rand.Next(90));
                     }
                  } else {
               ...
               if (this.getFakeInfectionLevel() <= 0.0F) {
                  for(var9 = 0; var9 < BodyPartType.ToIndex(BodyPartType.MAX); ++var9) {
                     ((BodyPart)this.getBodyParts().get(var9)).SetFakeInfected(false);
                  }
