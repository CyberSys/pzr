...
--[[
	if not isPaused and ISForageManager.getManager(playerObj) then
		if ISForageManager.getManager(playerObj).isSearchMode then
			self:addSlice(getText("ContextMenu_Forage"), getTexture("media/textures/Foraging/eyeconOn_UI.png"), self.onCommand, self, "ForageMode");
		else
			self:addSlice(getText("ContextMenu_Forage"), getTexture("media/textures/Foraging/eyeconOff_UI.png"), self.onCommand, self, "ForageMode");
		end;
	end;
	if not isPaused and ISForageManager.getManager(playerObj) then
		if ISForageManager.getManager(playerObj).isSearchMode then
			for _, icon in pairs(ISForageManager.getManager(playerObj).closeIcons) do
				self:addSlice(getText("ContextMenu_Forage"), icon.texture, self.onCommand, self, "ForageItem");
...
function ISBackButtonWheel:onCommand(command)
	...
	elseif command == "ForageMode" then
		local manager =	ISForageManager.getManager(playerObj);
		if manager then
