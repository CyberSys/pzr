...
function ISFitnessAction:setFitnessSpeed()
	...
	self.character:clearVariable("FitnessStruggle");
	
	local speed = (self.character:getPerkLevel(Perks.Fitness) / 5) / 1.1;
	speed = speed - (self.character:getMoodleLevel(MoodleType.Endurance) / 20)
	...
	if speed < 0.85 then
		speed = 1;
		self.character:setVariable("FitnessStruggle", true);
	end
	
	self.character:setVariable("FitnessSpeed", speed);
end
